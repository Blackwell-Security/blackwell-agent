...
from blackwell import security, __version__
...
from blackwell.core.security import revoke_tokens
...
@deprecate_endpoint(link=f'https://documentation.blackwell.com/{__version__}/user-manual/api/reference.html#'
...
async def deprecated_login_user(user: str, raw: bool = False) -> ConnexionResponse:
    ...
    return ConnexionResponse(body=token,
           ...
           ConnexionResponse(body=dumps(BlackwellResult({'data': TokenResponseModel(token=token)})),
...
async def get_user_me_policies(pretty: bool = False, wait_for_complete: bool = False) -> ConnexionResponse:
    ...
    """
    data = BlackwellResult({'data': request.context['token_info']['rbac_policies'],
...
async def revoke_all_tokens(pretty: bool = False) -> ConnexionResponse:
    ...
    data = raise_if_exc(await dapi.distribute_function())
    if type(data) == AffectedItemsBlackwellResult and len(data.affected_items) == 0:
        raise_if_exc(BlackwellPermissionError(4000, data.message))
