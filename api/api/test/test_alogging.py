...
REQUEST_HEADERS_TEST = {'authorization': 'Basic d2F6dWg6cGFzc3dvcmQxMjM='}  # blackwell:password123
...
def test_blackwelljsonformatter(message, dkt):
    """Check blackwell json formatter is working as expected.
    ...
    with patch('api.alogging.logging.LogRecord') as mock_record:
        ...
        wjf = alogging.BlackwellJsonFormatter()
        log_record = {}
...
def test_custom_logging(path, hash_auth_context, body, loggerlevel):
    ...
    user, remote, method = ('blackwell', '1.1.1.1', 'POST')
