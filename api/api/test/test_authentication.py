...
security_conf = BlackwellResult({
    'auth_token_exp_timeout': 900,
    'rbac_mode': 'black'
})
decoded_payload = {
    "iss": 'blackwell',
    "aud": 'Blackwell API REST',
    "nbf": 0,
...
@pytest.mark.asyncio
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None)
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.distribute_function', side_effect=None)
...
def test_generate_keypair(mock_open, mock_chown, mock_chmod, mock_change_keypair):
    ...
    calls = [call(authentication._private_key_path, authentication.blackwell_uid(), authentication.blackwell_gid()),
             call(authentication._public_key_path, authentication.blackwell_uid(), authentication.blackwell_gid())]
...
def test_check_token(mock_tokenmanager):
    result = authentication.check_token(username='blackwell_user', roles=tuple([1]), token_nbf_time=3600, run_as=False,
...
@patch('api.authentication.generate_keypair', return_value=('-----BEGIN PRIVATE KEY-----',
                                                            '-----BEGIN PUBLIC KEY-----'))
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None)
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.distribute_function', return_value=True)
@patch('api.authentication.raise_if_exc', side_effect=None)
async def test_decode_token(mock_raise_if_exc, mock_distribute_function, mock_dapi, mock_generate_keypair,
    ...
    mock_raise_if_exc.side_effect = [BlackwellResult({'valid': True, 'policies': {'value': 'test'}}),
                                     BlackwellResult(security_conf)]
    ...
    with patch('api.authentication.jwt.decode') as mock_decode:
        with patch('api.authentication.generate_keypair',
                   return_value=('-----BEGIN PRIVATE KEY-----',
                                 '-----BEGIN PUBLIC KEY-----')):
            with patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.__init__', return_value=None):
                with patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.distribute_function'):
                    with patch('api.authentication.raise_if_exc') as mock_raise_if_exc:
                        mock_decode.return_value = deepcopy(original_payload)

                        with pytest.raises(Unauthorized):
                            mock_raise_if_exc.side_effect = [BlackwellResult({'valid': False})]
                            authentication.decode_token(token='test_token')

                        with pytest.raises(Unauthorized):
                            mock_raise_if_exc.side_effect = [
                                BlackwellResult({'valid': True, 'policies': {'value': 'test'}}),
                                BlackwellResult({'auth_token_exp_timeout': 900,
                                             'rbac_mode': 'white'})]
                            authentication.decode_token(token='test_token')
