...
API_MAIN_PROCESS = 'blackwell-apid'
API_LOCAL_REQUEST_PROCESS = 'blackwell-apid_exec'
API_AUTHENTICATION_PROCESS = 'blackwell-apid_auth'
API_SECURITY_EVENTS_PROCESS = 'blackwell-apid_events'
...
def assign_blackwell_ownership(filepath: str):
    ...
    if not os.path.isfile(filepath):
        f = open(filepath, "w")
        f.close()
    if os.stat(filepath).st_gid != common.blackwell_gid() or \
        os.stat(filepath).st_uid != common.blackwell_uid():
        os.chown(filepath, common.blackwell_uid(), common.blackwell_gid())
...
def configure_ssl(params):
    ...
    try:
        ...
        if not os.path.exists(api_conf['https']['key']) \
            ...
            logger.info(
                f"Generated private key file in BLACKWELL_PATH/{to_relative_path(api_conf['https']['key'])}")
            generate_self_signed_certificate(private_key, api_conf['https']['cert'])
            logger.info(
                f"Generated certificate file in BLACKWELL_PATH/{to_relative_path(api_conf['https']['cert'])}")
        ...
        # Check and assign ownership to blackwell user for server.key and server.crt files
        assign_blackwell_ownership(api_conf['https']['key'])
        assign_blackwell_ownership(api_conf['https']['cert'])
        ...
        if exc.errno == 22:
            ...
            msg = f'Blackwell API SSL ERROR. Please, ensure ' \
                    f'if path to certificates is correct in the configuration ' \
                    f'file BLACKWELL_PATH/{to_relative_path(CONFIG_FILE_PATH)}'
            print(msg)
            logger.error(msg)
            raise exc from exc
...
def start(params: dict):
    """Run the Blackwell API.

    If another Blackwell API is running, this function will fail because uvicorn server will
    ...
    app.add_middleware(BlackwellAccessLoggerMiddleware, MiddlewarePosition.BEFORE_EXCEPTION)
...
def print_version():
    ...
    print('\n{} {} - {}\n\n{}'.format(__blackwell_name__, __version__, __author__, __licence__))
...
def add_debug2_log_level_and_error():
    """Add a new debug level used by blackwell api and framework."""
...
if __name__ == '__main__':
    ...
    from blackwell.core import common, pyDaemonModule, utils
    from blackwell.rbac.orm import check_database_integrity
    ...
    from api.middlewares import (
        ...
        BlackwellAccessLoggerMiddleware,
    ...
    try:
        ...
        print(f"Error when trying to start the Blackwell API. {e}")
        sys.exit(1)
    ...
    try:
        ...
        print(f"Error when trying to start the Blackwell API. {api_log_error}")
        sys.exit(1)

    # set permission on log files
    for handler in uvicorn_params['log_config']['handlers'].values():
        if 'filename' in handler:
            assign_blackwell_ownership(handler['filename'])
            os.chmod(handler['filename'], 0o660)

    # Configure and create the blackwell-api logger
    ...
    if 'use_only_authd' in api_conf:
        ...
        logger.warning(
            "'use_only_authd' option was deprecated on v4.3.0. Blackwell Authd will always be used")
    ...
    try:
        ...
        print(f'Internal error when trying to start the Blackwell API. {e}')
        sys.exit(1)
    finally:
