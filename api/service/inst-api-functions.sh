...
# Blackwell API Installer Functions
...
backup_old_api_4x() {
    ...
    if [ -d "${API_PATH}"/configuration ]; then
        if [ -n "$(ls -A "${API_PATH}"/configuration)" ]; then
            ...
            install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH_BACKUP}"/configuration
            ...
            if [ -d "${API_PATH}"/configuration/security ]; then
                if [ -n "$(ls -A "${API_PATH}"/configuration/security)" ]; then
                    install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH_BACKUP}"/configuration/security
            ...
            if [ -d "${API_PATH}"/configuration/ssl ]; then
                if [ -n "$(ls -A "${API_PATH}"/configuration/ssl)" ]; then
                    install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH_BACKUP}"/configuration/ssl
...
restore_old_api_4x() {
    ...
    if [ ! -d "${API_PATH}"/configuration ]; then
        install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH}"/configuration
    fi

    # Create security folder if it does not exists in the new api
    if [ ! -d "${API_PATH}"/configuration/security ]; then
        install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH}"/configuration/security
    fi

    # Create ssl folder if it does not exists in the new api
    if [ ! -d "${API_PATH}"/configuration/ssl ]; then
        install -o root -g ${BLACKWELL_GROUP} -m 0770 -d "${API_PATH}"/configuration/ssl
    fi
