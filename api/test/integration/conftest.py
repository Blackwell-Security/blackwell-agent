        ...
        while retries < max_retries:
            if build:
                build_process = subprocess.Popen(["docker", "compose", "--profile", env_mode,
                    "build", "--build-arg", f"BLACKWELL_BRANCH={current_branch}",
...
def check_health(node_type: str = 'manager', agents: list = None,
    ...
    """Check the Blackwell nodes health.
    ...
    if node_type == 'manager':
        ...
        for node in nodes_to_check:
            health = subprocess.check_output(
                f"docker inspect env-blackwell-{node}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
        ...
        for agent in agents:
            health = subprocess.check_output(
                f"docker inspect env-blackwell-agent{agent}-1 -f '{{{{json .State.Health.Status}}}}'",
                shell=True)
            if not health.startswith(b'"healthy"'):
                return False
...
def save_logs(test_name: str):
    """Save API, cluster and Blackwell logs from every cluster node and Blackwell logs from every agent if tests fail.
    Save haproxy-lb log.
        ...
        for log in logs:
            try:
                subprocess.check_output(
                    f"docker cp env-blackwell-{node}-1:{os.path.join(logs_path, log)} "
    ...
    for agent in agent_names:
        try:
            subprocess.check_output(
                f"docker cp env-blackwell-{agent}-1:{os.path.join(logs_path, 'ossec.log')} "
...
def api_test(request: _pytest.fixtures.SubRequest):
    ...
    while retries < max_retries:
        ...
        try:
            error_message = subprocess.check_output(["docker", "exec", "-t", "env-blackwell-master-1", "sh", "-c",
