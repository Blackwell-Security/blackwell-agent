"""
copyright: Copyright (C) 2015, Blackwell Inc.
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/api/configuration.html
...
from blackwell_testing.constants.api import CONFIGURATION_TYPES, MANAGER_CONFIGURATION_ROUTE
from blackwell_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from blackwell_testing.modules.api.utils import login, get_base_url
from blackwell_testing.utils.configuration import get_test_cases_data, load_configuration_template, get_blackwell_conf
...
def test_limit_eps(test_configuration, test_metadata, set_blackwell_configuration, add_configuration,
    ...
    blackwell_config = " ".join(get_blackwell_conf())
    assert f"<maximum>{test_metadata['expected_max_value']}</maximum>" in blackwell_config
    assert f"<timeframe>{test_metadata['expected_timeframe_value']}</timeframe>" in blackwell_config
