...
brief: These tests will check if the 'max_upload_size' setting of the API is working properly.
       ...
       The Blackwell API is an open source 'RESTful' API that allows for interaction with
       the Blackwell manager from a web browser, command line tool like 'cURL' or any script
...
from blackwell_testing.constants.api import CONFIGURATION_TYPES, GROUPS_ROUTE, CDB_LIST_ROUTE
...
def test_max_upload_size(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    assertions:
        - Verify that the 'blackwell-apid' daemon returns a proper HTTP status code depending on the value
