...
brief: These tests will check if the 'drop_privileges' setting of the API is working properly.
       This setting allows the user who starts the 'blackwell-apid' daemon to be different from
       the 'root' user. The Blackwell API is an open source 'RESTful' API that allows for interaction
       with the Blackwell manager from a web browser, command line tool like 'cURL' or any script
...
references:
    - https://documentation.blackwell.com/current/user-manual/api/getting-started.html
    - https://documentation.blackwell.com/current/user-manual/api/configuration.html#drop-privileges
...
from blackwell_testing.constants.api import CONFIGURATION_TYPES
from blackwell_testing.constants.daemons import API_DAEMONS_REQUIREMENTS
from blackwell_testing.constants.paths.api import BLACKWELL_API_SCRIPT
from blackwell_testing.utils.configuration import get_test_cases_data, load_configuration_template
from blackwell_testing.utils.services import search_process_by_command
...
def test_drop_privileges(test_configuration, test_metadata, add_configuration, truncate_monitored_files,
    ...
    description: Check if 'drop_privileges' affects the user of the API process. In this test, the 'PID' of the API
                 process is obtained. After that, it gets the user ('root' or 'blackwell') and checks if it matches the
    ...
    test_phases:
        ...
        - test:
            - Search blackwell-apid process and verify that it is present
    ...
    parameters:
        ...
        - add_configuration:
            type: fixture
            brief: Add configuration to the Blackwell API configuration files.
    ...
    assertions:
        - Verify that when 'drop_privileges' is enabled the user who has started the 'blackwell-apid' daemon is 'blackwell'.
        - Verify that when 'drop_privileges' is disabled the user who has started the 'blackwell-apid' daemon is 'root'.
    ...
    expected_output:
        - PID of the 'blackwell-apid' process.
        - blackwell (if drop_privileges is enabled)
        - root (if drop_privileges is disabled)
    """
    expected_user = test_metadata['expected_user']

    # Get blackwell-apid process info
    api_process = search_process_by_command(BLACKWELL_API_SCRIPT)
    assert api_process is not None, f"The process {BLACKWELL_API_SCRIPT} could not be found"
