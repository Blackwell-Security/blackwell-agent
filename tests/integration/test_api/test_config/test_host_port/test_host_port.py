...
brief: These tests will check that the settings related to the API host address and listening port
       are working correctly. The Blackwell API is an open source 'RESTful' API that allows for interaction
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/api/configuration.html#api-configuration-options
...
from blackwell_testing.modules.api.utils import get_base_url, login
...
def test_host_port(test_configuration, test_metadata, add_configuration, truncate_monitored_files, daemons_handler,
    ...
    test_phases:
        ...
        - test:
            - Check that expected host and port appear in the Blackwell API log
