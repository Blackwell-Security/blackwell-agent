...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/internal-options.html#analysisd
...
def test_remove_old_session_for_inactivity(configure_local_internal_options, test_configuration,
    ...
    '''
    description: Check if 'blackwell-logtest' correctly detects and handles the situation where trying to remove old
                 sessions due to inactivity. To do this, it creates more sessions than allowed and waits session_timeout
                 seconds, then checks that 'blackwell-logtest' has removed the session due to inactivity.
