...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_blackwell_conf.yaml')
...
def test_configuration_file(test_configuration, test_metadata, set_blackwell_configuration, daemons_handler):
    '''
    description: Checks if `blackwell-logtest` works as expected under different predefined configurations that cause
                 `blackwell-logtest` to start correctly, to be disabled, or to register an error. To do this, it checks
                 some values in these configurations from 'blackwell_conf.yaml' file.
    ...
    blackwell_log_monitor.start(callback=generate_callback(callback), timeout=session_parameters.default_timeout)
    assert blackwell_log_monitor.callback_result, 'Event not found'
