...
brief: These tests will check if the 'blackwell-authd' daemon is able to handle secure connections using
       the 'SSL' (Secure Socket Layer) protocol. The 'blackwell-authd' daemon can automatically add
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/registering/host-verification-registration.html
...
def test_authd_ssl_certs(test_configuration, test_metadata, set_blackwell_configuration,
                         generate_ca_certificate, truncate_monitored_files, daemons_handler,
                         wait_for_authd_startup):
    '''
    description:
        Checks if the 'blackwell-authd' daemon can manage 'SSL' connections with agents
    ...
    assertions:
        - Verify that the agent can only connect to the 'blackwell-authd' daemon socket using a valid certificate.
