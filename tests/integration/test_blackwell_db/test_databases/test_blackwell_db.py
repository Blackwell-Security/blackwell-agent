...
from blackwell_testing.utils.callbacks import make_callback
from blackwell_testing.modules.blackwell_db import BLACKWELL_DB_PREFIX
from blackwell_testing.utils import configuration
from blackwell_testing.utils.database import query_wdb
from blackwell_testing.utils.db_queries.agent_db import agent_integrity_check
...
# List items -> (blackwell_daemon: str,(
#                mitm: ManInTheMiddle
#                daemon_first: bool))
# Example1 -> ('blackwell-clusterd', None)              Only start blackwell-clusterd with no MITM
# Example2 -> ('blackwell-clusterd', (my_mitm, True))   Start MITM and then blackwell-clusterd
...
def validate_blackwell_db_response(expected_output, response):
    """ Method to validate the Blackwell-DB response.
...
def test_blackwell_db_messages_agent(test_metadata, configure_sockets_environment_module, connect_to_sockets_module,
                                 clean_databases, clean_registered_agents, insert_agents_test):
    '''
    description: Check that every input agent message in blackwell-db socket generates the proper output to blackwell-db
    ...
    parameters:
        ...
        - clean_registered_agents:
            type: fixture
            brief: Remove all agents of blackwelldb.
        - insert_agents_test:
            type: fixture
    ...
    for index, stage in enumerate(test_metadata['test_case']):
        ...
        if 'use_regex' in stage and stage['use_regex'] == 'yes':
            ...
            match = validate_blackwell_db_response(expected_output, response)
...
def test_blackwell_db_messages_global(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in blackwell-db socket generates the proper output to blackwell-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
                 stages (test_case, input field) and compare the result with the input list of stages (test_case,
                 output field).
...
def test_blackwell_db_range_checksum(configure_sockets_environment_module, connect_to_sockets_module,
    ...
    log_monitor.start(callback=make_callback('range checksum: Time: ', prefix=BLACKWELL_DB_PREFIX,
                                             escape=True), timeout=BLACKWELL_DB_CHECKSUM_CALCULUS_TIMEOUT)
    ...
    agent_integrity_check()
    log_monitor.start(callback=make_callback('range checksum avoided', prefix=BLACKWELL_DB_PREFIX,
                                             escape=True), timeout=BLACKWELL_DB_CHECKSUM_CALCULUS_TIMEOUT)
...
def test_blackwell_db_messages_tasks(test_metadata, daemons_handler_module, connect_to_sockets_module,
                                  clean_databases, clean_registered_agents):
    '''
    description: Check that every global input message in blackwell-db socket generates the proper output to blackwell-db
                 socket. To do this, it performs a query to the socket with a command taken from the input list of
