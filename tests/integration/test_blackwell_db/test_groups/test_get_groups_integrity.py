...
from blackwell_testing.utils.db_queries.global_db import create_or_update_agent, set_agent_group, sync_agent_groups, get_groups_integrity, delete_agent
...
def test_get_groups_integrity(daemons_handler, test_metadata, create_groups):
    '''
    description: Check that every input message using the 'get-groups-integrity' command in blackwell-db socket generates
                 the proper output to blackwell-db socket. To do this, it performs a query to the socket with a command
