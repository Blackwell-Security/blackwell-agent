...
from blackwell_testing.utils.db_queries.global_db import remove_metadata_value
...
# Configurations
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_blackwell_db_backup_command.yaml')
...
def test_wdb_backup_command(configure_sockets_environment_module, connect_to_sockets_module, remove_backups,
    ...
    '''
    description: Check that every input message using the 'backup' command in blackwell-db socket generates
                 the proper output to blackwell-db socket. To do this, it performs a series of queries to the socket with
    ...
    input_description:
        - Test cases are defined in the blackwell_db_backup_command.yaml file. This file contains the amount of backups to
