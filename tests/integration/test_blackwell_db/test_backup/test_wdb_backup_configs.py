...
brief: Blackwell-db is the daemon in charge of the databases with all the Blackwell persistent information, exposing a socket
       to receive requests and provide information. Blackwell-db has the capability to do automatic database backups, based
...
from blackwell_testing.constants.paths.logs import BLACKWELL_PATH, BLACKWELL_LOG_PATH
from blackwell_testing.utils.time import validate_interval_format, time_to_seconds
from blackwell_testing.modules.blackwell_db import patterns
...
# Configuration
t_config_path = Path(CONFIGURATIONS_FOLDER_PATH, 'configuration_blackwell_db_backups_conf.yaml')
t_cases_path = Path(TEST_CASES_FOLDER_PATH, 'cases_blackwell_db_backups_conf.yaml')
...
backups_path = Path(BLACKWELL_PATH, 'backup', 'db')
...
def test_wdb_backup_configs(test_configuration, test_metadata, set_blackwell_configuration,
    ...
    description: Check that given different wdb backup configuration parameters, the expected behavior is achieved.
                 For this, the test gets a series of parameters for the blackwell_db_backups_conf.yaml file and applies
    ...
    input_description:
        - Test cases are defined in the parameters and metada variables, that will be applied to the the
          blackwell_db_backup_command.yaml file. The parameters tested are: "enabled", "interval" and "max_files".
    ...
    try:
        ...
        if not validate_interval_format(test_interval):
            blackwell_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_INTERVAL_CALLBACK), timeout=timeout)
            assert blackwell_log_monitor.callback_result, 'Did not receive expected ' \
                                                    '"Invalid value element for interval..." event'

            return
        elif not isinstance(test_max_files, numbers.Number) or test_max_files==0:
            blackwell_log_monitor.start(callback=callbacks.generate_callback(patterns.WRONG_MAX_FILES_CALLBACK), timeout=timeout)
            assert blackwell_log_monitor.callback_result, 'Did not receive expected ' \
    ...
    # Manage if backup generation is enabled - one or more backups expected
        ...
        blackwell_log_monitor.start(callback=callbacks.generate_callback(patterns.BACKUP_CREATION_CALLBACK),
                                        timeout=timeout, accumulations=int(test_max_files)+1)
        result = blackwell_log_monitor.callback_result
