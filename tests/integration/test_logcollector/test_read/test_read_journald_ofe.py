...
from blackwell_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_blackwell_config, set_blackwell_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'blackwell-logcollector' daemon starts properly when the 'journald' tag is used 
                 ...
                 Finally, the test will verify that the Blackwell-logcollector read the logs, and the Blackwell API returns the correct values
    ...
    assertions:
        - Verify that the Blackwell component (agent or manager) can start when the 'jouranld' tag is used.
        - Verify that the Blackwell component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.
