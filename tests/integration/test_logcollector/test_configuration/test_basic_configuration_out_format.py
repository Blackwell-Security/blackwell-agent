...
brief: The 'blackwell-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'out_format' tag and the Blackwell API returns the same values for the configured
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/localfile.html#out-format
...
config_path = Path(CONFIGURATIONS_PATH, 'blackwell_basic_configuration_out_format.yaml')
...
def test_configuration_out_format(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'blackwell-logcollector' daemon detects invalid settings for the 'out_format' tag.
    ...
    input_description: A configuration template (test_basic_configuration_out_format) is contained in an
                       external YAML file (blackwell_basic_configuration.yaml). That template is combined with
                       different test cases defined in the module. Those include configuration settings
                       for the 'blackwell-logcollector' daemon.
    ...
    blackwell_log_monitor.start(callback, timeout=10)

    assert blackwell_log_monitor.callback_result, assert_error
