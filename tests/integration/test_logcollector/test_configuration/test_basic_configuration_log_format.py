...
brief: The 'blackwell-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'log_format' tag and the Blackwell API returns the same values for the configured
...
from blackwell_testing.constants.paths.logs import BLACKWELL_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from blackwell_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'blackwell_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'blackwell_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'blackwell_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'blackwell_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if Blackwell run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        blackwell_log_monitor.start(timeout=5,
        ...
        if test_metadata['log_format'] == 'full_command':
            ...
            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MONITORING_COMMAND, {
                                'command': test_metadata['command']})
        blackwell_log_monitor.start(timeout=5, callback=callback)
        assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_COMMAND_MONITORING
        ...
        assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            blackwell_log_monitor.start(timeout=5,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        blackwell_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if Blackwell fails because a invalid frequency configuration value.
    ...
    blackwell_log_monitor.start(timeout=5, callback=log_callback)
    assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_GENERIC_MESSAGE
...
def check_log_file_duplicated():
    """Check if Blackwell shows a warning message when the configuration is duplicated."""
    blackwell_log_monitor = file_monitor.FileMonitor(BLACKWELL_LOG_PATH)
    blackwell_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    blackwell_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (blackwell_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                    set_blackwell_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'blackwell-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the Blackwell API returns the same values for the 'localfile' section
    ...
    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (blackwell_basic_configuration.yaml, blackwell_duplicated_macos_configuration.yaml, and
                       blackwell_no_defined_location_macos_configuration.yaml). Those templates are combined with
