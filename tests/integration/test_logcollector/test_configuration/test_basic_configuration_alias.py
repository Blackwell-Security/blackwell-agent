...
brief: The 'blackwell-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       specified in the 'alias' tag when monitoring a command, and the Blackwell API returns the same
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/localfile.html#alias
...
config_path = Path(CONFIGURATIONS_PATH, 'blackwell_basic_configuration.yaml')
...
def test_configuration_alias(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'blackwell-logcollector' daemon changes a command name in the log messages by
                 ...
                 will verify that the Blackwell API returns the same values for the 'localfile' section that
                 the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_alias) is contained in an external YAML file
                       (blackwell_basic_configuration.yaml). That template is combined with two test cases defined
                       in the module. Those include configuration settings for the 'blackwell-logcollector' daemon.
    ...
    blackwell_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_READING_COMMAND_ALIAS, {
