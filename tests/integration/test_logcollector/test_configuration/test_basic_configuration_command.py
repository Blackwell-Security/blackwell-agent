...
brief: The 'blackwell-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the Blackwell API returns the same values for
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/localfile.html#command
...
config_path = Path(CONFIGURATIONS_PATH, 'blackwell_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'blackwell-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    blackwell_log_monitor.start(callback=callback)
