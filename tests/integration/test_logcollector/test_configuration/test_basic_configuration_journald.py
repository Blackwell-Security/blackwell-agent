...
brief: The 'blackwell-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'localfile' with 'journal' as 'log_format' is set in the configuration, and the Blackwell API returns the 
...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/localfile.html
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                remove_all_localfiles_blackwell_config, set_blackwell_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'blackwell-logcollector' daemon starts properly when the 'journald' tag is used as log_format.
                 For this purpose, the test will configure the logcollector to monitor a 'journald'.
                 Finally, the test will verify that the Blackwell component is started by checking its process, and the Blackwell API returns the correct values
    ...
    parameters:
        ...
        - remove_all_localfiles_blackwell_config:
            type: fixture
            brief: Remove all 'localfile' sections from the Blackwell configuration.
    ...
    assertions:
        - Verify that the Blackwell component (agent or manager) can start when the 'jouranld' tag is used as log_format.
        - Verify that the Blackwell API returns the correct values for the 'localfile' section.
    ...
    input_description: A configuration file with journal block settings and the expected output for each test case.
                       Those include configuration settings for `journal` configuration in 'blackwell-logcollector'.
