...
from blackwell_testing.constants.paths.logs import BLACKWELL_LOG_PATH, ACTIVE_RESPONSE_LOG_PATH
from blackwell_testing.modules.execd.active_response import patterns as ar_patterns
from blackwell_testing.modules.execd import patterns as execd_paterns
...
def test_execd_firewall_drop(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    blackwell_log_monitor.start(callback=generate_callback(execd_paterns.EXECD_EXECUTING_COMMAND))
    assert blackwell_log_monitor.callback_result, 'Execd `executing` command log not raised.'
