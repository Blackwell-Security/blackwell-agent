...
brief: Active responses execute a script in response to the triggering of specific alerts based
       ...
       which are executed by the 'blackwell-execd' daemon via scripts, run correctly.
...
references:
    - https://documentation.blackwell.com/current/user-manual/capabilities/active-response/#active-response
'''
import sys
import pytest
...
from blackwell_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, BLACKWELL_LOG_PATH
from blackwell_testing.constants.platforms import WINDOWS
from blackwell_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_BLACKWELL
from blackwell_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from blackwell_testing.modules.execd.configuration import EXECD_DEBUG
from blackwell_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-blackwell0 - restart-blackwell - 0\nrestart-blackwell0 - restart-blackwell.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                       set_blackwell_configuration, configure_ar_conf, remoted_simulator, authd_simulator,
                       daemons_handler, send_execd_message):
    '''
    description: Check if 'restart-blackwell' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the Blackwell agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-blackwell program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `blackwell-restart` did not fail.'
        return

    blackwell_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert blackwell_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_BLACKWELL))
    assert ar_monitor.callback_result, 'AR `restart-blackwell` program not used.'
