...
from blackwell_testing.utils.file import write_file
...
def test_file_size_disabled(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                            set_blackwell_configuration, folder_to_monitor, file_to_monitor, daemons_handler, detect_end_scan):
    '''
    ...
    blackwell_log_monitor.start(generate_callback(FILE_SIZE_LIMIT_REACHED), timeout=30)
    assert (blackwell_log_monitor.callback_result == None), f'Error, unexpected file_size limit event.'
    ...
    assert 'More changes...' in str(blackwell_log_monitor.callback_result[0]), 'Wrong content_changes field'
