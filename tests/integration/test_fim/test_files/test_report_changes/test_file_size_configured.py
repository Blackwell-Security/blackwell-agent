...
from blackwell_testing.modules.fim.patterns import DIFF_MAXIMUM_FILE_SIZE, ERROR_MSG_MAXIMUM_FILE_SIZE_EVENT, ERROR_MSG_WRONG_VALUE_MAXIMUM_FILE_SIZE
...
def test_diff_size_limit(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'blackwell-syscheckd' daemon limits the size of 'diff' information to generate from
    ...
    blackwell_log_monitor.start(generate_callback(DIFF_MAXIMUM_FILE_SIZE), timeout=30)
    callback_result = blackwell_log_monitor.callback_result
    assert callback_result, ERROR_MSG_MAXIMUM_FILE_SIZE_EVENT
    assert str(blackwell_log_monitor.callback_result[0]) == test_metadata.get('diff_size_limit_kb'), ERROR_MSG_WRONG_VALUE_MAXIMUM_FILE_SIZE
