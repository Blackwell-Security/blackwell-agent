...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/syscheck.html#file-size
...
from blackwell_testing.modules.fim.configuration import SYSCHECK_DEBUG, FILE_MAX_SIZE, RT_DELAY
from blackwell_testing.modules.agentd.configuration import AGENTD_WINDOWS_DEBUG
from blackwell_testing.modules.fim.patterns import FILE_SIZE_LIMIT_REACHED, EVENT_TYPE_REPORT_CHANGES, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED, ERROR_MSG_FILE_LIMIT_REACHED, \
                                               DIFF_FOLDER_DELETED, ERROR_MSG_FOLDER_DELETED, EVENT_UNABLE_DIFF
from blackwell_testing.modules.fim.utils import make_diff_file_path
from blackwell_testing.tools.monitors.file_monitor import FileMonitor
from blackwell_testing.utils.file import write_file, translate_size
...
def test_file_size_values(test_configuration, test_metadata, configure_local_internal_options,
                                    truncate_monitored_files, set_blackwell_configuration, folder_to_monitor, file_to_monitor, daemons_handler, detect_end_scan):
    '''
    description: Check if the 'blackwell-syscheckd' daemon limits the size of the monitored file to generate
    ...
    blackwell_log_monitor.start(callback=generate_callback(EVENT_TYPE_REPORT_CHANGES), timeout=30)
    assert blackwell_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    content_changes = str(blackwell_log_monitor.callback_result)
    ...
    blackwell_log_monitor.start(callback=generate_callback(DIFF_FOLDER_DELETED), timeout=30)
    assert blackwell_log_monitor.callback_result, ERROR_MSG_FOLDER_DELETED

    if os.path.exists(diff_file_path):
        pytest.raises(FileExistsError(f"{diff_file_path} found. It should not exist after incresing the size."))

    blackwell_log_monitor.start(callback=generate_callback(FILE_SIZE_LIMIT_REACHED), timeout=30)
    assert blackwell_log_monitor.callback_result, ERROR_MSG_FILE_LIMIT_REACHED

    # Check the content_changes field in the event
    blackwell_log_monitor.start(callback=generate_callback(EVENT_UNABLE_DIFF), timeout=30)
    assert blackwell_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'Unable to calculate diff due to \'file_size\' limit has been reached.' in blackwell_log_monitor.callback_result, 'Wrong content_changes field'
