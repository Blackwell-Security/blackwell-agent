...
from blackwell_testing.modules.fim.patterns import NUM_INOTIFY_WATCHES
...
def test_num_watches(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'blackwell-syscheckd' daemon detects the correct number of 'inotify watches' when
    ...
    blackwell_log_monitor.start(timeout=60, callback=generate_callback(NUM_INOTIFY_WATCHES))
    watches = int(blackwell_log_monitor.callback_result[0]) if blackwell_log_monitor.callback_result else 0
    ...
    blackwell_log_monitor.start(timeout=60, callback=generate_callback(NUM_INOTIFY_WATCHES), only_new_events=True)
