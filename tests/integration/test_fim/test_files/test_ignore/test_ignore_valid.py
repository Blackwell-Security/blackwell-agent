...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/syscheck.html#ignore
...
import pytest
from blackwell_testing import global_parameters
...
from blackwell_testing.modules.fim.patterns import EVENT_TYPE_ADDED, IGNORING_DUE_TO_SREGEX, IGNORING_DUE_TO_PATTERN
from blackwell_testing.modules.fim.utils import get_fim_event_data
...
def test_ignore_subdirectory(test_configuration, test_metadata, configure_local_internal_options,
                             ...
                             file_to_monitor):
    '''
    description: Check if the 'blackwell-syscheckd' daemon ignores the files that are in a monitored subdirectory
    ...
    input_description: Different test cases are contained in external YAML files
                       ...
                       for the 'blackwell-syscheckd' daemon and testing directories to monitor.
    ...
    if test_metadata.setdefault('triggers_event', False) == True:
        blackwell_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_ADDED))
        ...
        if test_metadata['is_pattern'] == False:
            blackwell_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_SREGEX))
            assert blackwell_log_monitor.callback_result
        else:
            blackwell_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_PATTERN))
            assert blackwell_log_monitor.callback_result
