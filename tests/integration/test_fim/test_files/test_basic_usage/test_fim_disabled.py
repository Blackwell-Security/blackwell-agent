...
from blackwell_testing.modules.fim.patterns import SENDING_FIM_EVENT
...
def test_fim_disabled(test_configuration, test_metadata, set_blackwell_configuration, truncate_monitored_files,
                      configure_local_internal_options, folder_to_monitor, daemons_handler):

    '''
    description: Check if the 'blackwell-syscheckd' daemon generates FIM events when it is disabled
    ...
    assertions:
        - Verify that when the 'blackwell-syscheckd' daemon is disabled, no FIM events are generated.
    ...
    blackwell_log_monitor.start(generate_callback(SENDING_FIM_EVENT))
