...
from blackwell_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED, MONITORING_PATH
...
def test_create_after_delete(test_configuration, test_metadata, configure_local_internal_options,
                             truncate_monitored_files, set_blackwell_configuration, folder_to_monitor, file_to_monitor, daemons_handler):
    '''
    ...
    input_description: The test cases are contained in external YAML file (cases_create_after_delete_dir.yaml)
                       which includes configuration parameters for the 'blackwell-syscheckd' daemon and testing
    ...
    blackwell_log_monitor.start(generate_callback(MONITORING_PATH), timeout=60)
    ...
    blackwell_log_monitor.start(generate_callback(EVENT_TYPE_DELETED), timeout=60)
    assert blackwell_log_monitor.callback_result
    assert get_fim_event_data(blackwell_log_monitor.callback_result)['mode'] == fim_mode

    file.recursive_directory_creation(folder_to_delete)
    time.sleep(2)
    file.write_file(filename, 'content')
    blackwell_log_monitor.start(generate_callback(EVENT_TYPE_ADDED), timeout=60)
    assert blackwell_log_monitor.callback_result
    assert get_fim_event_data(blackwell_log_monitor.callback_result)['mode'] == fim_mode
