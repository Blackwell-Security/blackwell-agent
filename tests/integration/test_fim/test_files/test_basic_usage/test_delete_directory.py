...
from blackwell_testing.modules.fim.patterns import EVENT_TYPE_DELETED, EVENT_TYPE_MODIFIED
...
def test_delete_dir(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                    truncate_monitored_files, folder_to_monitor, file_to_monitor, daemons_handler, start_monitoring):
    '''
    description: Check if the 'blackwell-syscheckd' daemon detects 'deleted' events from the files contained
    ...
    blackwell_log_monitor.start(generate_callback(EVENT_TYPE_MODIFIED))
