...
from blackwell_testing.modules.fim.patterns import AUDIT_RULES_RELOADED, EVENT_TYPE_MODIFIED, LINKS_SCAN_FINALIZED
...
from blackwell_testing.utils import file, commands
...
def test_audit_rules_with_symlink(test_configuration, test_metadata, set_blackwell_configuration, truncate_monitored_files,
                                  configure_local_internal_options, folder_to_monitor, symlink, symlink_new_target,
                                  daemons_handler, start_monitoring):
    '''
    description: Check if the 'blackwell-syscheckd' daemon removes the 'audit' rules when the target of
    ...
    blackwell_log_monitor.start(generate_callback(AUDIT_RULES_RELOADED))
