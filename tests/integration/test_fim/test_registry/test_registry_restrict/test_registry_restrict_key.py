...
references:
    ...
    - https://documentation.blackwell.com/current/user-manual/reference/ossec-conf/syscheck.html#windows-registry
...
from blackwell_testing.modules.fim.patterns import EVENT_TYPE_ADDED, EVENT_TYPE_DELETED, IGNORING_DUE_TO_RESTRICTION
...
from blackwell_testing.modules.fim.utils import get_fim_event_data, delete_registry
...
def test_restrict_key(test_configuration, test_metadata,configure_local_internal_options,
                            truncate_monitored_files, set_blackwell_configuration, daemons_handler, detect_end_scan, create_registry_key):
    '''
    description: Check if the 'blackwell-syscheckd' daemon detects or ignores events in monitored registry entries
    ...
    if test_metadata['triggers_event']:
        blackwell_log_monitor.start(callback=generate_callback(EVENT_TYPE_ADDED))
        ...
        blackwell_log_monitor.start(callback=generate_callback(EVENT_TYPE_DELETED))
        assert blackwell_log_monitor.callback_result
        event = get_fim_event_data(blackwell_log_monitor.callback_result)
        ...
        blackwell_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION))
        assert blackwell_log_monitor.callback_result

        delete_registry(win32con.HKEY_LOCAL_MACHINE, test_metadata['sub_key'], KEY_WOW64_64KEY if test_metadata['arch'] == 'x64' else KEY_WOW64_32KEY)

        blackwell_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_RESTRICTION), only_new_events=True)
        assert not blackwell_log_monitor.callback_result
