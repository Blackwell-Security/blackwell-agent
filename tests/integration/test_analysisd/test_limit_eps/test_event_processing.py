...
brief: The 'blackwell-analysisd' daemon uses a series of decoders and rules to analyze and interpret logs and events and
...
references:
    - https://documentation.blackwell.com/current/user-manual/ruleset/ruleset-xml-syntax/rules.html#if-sid
'''
import pytest
import time
import re
...
from blackwell_testing.modules.analysisd import utils, configuration as analysisd_config
from blackwell_testing.modules.remoted import configuration as remoted_config
from blackwell_testing.scripts.syslog_simulator import DEFAULT_MESSAGE_SIZE
from blackwell_testing.tools import thread_executor
from blackwell_testing.tools.simulators import run_syslog_simulator
from blackwell_testing.utils import configuration, file
...
def test_limitation(test_configuration, test_metadata, load_blackwell_basic_configuration, set_blackwell_configuration,
    ...
    # Check that blackwell-manager is processing syslog events
    ...
    # Check that the blackwell-manager is receiving events but it is not processing them due to the limitation
...
def test_queueing_events_after_limitation(test_configuration, test_metadata, load_blackwell_basic_configuration, set_blackwell_configuration,
                                          configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_dropping_events_when_queue_is_full(test_configuration, test_metadata, load_blackwell_basic_configuration, set_blackwell_configuration,
                                            configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_single_thread(test_configuration, test_metadata, load_blackwell_basic_configuration, set_blackwell_configuration,
                                                 configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
...
def test_event_processing_in_order_multi_thread(test_configuration, test_metadata, load_blackwell_basic_configuration, set_blackwell_configuration,
                                                configure_local_internal_options, truncate_monitored_files, daemons_handler):
    """
    description: Check that events are processed in order according to the position within the queue, and
        that events that are being received during the blocking phase are being added to the end of the queue when
