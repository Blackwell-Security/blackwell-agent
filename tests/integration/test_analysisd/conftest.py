...
from blackwell_testing.constants.paths.configurations import CUSTOM_RULES_PATH, CUSTOM_RULES_FILE, BLACKWELL_CONF_PATH
from blackwell_testing.constants.paths.logs import ALERTS_JSON_PATH, BLACKWELL_LOG_PATH
from blackwell_testing.constants.users import BLACKWELL_UNIX_GROUP, BLACKWELL_UNIX_USER
from blackwell_testing.modules.analysisd import patterns
from blackwell_testing.tools.monitors import file_monitor
from blackwell_testing.utils import callbacks, file


@pytest.fixture()
def prepare_custom_rules_file(request, test_metadata):
    """Configure a syscollector custom rules for testing.
    Restarting blackwell-analysisd is required to apply this changes.
    """
    ...
    shutil.chown(target_rule, BLACKWELL_UNIX_USER, BLACKWELL_UNIX_GROUP)
...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart Blackwell is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    """Configure a custom settting for testing. Restart Blackwell is needed for applying the configuration. """
    # Remove test case tags from ossec.conf
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), BLACKWELL_CONF_PATH)
