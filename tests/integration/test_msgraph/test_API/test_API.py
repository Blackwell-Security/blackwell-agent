...
brief: The Blackwell 'ms-graph' module is capable of communicating with Microsoft Graph & parsing its various
...
from blackwell_testing.modules.modulesd.configuration import MODULESD_DEBUG
...
def test_future_events_yes(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
    ...
    parameters:
        ...
        - daemons_handler:
            type: fixture
            brief: Manages daemons to reset Blackwell.
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*Bookmark updated'
        - r'.*blackwell-modulesd:ms-graph.*seconds to run first scan'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*Bookmark updated"))

    if(blackwell_log_monitor.callback_result != None):
        control_service('stop')
        truncate_file(BLACKWELL_LOG_PATH)
        control_service('start')
        blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*seconds to run first scan"))
        assert (blackwell_log_monitor.callback_result != None), f'Error, `first scan` not found in log'
    else:
...
def test_future_events_no(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*Bookmark updated'
        - r'.*blackwell-modulesd:ms-graph.*seconds to run next scan'
    '''
    ...
    if(blackwell_log_monitor.callback_result != None):
        ...
        blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*seconds to run next scan"))
        assert (blackwell_log_monitor.callback_result != None), f'Error, `next scan` not found in log'

        blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*seconds to run first scan"), timeout=10)
        assert (blackwell_log_monitor.callback_result == None), f'Error, `first scan` not found in log'
    else:
        assert (False), f'Error `Bookmark updated` not found in log'
...
def test_curl_max_size(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*Reached maximum CURL size'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*Reached maximum CURL size"))
    assert (blackwell_log_monitor.callback_result != None), f'Error, `maximum CURL size` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t4_configurations, t4_configuration_metadata), ids=t4_case_ids)
def test_valid_resource(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*microsoft.graph.security.alert'
        - r'.*blackwell-modulesd:ms-graph.*microsoft.graph.security.incident'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*microsoft.graph.security.alert"))
    assert (blackwell_log_monitor.callback_result != None), f'Error, `security.alert` not found in log'

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*microsoft.graph.security.incident"))
    assert (blackwell_log_monitor.callback_result != None), f'Error, `security.incident` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t5_configurations, t5_configuration_metadata), ids=t5_case_ids)
def test_invalid_resource(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*Received unsuccessful status
            code when attempting to get relationship \'invalid\'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(
        callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*Received unsuccessful "\
                                             r"status code when attempting to get relationship \'invalid\'"))
    assert (blackwell_log_monitor.callback_result != None), f'Error, `unsuccessful status code` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t6_configurations, t6_configuration_metadata), ids=t6_case_ids)
def test_valid_auth(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*INFO: Scanning tenant'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*INFO: Scanning tenant"))
    assert (blackwell_log_monitor.callback_result != None), f'Error, `Scanning tenant` not found in log'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t7_configurations, t7_configuration_metadata), ids=t7_case_ids)
def test_invalid_auth(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start, proxy_setup):
    '''
    description: Check 'ms-graph' behavior when `resource` tags `name` and `relationship` are invalid.
    blackwell_min_version: 4.6.0
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*WARNING: Recieved unsuccessful
    ...
    blackwell_log_monitor.start(
        callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*WARNING: Received unsuccessful "\
