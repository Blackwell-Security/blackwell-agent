...
def test_enabled(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*INFO: Started module'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*INFO: Started module"))
    assert (blackwell_log_monitor.callback_result != None), f'Error module enabled event not detected'

    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*{msg}", {
                              'msg': str(test_metadata['msg'])}))
    assert (blackwell_log_monitor.callback_result != None), f'Error module started or delayed event not detected'


@pytest.mark.parametrize('test_configuration, test_metadata', zip(t2_configurations, t2_configuration_metadata), ids=t2_case_ids)
def test_disabled(test_configuration, test_metadata, set_blackwell_configuration, configure_local_internal_options,
                 truncate_monitored_files, daemons_handler, wait_for_msgraph_start):
    '''
    ...
    expected_output:
        - r'.*blackwell-modulesd:ms-graph.*INFO: (Module disabled). Exiting.'
    '''

    blackwell_log_monitor = FileMonitor(BLACKWELL_LOG_PATH)
    blackwell_log_monitor.start(callback=callbacks.generate_callback(r".*blackwell-modulesd:ms-graph.*INFO: (Module disabled). Exiting."))

    assert (blackwell_log_monitor.callback_result != None), f'Error module disabled event not detected'
