...
This integration enrichs any alert generated by Blackwell via the Maltiverse API,
...
And restart Blackwell Manager:

    /etc/init.d/blackwell-manager restart
...
#    5 - Invalid URL
#    6 - Unable to connect to Blackwell
#    7 - Unexpected error sending message to Blackwell
...
def process_args(args: list):
    ...
    # Blackwell Manager in case of positive match
...
def send_event(msg: str, agent: dict = None):
    """Send an event to the Blackwell Manager.
    ...
    try:
        ...
        if e.errno == 111:
            print('ERROR: Blackwell is not running.')
            sys.exit(6)
        elif e.errno == 90:
            print('ERROR: Message too long to send to Blackwell.')
            sys.exit(7)
