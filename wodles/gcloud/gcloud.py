...
def main():
    ...
    try:
        ...
        if arguments.integration_type == "pubsub":
            ...
            with ThreadPoolExecutor() as executor:
                ...
                # check permissions
                subscriber_client = BlackwellGCloudSubscriber(credentials_file, project, logger, subscription_id)
                ...
                if messages_per_thread > 0:
                    for _ in range(n_threads - 1):
                        client = BlackwellGCloudSubscriber(credentials_file, project, logger, subscription_id)
    ...
    except exceptions.BlackwellIntegrationException as gcloud_exception:
        logging_func = logger.critical if \
            isinstance(gcloud_exception, exceptions.BlackwellIntegrationInternalError) else \
            logger.error
