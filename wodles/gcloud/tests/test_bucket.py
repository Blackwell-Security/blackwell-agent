...
from buckets.bucket import BlackwellGCloudBucket
...
def get_wodle_config(credentials_file: str = "credentials.json", logger: Logger = None,
    ...
    """Return a dict containing every parameter supported by BlackwellGCloudBucket. Used to simulate different ossec.conf
    configurations.
...
def test_BlackwellGCloudBucket__init__(mock_client):
    """Test if an instance of BlackwellGCloudBucket is created properly."""
    bucket = BlackwellGCloudBucket(**get_wodle_config())
...
def test_BlackwellGCloudBucket__init__ko(credentials_file, errcode):
    """Test that the appropriate exceptions are raised when the BlackwellGCloudBucket constructor is called with
    invalid parameters."""
    with pytest.raises(exceptions.GCloudError) as e:
        BlackwellGCloudBucket(**get_wodle_config(credentials_file=os.path.join(data_path, credentials_file)))
    assert e.value.errcode == errcode


@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_init_db(mock_client, clean_shared_cache):
...
@patch('buckets.bucket.BlackwellGCloudBucket.init_db')
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_get_last_processed_files(mock_client, mock_db, project_id, expected_length):
...
@patch('buckets.bucket.BlackwellGCloudBucket.init_db')
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_update_last_processed_files(mock_client, mock_db, project_id, prefix,
                                                       processed_files):
    """Test _update_last_processed_files updates the database by adding and removing rows when required."""
    bucket = BlackwellGCloudBucket(**get_wodle_config(prefix=prefix))
    bucket.db_connector = create_custom_database()
    bucket.db_table_name = TEST_TABLE_NAME
    bucket.project_id = project_id
...
@patch('buckets.bucket.BlackwellGCloudBucket.init_db')
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_update_last_processed_files_ko(mock_client, mock_db):
...
@patch('buckets.bucket.BlackwellGCloudBucket.init_db')
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_get_last_creation_time(mock_client, mock_db, project_id, expected_result):
...
def test_BlackwellGCloudBucket_check_permissions(mock_client, google_exception, errcode):
...
def test_BlackwellGCloudBucket_process_data(mock_client, mock_init_db, prefix, blob_creation_time,
    ...
    bucket = BlackwellGCloudBucket(**get_wodle_config(prefix=prefix, only_logs_after=only_logs_after, reparse=reparse))
    bucket.db_table_name = TEST_TABLE_NAME
...
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_load_information_from_file(mock_client):
...
def test_BlackwellGCloudBucket_process_blob(mock_client, mock_load_information, mock_socket, mock_send_msg, delete_file):
...
@patch('buckets.access_logs.GCSAccessLogs.load_information_from_file')
@patch('buckets.bucket.storage.client.Client.from_service_account_json')
def test_BlackwellGCloudBucket_process_blob_ko(mock_client, mock_load_information):
