...
from pubsub.subscriber import BlackwellGCloudSubscriber
...
def get_wodle_config(credentials_file: str = "credentials.json", project: str = "test_project",
    ...
    """Return a dict containing every parameter supported by BlackwellGCloudSubscriber. Used to simulate different
...
def test_BlackwellGCloudSubscriber__init__(mock_client):
    """Test if an instance of BlackwellGCloudSubscriber is created properly."""
    pubsub = BlackwellGCloudSubscriber(**get_wodle_config())
...
def test_BlackwellGCloudSubscriber__init__ko(credentials_file, errcode):
    """Test that the appropriate exceptions are raised when the BlackwellGCloudSubscriber constructor is called with
    invalid parameters.
    """
    with pytest.raises(GCloudError) as e:
        BlackwellGCloudSubscriber(**get_wodle_config(credentials_file=os.path.join(data_path, credentials_file)))
    assert e.value.errcode == errcode


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_get_subscriber_client(mock_credentials):
    """Test get_subscriber_client attempts to create a client object using the provided credentials file."""
    BlackwellGCloudSubscriber.get_subscriber_client("credentials.json")
    mock_credentials.assert_called_with("credentials.json")


@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_get_subscription_path(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_check_permissions(mock_credentials):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_check_permissions_ko(mock_credentials, errcode, msg):
    """Test check_permissions raises the expected GCloudError when the subscriber does not have the required
    permissions."""
    pubsub = BlackwellGCloudSubscriber(**get_wodle_config(credentials_file="credentials"))
    if errcode != 1206:
...
@patch('pubsub.subscriber.BlackwellGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_pull_request(mock_credentials, mock_send_msg, num_messages):
...
@patch('pubsub.subscriber.BlackwellGCloudSubscriber.send_msg')
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_pull_request_ko(mock_credentials, mock_send_msg):
...
@patch('pubsub.subscriber.pubsub.subscriber.Client.from_service_account_file')
def test_BlackwellGCloudSubscriber_process_messages(mock_credentials):
