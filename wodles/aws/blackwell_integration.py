...
BLACKWELL_DEFAULT_RETRY_CONFIGURATION = {aws_tools.RETRY_ATTEMPTS_KEY: 10, aws_tools.RETRY_MODE_BOTO_KEY: 'standard'}
MESSAGE_HEADER = "1:Blackwell-AWS:"


class BlackwellIntegration:
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, service_name=None, region=None,
        ...
        self.blackwell_path = utils.find_blackwell_path()
        self.blackwell_version = utils.get_blackwell_version()
        self.blackwell_queue = path.join(self.blackwell_path, "queue", "sockets", "queue")
        self.blackwell_wodle = path.join(self.blackwell_path, "wodles", "aws")
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when Blackwell Integration is instantiated and sets a default config using .aws/config file
        ...
        if path.exists(aws_tools.DEFAULT_AWS_CONFIG_PATH):
            ...
            try:
                ...
                if aws_tools.RETRY_ATTEMPTS_KEY in profile_config \
                    ...
                    # Set retry config
                    retries = copy.deepcopy(BLACKWELL_DEFAULT_RETRY_CONFIGURATION)
                    aws_tools.debug(
            ...
            args['config'] = config.Config(retries=copy.deepcopy(BLACKWELL_DEFAULT_RETRY_CONFIGURATION))
            aws_tools.debug(
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the Blackwell Queue
        ...
        try:
            ...
            s.connect(self.blackwell_queue)
            ...
            if e.errno == 111:
                aws_tools.error("Blackwell must be running.")
                sys.exit(11)
            elif e.errno == 90:
                aws_tools.error("Message too long to send to Blackwell.  Skipping message...")
                aws_tools.debug('+++ ERROR: Message longer than buffer socket for Blackwell. Consider increasing rmem_max. '
                                'Skipping message...', 1)
            else:
                aws_tools.error("Error sending message to blackwell: {}".format(e))
                sys.exit(13)
        except Exception as e:
            aws_tools.error("Error sending message to blackwell: {}".format(e))
            sys.exit(13)
...
class BlackwellAWSDatabase(BlackwellIntegration):
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, db_name,
        ...
        BlackwellIntegration.__init__(self, service_name=service_name,
        ...
        self.db_path = "{0}/{1}.db".format(self.blackwell_wodle, db_name)
        ...
        try:
            if self.db_cursor.execute(self.sql_find_table, {'name': 'metadata'}).fetchone():
                ...
                try:
                    ...
                    if metadata_version != self.blackwell_version:
                        self.db_cursor.execute(self.sql_update_version_metadata, {'blackwell_version': self.blackwell_version})
                ...
                try:
                    ...
                    self.db_cursor.execute(self.sql_insert_version_metadata, {'blackwell_version': self.blackwell_version})
