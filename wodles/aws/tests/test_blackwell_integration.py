...
@patch('blackwell_integration.BlackwellIntegration.get_client')
@patch('blackwell_integration.utils.find_blackwell_path', return_value=utils.TEST_BLACKWELL_PATH)
@patch('blackwell_integration.utils.get_blackwell_version')
def test_blackwell_integration_initializes_properly(mock_version, mock_path, mock_client):
    """Test if the instances of BlackwellIntegration are created properly."""

    args = utils.get_blackwell_integration_parameters()
    integration = blackwell_integration.BlackwellIntegration(**args)
    mock_path.assert_called_once()
    mock_version.assert_called_once()
    assert integration.blackwell_path == utils.TEST_BLACKWELL_PATH
    assert integration.blackwell_queue == os.path.join(integration.blackwell_path, utils.QUEUE_PATH)
    assert integration.blackwell_wodle == os.path.join(integration.blackwell_path, utils.WODLE_PATH)
...
@patch('blackwell_integration.config')
...
def test_default_config(mock_boto_config, file_exists, options, retry_attempts, retry_mode):
    """Test if `default_config` function returns the Blackwell default Retry configuration if there is no user-defined
    configuration.
    ...
    with patch('blackwell_integration.path.exists', return_value=file_exists):
        if file_exists:
            with patch('aws_tools.get_aws_config_params') as mock_config, \
                ...
                config = blackwell_integration.BlackwellIntegration.default_config(profile=utils.TEST_AWS_PROFILE)
            ...
            if aws_tools.RETRY_ATTEMPTS_KEY in profile_config or aws_tools.RETRY_MODE_CONFIG_KEY in profile_config:
                ...
                retries = blackwell_integration.BLACKWELL_DEFAULT_RETRY_CONFIGURATION

            assert config['config'].retries == retries
        else:
            config = blackwell_integration.BlackwellIntegration.default_config(profile=utils.TEST_AWS_PROFILE)

            mock_boto_config.Config.assert_called_with(retries=blackwell_integration.BLACKWELL_DEFAULT_RETRY_CONFIGURATION)
...
@pytest.mark.parametrize('region', list(blackwell_integration.DEFAULT_GOV_REGIONS) + ['us-east-1', None])
@pytest.mark.parametrize('service_name', list(blackwell_integration.SERVICES_REQUIRING_REGION) + ['other'])
def test_blackwell_integration_get_client_authentication(access_key, secret_key, profile, region, service_name):
    ...
    service_name : str
        Name of the service.
    """
    kwargs = utils.get_blackwell_integration_parameters(access_key=access_key, secret_key=secret_key, profile=profile,
    ...
    if region and service_name in blackwell_integration.SERVICES_REQUIRING_REGION:
        expected_conn_args['region_name'] = region
    else:
        expected_conn_args['region_name'] = region if region in blackwell_integration.DEFAULT_GOV_REGIONS else None

    with patch('blackwell_integration.utils.find_blackwell_path', return_value=utils.TEST_BLACKWELL_PATH), \
            patch('blackwell_integration.utils.get_blackwell_version', return_value=utils.BLACKWELL_VERSION), \
            patch('blackwell_integration.boto3.Session') as mock_boto:
        blackwell_integration.BlackwellIntegration(**kwargs)
...
def test_blackwell_integration_get_client(iam_role_arn, service_name, external_id):
    ...
    """
    kwargs = utils.get_blackwell_integration_parameters(access_key=None, secret_key=None, profile=None,
    ...
    sts_kwargs = {'aws_access_key_id': None, 'aws_secret_access_key': None, 'aws_session_token': utils.TEST_TOKEN,
                  'region_name': None}
    assume_role_kwargs = {'RoleArn': iam_role_arn, 'RoleSessionName': 'BlackwellLogParsing',
    ...
    with patch('blackwell_integration.utils.find_blackwell_path', return_value=utils.TEST_BLACKWELL_PATH), \
            patch('blackwell_integration.utils.get_blackwell_version', return_value=utils.BLACKWELL_VERSION), \
            patch('blackwell_integration.boto3.Session', side_effect=[mock_boto_session, mock_sts_session]) as mock_session:
        instance = blackwell_integration.BlackwellIntegration(**kwargs)
...
def test_blackwell_integration_get_client_handles_exceptions_on_botocore_error():
    """Test `get_client` function handles exceptions as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = blackwell_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    with patch('blackwell_integration.utils.find_blackwell_path', return_value=utils.TEST_BLACKWELL_PATH), \
            patch('blackwell_integration.utils.get_blackwell_version', return_value=utils.BLACKWELL_VERSION), \
            patch('blackwell_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
            blackwell_integration.BlackwellIntegration(**utils.get_blackwell_integration_parameters())
...
def test_blackwell_integration_get_sts_client(access_key, secret_key, profile):
    ...
    profile : str
        AWS profile name.
    """
    instance = utils.get_mocked_blackwell_integration(access_key=access_key, secret_key=secret_key, profile=profile)
    expected_conn_args = {}
    if access_key and secret_key:
        expected_conn_args['aws_access_key_id'] = access_key
        expected_conn_args['aws_secret_access_key'] = secret_key
    ...
    with patch('blackwell_integration.boto3.Session', return_value=mock_session) as mock_boto:
...
def test_blackwell_integration_get_sts_client_handles_exceptions_when_invalid_creds_provided():
    """Test `get_sts_client` function handles invalid credentials exception as expected."""
    mock_boto_session = MagicMock()
    mock_boto_session.client.side_effect = blackwell_integration.exceptions.ClientError({'Error': {'Code': 1}},
                                                                                             'operation')

    instance = utils.get_mocked_blackwell_integration(access_key=None, secret_key=None, profile=None)

    with patch('blackwell_integration.boto3.Session', return_value=mock_boto_session):
        with pytest.raises(SystemExit) as e:
...
def test_blackwell_integration_send_msg(dump_json):
    ...
    """
    instance = utils.get_mocked_blackwell_integration()
    msg = dumps(utils.TEST_MESSAGE) if dump_json else utils.TEST_MESSAGE
    with patch('blackwell_integration.socket.socket') as mock_socket:
        m = MagicMock()
        mock_socket.return_value = m
        instance.send_msg(utils.TEST_MESSAGE, dump_json=dump_json)
        mock_socket.assert_called_once()
        m.send.assert_called_with(f"{blackwell_integration.MESSAGE_HEADER}{msg}".encode())
...
def test_blackwell_integration_send_msg_socket_error(error_code, expected_exit_code):
...
@patch('io.BytesIO')
def test_blackwell_integration_decompress_file(mock_io):
    """Test 'decompress_file' method calls the expected function for a determined file type."""
    integration = utils.get_mocked_blackwell_integration()
    integration.client = MagicMock()
    # Instance that inherits from BlackwellIntegration sets the attribute bucket in its constructor
...
@patch('io.BytesIO')
def test_aws_blackwell_integration_decompress_file_handles_exceptions_when_decompress_fails(mock_io):
...
def test_blackwell_integration_send_msg_handles_exceptions():
...
@patch('blackwell_integration.utils.get_blackwell_version')
@patch('blackwell_integration.BlackwellIntegration.get_client')
@patch('blackwell_integration.BlackwellAWSDatabase.check_metadata_version')
@patch('blackwell_integration.sqlite3.connect')
def test_blackwell_aws_database_initializes_properly(mock_connect, mock_metadata, mock_client, mock_version, mock_path):
    """Test if the instances of BlackwellAWSDatabase are created properly."""
    mock_connect.return_value = MagicMock()
    args = utils.get_blackwell_aws_database_parameters()
    blackwell_aws_db = blackwell_integration.BlackwellAWSDatabase(**args)

    assert blackwell_aws_db.db_path == os.path.join(blackwell_aws_db.blackwell_wodle, f"{utils.TEST_DATABASE}.db")
    mock_connect.assert_called_once()
    blackwell_aws_db.db_connector.cursor.assert_called_once()
    mock_metadata.assert_called_once()


def test_blackwell_aws_database_create_table():
    """Test `create_table` function creates the table using the expected SQL."""
    instance = utils.get_mocked_blackwell_aws_database()
...
def test_blackwell_aws_database_create_table_handles_exceptions_when_table_not_created():
...
@patch('blackwell_integration.BlackwellAWSDatabase.create_table')
def test_blackwell_aws_database_db_initialization(mock_create_table, table_list):
...
def test_blackwell_aws_database_db_initialization_handles_exceptions():
...
def test_blackwell_aws_database_close_db():
...
def test_blackwell_aws_database_check_metadata_version_existing_table(custom_database):
    ...
    instance = utils.get_mocked_blackwell_aws_database(db_name=utils.TEST_DATABASE)
    ...
    assert old_metadata_value != utils.BLACKWELL_VERSION
    ...
    assert new_metadata_value == utils.BLACKWELL_VERSION


def test_blackwell_aws_database_check_metadata_version_no_table(custom_database):
...
def test_blackwell_aws_database_check_metadata_version_handles_exceptions(custom_database, table_exists):
...
def test_blackwell_aws_database_delete_deprecated_tables(custom_database):
    ...
    for table in blackwell_integration.DEPRECATED_TABLES:
