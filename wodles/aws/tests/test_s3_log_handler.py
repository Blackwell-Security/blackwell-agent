...
def test_aws_sl_subscriber_bucket_initializes_properly(mock_blackwell_integration, mock_client, mock_sts_client):
    ...
    mock_blackwell_integration.assert_called_with(integration, access_key=None, secret_key=None,
                                              profile=None,
                                              service_name='s3',
...
def test_aws_sl_subscriber_bucket_obtain_logs(mock_blackwell_integration, mock_sts_client):
...
@patch('blackwell_integration.BlackwellIntegration.get_sts_client')
@patch('blackwell_integration.BlackwellIntegration.__init__', side_effect=blackwell_integration.BlackwellIntegration.__init__)
def test_aws_sl_subscriber_bucket_obtain_logs_handles_exception(mock_blackwell_integration,
                                                                                    mock_sts_client):
...
@patch('blackwell_integration.BlackwellIntegration.send_msg')
@patch('blackwell_integration.BlackwellIntegration.get_sts_client')
@patch('blackwell_integration.BlackwellIntegration.__init__', side_effect=blackwell_integration.BlackwellIntegration.__init__)
def test_aws_sl_subscriber_bucket_process_file(mock_blackwell_integration, mock_sts_client, mock_send, mock_obtain):
...
def test_obtain_logs_processes_different_data_types(bucket_name, log_path, content, expected_logs):
    ...
    with patch('s3_log_handler.blackwell_integration.BlackwellIntegration.__init__'):
