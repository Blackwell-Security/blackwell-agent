...
def test_DockerListener__init__():
    ...
    for attribute in ['blackwell_path', 'blackwell_queue', 'msg_header', 'client', 'thread1', 'thread2']:
...
def test_DockerListener_process(mock_send):
    """Test process function sends the decoded events to the Blackwell socket."""
    event = b"test"
    dl = docker_listener.DockerListener()
    dl.process(event)
...
def test_DockerListener_send_msg(mock_socket):
    """Test send_msg sends the messages with the expected contents to the Blackwell socket."""
    msg = '{"test": "value"}'
    ...
    m.connect.assert_called_with(dl.blackwell_queue)
