...
class DockerListener:
    ...
    def __init__(self):
        ...
        if sys.platform == "win32":
            ...
            sys.exit(1)
        # socket variables
        self.blackwell_path = os.path.abspath(os.path.join(__file__, "..", "..", ".."))
        self.blackwell_queue = os.path.join(self.blackwell_path, "queue", "sockets", "queue")
        self.msg_header = "1:Blackwell-Docker:"
        # docker variables
        self.client = None
        self.thread1 = None
        self.thread2 = None
    ...
    def send_msg(self, msg):
        """
        Sends a Docker event to the Blackwell Queue
        ...
        try:
            ...
            if e.errno == 111:
                sys.stderr.write('Blackwell must be running.\n')
                sys.exit(11)
            else:
                sys.stderr.write("Error sending message to blackwell: {}\n".format(e))
                sys.exit(13)
        except Exception as e:
            sys.stderr.write("Error sending message to blackwell: {}\n".format(e))
            sys.exit(13)
