...
#include "../blackwell_db/wdb_state.h"
...
static int test_setup(void ** state) {
    ...
    wdb_state.queries_breakdown.blackwelldb_queries = 212;
    wdb_state.queries_breakdown.blackwelldb_breakdown.remove_queries = 212;
    wdb_state.queries_breakdown.blackwelldb_breakdown.remove_time.tv_sec = 0;
    wdb_state.queries_breakdown.blackwelldb_breakdown.remove_time.tv_usec = 132156;
...
void test_blackwelldb_create_state_json(void ** state) {
    ...
    *state = (void *)state_json;
    ...
    assert_non_null(cJSON_GetObjectItem(received_breakdown, "blackwelldb"));
    assert_int_equal(cJSON_GetObjectItem(received_breakdown, "blackwelldb")->valueint, 212);

    cJSON* blackwelldb_queries_breakdown = cJSON_GetObjectItem(received_breakdown, "blackwelldb_breakdown");

    cJSON* blackwelldb_queries_db = cJSON_GetObjectItem(blackwelldb_queries_breakdown, "db");
    assert_non_null(cJSON_GetObjectItem(blackwelldb_queries_db, "remove"));
    assert_int_equal(cJSON_GetObjectItem(blackwelldb_queries_db, "remove")->valueint, 212);
    ...
    assert_non_null(cJSON_GetObjectItem(execution_breakdown, "blackwelldb"));
    assert_int_equal(cJSON_GetObjectItem(execution_breakdown, "blackwelldb")->valueint, 132);

    cJSON* blackwelldb_time_breakdown = cJSON_GetObjectItem(execution_breakdown, "blackwelldb_breakdown");

    cJSON* blackwelldb_time_db = cJSON_GetObjectItem(blackwelldb_time_breakdown, "db");
    assert_non_null(cJSON_GetObjectItem(blackwelldb_time_db, "remove"));
    assert_int_equal(cJSON_GetObjectItem(blackwelldb_time_db, "remove")->valueint, 132);
}

int main(void) {
    const struct CMUnitTest tests[] = {
        // Test wdb_create_state_json
        cmocka_unit_test_setup_teardown(test_blackwelldb_create_state_json, test_setup, test_teardown),
    };
