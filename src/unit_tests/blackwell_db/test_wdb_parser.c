...
#include "../wrappers/blackwell/blackwell_db/wdb_syscollector_wrappers.h"
#include "../wrappers/blackwell/blackwell_db/wdb_wrappers.h"
#include "../wrappers/blackwell/blackwell_db/wdb_agents_wrappers.h"
#include "../wrappers/blackwell/blackwell_db/wdb_delta_event_wrappers.h"
...
void test_wdb_parse_sca_invalid_insert_not_number_id(void **state) {
    int ret;
    test_struct_t *data  = (test_struct_t *)*state;
    char query [25] = "insert {\"id\":\"blackwell\"}";

    expect_string(__wrap__mdebug1, formatted_msg, "Malformed JSON: field 'id' must be a number");

    ret = wdb_parse_sca(data->wdb, query, data->output);

    assert_string_equal(data->output, "err Invalid Security Configuration Assessment query syntax, near '{\"id\":\"blackwell\"}'");
    assert_int_equal(ret, -1);
}
