...
#include "../wrappers/blackwell/blackwell_db/wdb_state_wrappers.h"
...
void test_wdbcom_output_builder(void ** state) {
    ...
    cJSON_AddStringToObject(data_json, "test2", "blackwelldb");
    ...
    *state = msg;
    ...
    assert_non_null(msg);
    assert_string_equal(msg, "{\"error\":5,\"message\":\"test msg\",\"data\":{\"test1\":18,\"test2\":\"blackwelldb\"}}");
}
...
void test_wdbcom_dispatch_getstats(void ** state) {
    ...
    *response = '\0';
    ...
    assert_string_equal(response, "{\"error\":0,\"message\":\"ok\",\"data\":{\"test1\":18,\"test2\":\"blackwelldb\"}}");
}
...
void test_wdbcom_dispatch_getconfig(void ** state) {
    ...
    *response = '\0';
    ...
    assert_non_null(response);
    assert_string_equal(response, "{\"error\":0,\"message\":\"ok\",\"data\":{\"test1\":12,\"test2\":\"blackwelldb\"}}");
}
