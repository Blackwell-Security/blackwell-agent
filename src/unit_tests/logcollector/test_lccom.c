...
void _test_lccom_getstate_tmp (char *fullJson, char *ExpectedBlock, bool getNextPage){
    ...
    if (strstr(fullJson, outjson2) == NULL) {
        expect_string(__wrap_stat, __file, "var/run/blackwell-logcollector.state");
        ...
        expect_string(__wrap__mdebug2, formatted_msg, " Wed Dec 31 19:00:00 1969 var/run/blackwell-logcollector.state");
    }
