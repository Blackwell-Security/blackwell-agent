...
{
    char *argv0 = "/usr/share/blackwell/bin/test";
    ...
    expect_string(__wrap_stat, __file, "/usr/share/blackwell");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/usr/share/blackwell");
    free(val);
}
...
void test_w_homedir_env_var(void **state)
{
    ...
    expect_string(__wrap_getenv, name, BLACKWELL_HOME_ENV);
    will_return(__wrap_getenv, "/home/blackwell");

    expect_string(__wrap_stat, __file, "/home/blackwell");
    will_return(__wrap_stat, &stat_buf);
    will_return(__wrap_stat, 0);

    val = w_homedir(argv0);
    assert_string_equal(val, "/home/blackwell");
    free(val);
}
...
void test_w_homedir_stat_fail(void **state)
{
    ...
    expect_string(__wrap__merror_exit, formatted_msg, "(1108): Unable to find Blackwell install directory. Export it to BLACKWELL_HOME environment variable.");
