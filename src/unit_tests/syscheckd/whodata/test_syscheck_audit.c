...
#include "wrappers/blackwell/syscheckd/audit_parse_wrappers.h"
...
#define AUDIT_RULES_FILE            "etc/audit_rules_blackwell.rules"
#define AUDIT_RULES_LINK            "/etc/audit/rules.d/audit_rules_blackwell.rules"
...
void test_set_auditd_config_audit3_plugin_created(void **state) {
    ...
    const char *audit3_socket = "/etc/audit/plugins.d/af_blackwell.conf";
...
void test_set_auditd_config_audit2_plugin_created(void **state) {
    ...
    // Plugin already created
    const char *audit2_socket = "/etc/audisp/plugins.d/af_blackwell.conf";
...
void test_set_auditd_config_audit_plugin_tampered_configuration(void **state) {
    ...
    expect_string(__wrap__minfo, formatted_msg, "(6024): Generating Auditd socket configuration file: 'etc/af_blackwell.conf'");
    ...
    expect_string(__wrap_wfopen, path, "etc/af_blackwell.conf");
    ...
    // Create plugin
    expect_string(__wrap_symlink, path1, "etc/af_blackwell.conf");
    expect_string(__wrap_symlink, path2, audit3_socket);
    will_return(__wrap_symlink, -1);
    errno = EEXIST;

    expect_string(__wrap_unlink, file, "/etc/audit/plugins.d/af_blackwell.conf");
    ...
    expect_string(__wrap__minfo, formatted_msg, "(6025): Audit plugin configuration (etc/af_blackwell.conf) was modified. Restarting Auditd service.");
...
void test_set_auditd_config_audit_plugin_not_created_fopen_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1103): Could not open file 'etc/af_blackwell.conf' due to [(0)-(Success)].");
...
void test_set_auditd_config_audit_plugin_not_created_fclose_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1140): Could not close file 'etc/af_blackwell.conf' due to [(0)-(Success)].");
...
void test_set_auditd_config_audit_plugin_not_created_recreate_symlink_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1134): Unable to link from '/etc/audit/plugins.d/af_blackwell.conf' to 'etc/af_blackwell.conf' due to [(17)-(File exists)].");
...
void test_set_auditd_config_audit_plugin_not_created_recreate_symlink_unlink_error(void **state) {
    ...
    expect_string(__wrap__merror, formatted_msg, "(1123): Unable to delete file: '/etc/audit/plugins.d/af_blackwell.conf' due to [(17)-(File exists)].");
...
void test_audit_read_events_select_success_recv_success(void **state) {
    ...
    char * buffer = " \
        ...
        type=SYSCALL msg=audit(1571914029.306:3004255): arch=c000003e syscall=263 success=yes exit=0 a0=ffffff9c a1=55c5f8170490 a2=0 a3=7ff365c5eca0 items=2 ppid=3211 pid=44082 auid=4294967295 uid=0 gid=0 euid=0 suid=0 fsuid=0 egid=0 sgid=0 fsgid=0 tty=pts3 ses=5 comm=\"test\" exe=\"74657374C3B1\" key=\"blackwell_fim\"\n\
...
void test_audit_create_rules_file(void **state) {
    ...
    expect_string(__wrap_fprintf, formatted_msg, "-w /test0 -p wa -k blackwell_fim\n");
    will_return(__wrap_fprintf, 0);

    expect_string(__wrap__mdebug2, formatted_msg, "(6365): Added directory '/test1' to audit rules file.");

    expect_any(__wrap_fprintf, __stream);
    expect_string(__wrap_fprintf, formatted_msg, "-w /test1 -p wa -k blackwell_fim\n");
    will_return(__wrap_fprintf, 0);
