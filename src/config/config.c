                              ...
                              void *d2)
{
    ...
    const char *oswmodule = "wodle";                            /* Wodle - Blackwell Module  */
    ...
    const char *osgcp_pub = "gcp-pubsub";                       /* Google Cloud PubSub - Blackwell Module */
    const char *osgcp_bucket = "gcp-bucket";                    /* Google Cloud Bucket - Blackwell Module */
    const char *wlogtest = "rule_test";                         /* Blackwell Logtest */
    const char *agent_upgrade = "agent-upgrade";                /* Agent Upgrade Module */
    const char *task_manager = "task-manager";                  /* Task Manager Module */
    const char *blackwell_db = "wdb";                               /* Blackwell-DB Daemon */
#ifndef WIN32
        ...
        if (chld_node && (strcmp(node[i]->element, osglobal) == 0)) {
            ...
            if ((modules & CWMODULE)) {
                mwarn(
                    ...
                    "See https://documentation.blackwell.com",
                    osvulndetector,
                    osvulndetection);
            ...
            if ((modules & CWMODULE) && !(modules & CAGENT_CONFIG) && (Read_AgentUpgrade(xml, node[i], d1) < 0)) {
                ...
                mwarn("%s configuration is only set in the manager.", node[i]->element);
            #endif
        }  else if (chld_node && (strcmp(node[i]->element, blackwell_db) == 0)) {
            #if !defined(CLIENT)
                if ((modules & BLACKWELLDB) && (Read_BlackwellDB(xml, chld_node) < 0)) {
                    goto fail;
                }
            #else
                mwarn("%s configuration is only set in the manager.", node[i]->element);
