...
void *dispatch_buffer(__attribute__((unused)) void * arg){
    ...
    while(1){
        ...
        if (buff.warn){
            ...
            snprintf(warn_msg, OS_MAXSTR, "%c:%s:%s", LOCALFILE_MQ, "blackwell-agent", warn_str);
            send_msg(warn_msg, -1);
        }
        ...
        if (buff.full){
            ...
            buff.full = 0;
            mwarn(FULL_BUFFER);
            snprintf(full_msg, OS_MAXSTR, "%c:%s:%s", LOCALFILE_MQ, "blackwell-agent", OS_FULL_BUFFER);
            send_msg(full_msg, -1);
        }
        ...
        if (buff.flood){
            ...
            snprintf(flood_msg, OS_MAXSTR, "%c:%s:%s", LOCALFILE_MQ, "blackwell-agent", OS_FLOOD_BUFFER);
        ...
        if (buff.normal){
            ...
            snprintf(normal_msg, OS_MAXSTR, "%c:%s:%s", LOCALFILE_MQ, "blackwell-agent", OS_NORMAL_BUFFER);
