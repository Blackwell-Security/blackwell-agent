...
#include "blackwell_modules/wm_sca.h"
    ...
    while (1) {
        ...
        if (IsValidHeader(tmp_msg)) {
            ...
            /* Force reconnect agent to the manager */
                ...
                minfo("Blackwell Agent will be reconnected because a reconnect message was received");
                os_setwait();
                        ...
                        if (final_file) {
                            if (strcmp(final_file + 1, SHAREDCFG_FILENAME) == 0) {
                                ...
                                if (!UnmergeFiles(file, SHAREDCFG_DIR, OS_TEXT, &ignore_list)) {
                                    ...
                                    snprintf(msg_output, OS_MAXSTR, "%c:%s:%s",  LOCALFILE_MQ, "blackwell-agent", AG_IN_UNMERGE);
