...
static LPTSTR g_lpszServiceName        = "BlackwellSvc";
static LPTSTR g_lpszServiceDisplayName = "Blackwell";
static LPTSTR g_lpszServiceDescription = "Blackwell Windows Agent";
...
int os_stop_service()
{
    ...
    * newer instances of blackwell-agent.
    */
    ...
    if (ossecServiceStatusHandle) {
        switch (dwOpcode) {
            case SERVICE_CONTROL_STOP:
                ...
                // Kill children processes spawned by modules, only in blackwell-agent
                wm_kill_children();
                stop_wmodules();
