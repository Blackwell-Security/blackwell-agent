...
!define REVISION "41101"
!define NAME "Blackwell"
!define SERVICE "BlackwellSvc"

; output file
!ifndef OutFile
    !define OutFile "blackwell-agent-${VERSION}.exe"
!endif

Var is_upgrade

Name "${NAME} Windows Agent v${VERSION}"
BrandingText "Copyright (C) 2015, Blackwell Inc."
OutFile "${OutFile}"

VIProductVersion "4.10.0.0"
VIAddVersionKey ProductName "${NAME}"
VIAddVersionKey CompanyName "Blackwell Inc."
VIAddVersionKey LegalCopyright "2023 - Blackwell Inc."
VIAddVersionKey FileDescription "Blackwell Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "Blackwell Agent"
...
Section "Blackwell Agent (required)" MainSec
    ...
    ; install files
    File blackwell-agent.exe
    File blackwell-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-blackwell.exe restart-blackwell.exe
    ...
    File /oname=libblackwellext.dll ..\libblackwellext.dll
    File /oname=libblackwellshared.dll ..\libblackwellshared.dll
    ...
    File /oname=syscollector.dll ..\blackwell_modules\syscollector\build\bin\syscollector.dll
    File /oname=libfimdb.dll ..\syscheckd/build/bin/libfimdb.dll
    File /oname=queue\syscollector\norm_config.json ..\blackwell_modules\syscollector\norm_config.json
    File VERSION
    File REVISION
    ...
    ; use appropriate version of "blackwell-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\blackwell-agent.exe"
        Rename "$INSTDIR\blackwell-agent-eventchannel.exe" "$INSTDIR\blackwell-agent.exe"
    ${Else}
        Delete "$INSTDIR\blackwell-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "Blackwell, Inc."
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://blackwell.com"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://blackwell.com"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\blackwell-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\blackwell-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
