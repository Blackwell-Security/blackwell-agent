...
int wdb_insert_agent(int id,
    ...
    if (!data_in) {
        mdebug1("Error creating data JSON for Blackwell DB.");
        return OS_INVALID;
    }
...
int wdb_update_agent_status_code(int id, agent_status_code_t status_code, const char *version, const char *sync_status, int *sock) {
    ...
    cJSON_AddNumberToObject(data_in, "status_code", status_code);
    if (version != NULL) {
        char blackwell_version[OS_SIZE_128 + 1] = "";
        snprintf(blackwell_version, OS_SIZE_128, "%s %s", __ossec_name, version);
        cJSON_AddStringToObject(data_in, "version", blackwell_version);
    }
    cJSON_AddStringToObject(data_in, "sync_status", sync_status);
    data_in_str = cJSON_PrintUnformatted(data_in);
...
rb_tree* wdb_get_all_agents_rbtree(bool include_manager, int *sock) {
    ...
    if (status == WDBC_ERROR) {
        merror("Error querying Blackwell DB to get agent's IDs.");
        rbtree_destroy(tree);
        tree = NULL;
    }
...
int wdb_find_agent(const char *name, const char *ip, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB for agent ID.");
        return OS_INVALID;
    }
...
cJSON* wdb_get_agent_info(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to get the agent's %d information.", id);
        return NULL;
    }
...
cJSON* wdb_get_agent_labels(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to get the agent's %d labels.", id);
        return NULL;
    }
...
char* wdb_get_agent_name(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to get the agent's %d name.", id);
        return NULL;
    }
...
char* wdb_get_agent_group(int id, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to get the agent's %d group.", id);
        return NULL;
    }
...
int wdb_find_group(const char *name, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to get the agent group id.");
        return OS_INVALID;
    }
...
int wdb_update_groups(const char *dirname, int *sock) {
    ...
    if (!root) {
        merror("Error querying Blackwell DB to update groups.");
        if (!sock) {
            wdbc_close(&aux_sock);
        }
        return OS_INVALID;
...
cJSON* wdb_get_distinct_agent_groups(int *sock) {
    ...
    if (status == WDBC_ERROR) {
        merror("Error querying Blackwell DB to get agent's groups.");
        cJSON_Delete(root);
        root = NULL;
    }
