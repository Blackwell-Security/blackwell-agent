/* blackwelldb state management functions
 * May 27, 2022
 *
...
void w_inc_blackwelldb() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.blackwelldb_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_blackwelldb_remove() {
    w_mutex_lock(&db_state_t_mutex);
    wdb_state.queries_breakdown.blackwelldb_breakdown.remove_queries++;
    w_mutex_unlock(&db_state_t_mutex);
}

void w_inc_blackwelldb_remove_time(struct timeval time) {
    w_mutex_lock(&db_state_t_mutex);
    timeradd(&wdb_state.queries_breakdown.blackwelldb_breakdown.remove_time, &time, &wdb_state.queries_breakdown.blackwelldb_breakdown.remove_time);
    w_mutex_unlock(&db_state_t_mutex);
}
...
cJSON* wdb_create_state_json() {
    ...
    cJSON_AddNumberToObject(_received_breakdown, "blackwelldb", wdb_state_cpy.queries_breakdown.blackwelldb_queries);

    cJSON *_blackwelldb_breakdown = cJSON_CreateObject();
    cJSON_AddItemToObject(_received_breakdown, "blackwelldb_breakdown", _blackwelldb_breakdown);

    cJSON *_blackwelldb_db = cJSON_CreateObject();
    cJSON_AddItemToObject(_blackwelldb_breakdown, "db", _blackwelldb_db);

    cJSON_AddNumberToObject(_blackwelldb_db, "remove", wdb_state_cpy.queries_breakdown.blackwelldb_breakdown.remove_queries);
    ...
    cJSON_AddNumberToObject(_execution_breakdown, "blackwelldb", timeval_to_milis(wdb_state_cpy.queries_breakdown.blackwelldb_breakdown.remove_time));

    cJSON *_blackwelldb_breakdown_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_execution_breakdown, "blackwelldb_breakdown", _blackwelldb_breakdown_t);

    cJSON *_blackwelldb_db_t = cJSON_CreateObject();
    cJSON_AddItemToObject(_blackwelldb_breakdown_t, "db", _blackwelldb_db_t);

    cJSON_AddNumberToObject(_blackwelldb_db_t, "remove", timeval_to_milis(wdb_state_cpy.queries_breakdown.blackwelldb_breakdown.remove_time));
...
STATIC uint64_t get_time_total(wdb_state_t *state){
    return get_task_time(state) + get_global_time(state) + get_agent_time(state) + timeval_to_milis(state->queries_breakdown.blackwelldb_breakdown.remove_time) + timeval_to_milis(state->queries_breakdown.mitre_breakdown.sql_time);
}
