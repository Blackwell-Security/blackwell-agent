...
/* Look for a configuration assessment entry in Blackwell DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan configuration assessment entry in Blackwell DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Look for a scan policy entry in Blackwell DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
int wdb_sca_policy_find(wdb_t * wdb, char *id, char * output);

/* Gets the result of all checks in Blackwell DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
...
/* Gets the result of all policies in Blackwell DB. Returns 1 if found, 0 if not, or -1 on error. (new) */
 ...
 *
 * @param [in] input The chunk obtained from BlackwellDB to be parsed.
 ...
 * a message to process in blackwell-db process.
 *
 * @param wdb The Global struct database.
 * @param input Buffer input
 * @param output Buffer output, on success responses are:
...
/**
 * @brief Function to get the information of a particular agent stored in Blackwell DB.
 *
 * @param wdb The Global struct database.
 * @param id Agent id.
...
/**
 * @brief Method to read the internal blackwell-db configuration.
 *
