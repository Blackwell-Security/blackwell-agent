...
int main(int argc, char ** argv)
{
    ...
    wconfig.worker_pool_size = getDefine_Int("blackwell_db", "worker_pool_size", 1, 32);
    wconfig.commit_time_min = getDefine_Int("blackwell_db", "commit_time_min", 1, 3600);
    wconfig.commit_time_max = getDefine_Int("blackwell_db", "commit_time_max", 1, 3600);
    wconfig.open_db_limit = getDefine_Int("blackwell_db", "open_db_limit", 1, 4096);
    nofile = getDefine_Int("blackwell_db", "rlimit_nofile", 1024, 1048576);

    wconfig.fragmentation_threshold = getDefine_Int("blackwell_db", "fragmentation_threshold", 0, 100);
    wconfig.fragmentation_delta = getDefine_Int("blackwell_db", "fragmentation_delta", 0, 100);
    wconfig.free_pages_percentage = getDefine_Int("blackwell_db", "free_pages_percentage", 0, 99);
    wconfig.max_fragmentation = getDefine_Int("blackwell_db", "max_fragmentation", 0, 100);
    wconfig.check_fragmentation_interval = getDefine_Int("blackwell_db", "check_fragmentation_interval", 1, 30758400);
    ...
    if (ReadConfig(modules, OSSECCONF, &gconfig, NULL) < 0) {
        merror_exit("Invalid configuration block for Blackwell-DB.");
    }

    if (!isDebug()) {
        int debug_level;

        for (debug_level = getDefine_Int("blackwell_db", "debug", 0, 2); debug_level; debug_level--) {
            nowDebug();
        }
    }

    mdebug1(BLACKWELL_HOMEDIR, home_path);
