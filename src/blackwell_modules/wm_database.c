/*
 * Blackwell Module for SQLite database syncing
 ...
 *        In addition it will remove from global.db in blackwell-db all the agents that
...
/**
 * @brief This function removes the blackwell-db agent DB and the diff folder of an agent.
 *
...
void wm_clean_agent_artifacts(int agent_id, const char* agent_name) {
    ...
    // Removing blackwell-db database
    char wdbquery[OS_SIZE_128 + 1];
    char wdboutput[OS_SIZE_1024];
    snprintf(wdbquery, OS_SIZE_128, "blackwelldb remove %d", agent_id);
    if (result = wdbc_query_ex(&wdb_wmdb_sock, wdbquery, wdboutput, sizeof(wdboutput)), result) {
        mtdebug1(WM_DATABASE_LOGTAG, "Could not remove the blackwell-db DB of the agent %d.", agent_id);
    }
...
wmodule* wm_database_read() {
    ...
    data.sync_agents = getDefine_Int("blackwell_database", "sync_agents", 0, 1);
    data.real_time = getDefine_Int("blackwell_database", "real_time", 0, 1);
    data.interval = getDefine_Int("blackwell_database", "interval", 0, 86400);
    data.max_queued_events = getDefine_Int("blackwell_database", "max_queued_events", 0, INT_MAX);
