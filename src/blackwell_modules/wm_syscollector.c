...
static void wm_sys_send_message(const void* data, const char queue_id) {
    ...
    #ifdef CLIENT
            mterror(WM_SYS_LOGTAG, "Unable to send message to '%s' (blackwell-agentd might be down). Attempting to reconnect.", DEFAULTQUEUE);
    #else
            mterror(WM_SYS_LOGTAG, "Unable to send message to '%s' (blackwell-analysisd might be down). Attempting to reconnect.", DEFAULTQUEUE);
    #endif
