...
STATIC int wm_agent_upgrade_analyze_agent(int agent_id, wm_agent_task *agent_task) {
    ...
    if (agent_info && agent_info->child) {
        ...
        if(cJSON_IsString(value) && value->valuestring != NULL){
            os_strdup(value->valuestring, agent_task->agent_info->blackwell_version);
        }
...
STATIC int wm_agent_upgrade_validate_agent_task(const wm_agent_task *agent_task) {
    ...
    // Validate Blackwell version to upgrade
    validate_result = wm_agent_upgrade_validate_version(agent_task->agent_info->blackwell_version, agent_task->agent_info->platform, agent_task->task_info->command, agent_task->task_info->task);
