/*
 * Blackwell Module for Agent Upgrading
...
int wm_agent_upgrade_validate_version(const char *blackwell_version, const char *platform, wm_upgrade_command command, void *task) {
    char *tmp_agent_version = NULL;
    char *manager_version = NULL;
    int return_code = WM_UPGRADE_GLOBAL_DB_FAILURE;

    if (blackwell_version) {
        if (tmp_agent_version = strchr(blackwell_version, 'v'), tmp_agent_version) {

            if (compare_blackwell_versions(tmp_agent_version, WM_UPGRADE_MINIMAL_VERSION_SUPPORT, true) < 0) {
                return_code = WM_UPGRADE_NOT_MINIMAL_VERSION_SUPPORTED;
            } else if (compare_blackwell_versions(tmp_agent_version, WM_UPGRADE_MINIMAL_VERSION_SUPPORT_MACOS, true) < 0 && !strcmp(platform, "darwin")) {
                return_code = WM_UPGRADE_NOT_MINIMAL_VERSION_SUPPORTED;
                ...
                if (manager_version = strchr(__ossec_version, 'v'), manager_version) {
                    ...
                    if (!upgrade_task->force_upgrade) {
                        if (compare_blackwell_versions(tmp_agent_version, upgrade_task->wpk_version, true) >= 0) {
                            return_code = WM_UPGRADE_NEW_VERSION_LEES_OR_EQUAL_THAT_CURRENT;
                        } else if (compare_blackwell_versions(upgrade_task->wpk_version, manager_version, true) > 0) {
...
int wm_agent_upgrade_validate_wpk_version(wm_agent_info *agent_info, wm_upgrade_task *task, const char *wpk_repository_config) {
    ...
    if (!task->wpk_repository) {
        ...
        } else if (compare_blackwell_versions(task->wpk_version, "v4.0.0", true) < 0) {
    ...
    if (!strcmp(agent_info->platform, "windows")) {
        snprintf(path_url, OS_SIZE_2048, "%swindows/",
                 repository_url);
        snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_windows.wpk",
                 task->wpk_version);
    } else if (!strcmp(agent_info->platform, "darwin")) {
        if (compare_blackwell_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
            ...
            snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_macos_%s.%s.wpk",
            ...
            snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_macos_%s.wpk",
                    task->wpk_version, agent_info->architecture);
        }
    } else {
        if (compare_blackwell_versions(task->wpk_version, WM_UPGRADE_NEW_LINUX_VERSION_REPOSITORY, true) >= 0) {
            if (compare_blackwell_versions(task->wpk_version, WM_UPGRADE_NEW_VERSION_STRUCTURE_REPOSITORY, true) >= 0) {
                ...
                snprintf(path_url, OS_SIZE_2048, "%slinux/%s/%s/",
                         repository_url, agent_info->package_type, package_architecture);
                snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_linux_%s.%s.wpk",
                         task->wpk_version, package_architecture, agent_info->package_type);
            } else {
                snprintf(path_url, OS_SIZE_2048, "%slinux/%s/",
                         repository_url, agent_info->architecture);
                snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_linux_%s.wpk",
                         task->wpk_version, agent_info->architecture);
            }
            ...
            snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_%s_%s.%s_%s.wpk",
            ...
            snprintf(file_url, OS_SIZE_2048, "blackwell_agent_%s_%s_%s_%s.wpk",
            ...
            if (next_line = strchr(version, '\n'), next_line) {
                *next_line = '\0';
                if (sha1 = strchr(version, ' '), sha1) {
                    *sha1 = '\0';
                    if (compare_blackwell_versions(task->wpk_version, version, true) == 0) {
