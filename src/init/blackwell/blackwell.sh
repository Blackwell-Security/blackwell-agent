...
# Install functions for Blackwell
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/blackwell.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/blackwell.pp /tmp && semodule -i /tmp/blackwell.pp
                rm -f /tmp/blackwell.pp
...
BlackwellUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown blackwell:blackwell $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for Blackwell DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/blackwell
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated Blackwell tools
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:blackwell
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown blackwell:blackwell
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:blackwell {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown blackwell:blackwell {} \;
        fi
    fi
