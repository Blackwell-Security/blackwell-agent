...
# blackwell-control        This shell script takes care of starting
...
AUTHOR="Blackwell Inc."
USE_JSON=false
DAEMONS="blackwell-clusterd blackwell-modulesd blackwell-monitord blackwell-logcollector blackwell-remoted blackwell-syscheckd blackwell-analysisd blackwell-maild blackwell-execd blackwell-db blackwell-authd blackwell-agentlessd blackwell-integratord blackwell-dbd blackwell-csyslogd blackwell-apid"
OP_DAEMONS="blackwell-clusterd blackwell-maild blackwell-agentlessd blackwell-integratord blackwell-dbd blackwell-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting Blackwell $VERSION..."
    fi

    TEST=$(${DIR}/bin/blackwell-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/blackwell-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If blackwell-maild is disabled, don't try to start it.
        if [ X"$i" = "Xblackwell-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If blackwell-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xblackwell-clusterd" ]; then
        ...
        ## If blackwell-authd is disabled, don't try to start it.
        if [ X"$i" = "Xblackwell-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by Blackwell, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "Blackwell $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"BLACKWELL_VERSION":"'${VERSION}'"},'
            echo -n '{"BLACKWELL_REVISION":"'${REVISION}'"},'
            echo -n '{"BLACKWELL_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "BLACKWELL_VERSION=\"${VERSION}\""
            echo "BLACKWELL_REVISION=\"${REVISION}\""
            echo "BLACKWELL_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/blackwell-execd//')
    restart_service
    ;;
status)
    lock
