...
DAEMONS="blackwell-modulesd blackwell-monitord blackwell-logcollector blackwell-syscheckd blackwell-analysisd blackwell-maild blackwell-execd blackwell-db blackwell-agentlessd blackwell-integratord blackwell-dbd blackwell-csyslogd"
...
start_service()
{
    ...
    TEST=$(${DIR}/bin/blackwell-logtest-legacy -t  2>&1)
    echo $TEST

    if [ ! -z "$TEST" ]; then
        echo "blackwell-analysisd: Configuration error. Exiting."
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/blackwell-control start
    fi
...
stop_service()
{
    ...
    if [ $? = 0 ]; then
        ...
        ${DIR}/ossec-agent/bin/blackwell-control stop
    fi
