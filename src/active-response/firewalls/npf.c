...
int main (int argc, char **argv) {
    ...
    flag = false;
    while (fgets(output_buf, OS_MAXSTR, wfd->file_out)) {
        const char *pos = strstr(output_buf, "table <blackwell_blacklist>");
    ...
    if (flag == false) {
        memset(log_msg, '\0', OS_MAXSTR);
        snprintf(log_msg, OS_MAXSTR -1, "Unable to find 'table <blackwell_blacklist>'");
        write_debug_file(argv[0], log_msg);
        cJSON_Delete(input_json);
        os_free(npfctl_path);
        return OS_INVALID;
    ...
    if (action == ADD_COMMAND) {
        const char *arg3[6] = { npfctl_path, "table", "blackwell_blacklist", "add", srcip, NULL };
        memcpy(exec_cmd2, arg3, sizeof(exec_cmd2));
    } else {
        const char *arg3[6] = { npfctl_path, "table", "blackwell_blacklist", "del", srcip, NULL };
        memcpy(exec_cmd2, arg3, sizeof(exec_cmd2));
    }
