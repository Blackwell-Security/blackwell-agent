...
#define RULE_NAME "BLACKWELL ACTIVE RESPONSE BLOCKED IP"
...
int main (int argc, char **argv) {
    ...
    if (1 == checkVista()) {
        ...
        char *exec_args_delete[12] = { netsh_path, "ipsec", "static", "delete", "filter", "filterlist=\"blackwell_filter\"", "srcmask=\"255.255.255.255\"", remoteip, "dstaddr=Me", "protocol=\"any\"", "mirrored=yes", NULL };
        char *exec_args_filter[12] = { netsh_path, "ipsec", "static", "add", "filter", "filterlist=\"blackwell_filter\"", "srcmask=\"255.255.255.255\"", remoteip, "dstaddr=Me", "protocol=\"any\"", "mirrored=yes", NULL };
        char *exec_args_faction[8] = { netsh_path, "ipsec", "static", "add", "filteraction", "name=\"blackwell_action\"", "action=block", NULL };
        char *exec_args_policy[9]  = { netsh_path, "ipsec", "static", "add", "policy", "name=\"blackwell_policy\"", "assign=yes", description, NULL };
        char *exec_args_rule[10]   = { netsh_path, "ipsec", "static", "add", "rule", "name=blackwell_rule", "policy=blackwell_policy", "filterlist=blackwell_filter", "filteraction=blackwell_action", NULL };

        if (action == ADD_COMMAND) {
            wfd = wpopenv(netsh_path, exec_args_filter, W_BIND_STDERR);
            if (!wfd) {
                memset(log_msg, '\0', OS_MAXSTR);
                snprintf(log_msg, OS_MAXSTR -1, "Unable to run netsh, action: 'ADD', 'blackwell_filter'");
                write_debug_file(argv[0], log_msg);
            } else {
                wpclose(wfd);
            }
            ...
            if (!wfd) {
                memset(log_msg, '\0', OS_MAXSTR);
                snprintf(log_msg, OS_MAXSTR -1, "Unable to run netsh, action: 'ADD', 'blackwell_action'");
                write_debug_file(argv[0], log_msg);
            } else {
                wpclose(wfd);
            }
            ...
            if (!wfd) {
                memset(log_msg, '\0', OS_MAXSTR);
                snprintf(log_msg, OS_MAXSTR -1, "Unable to run netsh, action: 'ADD', 'blackwell_policy'");
                write_debug_file(argv[0], log_msg);
            } else {
                wpclose(wfd);
            }
            ...
            if (!wfd) {
                memset(log_msg, '\0', OS_MAXSTR);
                snprintf(log_msg, OS_MAXSTR -1, "Unable to run netsh, action: 'ADD', 'blackwell_rule'");
                write_debug_file(argv[0], log_msg);
            } else {
                wpclose(wfd);
            }
            ...
            if (!wfd) {
                memset(log_msg, '\0', OS_MAXSTR);
                snprintf(log_msg, OS_MAXSTR -1, "Unable to run netsh, action: 'DELETE', rule: 'blackwell_rule'");
                write_debug_file(argv[0], log_msg);
            } else {
                wpclose(wfd);
            }
