...
if [ "x$1" = "xlist" ]; then
    ...
    if [ "X${2}" = "X" ]; then
        echo "ERROR: You must specify the installation path. i.e.: ${0} $1 BLACKWELL_HOME"
        exit 1;
    fi    
    BLACKWELL_HOME=${2}

    eval $(${BLACKWELL_HOME}/bin/blackwell-control info 2>/dev/null)    
    if [ "X$BLACKWELL_TYPE" = "X" ]; then
        echo "ERROR: Unable to save rules. You must have Blackwell installed to do so."
        exit 1;
    fi
    if [ "$BLACKWELL_TYPE" = "agent" ]; then
        echo "ERROR: You must execute this script on Blackwell Manager"
        exit 1;
    fi

    ls ${BLACKWELL_HOME}/compiled_rules > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        mkdir ${BLACKWELL_HOME}/compiled_rules > /dev/null 2>&1
        if [ ! $? = 0 ]; then
    ...
    cp .function_list ${BLACKWELL_HOME}/compiled_rules/function_list > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to save rules. You must be root to do so."
        exit 1;
    fi
    ...
    for i in `ls *.c`; do
        if [ ! "x$i" = "xgeneric_samples.c" ]; then
            cp $i ${BLACKWELL_HOME}/compiled_rules/ > /dev/null 2>&1
        fi
    done
    echo "*Save completed at ${BLACKWELL_HOME}/compiled_rules/";
    exit 0;
    ...
    ls ${BLACKWELL_HOME}/compiled_rules/function_list > /dev/null 2>&1
    if [ ! $? = 0 ]; then
        echo "*No local compiled rules available to restore."
        exit 0;
    fi

    cat  ${BLACKWELL_HOME}/compiled_rules/function_list >> .function_list
    if [ ! $? = 0 ]; then
        echo "ERROR: Unable to restore rules. Function list not present."
        exit 1;
    fi

    for i in `ls ${BLACKWELL_HOME}/compiled_rules/*.c`; do
        if [ ! "x$i" = "xgeneric_samples.c" ]; then
            cp $i ./ > /dev/null 2>&1
        fi
    done
    echo "*Restore completed from ${BLACKWELL_HOME}/compiled_rules/";
    exit 0;
