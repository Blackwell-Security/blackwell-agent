...
/**
 * @brief Connects to Blackwell-DB socket
 *
...
/**
 * @brief Connects to Blackwell-DB socket with a maximum number of attempts.
 *
...
/**
 * @brief Sends query to Blackwell-DB and stores the response.
 *
 * @param[in] sock Client socket descriptor.
 * @param[in] query Query to be sent to Blackwell-DB.
 * @param[out] response Char pointer where the response from Blackwell-DB will be stored.
 * @param[in] len Lenght of the response param.
 * @post This function will read up to len bytes from Blackwell DB.
...
/**
 * @brief Check connection to Blackwell-DB, sends query and stores the response.
 *
...
int wdbc_query_ex(int *sock, const char *query, char *response, const int len) {
    ...
    if (retval = wdbc_query(*sock, query, response, len), retval != 0) {
        if (errno == EAGAIN || errno == EWOULDBLOCK) {
            ...
            // Retry to connect
            merror("Connection with blackwell-db lost. Reconnecting.");
            close(*sock);
...
/**
 * @brief Parse the result of the query to Blackwell-DB
 *
 ...
 *
 * @param result Result from the query to Blackwell-DB.
...
cJSON * wdbc_query_parse_json(int *sock, const char *query, char *response, const int len) {
    ...
    case -2:
        ...
        return NULL;
    case -1:
        merror("No response from blackwell-db.");
        return NULL;
    }
    ...
    case WDBC_OK:
        break;
    case WDBC_ERROR:
        merror("Bad response from blackwell-db: %s", arg);
        // Fallthrough
    default:
        return NULL;
    }
 ...
 * @param[out] response Char pointer where the response from Blackwell-DB will be stored.
 * @param[in] len Lenght of the response param.
 * @param[out] payload Char pointer where the payload from Blackwell-DB will be stored.
 * @return Enum wdbc_result.
 */
...
wdbc_result wdbc_query_parse(int *sock, const char *query, char *response, const int len, char** payload) {
    ...
    if (OS_SUCCESS == result) {
        ...
        if (status == WDBC_ERROR){
            merror("Bad response from blackwell-db: %s", _payload);
        }
    }
    else if (-2 == result) {
        merror("Unable to connect to socket '%s'", WDB_LOCAL_SOCK);
