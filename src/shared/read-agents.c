...
int send_msg_to_agent(int msocket, const char *msg, const char *agt_id, const char *exec)
{
    ...
    if (!exec) {
        ...
        for (size_t i = 0; id_array[i] != OS_INVALID; i++) {
            ...
            if (!json_agt_info) {
                merror("Failed to get agent '%d' information from Blackwell DB.", id_array[i]);
                continue;
            }
...
agent_info *get_agent_info(const char *agent_id){
    ...
    if (!json_agt_info) {
        mdebug1("Failed to get agent '%s' information from Blackwell DB.",agent_id);
        return NULL;
    }
...
agent_status_t get_agent_status(int agent_id){
    ...
    if (!json_agt_info) {
        mdebug1("Failed to get agent '%d' information from Blackwell DB.", agent_id);
        return status;
    }
...
char **get_agents(int flag){
    ...
    for (i = 0; id_array[i] != -1; i++){
        ...
        json_agt_info = wdb_get_agent_info(id_array[i], &sock);
        if (!json_agt_info) {
            mdebug1("Failed to get agent '%d' information from Blackwell DB.", id_array[i]);
            continue;
        }
...
time_t scantime_fim (const char *agent_id, const char *scan) {
    ...
    if (wdbc_query_ex(&wdb_socket, blackwelldb_query, response, OS_SIZE_6144) == 0) {
