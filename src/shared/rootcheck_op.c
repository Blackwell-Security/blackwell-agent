...
int send_rootcheck_log(const char* agent_id, long int date, const char* log, char* response) {
    char blackwelldb_query[OS_SIZE_6144];
    int db_result;
    int socket = -1;

    snprintf(blackwelldb_query, OS_SIZE_6144, "agent %s rootcheck save %li %s", agent_id, date, log);
    db_result = wdbc_query_ex(&socket, blackwelldb_query, response, OS_SIZE_6144);
    close(socket);

    if (db_result == -2) {
        merror("Bad load query: '%s'.", blackwelldb_query);
    }
