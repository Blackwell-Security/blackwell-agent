...
from blackwell.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> BlackwellLogger:
    """Set cluster logger.
...
def print_version():
    """Print Blackwell metadata."""
    from blackwell.core.cluster import __author__, __licence__, __version__, __blackwell_name__
    print(f"\n{__blackwell_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('blackwell-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('blackwell-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: BlackwellLogger):
    ...
    logger : BlackwellLogger
        Cluster logger.
    """
    from blackwell.core.cluster import local_server, master
    from blackwell.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: BlackwellLogger):
    ...
    from blackwell.core.cluster import local_server, worker
...
def main():
    """Main function of the blackwell-clusterd script in charge of starting the cluster process."""
    import blackwell.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.BLACKWELL_PATH}/logs/cluster.log'):
        os.chown(f'{common.BLACKWELL_PATH}/logs/cluster.log', common.blackwell_uid(), common.blackwell_gid())
        os.chmod(f'{common.BLACKWELL_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        blackwell.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    blackwell.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('blackwell-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to blackwell
    if not args.root:
        os.setgid(common.blackwell_gid())
        os.setuid(common.blackwell_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('blackwell-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('blackwell-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('blackwell-clusterd', pid)


if __name__ == '__main__':
    import blackwell.core.cluster.utils as cluster_utils
    from blackwell.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('blackwell_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
