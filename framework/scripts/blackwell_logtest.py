...
from blackwell.core.common import LOGTEST_SOCKET
...
def main():
    """blackwell-logtest main function."""
    # Parse cmdline args
    ...
    if args.version:
        logging.info('%s', Blackwell.get_description())
        logging.info('%s', Blackwell.get_license())
        sys.exit(0)
    ...
    # Initialize blackwell-logtest component
    w_logtest = BlackwellLogtest(location=args.location)
    logging.info('Starting blackwell-logtest %s', Blackwell.get_version_str())
    ...
    while True:
        ...
        try:
            ...
            logging.error('** Blackwell-logtest error ' + str(error))
            continue
        except ConnectionError:
            logging.error('** Blackwell-logtest error when connecting with blackwell-analysisd')
            continue
        ...
        if 'messages' in output.keys():
            ...
            for message in output['messages']:
                if message.startswith("WARNING"):
                    logging.warning('** Blackwell-Logtest: %s', message)
        ...
        # Show blackwell-logtest output
        BlackwellLogtest.show_output(output)
...
class BlackwellDeamonProtocol:
    """Encapsulate logic communication aspects between Blackwell daemons."""

    def __init__(self, version: int = 1, origin_module: str = "blackwell-logtest", module_name: str = "blackwell-logtest"):
        """Class constructor.
        ...
        origin_module : str
            Origin source module. Default: "blackwell-logtest"
        module_name : str
            Source module name. Default: "blackwell-logtest"
        """
        self.protocol = dict()
    ...
    def wrap(self, command: str, parameters: dict) -> str:
        """Wrap data with Blackwell daemon protocol information.
    ...
    def unwrap(self, msg: dict) -> dict:
        """Unwrap data from Blackwell daemon protocol information.
...
class BlackwellSocket:
    """Encapsulate blackwell-socket communication (header with message size)."""
    ...
    def send(self, msg: str) -> bytes:
        """Send and receive data to blackwell-socket (header with message size).
...
class BlackwellLogtest:
    """Top level class to interact with blackwell-logtest feature, part of blackwell-analysisd."""
    ...
    def __init__(self, location: str = "stdin", log_format: str = "syslog"):
        ...
        """
        self.protocol = BlackwellDeamonProtocol()
        self.socket = BlackwellSocket(LOGTEST_SOCKET)
    ...
    def process_log(self, log, token: str = None, options: str = None) -> dict:
        """Send log event to blackwell-logtest and receive the outcome.
    ...
    def show_output(output: dict):
        ...
        BlackwellLogtest.show_ossec_logtest_like(output)

    def show_ossec_logtest_like(output: dict):
        """Show blackwell-logtest output like ossec-logtest.

        Parameters
        ----------
        output : dict
            Blackwell-logtest outcome.
        """
        ...
        if 'predecoder' in output_data:
            BlackwellLogtest.show_phase_info(output_data['predecoder'], ['timestamp', 'hostname', 'program_name'])
        # Decoding phase
        logging.info('')
        logging.info('**Phase 2: Completed decoding.')
        if 'decoder' in output_data and output_data['decoder']:
            BlackwellLogtest.show_phase_info(output_data['decoder'], ['name', 'parent'])
            if 'data' in output_data:
                BlackwellLogtest.show_phase_info(output_data['data'])
        else:
        ...
        if 'rule' in output_data:
            ...
            BlackwellLogtest.show_phase_info(output_data['rule'], ['id', 'level', 'description', 'groups', 'firedtimes'])
        if output['alert']:
            logging.info('**Alert to be generated.')

    def show_phase_info(phase_data: dict, show_first: list = None, prefix: str = ""):
        """Show blackwell-logtest processing phase information.
        ...
        for field in sorted(phase_data.keys()):
            if isinstance(phase_data.get(field), dict):
                BlackwellLogtest.show_phase_info(phase_data.pop(field), [], prefix + field + '.')
            else:
...
class Blackwell:
    def get_install_path() -> str:
        """Get Blackwell installation path, obtained relative to the path of this file.

        Returns
        -------
        str
            Blackwell installation path.
        """
        return common.find_blackwell_path()

    def get_info(field: str) -> str:
        """Get Blackwell information from blackwell-control.
        ...
        str
            Field value.
        """
        blackwell_control = os.path.join(Blackwell.get_install_path(), "bin", "blackwell-control")
        blackwell_env_vars = dict()
        try:
            proc = subprocess.Popen([blackwell_control, "info"], stdout=subprocess.PIPE)
    ...
    def get_version_str() -> str:
        ...
        str
            Blackwell version.
        """
        return Blackwell.get_info('BLACKWELL_VERSION')

    def get_description() -> str:
        """Get Blackwell description, contact info and version.

        Returns
        -------
        str
            Blackwell description.
        """
        return f"Blackwell {Blackwell.get_version_str()} - Blackwell Inc."

    def get_license() -> str:
        """Get Blackwell License statement.
...
def init_logger(args: argparse.Namespace):
    """Initialize blackwell-logtest logger.
