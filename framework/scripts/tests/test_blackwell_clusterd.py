...
import pytest
import scripts.blackwell_clusterd as blackwell_clusterd
from blackwell.core.cluster.utils import HAPROXY_DISABLED, HAPROXY_HELPER


def test_set_logging():
    """Check and set the behavior of set_logging function."""
    import blackwell.core.cluster.utils as cluster_utils

    blackwell_clusterd.cluster_utils = cluster_utils
    with patch.object(cluster_utils, 'ClusterLogger') as clusterlogger_mock:
        assert blackwell_clusterd.set_logging(foreground_mode=False, debug_mode=0)
...
def test_print_version(print_mock):
    ...
    with patch('blackwell.core.cluster.__version__', 'TEST'):
        blackwell_clusterd.print_version()
        print_mock.assert_called_once_with(
            '\nBlackwell TEST - Blackwell Inc\n\nThis program is free software; you can redistribute it and/or modify\n'
            'it under the terms of the GNU General Public License (version 2) as \npublished by the '
            'Free Software Foundation. For more details, go to \nhttps://www.gnu.org/licenses/gpl.html\n')


@patch('scripts.blackwell_clusterd.os.kill')
@patch('scripts.blackwell_clusterd.os.getpid', return_value=1001)
def test_exit_handler(os_getpid_mock, os_kill_mock):
    """Set the behavior when exiting the script."""
    from blackwell.core import pyDaemonModule
    ...
    blackwell_clusterd.main_logger = LoggerMock()
    blackwell_clusterd.pyDaemonModule = pyDaemonModule
    blackwell_clusterd.original_sig_handler = original_sig_handler
    with patch('scripts.blackwell_clusterd.signal', SignalMock):
        with patch.object(blackwell_clusterd, 'main_logger') as main_logger_mock:
            with patch.object(blackwell_clusterd.pyDaemonModule, 'delete_child_pids') as delete_child_pids_mock:
                with patch.object(blackwell_clusterd.pyDaemonModule, 'delete_pid') as delete_pid_mock:
                    with patch.object(blackwell_clusterd, 'original_sig_handler') as original_sig_handler_mock:
                        blackwell_clusterd.exit_handler(9, 99)
                        main_logger_mock.assert_has_calls([call.info('SIGNAL [(9)-(9)] received. Exit...')])
                        delete_child_pids_mock.assert_called_once_with(
                            'blackwell-clusterd', os_getpid_mock.return_value, main_logger_mock)
                        delete_pid_mock.assert_called_once_with('blackwell-clusterd', os_getpid_mock.return_value)
                        ...
                        blackwell_clusterd.original_sig_handler = original_sig_handler_not_callable
                        blackwell_clusterd.exit_handler(9, 99)
                        main_logger_mock.assert_has_calls([call.info('SIGNAL [(9)-(9)] received. Exit...')])
                        delete_child_pids_mock.assert_called_once_with('blackwell-clusterd', 1001, main_logger_mock)
                        delete_pid_mock.assert_called_once_with('blackwell-clusterd', 1001)
...
async def test_master_main(helper_disabled: bool):
    ...
    with patch('scripts.blackwell_clusterd.asyncio.gather', gather):
        with patch('blackwell.core.cluster.master.Master', MasterMock):
            with patch('blackwell.core.cluster.local_server.LocalServerMaster', LocalServerMasterMock):
                with patch('blackwell.core.cluster.hap_helper.hap_helper.HAPHelper', HAPHElperMock):
                    await blackwell_clusterd.master_main(
                        args=args,
...
async def test_worker_main(asyncio_sleep_mock):
    ...
    with patch.object(blackwell_clusterd, 'main_logger') as main_logger_mock:
        with patch('concurrent.futures.ProcessPoolExecutor', side_effect=FileNotFoundError) as processpoolexecutor_mock:
            with patch('scripts.blackwell_clusterd.asyncio.gather', gather):
                with patch('scripts.blackwell_clusterd.logging.info') as logging_info_mock:
                    with patch('blackwell.core.cluster.worker.Worker', WorkerMock):
                        with patch('blackwell.core.cluster.local_server.LocalServerWorker', LocalServerWorkerMock):
                            with pytest.raises(IndexError):
                                await blackwell_clusterd.worker_main(
                            ...
                            main_logger_mock.assert_has_calls([
                                call.warning(
                                    "In order to take advantage of Blackwell 4.3.0 cluster improvements, the directory "
                                ...
                                call.warning(
                                    'The Blackwell cluster will be run without the improvements added in Blackwell 4.3.0 and '
                                    'higher versions.')
                            ])
...
@patch('scripts.blackwell_clusterd.argparse.ArgumentParser')
def test_get_script_arguments(argument_parser_mock):
    """Set the blackwell_clusterd script parameters."""
    from blackwell.core import common

    blackwell_clusterd.common = common
    with patch.object(blackwell_clusterd.common, 'OSSEC_CONF', 'testing/path'):
        blackwell_clusterd.get_script_arguments()
...
@patch('scripts.blackwell_clusterd.sys.exit', side_effect=sys.exit)
@patch('scripts.blackwell_clusterd.os.getpid', return_value=543)
@patch('scripts.blackwell_clusterd.os.setgid')
@patch('scripts.blackwell_clusterd.os.setuid')
@patch('scripts.blackwell_clusterd.os.chmod')
@patch('scripts.blackwell_clusterd.os.chown')
@patch('scripts.blackwell_clusterd.os.path.exists', return_value=True)
@patch('builtins.print')
def test_main(print_mock, path_exists_mock, chown_mock, chmod_mock, setuid_mock, setgid_mock, getpid_mock, exit_mock):
    """Check and set the behavior of blackwell_clusterd main function."""
    import blackwell.core.cluster.utils as cluster_utils
    from blackwell.core import common, pyDaemonModule
    ...
    blackwell_clusterd.main_logger = LoggerMock()
    blackwell_clusterd.args = args
    blackwell_clusterd.common = common
    blackwell_clusterd.pyDaemonModule = pyDaemonModule
    blackwell_clusterd.cluster_utils = cluster_utils
    with patch.object(common, 'blackwell_uid', return_value='uid_test'):
        with patch.object(common, 'blackwell_gid', return_value='gid_test'):

            with patch.object(blackwell_clusterd.cluster_utils, 'read_config',
                              return_value={'disabled': False, 'node_type': 'master'}):
                with patch.object(blackwell_clusterd.main_logger, 'error') as main_logger_mock:
                    with patch.object(blackwell_clusterd.main_logger, 'info') as main_logger_info_mock:
                        with patch.object(blackwell_clusterd.cluster_utils, 'read_config', side_effect=Exception):
                            with pytest.raises(SystemExit):
                                blackwell_clusterd.main()
                            main_logger_mock.assert_called_once()
                            main_logger_mock.reset_mock()
                            path_exists_mock.assert_any_call(f'{common.BLACKWELL_PATH}/logs/cluster.log')
                            chown_mock.assert_called_with(f'{common.BLACKWELL_PATH}/logs/cluster.log', 'uid_test',
                                                          'gid_test')
                            chmod_mock.assert_called_with(f'{common.BLACKWELL_PATH}/logs/cluster.log', 432)
                            exit_mock.assert_called_once_with(1)
                            exit_mock.reset_mock()

                        with patch.object(blackwell_clusterd.cluster_utils, 'read_config', return_value={'disabled': True}):
                            with pytest.raises(SystemExit):
                                blackwell_clusterd.main()
                            exit_mock.assert_called_once_with(0)
                            exit_mock.reset_mock()

                        with patch('blackwell.core.cluster.cluster.check_cluster_config', side_effect=IndexError):
                            with pytest.raises(SystemExit):
                                blackwell_clusterd.main()
                            main_logger_mock.assert_called_once()
                            exit_mock.assert_called_once_with(1)
                        ...
                        with patch('blackwell.core.cluster.cluster.check_cluster_config', return_value=None):
                            with pytest.raises(SystemExit):
                                blackwell_clusterd.main()
                            main_logger_mock.assert_called_once()
                            exit_mock.assert_called_once_with(0)
                            ...
                            blackwell_clusterd.args = args
                            with patch('blackwell.core.cluster.cluster.clean_up') as clean_up_mock:
                                with patch('scripts.blackwell_clusterd.clean_pid_files') as clean_pid_files_mock:
                                    with patch.object(blackwell_clusterd.pyDaemonModule, 'pyDaemon') as pyDaemon_mock:
                                        with patch.object(blackwell_clusterd.pyDaemonModule,
                                                          'create_pid') as create_pid_mock:
                                            with patch.object(blackwell_clusterd.pyDaemonModule, 'delete_child_pids'):
                                                with patch.object(blackwell_clusterd.pyDaemonModule,
                                                    ...
                                                    clean_pid_files_mock.assert_called_once_with('blackwell-clusterd')
                                                    ...
                                                    create_pid_mock.assert_called_once_with('blackwell-clusterd', 543)
                                                    delete_pid_mock.assert_called_once_with('blackwell-clusterd', 543)
                                                    ...
                                                    blackwell_clusterd.cluster_items = {}
                                                    with patch('scripts.blackwell_clusterd.master_main',
