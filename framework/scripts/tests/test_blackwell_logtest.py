...
import scripts.blackwell_logtest as blackwell_logtest


class BlackwellSocketMock:
    """Auxiliary mock."""
...
class BlackwellDeamonProtocolMock:
    """Auxiliary mock."""
...
def test_init_argparse(argument_parser_mock):
    ...
    blackwell_logtest.init_argparse()
...
@patch('scripts.blackwell_logtest.init_logger')
@patch('scripts.blackwell_logtest.BlackwellLogtest')
@patch('scripts.blackwell_logtest.init_argparse')
@patch('builtins.input', return_value='mock')
def test_main(input_mock, argparse_mock, blackwell_logtest_class_mock, init_logger_mock, register_mock, logger_warning_mock,
    ...
    blackwell_logtest_class_mock.return_value = BlackwellLogtestMock()

    # Test the first 'try' present in the 'while'
    try:
        blackwell_logtest.main()
    ...
    register_mock.assert_called_once_with(blackwell_logtest_class_mock.return_value.remove_last_session)
    sys_exit_mock.assert_has_calls([call(0), call(1)])

    logger_info_mock.assert_has_calls([call('%s', 'Blackwell ERROR - Blackwell Inc.'),
                                       call('%s', '\nThis program is free software; you can redistribute it and/or '
                                                  'modify\nit under the terms of the GNU General Public License '
                                                  '(version 2) as\npublished by the Free Software Foundation. For more '
                                                  'details, go to\nhttps://www.gnu.org/licenses/gpl.html\n'),
                                       call('Starting blackwell-logtest %s', 'ERROR'),
                                       call('Type one log per line')])
    logger_error_mock.assert_called_once_with('Unit test configuration wrong syntax: %s',
                                              blackwell_logtest_class_mock.return_value.get_last_ut())
    logger_warning_mock.assert_has_calls([call('** Blackwell-Logtest: %s', 'WARNING'), call('')])

    assert blackwell_logtest_class_mock.return_value.show_last_ut_result_called is True
    assert blackwell_logtest_class_mock.return_value.remove_last_session_called is False
    assert blackwell_logtest_class_mock.return_value.get_last_ut_called is True
    assert blackwell_logtest_class_mock.return_value.process_log_called is True
    ...
    blackwell_logtest_class_mock.return_value = BlackwellLogtestMock(True)
    ...
    blackwell_logtest_class_mock.return_value = BlackwellLogtestMock(process_log_exception='ValueError')
    ...
    logger_error_mock.assert_called_once_with('** Blackwell-logtest error ')

    # Test the third exception
    logger_error_mock.reset_mock()
    blackwell_logtest_class_mock.return_value = BlackwellLogtestMock(process_log_exception='ConnectionError')
    logger_error_mock.side_effect = Exception()
    ...
    logger_error_mock.assert_called_once_with('** Blackwell-logtest error when connecting with blackwell-analysisd')


# Test BlackwellDaemonProtocol class methods

def create_blackwell_daemon_protocol_class():
    """Create new BlackwellDaemonProtocol class."""
    return blackwell_logtest.BlackwellDeamonProtocol()


def test_wdp_init():
    """Test the init method, checking the initial status of its attributes."""
    wdp = create_blackwell_daemon_protocol_class()

    assert isinstance(wdp.protocol, dict)
    assert wdp.protocol['version'] == 1
    assert isinstance(wdp.protocol['origin'], dict)
    assert wdp.protocol['origin']['name'] == 'blackwell-logtest'
    assert wdp.protocol['origin']['module'] == 'blackwell-logtest'


@patch('json.dumps', return_value='')
def test_wdp_wrap(json_dumps_mock):
    """Test if the data is being properly wrapped with blackwell daemon protocol information."""
    wdp = create_blackwell_daemon_protocol_class()
    assert wdp.wrap(command='command', parameters={'parameters': 'parameters'}) == json_dumps_mock.return_value
    json_dumps_mock.assert_called_once_with({'version': 1, 'origin': {'name': 'blackwell-logtest', 'module':
        'blackwell-logtest'}, 'command': 'command', 'parameters':
                                                 {'parameters': 'parameters'}})


@patch('json.loads', return_value={'error': 'error', 'message': 'message'})
def test_wdp_unwrap(json_loads_mock):
    """Test if the data is being properly unwrapped from the blackwell daemon protocol."""
    wdp = create_blackwell_daemon_protocol_class()
    msg = {}
...
# Test BlackwellSocket methods

def create_blackwell_socket_class(file):
    """Create a new BlackwellSocket class."""
    return blackwell_logtest.BlackwellSocket(file=file)


def test_ws_init():
    """Test the correct start of the BlackwellSocket class."""
    file = ''
    assert create_blackwell_socket_class(file).file == file
...
def test_ws_sent(unpack_mock, pack_mock, msg_waitall_mock, stream_mock, unix_mock, socket_socket_mock):
    ...
    ws = create_blackwell_socket_class(file=file)
...
# Test BlackwellLogtest class methods

@patch('scripts.blackwell_logtest.BlackwellSocket', return_value=BlackwellSocketMock())
@patch('scripts.blackwell_logtest.LOGTEST_SOCKET')
@patch('scripts.blackwell_logtest.BlackwellDeamonProtocol', return_value=BlackwellDeamonProtocolMock())
def create_blackwell_logtest_class(blackwell_deamon_mock, logtest_socket_mock, blackwell_socket_mock):
    """Create new BlackwellLogtest class."""
    return blackwell_logtest.BlackwellLogtest()


def test_wl_init():
    """Test the correct initialization of BlackwellLogtest class."""
    wl = create_blackwell_logtest_class()

    assert isinstance(wl.protocol, BlackwellDeamonProtocolMock)
    assert isinstance(wl.socket, BlackwellSocketMock)
...
@patch('scripts.blackwell_logtest.BlackwellLogtest.remove_session')
...
@patch('logging.debug')
@patch('json.dumps', return_value='')
@patch('scripts.blackwell_logtest.BlackwellLogtest.show_ossec_logtest_like')
...
@patch('logging.info')
@patch('scripts.blackwell_logtest.BlackwellLogtest.show_phase_info')
def test_wl_show_ossec_logtest_like(show_phase_info_mock, info_mock):
    """Test if blackwell-logtest output is being shown as ossec-logtest output."""
    ...
    # Test the third 'if'
    blackwell_logtest.BlackwellLogtest.show_ossec_logtest_like(output)
...
def test_wl_show_phase_info(info_mock):
    """Check if blackwell-logtest is processing phase information."""
    phase_data = {'key': 'value', 'key2': 'value2', 'key3': {'1': '2'}}
    show_first = ['key']

    blackwell_logtest.BlackwellLogtest.show_phase_info(phase_data=phase_data, show_first=show_first)
    info_mock.assert_has_calls([call("\t%s: '%s'", 'key', 'value'), call("\t%s: '%s'", 'key2', 'value2')])


@patch('logging.info')
@patch('scripts.blackwell_logtest.BlackwellLogtest.get_last_ut', return_value='mock')
...
# Test Blackwell class

def create_blackwell_class():
    """Auxiliary function to create a Blackwell class."""
    return blackwell_logtest.Blackwell


@patch('scripts.blackwell_logtest.common.find_blackwell_path', return_value='')
def test_blackwell_get_install_path(find_blackwell_path_mock):
    """Test is we can get the installation path correctly."""
    assert create_blackwell_class().get_install_path() == find_blackwell_path_mock.return_value


@patch('subprocess.PIPE')
@patch('subprocess.Popen')
@patch('os.path.join', return_value='')
@patch('scripts.blackwell_logtest.Blackwell.get_install_path', return_value='')
def test_blackwell_get_info(get_install_mock, join_mock, popen_mock, pipe_mock):
    """Check if we can properly obtain information from blackwell-control."""
    ...
    blackwell = create_blackwell_class()
    popen_mock.return_value = ProcMock()
    field = 'key'

    # Test the 'try'
    assert blackwell.get_info(field=field) == field
    get_install_mock.assert_called_once_with()
    join_mock.assert_called_once_with(get_install_mock.return_value, 'bin', 'blackwell-control')
    ...
    assert blackwell.get_info(field=field) == 'ERROR'


@patch('scripts.blackwell_logtest.Blackwell.get_info', return_value='')
def test_blackwell_get_version_str(get_info_mock):
    """Test if the version is being properly retrieved."""
    assert create_blackwell_class().get_version_str() == get_info_mock.return_value
    get_info_mock.assert_called_once_with('BLACKWELL_VERSION')


@patch('scripts.blackwell_logtest.Blackwell.get_version_str', return_value='')
def test_blackwell_get_description(get_version_mock):
    """Test if the description is being properly retrieved."""
    assert create_blackwell_class().get_description() == f"Blackwell {get_version_mock.return_value} - Blackwell Inc."
    get_version_mock.assert_called_once_with()


@patch('textwrap.dedent', return_value='')
def test_blackwell_get_license(dedent_mock):
    """Test that the license was not changed."""
    assert create_blackwell_class().get_license() == dedent_mock.return_value
...
def test_init_logger(logging_mock):
    ...
    blackwell_logtest.init_logger(ArgsMock())
