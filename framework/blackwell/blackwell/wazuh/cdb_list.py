...
from blackwell.core.cdb_list import iterate_lists, get_list_from_file, REQUIRED_FIELDS, SORT_FIELDS, delete_list, \
...
from blackwell.core.utils import process_array, safe_move, delete_file_with_backup, upload_file, to_relative_path
...
def get_lists(filename: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, select: list = None,
              ...
              relative_dirname: str = None, q: str = None, distinct: bool = False) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Lists content.
    """
    result = AffectedItemsBlackwellResult(all_msg='All specified lists were returned',
                                      some_msg='Some lists were not returned',
                                      none_msg='No list was returned')
    dirname = join(common.BLACKWELL_PATH, relative_dirname) if relative_dirname else None
...
@expose_resources(actions=['lists:read'], resources=['list:file:{filename}'])
def get_list_file(filename: list = None, raw: bool = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        CDB list content.
    """
    result = AffectedItemsBlackwellResult(all_msg='CDB list was returned',
                                      none_msg='No list was returned')

    try:
        # Recursively search for filename inside {blackwell_path}/etc/lists/
...
def upload_list_file(filename: str = None, content: str = None, overwrite: bool = False) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='CDB list file uploaded successfully',
                                      none_msg='Could not upload CDB list file')
    full_path = join(common.USER_LISTS_PATH, filename)
    backup_file = ''

    try:
        # Raise BlackwellError if CDB list is not valid
...
def delete_list_file(filename: list) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Confirmation message.
    """
    result = AffectedItemsBlackwellResult(all_msg='CDB list file was successfully deleted',
...
def get_path_lists(filename: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
                   ...
                   search_in_fields: str = None, relative_dirname: str = None) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='All specified paths were returned',
    ...
    for item in list(lists):
        if any([relative_dirname is not None and item['relative_dirname'] != relative_dirname,
                join(common.BLACKWELL_PATH, item['relative_dirname'], item['filename']) not in paths]):
            lists.remove(item)
