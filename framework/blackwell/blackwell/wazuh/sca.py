...
from blackwell.core.agent import get_agents_info
...
from blackwell.core.sca import (
    BlackwellDBQueryDistinctSCACheck, BlackwellDBQuerySCA, BlackwellDBQuerySCACheck, BlackwellDBQuerySCACheckIDs,
    BlackwellDBQuerySCACheckRelational, SCA_CHECK_COMPLIANCE_DB_FIELDS, SCA_CHECK_RULES_DB_FIELDS, SCA_CHECK_DB_FIELDS)
...
def get_sca_list(agent_list: list = None, q: str = "", offset: int = 0, limit: int = common.DATABASE_LIMIT,
                 ...
                 filters: dict = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected SCA information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            with BlackwellDBQuerySCA(agent_id=agent_list[0], offset=offset, limit=limit, sort=sort, search=search,
            ...
            result.add_failed_item(id_=agent_list[0], error=BlackwellResourceNotFound(1701))
...
def get_sca_checks(policy_id: str = None, agent_list: list = None, q: str = "", offset: int = 0,
                   ...
                   filters: dict = None, distinct: bool = False) -> AffectedItemsBlackwellResult:
    ...
    BlackwellInternalError(2007)
        If there was an error retrieving data from Blackwell DB.
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected SCA/policy information was returned',
    ...
    if len(agent_list) != 0:
        if agent_list[0] in get_agents_info():
            ...
            if not distinct:
                ...
                with BlackwellDBQuerySCACheckIDs(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
                ...
                if id_check_list:
                    ...
                    if not select or select_compliance:
                        with BlackwellDBQuerySCACheckRelational(
            ...
            # Workaround for distinct=True
                ...
                with BlackwellDBQueryDistinctSCACheck(agent_id=agent_list[0], offset=offset, limit=limit, filters=filters,
