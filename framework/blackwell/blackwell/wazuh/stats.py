...
from blackwell.core.exception import BlackwellException, BlackwellResourceNotFound
from blackwell.core.results import AffectedItemsBlackwellResult
from blackwell.core.stats import get_daemons_stats_, get_daemons_stats_socket, hourly_, totals_, weekly_
...
def totals(date: datetime.date) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='Statistical information for each node was successfully read',
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def hourly() -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='Statistical information per hour for each node was successfully read',
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def weekly() -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='Statistical information per week for each node was successfully read',
...
def get_daemons_stats_agents(daemons_list: list = None, agent_list: list = None):
    ...
    daemon_socket_mapping = {'blackwell-remoted': common.REMOTED_SOCKET,
                             'blackwell-analysisd': common.ANALYSISD_SOCKET}
    result = AffectedItemsBlackwellResult(all_msg='Statistical information for each daemon was successfully read',
    ...
    if agent_list:
        if 'all' not in agent_list:
            ...
            with BlackwellDBQueryAgents(limit=None, select=["id", "status"], **rbac_filters) as db_query:
                data = db_query.run()
            ...
            with contextlib.suppress(KeyError):
                ...
                result.add_failed_item('000', exception.BlackwellError(1703))

            # Add non-existent agents to failed_items
            not_found_agents = agent_list - system_agents
            [result.add_failed_item(id_=agent, error=exception.BlackwellResourceNotFound(1701)) for agent in
             not_found_agents]

            # Add non-active agents to failed_items
            non_active_agents = [agent['id'] for agent in data['items'] if agent['status'] != 'active']
            [result.add_failed_item(id_=agent, error=exception.BlackwellError(1707)) for agent in non_active_agents]
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_daemons_stats(daemons_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    daemon_socket_mapping = {'blackwell-remoted': common.REMOTED_SOCKET,
                             'blackwell-analysisd': common.ANALYSISD_SOCKET,
                             'blackwell-db': common.WDB_SOCKET}
    result = AffectedItemsBlackwellResult(all_msg='Statistical information for each daemon was successfully read',
                                      some_msg='Could not read statistical information for some daemons',
                                      none_msg='Could not read statistical information for any daemon')
...
def get_agents_component_stats_json(agent_list: list = None, component: str = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Component stats.
    """
    result = AffectedItemsBlackwellResult(all_msg='Statistical information for each agent was successfully read',
    ...
    for agent_id in agent_list:
        try:
            if agent_id not in system_agents:
                raise exception.BlackwellResourceNotFound(1701)
