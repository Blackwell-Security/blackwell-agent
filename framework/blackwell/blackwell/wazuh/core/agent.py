...
from blackwell.core import common, configuration, stats
from blackwell.core.InputValidator import InputValidator
from blackwell.core.cluster.utils import get_manager_status
from blackwell.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from blackwell.core.exception import BlackwellException, BlackwellError, BlackwellInternalError, BlackwellResourceNotFound
from blackwell.core.utils import BlackwellVersion, plain_dict_to_nested_dict, get_fields_to_nest, BlackwellDBQuery, \
    BlackwellDBQueryDistinct, BlackwellDBQueryGroupBy, BlackwellDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from blackwell.core.blackwell_queue import BlackwellQueue
from blackwell.core.blackwell_socket import BlackwellSocket, BlackwellSocketJSON, create_blackwell_socket_message
from blackwell.core.wdb import BlackwellDBConnection
...
class BlackwellDBQueryAgents(BlackwellDBQuery):
    """Class used to query Blackwell agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_blackwell_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = BlackwellDBBackend(query_format='global')
        BlackwellDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the Blackwell query."""
        BlackwellDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return BlackwellDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the Blackwell query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            BlackwellDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise BlackwellError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            BlackwellDBQuery._process_filter(self, field_name, field_filter, q_filter)


class BlackwellDBQueryGroup(BlackwellDBQuery):
    """Class used to query Blackwell groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = BlackwellDBBackend(query_format='global')
        BlackwellDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class BlackwellDBQueryDistinctAgents(BlackwellDBQueryDistinct, BlackwellDBQueryAgents):
    pass


class BlackwellDBQueryGroupByAgents(BlackwellDBQueryGroupBy, BlackwellDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        BlackwellDBQueryAgents.__init__(self, *args, **kwargs)
        BlackwellDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return BlackwellDBQuery._format_data_into_dictionary(self)


class BlackwellDBQueryMultigroups(BlackwellDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        BlackwellDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        Raises
        ------
        BlackwellResourceNotFound(1701)
            Agent does not exist.
        """
        with BlackwellDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
                                query="id={}".format(self.id), count=False, get_data=True,
                                remove_extra_fields=False) as db_query:
            try:
                data = db_query.run()['items'][0]
            except IndexError:
                raise BlackwellResourceNotFound(1701)
    ...
    def get_key(self) -> str:
        ...
        if self.id != "000":
            ...
            raise BlackwellError(1703)

        return self.key

    def reconnect(self, wq: BlackwellQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : BlackwellQueue
            BlackwellQueue used for the active response message.
        ...
        -------
        str
            Message generated by Blackwell.
        """
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise BlackwellError(1707)

        # Send force reconnect message to the BlackwellQueue
        ret_msg = wq.send_msg_to_agent(BlackwellQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        str
            Message generated by Blackwell.
        """
        # Check that blackwell-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except BlackwellInternalError as e:
            # blackwell-authd is not running due to a problem with /proc availability
            raise BlackwellError(1726, extra_message=str(e))

        if manager_status.get('blackwell-authd') != 'running':
            # blackwell-authd is not running
            raise BlackwellError(1726)
        ...
        try:
            ...
            raise BlackwellInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = BlackwellSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise BlackwellError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise BlackwellInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to Blackwell using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        try:
            ...
            if e.code == 9008:
                raise BlackwellError(1705, extra_message=name)
            elif e.code == 9007:
                raise BlackwellError(1706, extra_message=ip)
            elif e.code == 9012:
                raise BlackwellError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = BlackwellDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = BlackwellDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        BlackwellResourceNotFound(1710)
        ...
        if not force:
            ...
            if not Agent.group_exists(group_id):
                raise BlackwellResourceNotFound(1710)
        ...
        if group_id not in group_list:
            raise BlackwellError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise BlackwellError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or Blackwell vX.Y.Z.
        ...
        """
        if BlackwellVersion(agent_version) < BlackwellVersion(common.ACTIVE_CONFIG_VERSION):
            raise BlackwellInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        BlackwellInternalError(1015)
            Agent version is null.
        BlackwellInternalError(1735)
        ...
        if self.version is None:
            raise BlackwellInternalError(1015)
        agent_version = BlackwellVersion(self.version.split(" ")[1])
        required_version = BlackwellVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise BlackwellInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_blackwell_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_blackwell_version_format(filters: dict):
    """Verify and format the specified blackwell version into the 'blackwell vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"blackwell {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^blackwell \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: BlackwellQueue = None) -> str:
    ...
    wq : BlackwellQueue
        BlackwellQueue used for the active response messages.
    ...
    str
        Message generated by Blackwell.
    """
    # If the Blackwell agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if BlackwellVersion(agent_version) >= BlackwellVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(BlackwellQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(BlackwellQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_blackwell_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_blackwell_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = BlackwellSocket(common.UPGRADE_SOCKET)
