...
class AsyncBlackwellDBConnection:
    """
    ...
    async def _send(self, msg, raw=False):
        """Format and send message to blackwell-db socket without blocking event loop.

        Parameters
        ----------
        msg : str
            Message to be sent to blackwell-db.
        raw : bool
            If `True`, the status message from blackwell-db is included in the response.

        Returns
        -------
        str, list
            Result for the request sent to blackwell-db.
        """
        try:
            ...
            try:
                ...
                raise BlackwellInternalError(2010, extra_message=e)

            if raw:
                return data
            elif data[0] == "err":
                raise BlackwellError(2003, data[1])
            else:
                return json.loads(data[1], object_hook=BlackwellDBConnection.json_decoder)
        except (FileNotFoundError, ConnectionError) as e:
            with contextlib.suppress(Exception):
                await self.open_connection()
            raise BlackwellInternalError(2005, extra_message=e)
    ...
    async def run_wdb_command(self, command):
        ...
        ----------
        command : str
            Command to be executed inside blackwell-db
        ...
        if len(result) > 1:
            if result[0] == 'err':
                raise BlackwellInternalError(2007, extra_message=result[1])

        else:
            if result[0] != 'ok':
                raise BlackwellInternalError(2007)

        return result


class BlackwellDBConnection:
    """
    ...
    def __init__(self, request_slice=500):
        ...
        ----------
        request_slice : int
            Maximum number of items to request from blackwell-db on the first call.
        """
        self.socket_path = common.WDB_SOCKET
        ...
        try:
            ...
            raise BlackwellInternalError(2005, e)
    ...
    def __query_input_validation(self, query: str):
        ...
        for check, error_text in input_val_errors:
            if not check:
                raise BlackwellError(2004, error_text)
    ...
    def _send(self, msg: str, raw: bool = False) -> dict:
        ...
        Raises
        ------
        BlackwellInternalError(2009)
            Pagination error. Response from blackwell-db was over the maximum socket buffer size.
        BlackwellError(2003)
        ...
        if data_size >= MAX_SOCKET_BUFFER_SIZE:
            raise BlackwellInternalError(2009)
        ...
        if data[0] == "err":
            ...
            return BlackwellDBConnection.loads(data[1])
    ...
    def loads(string: str) -> dict:
        ...
        ----------
        string : str
            String response from `blackwell-db`. It must be a dumped JSON.
        ...
        dict
            JSON object.
        """
        data = json.loads(string, object_hook=BlackwellDBConnection.json_decoder)
        if '"(null)"' in string:
    ...
    def delete_agents_db(self, agents_id: List[str]) -> dict:
        """Delete agents db through blackwell-db service.
        ...
        -------
        dict
            Dict received from blackwell db in the form: {"agents": {"ID": "MESSAGE"}}, where MESSAGE may be one of the
            following:
                - Ok
                - Invalid agent ID
                ...
                - DB not found
        """
        return self._send(f"blackwelldb remove {' '.join(agents_id)}")
    ...
    def send(self, query: str, raw: bool = True) -> Union[str, dict]:
        ...
        ----------
        query : str
            Query to be executed in blackwell-db.
        raw : bool
    ...
    def execute(self, query, count=False, delete=False, update=False):
        ...
        def send_request_to_wdb(query_lower, step, off, response):
            try:
                ...
                response.extend(BlackwellDBConnection.loads(request_response))
        ...
        if delete:
            ...
            if regex.match(query_lower) is None:
                raise BlackwellError(2004, "Delete query is wrong")
        ...
        if update:
            regex = re.compile(r"\w+ \d+? sql update ([\w\d,*_ ]+) set value = '([\w\d,*_ ]+)' where key (=|like)?"
                               r" '([a-z0-9,*_%\- ]+)'")
            if regex.match(query_lower) is None:
                raise BlackwellError(2004, "Update query is wrong")
            return self._send(query_lower)
        ...
        if not re.search(r'.?select count\([\w \*]+\)( as [^,]+)? from', query_without_where):
            ...
            try:
                ...
                raise BlackwellError(2006, str(e))
            except (BlackwellError, BlackwellInternalError) as e:
                raise e
            except Exception as e:
                raise BlackwellInternalError(2007, str(e))
