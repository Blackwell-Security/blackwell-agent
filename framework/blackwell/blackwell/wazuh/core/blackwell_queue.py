...
from blackwell.core.common import origin_module
...
def create_blackwell_queue_socket_msg(flag: str, str_agent_id: str, msg: str, is_restart: bool = False) -> str:
    """Create message that will be sent to the BlackwellQueue socket.
    ...
    -------
    str
        Message that will be sent to the BlackwellQueue socket.
    """
...
class BaseQueue:
    ...
    def _connect(self):
        try:
            ...
            if length_send_buffer < BlackwellQueue.MAX_MSG_SIZE:
                self.socket.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, BlackwellQueue.MAX_MSG_SIZE)
        except Exception:
            raise BlackwellInternalError(1010, self.path)
    ...
    def _send(self, msg: bytes) -> None:
        ...
        try:
            ...
            if sent == 0:
                raise BlackwellInternalError(1011, self.path)
        except socket.error:
            raise BlackwellInternalError(1011, self.path)
...
class BlackwellQueue(BaseQueue):
    """
    BlackwellQueue Object.
    """
    ...
    RESTART_AGENTS_JSON = json.dumps(create_blackwell_socket_message(origin={'module': origin_module.get()},
    ...
    def send_msg_to_agent(self, msg: str = '', agent_id: str = '', msg_type: str = '') -> str:
        ...
        msg_is_no_ar = msg in [BlackwellQueue.HC_SK_RESTART, BlackwellQueue.HC_FORCE_RECONNECT]
        msg_is_restart = msg in [BlackwellQueue.RESTART_AGENTS, BlackwellQueue.RESTART_AGENTS_JSON]
        ...
        # AR
        if msg_type == BlackwellQueue.AR_TYPE:
            socket_msg = create_blackwell_queue_socket_msg(flag, str_agent_id, msg) if agent_id != '000' else msg
            # Return message
        ...
        # Restart agents
            ...
            # If msg is not a non active-response command and not a restart command, raises BlackwellInternalError
            if not msg_is_no_ar and not msg_is_restart:
                raise BlackwellInternalError(1012, msg)
            socket_msg = create_blackwell_queue_socket_msg(flag, str_agent_id, msg, is_restart=msg_is_restart)
            # Return message
            if msg == BlackwellQueue.HC_SK_RESTART:
                ret_msg = "Restarting Syscheck on agent" if agent_id else "Restarting Syscheck on all agents"
            elif msg == BlackwellQueue.HC_FORCE_RECONNECT:
                ret_msg = "Reconnecting agent" if agent_id else "Reconnecting all agents"
            else:  # msg == BlackwellQueue.RESTART_AGENTS or msg == BlackwellQueue.RESTART_AGENTS_JSON
        ...
        try:
            ...
            raise BlackwellError(1014, extra_message=f": BlackwellQueue socket with path {self.path}")

        return ret_msg


class BlackwellAnalysisdQueue(BaseQueue):
    """
    BlackwellAnalysisdQueue Object.
    """
    ...
    def send_msg(self, msg_header: str, msg: str):
        ...
        try:
            ...
            raise BlackwellError(1014, extra_message=f': BlackwellAnalysisdQueue socket with path {self.path}. {str(e)}')
