...
from blackwell.core.cluster import client
...
class LocalClient(client.AbstractClientManager):
    ...
    def __init__(self):
        """Class constructor"""
        super().__init__(configuration=blackwell.core.cluster.utils.read_config(), enable_ssl=False, performance_test=0,
                         concurrency_test=0, file='', string=0, logger=logging.getLogger(), tag="Local Client",
                         cluster_items=blackwell.core.cluster.utils.get_cluster_items())
    ...
    async def start(self):
        ...
        try:
            self.transport, self.protocol = await loop.create_unix_connection(
                                             ...
                                             path=os.path.join(common.BLACKWELL_PATH, 'queue', 'cluster', 'c-internal.sock'))
        except (ConnectionRefusedError, FileNotFoundError):
            raise exception.BlackwellInternalError(3012)
        except MemoryError:
            raise exception.BlackwellInternalError(1119)
        except Exception as e:
            raise exception.BlackwellInternalError(3009, str(e))
        ...
        while True:
            ...
            try:
                ...
                if min_timeout < self.cluster_items['intervals']['worker']['keep_alive']:
                    raise exception.BlackwellInternalError(3020)
                else:
                    try:
