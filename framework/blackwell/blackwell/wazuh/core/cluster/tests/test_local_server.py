...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        from blackwell.core.cluster.local_server import *
...
async def test_LocalServerHandler_connection_made(event_loop):
    ...
    with patch.object(logger, "debug") as logger_debug_mock:
        with patch("blackwell.core.cluster.local_server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.local_server.server.AbstractServerHandler.process_request")
...
async def test_LocalServerHandler_get_ruleset_hashes(event_loop):
    ...
    lsh = LocalServerHandler(server=ServerMock(), loop=event_loop, fernet_key=None, cluster_items={})
    with patch("blackwell.core.cluster.local_server.cluster.get_ruleset_status",
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.local_server.LocalServerHandler.send_request", return_value='changed')
...
async def test_LocalServerHandlerMaster_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(BlackwellClusterError, match=".* 3022 .*"):
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.local_server.LocalServerHandler.process_request")
async def test_LocalServerHandlerWorker_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(BlackwellClusterError, match=".* 3023 .*"):
    ...
    with patch.object(ls.node.client, "send_request", return_value='') as send_request_mock:
        with patch.object(lshw, 'log_exceptions', return_value='') as log_exceptions_mock:
            with patch.object(lshw, 'get_api_response', return_value='') as callback_mock:
                assert lshw.send_request_to_master(command=b"test", arguments=b"blackwell") == \
                    (b"ok", b"Sent request to master node")
                await wait_function_called(callback_mock)
                await wait_function_called(log_exceptions_mock)
                send_request_mock.assert_called_once_with(b"test", b"blackwell")
