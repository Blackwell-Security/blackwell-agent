...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        from blackwell.core.cluster.local_client import *
        from blackwell.core.cluster.common import InBuffer
...
@patch("blackwell.core.cluster.client.asyncio.get_running_loop")
...
async def test_localclient_start():
    ...
    with patch("uvloop.Loop.create_unix_connection", side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch("os.path.join", return_value="path/test"):
            with patch("blackwell.core.cluster.client.asyncio.get_running_loop", return_value=mocked_loop):
                lc = LocalClient()
                await lc.start()
...
async def test_localclient_start_ko(mock_get_running_loop):
    ...
    with pytest.raises(BlackwellInternalError, match=r'.* 3009 .*'):
        await LocalClient().start()

    with patch("blackwell.core.cluster.local_client.os.path.join", side_effect=MemoryError):
        with pytest.raises(BlackwellInternalError, match=r'.* 1119 .*'):
            await LocalClient().start()

    with patch("blackwell.core.cluster.local_client.os.path.join", side_effect=FileNotFoundError):
        with pytest.raises(BlackwellInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()

    with patch("blackwell.core.cluster.local_client.os.path.join", side_effect=ConnectionRefusedError):
        with pytest.raises(BlackwellInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()
...
async def test_wait_for_response():
    ...
    lc.protocol.send_request.side_effect = [b"None", exception.BlackwellClusterError(3018)]

    with patch("asyncio.Event.wait", side_effect=asyncio.TimeoutError):
        with pytest.raises(BlackwellInternalError, match=rf".* 3020 .*"):
...
async def test_localclient_execute():
    ...
    with patch("blackwell.core.cluster.local_client.LocalClient.start"):
        with patch("blackwell.core.cluster.local_client.LocalClient.send_api_request", return_value="Test"):
...
async def test_localclient_send_file():
    ...
    with patch("blackwell.core.cluster.local_client.LocalClient.start"):
        with patch("blackwell.core.cluster.local_client.LocalClient.send_api_request", return_value=b"blackwell/test python"):
            lc = LocalClient()
            assert await lc.send_file(path="blackwell/test", node_name="python") == b"blackwell/test python"
