...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        import blackwell.core.cluster.client as client
...
def test_acm_init():
    ...
    assert abstract_client_manager.logger == logging.getLogger("blackwell")
...
@patch('blackwell.core.cluster.client.AbstractClientManager.add_tasks', return_value=[])
...
def test_ac_connection_result():
    ...
    with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
        ...
        with patch.object(abstract_client.transport, "close") as close_mock:
            ...
            future.set_result([BlackwellClusterError(3020)])
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(BlackwellClusterError(3020))}.")
            ...
            future = asyncio.Future()
            future.set_exception(BlackwellClusterError(3020))
            abstract_client.connection_result(future)
            logger_mock.assert_called_once_with(f"Could not connect to master: {str(BlackwellClusterError(3020))}.")
            close_mock.assert_called_once()

    # Check second condition
    with patch.object(logging.getLogger('blackwell'), "info") as logger_mock:
        abstract_client.transport = CloseMock()
        future = asyncio.Future()
...
@patch('blackwell.core.cluster.client.AbstractClient._cancel_all_tasks')
def test_ac_connection_lost(cancel_tasks_mock):
    ...
    with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
        abstract_client.connection_lost(exc=BlackwellException(1001))
        logger_mock.assert_called_once_with(f"Connection closed due to an unhandled error: {BlackwellException(1001)}\n",
                                            exc_info=False)
...
def test_ac_process_response():
    ...
    # Test the second condition
    with patch('blackwell.core.cluster.common.Handler.process_response', return_value=b'ok') as pr_mock:
...
def test_ac_process_request():
    ...
    # Test the fist condition
    with patch('blackwell.core.cluster.client.AbstractClient.echo_client', return_value=b'ok') as echo_mock:
        assert (abstract_client.process_request(command=b"echo-m", data=b"data") == b'ok')
        echo_mock.assert_called_once_with(b'data')

    # Test the second condition
    with patch('blackwell.core.cluster.common.Handler.process_request', return_value=b'ok') as pr_mock:
...
@patch('blackwell.core.cluster.client.AbstractClient.send_request', return_value=b"ok")
async def test_ac_client_echo_ok(send_request_mock, done_mock, asyncio_sleep_mock):
    ...
    # Test try
    with patch.object(logging.getLogger("blackwell"), "info") as logger_mock:
        with patch('blackwell.core.cluster.common.Handler.setup_task_logger',
                   return_value=logging.getLogger("blackwell")) as setup_logger_mock:
            try:
...
@patch.object(FutureMock, "done", return_value=False)
@patch('blackwell.core.cluster.client.perf_counter', return_value=0)
@patch('blackwell.core.cluster.client.AbstractClient.send_request', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('blackwell.core.cluster.client.perf_counter', return_value=0)
@patch('blackwell.core.cluster.client.AbstractClient.send_file', return_value="ok")
...
@freeze_time("2022-01-01")
@patch('blackwell.core.cluster.client.perf_counter', return_value=0)
@patch('blackwell.core.cluster.client.AbstractClient.send_string', return_value="ok")
