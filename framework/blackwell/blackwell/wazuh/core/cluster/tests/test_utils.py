    ...
    with patch('blackwell.core.common.getpwnam'):
        with patch('blackwell.core.common.blackwell_uid'):
            with patch('blackwell.core.common.blackwell_gid'):
                ...
                from blackwell.core.cluster import utils
                from blackwell.core.exception import BlackwellError, BlackwellException, BlackwellInternalError, BlackwellPermissionError, \
                    BlackwellResourceNotFound, BlackwellHAPHelperError
...
def test_read_cluster_config():
    ...
    with patch('blackwell.core.cluster.utils.get_ossec_conf', side_effect=BlackwellError(1001)):
        with pytest.raises(BlackwellError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('blackwell.core.configuration.load_blackwell_xml', return_value=SystemExit):
        with pytest.raises(SystemExit) as pytest_wrapped_e:
            utils.read_cluster_config(from_import=True)
        assert pytest_wrapped_e.type == SystemExit
        assert pytest_wrapped_e.value.code == 0

    with patch('blackwell.core.cluster.utils.get_ossec_conf', side_effect=KeyError(1)):
        with pytest.raises(BlackwellError, match='.* 3006 .*'):
            utils.read_cluster_config()

    with patch('blackwell.core.cluster.utils.get_ossec_conf', return_value={'cluster': default_cluster_config}):
        ...
        with pytest.raises(BlackwellError, match='.* 3004 .*'):
            utils.read_cluster_config()
...
def test_parse_haproxy_helper_config_ko(config: dict, exception_type: BlackwellException, expected_error_code: str):
...
def test_get_manager_status():
    ...
    with patch('blackwell.core.cluster.utils.glob', return_value=['ossec-0.pid']):
        with patch('re.match', return_value='None'):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'

        # with patch('blackwell.core.cluster.utils.join', return_value='failed') as join_mock:
        with patch('blackwell.core.cluster.utils.os.path.exists', side_effect=exist_mock):
            status = utils.get_manager_status()
            for value in status.values():
                assert value == 'failed'
...
def test_get_manager_status_ko(mock_stat, exc):
    ...
    with pytest.raises(BlackwellInternalError, match='.* 1913 .*'):
...
def test_get_cluster_status():
    ...
    BlackwellInternalError is raised."""
    status = utils.get_cluster_status()
    assert {'enabled': 'no', 'running': 'no'} == status

    with patch('blackwell.core.cluster.utils.get_manager_status', side_effect=BlackwellInternalError(1913)):
        status = utils.get_cluster_status()
        assert {'enabled': 'no', 'running': 'no'} == status
...
def test_manager_restart():
    ...
    with patch('blackwell.core.cluster.utils.open', side_effect=None):
            ...
            with patch('os.path.exists', return_value=True):
                ...
                with patch('socket.socket.connect', side_effect=None):
                    ...
                    with patch('socket.socket.send', side_effect=None):
                        ...
                        assert BlackwellResult({'message': 'Restart request sent'}) == status
...
def test_get_cluster_items():
    ...
    with patch('os.path.abspath', side_effect=FileNotFoundError):
        with pytest.raises(BlackwellException, match='.* 3005 .*'):
...
def test_ClusterLogger():
    """Verify that ClusterLogger defines the logger used by blackwell-clusterd."""
...
@patch('blackwell.core.cluster.utils.pyDaemonModule.create_pid')
def test_process_spawn_sleep(pyDaemon_create_pid_mock, get_pid_mock):
    ...
    pyDaemon_create_pid_mock.assert_called_once_with(f'blackwell-clusterd_child_{child}', get_pid_mock.return_value)


@pytest.mark.asyncio
@patch('concurrent.futures.ThreadPoolExecutor')
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI')
...
@patch('blackwell.core.cluster.utils.read_cluster_config')
