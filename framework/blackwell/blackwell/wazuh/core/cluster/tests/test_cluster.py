...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        import blackwell.core.cluster.cluster as cluster
...
def test_check_cluster_config_ko(read_config, message):
    ...
    with patch('blackwell.core.cluster.utils.get_ossec_conf', return_value=read_config) as m:
        with pytest.raises(BlackwellException, match=rf'.* 3004 .* {message}'):
            configuration = blackwell.core.cluster.utils.read_config()
...
def test_get_node():
    ...
    with patch('blackwell.core.cluster.cluster.read_config', return_value=test_dict):
...
@patch('blackwell.core.cluster.cluster.blake2b', return_value="hash")
@patch("blackwell.core.cluster.cluster.path.join", return_value="/mock/foo/bar")
@patch('blackwell.core.cluster.cluster.walk', return_value=[('/foo/bar', (), ('spam', 'eggs', '.merged'))])
def test_walk_dir(walk_mock, path_join_mock, blake2b_mock, getmtime_mock):
    ...
    path_join_mock.assert_called_once_with(common.BLACKWELL_PATH, '/foo/bar')
    ...
    walk_mock.assert_called_once_with(path_join_mock.return_value, topdown=True)
    path_join_mock.assert_has_calls([call(common.BLACKWELL_PATH, '/foo/bar'),
...
@patch('blackwell.core.cluster.cluster.walk', return_value=[('/foo/bar', (), ['spam'])])
...
def test_walk_dir_ko(mock_path_join, mock_walk):
    ...
    with patch('blackwell.core.cluster.cluster.walk', side_effect=OSError):
        with pytest.raises(BlackwellInternalError, match=r'.* 3015 .*'):
...
@patch('blackwell.core.cluster.cluster.get_cluster_items', return_value={
    "files": {
        "etc/": {
            "permissions": 416,
            "source": "master",
...
def test_get_files_status(mock_get_cluster_items):
    ...
    with patch('blackwell.core.cluster.cluster.walk_dir', return_value=(test_dict, {})):
    ...
    with patch('blackwell.core.cluster.cluster.walk_dir', side_effect=Exception):
...
def test_get_ruleset_status(mock_get_cluster_items):
    ...
    with patch("blackwell.core.cluster.cluster.walk_dir", return_value=(test_dict, {})) as walk_dir_mock:
    ...
    with patch("blackwell.core.cluster.cluster.walk_dir", side_effect=Exception):
        with patch.object(blackwell.core.cluster.cluster.logger, "warning") as logger_mock:
...
@patch('blackwell.core.cluster.cluster.get_cluster_items')
@patch('blackwell.core.cluster.cluster.mkdir_with_mode')
@patch('blackwell.core.cluster.cluster.path.dirname', return_value='/some/path')
@patch('blackwell.core.cluster.cluster.path.exists', return_value=False)
def test_compress_files_ok(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode, mock_get_cluster_items,
    ...
    with patch('builtins.open', mock_open(read_data='test_content')) as open_mock:
        assert isinstance(cluster.compress_files('some_name', ['some/path', 'another/path'], {'ko_file': 'file'}),
                          tuple)
        assert open_mock.call_args_list == [call(ANY, 'ab'), call(os.path.join(common.BLACKWELL_PATH, 'some/path'), 'rb'),
                                            call(os.path.join(common.BLACKWELL_PATH, 'another/path'), 'rb')]
...
def test_compress_files_ko(mock_path_exists, mock_path_dirname, mock_mkdir_with_mode, mock_get_cluster_items):
    ...
    with patch('builtins.open', mock_open(read_data='test_content')):
        ...
        assert logs['warning']['some/path'] == [f'File too large to be synced: '
                                                f'{os.path.join(common.BLACKWELL_PATH, "some/path")}']
        ...
        with patch('zlib.compress', side_effect=zlib.error):
            with pytest.raises(BlackwellError, match=r'.* 3001 .*'):
...
@pytest.mark.asyncio
@patch('blackwell.core.cluster.cluster.decompress_files', return_value="OK")
...
@patch('blackwell.core.cluster.cluster.remove')
@patch('blackwell.core.cluster.cluster.mkdir_with_mode')
        ...
        with patch('builtins.open', mock_open(read_data=f'path{cluster.PATH_SEP}content'.encode())):
            with patch('os.path.exists', return_value=False):
                with patch('os.makedirs', side_effect=PermissionError) as mock_makedirs:
                    with patch('blackwell.core.cluster.cluster.remove'):
...
def test_compare_files(mock_get_cluster_items):
    ...
    # First condition
    with patch('blackwell.core.cluster.cluster.merge_info', return_values=[1, "random/path/"]):
...
@patch('blackwell.core.cluster.cluster.get_cluster_items')
@patch.object(blackwell.core.cluster.cluster.logger, "error")
def test_compare_files_ko(logger_mock, mock_get_cluster_items):
    ...
    with pytest.raises(Exception):
        ...
        blackwell_db_query_mock.assert_called_once_with()
...
def test_clean_up_ok():
    ...
    with patch('os.path.join', return_value="some/path/"):
        with patch.object(blackwell.core.cluster.cluster.logger, "debug") as mock_logger:
            with patch('os.path.exists', return_value=False) as path_exists_mock:
                ...
                with patch('blackwell.core.cluster.cluster.listdir',
...
def test_clean_up_ko():
    ...
    with patch('os.path.join') as path_join_mock:
        with patch.object(blackwell.core.cluster.cluster.logger, "error") as mock_error_logger:
            with patch.object(blackwell.core.cluster.cluster.logger, "debug") as mock_debug_logger:
...
@patch('blackwell.core.cluster.cluster.listdir', return_value=['005', '006'])
@patch('blackwell.core.cluster.cluster.stat')
def test_merge_info(stat_mock, listdir_mock):
    ...
    with patch('builtins.open', mock_open(read_data=agent_groups)) as open_mock:
        ...
        open_mock.assert_any_call(common.BLACKWELL_PATH + '/queue/cluster/worker1/testing-shared.merged', 'wb')
        open_mock.assert_any_call(common.BLACKWELL_PATH + '/queue/testing/005', 'rb')
        open_mock.assert_any_call(common.BLACKWELL_PATH + '/queue/testing/006', 'rb')
...
def test_unmerge_info():
    ...
    with patch('builtins.open', mock_open(read_data=agent_info)):
        with patch('blackwell.core.cluster.cluster.stat') as stat_mock:
            ...
            with patch.object(blackwell.core.cluster.cluster.logger, "warning") as mock_logger:
...
async def test_run_in_pool(event_loop):
    ...
    with patch('blackwell.core.cluster.cluster.wait_for', return_value="OK") as wait_for_mock:
...
def test_validate_haproxy_helper_config_ko():
    """Verify that validate_haproxy_helper_config raises BlackwellError when there is a validation error."""

    config = {cluster.AGENT_CHUNK_SIZE: 120, cluster.AGENT_RECONNECTION_TIME: 10}

    with patch.object(cluster, 'validate', side_effect=cluster.ValidationError(message='Error test', path=['test'])):
        with pytest.raises(cluster.BlackwellError, match='.* 3004 .*'):
            cluster.validate_haproxy_helper_config(config)
