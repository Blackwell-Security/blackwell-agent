...
from blackwell.core import exception
...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        import blackwell.core.cluster.common as cluster_common
        import blackwell.core.results as wresults
        from blackwell.core import common
        from blackwell.core.wdb import AsyncBlackwellDBConnection
...
blackwell_common = cluster_common.BlackwellCommon()
...
@patch("blackwell.core.cluster.common.SendStringTask.set_up_coro")
def test_sst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        sst_task = cluster_common.SendStringTask(blackwell_common=cluster_common.BlackwellCommon(), logger='')
        assert sst_task.logger == ''
        assert sst_task.task in sst_task.tasks_hard_reference
        assert isinstance(sst_task.blackwell_common, cluster_common.BlackwellCommon)
...
def test_sst_done_callback(setup_coro_mock, create_task_mock):
    ...
    create_task_mock.return_value = TaskMock()
    blackwell_common_mock = BlackwellCommon()
    ...
    with patch.object(TaskMock, "add_done_callback"):
        ...
        with patch.object(logger, "error") as logger_mock:
            sst_task = cluster_common.SendStringTask(blackwell_common=blackwell_common_mock, logger=logger)
            assert sst_task.task in sst_task.tasks_hard_reference
...
@patch("asyncio.create_task")
@patch("blackwell.core.cluster.common.ReceiveStringTask.set_up_coro")
def test_rst_init(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        string_task = cluster_common.ReceiveStringTask(blackwell_common=cluster_common.BlackwellCommon(), logger='',
                                                       info_type='testing', task_id=b"010")
        assert isinstance(string_task.blackwell_common, cluster_common.BlackwellCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
...
async def test_rst_str_method(logger_mock, event_loop):
    ...
    with patch('blackwell.core.cluster.common.ReceiveStringTask.set_up_coro', side_effect = return_coro) as setup_coro_mock:
        string_task = cluster_common.ReceiveStringTask(cluster_common.BlackwellCommon(), '', b"task", logger_mock)
...
@patch('logging.Logger')
@patch('blackwell.core.cluster.common.BlackwellCommon')
def test_rst_set_up_coro_ko(blackwell_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveStringTask(blackwell_common_mock, logger_mock, b"task")
...
def test_rst_done_callback(setup_coro_mock, create_task_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        logger = logging.getLogger('blackwell')
        with patch.object(logger, "error") as logger_mock:
            string_task = cluster_common.ReceiveStringTask(blackwell_common=blackwell_common_mock, logger=logger,
                                                           info_type='agent-groups', task_id=b"010")
            string_task.done_callback()
            assert string_task.blackwell_common.in_str == {b"011": b"123456789"}
            assert string_task.blackwell_common.sync_tasks == {b"011": b"123456789"}
...
@patch('asyncio.Event')
@patch("asyncio.create_task")
@patch.object(logging.getLogger("blackwell"), "error")
@patch("blackwell.core.cluster.common.ReceiveFileTask.set_up_coro")
@patch("blackwell.core.cluster.common.uuid4", return_value="e6c45993-b0ae-438f-8914-3b5175b2bbfd")
def test_rft_init(uuid_mock, setup_coro_mock, logger_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback") as done_callback_mock:
        file_task = cluster_common.ReceiveFileTask(blackwell_common=cluster_common.BlackwellCommon(), logger=logger_mock,
                                                   task_id=b"010")
        assert isinstance(file_task.blackwell_common, cluster_common.BlackwellCommon)
        setup_coro_mock.assert_called_once()
        done_callback_mock.assert_called_once()
        ...
        string_task = cluster_common.ReceiveFileTask(blackwell_common=cluster_common.BlackwellCommon(), logger=logger_mock,
                                                     task_id="")
...
@patch('logging.Logger')
@patch('blackwell.core.cluster.common.BlackwellCommon')
def test_rft_set_up_coro(blackwell_common_mock, logger_mock):
    """Test if the exception is being properly raised when an Exception takes place."""

    with pytest.raises(NotImplementedError):
        cluster_common.ReceiveFileTask(blackwell_common_mock, logger_mock, b"task")
...
def test_rft_done_callback(set_up_coro_mock, create_task_mock, event_mock):
    ...
    with patch.object(TaskMock, "add_done_callback"):
        with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
            file_task = cluster_common.ReceiveFileTask(blackwell_common=blackwell_common_mock,
                                                       logger=logging.getLogger('blackwell'), task_id=b"010")
            file_task.done_callback()
            assert file_task.blackwell_common.sync_tasks == {"011": b"123456789"}
            logger_mock.assert_called_once_with(Exception, exc_info=False)
...
def test_handler_init():
    ...
    with patch('blackwell.core.cluster.utils.context_tag', ContextVar('', default="")) as cv:
        ...
        assert handler.logger == logging.getLogger("blackwell")
...
def test_handler_msg_build_ko():
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3024 .*'):
...
def test_handler_get_messages_ok():
    ...
    with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
        with patch('blackwell.core.cluster.common.Handler.msg_parse', return_value=True) as handler_mock:
...
@patch('blackwell.core.cluster.common.Handler.msg_parse', return_value=True)
...
def test_handler_get_messages_ko(decrypt_mock, msg_parse_mock):
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3025 .*'):
        handler.in_msg.total = 0
        list(handler.get_messages())


@pytest.mark.asyncio
@patch('blackwell.core.cluster.common.Handler.push')
@patch('blackwell.core.cluster.common.Handler.next_counter', return_value=30)
@patch('blackwell.core.cluster.common.Handler.msg_build', return_value=["some", "messages"])
async def test_handler_send_request_ok(msg_build_mock, next_counter_mock, push_mock):
    ...
    with patch('blackwell.core.cluster.common.Response.read', return_value="some value") as read_mock:
    ...
    with patch('blackwell.core.cluster.common.Response.read', side_effect=delay):
        with pytest.raises(exception.BlackwellClusterError, match=r'\b3020\b'):
...
async def test_handler_send_request_ko():
    ...
    with patch('blackwell.core.cluster.common.Handler.msg_build', side_effect=MemoryError):
        with pytest.raises(exception.BlackwellClusterError, match=r'.* 3026 .*'):
            await handler.send_request(b'some bytes', b'some data')

    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3018 .*'):
        await handler.send_request(b'some bytes', b'some data')


@pytest.mark.asyncio
@patch('blackwell.core.cluster.common.Handler.send_request')
async def test_handler_get_chunks_in_task_id(send_request_mock):
    ...
    # Test KeyError
    with pytest.raises(exception.BlackwellClusterError, match='.* 3035 .*'):
    ...
    with pytest.raises(exception.BlackwellClusterError, match='.* 3036 .*'):
...
async def test_handler_update_chunks_wdb(send_request_mock):
    ...
    with patch('blackwell.core.cluster.cluster.run_in_pool',
        ...
        with patch.object(LoggerMock, "debug") as logger_debug_mock:
            with patch.object(LoggerMock, "debug2") as logger_debug2_mock:
                with patch.object(LoggerMock, "error") as logger_error_mock:
                    ...
                    logger_debug_mock.assert_has_calls([call('2/5 chunks updated in blackwell-db in 6.000s.')])
                    ...
                    logger_error_mock.assert_has_calls([call('other1'), call('other2'),
                                                        call('Blackwell-db response for chunk 1/5 was not "ok": 0'),
                                                        call('Blackwell-db response for chunk 2/5 was not "ok": 1')])

    # Test Exception
    send_request_mock.reset_mock()
    with pytest.raises(exception.BlackwellClusterError,
...
@patch('blackwell.core.cluster.common.Handler.send_request', return_value=b'some data')
...
async def test_handler_send_file_ko():
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3034 .*'):
...
async def test_handler_send_string():
    ...
    with patch('blackwell.core.cluster.common.Handler.send_request', return_value=b"some data"):
        assert (await handler.send_string(b"something") == b"some data")

    with patch('blackwell.core.cluster.common.Handler.send_request', side_effect=exception.BlackwellClusterError(3020)):
        with patch.object(logging.getLogger("blackwell"), "error") as logger_mock:
            assert exception.BlackwellClusterError(3020).message.encode() in await handler.send_string(b"something")
...
async def test_handler_forward_dapi_response_ok():
    ...
    with patch('blackwell.core.cluster.common.Handler.get_manager', return_value=mock_manager):
...
async def test_handler_forward_dapi_response_ko():
    ...
    with patch('blackwell.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_string",
                          side_effect=exception.BlackwellException(1001)) as send_string_mock:
            with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
                with patch('blackwell.core.cluster.common.Handler.send_request', return_value="some value"):
                    with patch('json.dumps', return_value="some value"):
                        ...
                        logger_mock.assert_called_once_with(f"Error sending API response to local client: "
                                                            f"{exception.BlackwellException(1001)}")
...
@pytest.mark.parametrize('exception, expected_error', [
    (exception.BlackwellException(1001), exception.BlackwellException(1001)),
    (exception.BlackwellClusterError(3020), None),
    (Exception('Test'), exception.BlackwellClusterError(1000, extra_message='Test'))
])
async def test_handler_forward_sendsync_response_ko(exception, expected_error):
    ...
    with patch('blackwell.core.cluster.common.Handler.get_manager', return_value=mock_manager):
        ...
        with patch.object(mock_manager.local_server, "send_request", side_effect=exception):
            with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
                with patch('blackwell.core.cluster.common.Handler.send_request',
                    ...
                    if expected_error:
                        ...
                        send_request_mock.assert_called_once_with(
                            b'sendsync_err',
                            json.dumps(expected_error, cls=cluster_common.BlackwellJSONEncoder).encode()
                        )
                    else:
...
def test_handler_data_received_ok():
    ...
    # Test first if
    with patch('blackwell.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"d")]):
        # Test try
    ...
    with patch('blackwell.core.cluster.common.Handler.get_messages', return_value=[(b"bytes1", 123, b"bytes2", b"bytes3")]):
        with patch('cryptography.fernet.Fernet.decrypt', return_value="decrypted payload"):
            ...
            with patch('blackwell.core.cluster.common.Handler.dispatch') as dispatch_mock:
                handler.data_received(b"message")
...
@patch('blackwell.core.cluster.common.Handler.msg_build', return_value=["msg"])
@patch('blackwell.core.cluster.common.Handler.push')
@patch('blackwell.core.cluster.common.Handler.process_request', return_value=(b"command", b"payload"))
def test_handler_dispatch(process_request_mock, push_mock, msg_build_mock):
    ...
    process_request_mock.side_effect = exception.BlackwellException(1001)
    with patch.object(logging.getLogger('blackwell'), "error") as logger_mock:
        handler.dispatch(None, 123, b"payload")
        logger_mock.assert_called_with(f"Internal error processing request '{None}': {exception.BlackwellException(1001)}")
...
def test_handler_process_request():
    ...
    with patch('blackwell.core.cluster.common.Handler.echo') as echo_mock:
        handler.process_request(b"echo", b"data")
        echo_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.receive_file') as receive_file_mock:
        handler.process_request(b"new_file", b"data")
        receive_file_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.receive_str') as receive_str_mock:
        handler.process_request(b"new_str", b"data")
        receive_str_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.update_file') as update_file_mock:
        handler.process_request(b"file_upd", b"data")
        update_file_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.str_upd') as str_upd_mock:
        handler.process_request(b"str_upd", b"data")
        str_upd_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.process_error_str') as process_error_str_mock:
        handler.process_request(b"err_str", b"data")
        process_error_str_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.end_file') as end_file_mock:
        handler.process_request(b"file_end", b"data")
        end_file_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.cancel_task') as cancel_task_mock:
        handler.process_request(b"cancel_task", b"data")
        cancel_task_mock.assert_called_once_with(b"data")
    with patch('blackwell.core.cluster.common.Handler.process_dapi_error') as process_dapi_err_mock:
    ...
    with patch('blackwell.core.cluster.common.Handler.process_unknown_cmd') as process_unknown_cmd_mock:
...
def test_handler_process_response():
    ...
    with patch('blackwell.core.cluster.common.Handler.process_error_from_peer', return_value=b"payload"):
...
def test_handler_str_upd():
    ...
    with patch('blackwell.core.cluster.common.InBuffer.receive_data'):
...
def test_handler_process_dapi_error_ko():
    ...
    handler = cluster_common.Handler(fernet_key, cluster_items)
    with pytest.raises(exception.BlackwellClusterError, match=r".* 3032 .*"):
        handler.server = ManagerMock()
        handler.process_dapi_error(data=b"data 2")

    with pytest.raises(exception.BlackwellClusterError, match=r".* 3025 .*"):
        with patch("asyncio.create_task", side_effect=exception.BlackwellClusterError(1001)):
            handler.server = ManagerMock()
...
def test_handler_process_error_from_peer():
    ...
    assert isinstance(handler.process_error_from_peer(b"data to decode"), exception.BlackwellClusterError)
...
def test_handler_setup_task_logger():
    ...
    with patch.object(logging.getLogger("blackwell"), "getChild", return_value=TaskLoggerMock()) as get_child_mock:
...
async def test_handler_wait_for_file_ko(send_request_mock):
    """Check if expected exception is raised.
        Condition 1: when event.wait() exceeds the timeout, BlackwellClusterError 3039 is raised
        Condition 2: when any other exception occurs, BlackwellClusterError 3040  is raised"""
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3039 .*'):
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r".* 3040 .*"):
...
# Test 'BlackwellCommon' class methods

def test_blackwell_common_init():
    """Test the '__init__' method correct functioning."""

    blackwell_common_test = cluster_common.BlackwellCommon()
    assert blackwell_common_test.sync_tasks == {}


@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.AsyncBlackwellDBConnection", return_value=AsyncMock())
async def test_blackwell_common_recalculate_group_hash(asyncblackwelldbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    await blackwell_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']


@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.AsyncBlackwellDBConnection")
async def test_blackwell_common_recalculate_group_hash_ko(asyncblackwelldbconnection_mock):
    class LoggerMock:
        """Auxiliary class."""
    ...
    logger = LoggerMock()
    asyncblackwelldbconnection_mock.side_effect = [exception.BlackwellInternalError(2007), exception.BlackwellError(2003)]

    await blackwell_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2007 executing recalculate agent-group hash command: '
                               'Error retrieving data from Blackwell DB']

    logger = LoggerMock()
    await blackwell_common.recalculate_group_hash(logger)
    assert logger._debug == ['Recalculating agent-group hash.']
    assert logger._warning == ['Error 2003 executing recalculate agent-group hash command: Error in blackwelldb request']

def test_blackwell_common_get_logger():
    """Check if a Logger object is properly returned."""

    with pytest.raises(NotImplementedError):
        blackwell_common.get_logger()


def test_blackwell_common_setup_send_info():
    ...
    with patch('blackwell.core.cluster.common.BlackwellCommon.get_logger'):
        first_output, second_output = blackwell_common.setup_send_info(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)


def test_blackwell_common_setup_receive_file():
    ...
    with patch('blackwell.core.cluster.common.BlackwellCommon.get_logger'):
        first_output, second_output = blackwell_common.setup_receive_file(mock_object)
        assert first_output == b'ok'
        assert isinstance(second_output, bytes)
        assert MyTaskMock().task_id in blackwell_common.sync_tasks
        assert isinstance(blackwell_common.sync_tasks[MyTaskMock().task_id], MyTaskMock)


@patch('blackwell.core.cluster.common.BlackwellCommon')
@patch('logging.Logger')
def test_blackwell_common_end_receiving_file_ok(logger_mock, blackwell_common_mock):
    """Check if the full path to the received file is properly stored and availability is notified."""

    with patch('blackwell.core.cluster.common.ReceiveFileTask.set_up_coro'):
        with patch('asyncio.create_task'):
            file_task = cluster_common.ReceiveFileTask(blackwell_common_mock, logger_mock, b"task")

    blackwell_common.sync_tasks = {'task_ID': file_task}
    assert blackwell_common.end_receiving_file("task_ID filepath") == (b'ok', b'File correctly received')
    assert isinstance(blackwell_common.sync_tasks["task_ID"], cluster_common.ReceiveFileTask)


@patch('os.remove')
@patch('os.path.exists', return_value=True)
def test_blackwell_common_end_receiving_file_ko(path_exists_mock, os_remove_mock):
    """Test the 'end_receiving_file' correct functioning in a failure scenario."""

    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3027 .*'):
        blackwell_common.end_receiving_file("not_task_ID filepath")
...
@patch('json.loads')
def test_blackwell_common_error_receiving_file_ok(json_loads_mock):
    """Check how error are handled by peer in the sent file process."""

    with patch('os.path.exists', return_value=True):
        with patch('os.remove'):
            # Test first condition and its nested condition
            assert blackwell_common.error_receiving_file("task_ID error_details") == (b'ok', b'Error received')

    # Test second condition
    with patch('blackwell.core.cluster.common.BlackwellCommon.get_logger'):
        assert blackwell_common.error_receiving_file("not_task_ID error_details") == (b'ok', b'Error received')


def test_blackwell_common_error_receiving_file_ko():
...
def test_blackwell_common_get_node():
    """Check if it is possible to obtain basic information about the node."""

    class MockClass(cluster_common.BlackwellCommon, cluster_common.Handler, abc.ABC):
        def __init__(self):
            super().__init__()
    ...
    with patch('blackwell.core.cluster.common.Handler.get_manager', return_value=mock_manager) as manager_mock:
        mock_class.get_node()
        manager_mock.assert_called_once()


# Test SyncBlackwelldb class

def test_sync_blackwell_db_init():
    """Test the '__init__' method from the SyncBlackwelldb class."""

    sync_blackwell_db = cluster_common.SyncBlackwelldb(
        manager=cluster_common.Handler(fernet_key, cluster_items), logger=logging.getLogger("blackwell"), cmd=b"cmd",
        data_retriever=None, get_data_command="get_command", set_data_command="set_command", pivot_key=None)

    assert sync_blackwell_db.get_data_command == "get_command"
    assert sync_blackwell_db.set_data_command == "set_command"
    assert sync_blackwell_db.data_retriever is None


@pytest.mark.asyncio
@patch("blackwell.core.wdb.socket.socket")
async def test_sync_blackwell_db_retrieve_information(socket_mock):
    ...
    wdb_conn = AsyncBlackwellDBConnection()
    logger = logging.getLogger("blackwell")
    handler = cluster_common.Handler(fernet_key, cluster_items)
    sync_object = cluster_common.SyncBlackwelldb(manager=handler, logger=logger, cmd=b'syn_a_w_m',
    ...
    # data_retriever returns a JSON that does not follow the pivoting scheme
    with patch.object(sync_object, 'data_retriever', side_effect=exception.BlackwellException(1000)):
        with patch.object(sync_object.logger, 'error') as logger_error_mock:
            ...
            logger_error_mock.assert_called_with(
                'Could not obtain data from blackwell-db: Error 1000 - Blackwell Internal Error')


@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch("json.dumps", return_value="")
@patch('blackwell.core.cluster.common.time.perf_counter', return_value=0)
async def test_sync_blackwell_db_sync_ok(perf_counter_mock, json_dumps_mock):
    ...
    # Test try and if
    with patch.object(sync_blackwell_db.logger, "debug") as logger_debug_mock:
        with patch("blackwell.core.cluster.common.Handler.send_string", return_value=b"OK") as send_string_mock:
            with patch("blackwell.core.cluster.common.Handler.send_request",
                       side_effect=None) as send_request_mock:
                assert await sync_blackwell_db.sync(start_time=10, chunks=['a', 'b']) is True
    ...
    # Test else
    with patch.object(sync_blackwell_db.logger, "info") as logger_info_mock:
        assert await sync_blackwell_db.sync(start_time=-10, chunks=[]) is True
        logger_info_mock.assert_called_once_with(f"Finished in 10.000s. Updated 0 chunks.")

    # Test except
    with patch("blackwell.core.cluster.common.Handler.send_string", return_value=b'Error 1'):
        sync_blackwell_db.server = cluster_common.Handler(fernet_key, cluster_items)
        with pytest.raises(exception.BlackwellClusterError, match=r".* 3016 .*"):
            await sync_blackwell_db.sync(start_time=10, chunks=['a'])
...
def test_end_sending_agent_information(perf_counter_mock, json_loads_mock):
    ...
    with patch('blackwell.core.cluster.common.utils.get_utc_now', side_effect=get_utc_now_mock):
...
@patch("blackwell.core.cluster.common.BlackwellDBConnection")
def test_send_data_to_wdb(BlackwellDBConnection_mock):
    """Check if the data chunks are being properly forward to the Blackwell-db socket."""

    class MockBlackwellDBConnection:
        """Auxiliary class."""
    ...
    BlackwellDBConnection_mock.return_value = MockBlackwellDBConnection()
    ...
    BlackwellDBConnection_mock.return_value.exceptions += 1
    ...
    with patch('blackwell.core.cluster.master.utils.Timeout', side_effect=Exception):
        result = cluster_common.send_data_to_wdb(data={'chunks': ['1chunk', '2chunk'], 'set_data_command': ''},
                                                 timeout=15)
...
def test_blackwell_json_encoder_default():
    """Test if a special JSON encoder is defined for Blackwell."""

    blackwell_encoder = cluster_common.BlackwellJSONEncoder()

    # Test first condition
    assert isinstance(blackwell_encoder.default(object), dict)
    assert blackwell_encoder.default(object) == {'__callable__': {'__name__': 'object', '__module__': 'builtins',
                                                              '__qualname__': 'object', '__type__': 'type'}}

    class BlackwellMock(json.JSONEncoder):
        __self__ = Blackwell()
        a = 0
        b = 0
    ...
    blackwell_encoder.default(BlackwellMock)

    with patch('builtins.callable', return_value=False):
        # Test second condition
        assert isinstance(blackwell_encoder.default(exception.BlackwellException(3012)), dict)
        assert blackwell_encoder.default(exception.BlackwellException(3012)) == \
               {'__blackwell_exception__': {'__class__': 'BlackwellException',
                                        '__object__': {'type': 'about:blank', 'title': 'BlackwellException', 'code': 3012,
                                                       'extra_message': None, 'extra_remediation': None,
                                                       'cmd_error': False, 'dapi_errors': {}}}}

        # Test third condition
        abstract_blackwell_result = wresults.AffectedItemsBlackwellResult()

        assert isinstance(blackwell_encoder.default(abstract_blackwell_result), dict)
        assert blackwell_encoder.default(abstract_blackwell_result) == \
               {'__blackwell_result__': {'__class__': 'AffectedItemsBlackwellResult',
        ...
        assert isinstance(blackwell_encoder.default(date), dict)
        assert blackwell_encoder.default(date) == {'__blackwell_datetime__': '2021-10-15T00:00:00'}

        # Test fifth condition
        exc = ValueError('test')
        assert isinstance(blackwell_encoder.default(exc), dict)
        assert blackwell_encoder.default(exc) == {'__unhandled_exc__': {'__class__': 'ValueError',
                                                                    '__args__': ('test',)}}

        # Test simple return
        with pytest.raises(TypeError):
            blackwell_encoder.default({"key": "value"})


def test_as_blackwell_object_ok():
    """Test the different outputs taking into account the input values."""

    # Test the first condition and nested if
    assert cluster_common.as_blackwell_object({"__callable__": {"__name__": "type", "__blackwell__": "version"}}) == "server"

    # Test the first condition and nested else
    assert isinstance(
        cluster_common.as_blackwell_object({"__callable__": {"__name__": "path", "__qualname__": "__loader__.value",
                                                         "__module__": "os"}}), str)

    assert cluster_common.as_blackwell_object({"__callable__": {"__name__": "__name__", "__qualname__": "value",
                                                            "__module__": "itertools"}}) == "itertools"

    # Test the second condition
    assert isinstance(cluster_common.as_blackwell_object(
        {"__blackwell_exception__": {"__class__": "BlackwellException", "__object__": {"code": 1001}}}),
        exception.BlackwellException)

    # Test the third condition
    with patch('blackwell.core.results.AbstractBlackwellResult.decode_json', return_value=wresults.AbstractBlackwellResult):
        assert isinstance(cluster_common.as_blackwell_object(
            {"__blackwell_result__": {"__class__": "AbstractBlackwellResult", "__object__": {"code": 1001}}}), abc.ABCMeta)

    # Test the fourth condition
    assert isinstance(cluster_common.as_blackwell_object({"__blackwell_datetime__": "2021-10-14"}), datetime)
    assert cluster_common.as_blackwell_object({"__blackwell_datetime__": "2021-10-14"}) == datetime(2021, 10, 14)

    # Test the fifth condition
    result = cluster_common.as_blackwell_object({'__unhandled_exc__': {'__class__': 'ValueError',
                                                                   '__args__': ('test',)}})
    assert result == {"ValueError": ["test"]}

    result = cluster_common.as_blackwell_object({'__unhandled_exc__': {'__class__': 'exit',
                                                                   '__args__': []}})
    assert result == {"exit": []}

    # No condition fulfilled
    assert isinstance(cluster_common.as_blackwell_object({"__blackwell_datetime_bad__": "2021-10-14"}), dict)
    assert cluster_common.as_blackwell_object({"__blackwell_datetime_bad__": "2021-10-14"}) == \
           {"__blackwell_datetime_bad__": "2021-10-14"}


def test_as_blackwell_object_ko():
    """Test if the exceptions are correctly raised."""

    with pytest.raises(exception.BlackwellInternalError, match=r'.* 1000 .*'):
        cluster_common.as_blackwell_object({"__callable__": {"__name__": "value", "__blackwell__": "value"}})


def get_handler():
    """Return a Handler object. This is an auxiliary method."""
    return cluster_common.Handler(fernet_key=fernet_key, cluster_items=cluster_items, logger=logging.getLogger("blackwell"))
...
def test_sync_task_init():
    ...
    sync_task = cluster_common.SyncTask(b"cmd", logging.getLogger("blackwell"), get_handler())

    assert sync_task.cmd == b"cmd"
    assert sync_task.logger == logging.getLogger("blackwell")
    assert isinstance(sync_task.server, cluster_common.Handler)


@pytest.mark.asyncio
@patch('blackwell.core.cluster.common.Handler.send_request', side_effect=Exception())
async def test_sync_task_request_permission(send_request_mock):
    ...
    with patch.object(logging.getLogger("blackwell"), "debug") as logger_mock:
        # Test second condition
...
@patch("blackwell.core.cluster.cluster.compress_files", return_value=("files/path/", {}))
@patch("blackwell.core.cluster.utils.log_subprocess_execution")
async def test_sync_files_sync_ok(log_subprocess_mock, compress_files_mock, unlink_mock, relpath_mock, json_dumps_mock):
    ...
    class WorkerMock:
        ...
        async def send_request(self, command, data):
            ...
            if command == b"cmd" and data == b"" and self.count == 1:
                raise exception.BlackwellClusterError(3020, extra_message=command.decode())
                ...
                raise exception.BlackwellClusterError(3016, extra_message=command.decode())
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("blackwell"), worker_mock)
    ...
    with patch.object(logging.getLogger("blackwell"), "error") as logger_mock:
        ...
        json_dumps_mock.assert_called_once_with(
            exception.BlackwellClusterError(code=3020, extra_message="cmd"),
            cls=cluster_common.BlackwellJSONEncoder)
    ...
    with patch.object(WorkerMock, "send_file", return_value=1000) as send_file_mock:
        ...
        with patch.object(logging.getLogger("blackwell"), "debug") as logger_debug_mock:
            with patch.object(logging.getLogger("blackwell"), "error") as logger_error_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
                ...
                relpath_mock.assert_called_once_with('files/path/', common.BLACKWELL_PATH)
                ...
                log_subprocess_mock.assert_called()
                logger_error_mock.assert_called_once_with(
                    f"Error sending zip file: {exception.BlackwellException(3016, 'cmd_e')}")
...
async def test_sync_files_sync_ko(send_request_mock):
    ...
    sync_files = cluster_common.SyncFiles(b"cmd", logging.getLogger("blackwell"), handler)
    ...
    with patch('os.unlink', side_effect=FileNotFoundError):
        ...
        with patch("blackwell.core.cluster.cluster.compress_files", return_value=(compressed_data, {})):
            send_request_mock.side_effect = None
            logger = logging.getLogger('blackwell')
            with patch.object(logger, "error") as logger_mock:
                await sync_files.sync(files_to_sync, files_metadata, 1, task_pool=None)
