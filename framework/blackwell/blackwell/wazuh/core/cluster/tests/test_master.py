...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core.cluster import common as cluster_common, client, master
        from blackwell.core.cluster.master import DEFAULT_DATE
        from blackwell.core import common
        from blackwell.core.cluster.dapi import dapi
...
async def test_rit_init():
    ...
    with patch.object(master_common, 'integrity_check') as integrity_check_mock:
        receive_integrity_task = master.ReceiveIntegrityTask(blackwell_common=master_common,
                                                            logger=logging.getLogger("blackwell"))
        assert isinstance(receive_integrity_task.blackwell_common, cluster_common.BlackwellCommon)
...
def test_rit_set_up_coro(create_task_mock):
    ...
    blackwell_common_mock = BlackwellCommonMock()
    receive_integrity_task = master.ReceiveIntegrityTask(blackwell_common=blackwell_common_mock,
                                                         logger=logging.getLogger("blackwell"))
    assert receive_integrity_task.set_up_coro() == blackwell_common_mock.integrity_check
    create_task_mock.assert_called_once()


@patch("asyncio.create_task")
@patch("blackwell.core.cluster.common.ReceiveFileTask.done_callback")
def test_rit_done_callback(super_callback_mock, create_task_mock):
    ...
    assert blackwell_common_mock.sync_integrity_free[0] is True


# Test ReceiveExtraValidTask class

@patch("asyncio.create_task")
@patch("blackwell.core.cluster.master.ReceiveExtraValidTask.set_up_coro")
def test_revt_init(set_up_coro_mock, create_task_mock):
    """Test the correct initialization of the ReceiveExtraValidTask class."""

    receive_extra_valid_task = master.ReceiveExtraValidTask(blackwell_common=cluster_common.BlackwellCommon(),
                                                            logger=logging.getLogger("blackwell"))

    assert isinstance(receive_extra_valid_task.blackwell_common, cluster_common.BlackwellCommon)
...
def test_revt_set_up_coro(create_task_mock):
    ...
    blackwell_common_mock = BlackwellCommonMock()
    receive_extra_valid_task = master.ReceiveExtraValidTask(blackwell_common=blackwell_common_mock,
                                                            logger=logging.getLogger("blackwell"))
    assert receive_extra_valid_task.set_up_coro() == blackwell_common_mock.sync_extra_valid
    create_task_mock.assert_called_once()
...
def test_revt_done_callback(set_up_coro_mock, super_callback_mock, create_task_mock):
    ...
    set_up_coro_mock.assert_called_once()
    assert blackwell_common_mock.sync_integrity_free[0] is True
    assert blackwell_common_mock.extra_valid_requested is False


@patch("asyncio.create_task")
@patch("blackwell.core.cluster.master.ReceiveAgentInfoTask.set_up_coro")
def test_rait_init(set_up_coro_mock, create_task_mock):
    """Test the initialization of the ReceiveAgentInfoTask object."""

    receive_agent_info_task = master.ReceiveAgentInfoTask(blackwell_common=cluster_common.BlackwellCommon(),
                                                          logger=logging.getLogger("blackwell"), task_id="0101")

    assert isinstance(receive_agent_info_task.blackwell_common, cluster_common.BlackwellCommon)
...
@patch("asyncio.create_task")
@patch("blackwell.core.cluster.master.SendEntireAgentGroupsTask.set_up_coro")
def test_seagt_init(set_up_coro_mock, create_task_mock):
    """Test the initialization of the SendEntireAgentGroupsTask object."""

    receive_agent_groups_task = master.SendEntireAgentGroupsTask(blackwell_common=cluster_common.BlackwellCommon(),
                                                                 logger=logging.getLogger("blackwell"))
...
def test_rait_set_up_coro(create_task_mock):
    ...
    class BlackwellCommonMock:
        ...
        def sync_blackwell_db_info(self, blackwell_common, task_id):
            """Auxiliary method."""
            pass

    blackwell_common_mock = BlackwellCommonMock()
    receive_agent_info_task = master.ReceiveAgentInfoTask(blackwell_common=blackwell_common_mock,
                                                          logger=logging.getLogger("blackwell"), task_id="0101")
    assert receive_agent_info_task.set_up_coro() == blackwell_common_mock.sync_blackwell_db_info
    create_task_mock.assert_called_once()


@patch("asyncio.create_task")
@patch("blackwell.core.cluster.common.ReceiveStringTask.done_callback")
@patch("blackwell.core.cluster.master.ReceiveAgentInfoTask.set_up_coro")
def test_rait_done_callback(set_up_coro_mock, super_callback_mock, create_task_mock):
    ...
    super_callback_mock.assert_called_once_with(None)
    set_up_coro_mock.assert_called_once()
    assert blackwell_common_mock.sync_agent_info_free is True
...
def test_master_handler_init():
    ...
    with patch('blackwell.core.cluster.master.context_tag', ContextVar('', default="")) as cv:
...
def test_master_handler_process_request(logger_mock):
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.get_permission", return_value=b"ok") as get_permission_mock:
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.setup_sync_integrity",
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.end_receiving_integrity_checksums",
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.process_sync_error_from_worker",
    ...
    with patch("blackwell.core.cluster.common.end_sending_agent_information",
    ...
    with patch("blackwell.core.cluster.common.error_receiving_agent_information",
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.process_dapi_res", return_value=b"ok") as process_dapi_res_mock:
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.get_nodes", return_value=(["cmd", "res"])) as get_nodes_mock:
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.get_health", return_value=(["cmd", "res"])) as get_health_mock:
        with patch("json.loads", return_value=b"ok") as json_loads_mock:
            with patch("json.dumps", return_value="ok") as json_dumps_mock:
    ...
    with patch("blackwell.core.cluster.server.AbstractServerHandler.process_request",
...
@patch("blackwell.core.cluster.master.uuid4", return_value=10101010)
async def test_master_handler_execute_ok(uuid4_mock, wait_for_mock):
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.send_request", return_value=b"result") as send_request_mock:
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.process_request",
...
async def test_master_handler_execute_ko(uuid4_mock):
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3022 .*'):
    ...
    with patch("blackwell.core.cluster.master.MasterHandler.send_request", return_value=b"result"):
        with pytest.raises(exception.BlackwellClusterError, match=r".* 3021 .*"):
            await master_handler.execute(command=b'dapi', data=b"client request", wait_for_complete=False)

    # Test the third exception
    with patch("blackwell.core.cluster.master.MasterHandler.process_request", return_value=[b"error", b""]):
        with pytest.raises(exception.BlackwellClusterError, match=r".* 3022 .*"):
...
@patch("blackwell.core.cluster.common.SyncBlackwelldb", return_value="SyncBlackwelldbMock")
@patch("blackwell.core.cluster.common.SyncFiles", return_value="SyncFilesMock")
@patch("os.path.exists", return_value=False)
@patch("os.path.join", return_value="/some/path")
@patch("blackwell.core.cluster.master.utils.mkdir_with_mode")
@patch("blackwell.core.cluster.master.metadata.__version__", "version")
@patch("blackwell.core.cluster.server.AbstractServerHandler.hello", return_value=(b"ok", "payload"))
def test_master_handler_hello_ok(super_hello_mock, mkdir_with_mode_mock, join_mock, path_exists_mock, sync_files_mock,
    ...
    join_mock.assert_called_once_with(common.BLACKWELL_PATH, "queue", "cluster", None)
    ...
    assert master_handler.agent_groups == "SyncBlackwelldbMock"


@patch("blackwell.core.cluster.master.metadata.__version__", "random")
@patch("blackwell.core.cluster.server.AbstractServerHandler.hello", return_value=(b"ok", "payload"))
def test_master_handler_hello_ko(super_hello_mock):
    ...
    #  Test the first exception
    with pytest.raises(exception.BlackwellClusterError, match=r".* 3030 .*"):
        master_handler.hello(b"name cluster_name node_type version")

    #  Test the second exception
    master_handler.server.configuration["name"] = "cluster_name"
    with pytest.raises(exception.BlackwellClusterError, match=r".* 3031 .*"):
        master_handler.hello(b"name cluster_name node_type version")
...
def test_master_handler_get_permission():
    ...
    class MockServer:
        ...
        def __init__(self):
            self.integrity_already_executed = ['not blackwell']

    master_handler = get_master_handler()
    master_handler.server = MockServer()
    master_handler.name = 'blackwell'
...
@patch("blackwell.core.cluster.common.BlackwellCommon.setup_send_info", return_value=b"ok")
...
@patch("blackwell.core.cluster.common.BlackwellCommon.error_receiving_file", return_value=b"ok")
...
@patch("blackwell.core.cluster.common.BlackwellCommon.end_receiving_file", return_value=b"ok")
...
@patch('blackwell.core.cluster.common.Handler.send_request', return_value='some_data')
...
async def test_master_handler_sync_blackwell_db_info(get_chunks_mock, update_chunks_mock, send_request_mock):
    """Check that the blackwell-db data reception task is created and chunks are obtained and updated in DB."""
    class LoggerMock:
        """Auxiliary class."""
    ...
    assert await master_handler.sync_blackwell_db_info(task_id=b'17', info_type='agent-groups') == 'some_data'
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.master.MasterHandler.recalculate_group_hash", return_value=AsyncMock())
@patch('blackwell.core.cluster.common.SyncBlackwelldb')
async def test_manager_handler_send_entire_agent_groups_information(syncblackwelldb_mock, recalculate_group_hash_mock):
    ...
    syncblackwelldb_mock.assert_called_once_with(manager=master_handler, logger=logger, cmd=b'syn_g_m_w_c',
                                             data_retriever=ANY,
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.master.AsyncBlackwellDBConnection")
async def test_manager_handler_send_agent_groups_information(asyncblackwelldbconnection_mock):
    """Check if the data chunks are being properly forward to the Blackwell-db socket."""
...
@patch("shutil.rmtree")
@patch("asyncio.wait_for")
@patch("blackwell.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
@patch('blackwell.core.cluster.master.cluster.run_in_pool',
...
async def test_master_handler_sync_worker_files_ok(run_in_pool_mock, decompress_files_mock, wait_for_mock, rmtree_mock):
    ...
    await asyncio.gather(
        master_handler.sync_worker_files("task_id", event, logging.getLogger("blackwell")),
        unlock_event(event))
...
@pytest.mark.asyncio
@patch("shutil.rmtree")
@patch('blackwell.core.cluster.master.cluster.run_in_pool', side_effect=Exception)
@patch("blackwell.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
@patch("blackwell.core.cluster.master.MasterHandler.wait_for_file")
async def test_master_handler_sync_worker_files_ko(wait_for_mock, decompress_files_mock, run_in_pool_mock, rmtree_mock):
    ...
    with pytest.raises(Exception):
        await master_handler.sync_worker_files("task_id", asyncio.Event(), logging.getLogger("blackwell"))

    # Test the second exception
    with pytest.raises(exception.BlackwellClusterError, match=r'.* 3038 .*'):
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.master.MasterHandler.set_date_end_master")
@patch("blackwell.core.cluster.master.MasterHandler.sync_worker_files")
async def test_master_handler_sync_extra_valid(sync_worker_files_mock, set_date_end_master_mock):
    ...
    master_handler = get_master_handler()
    master_handler.task_loggers["Integrity sync"] = logging.getLogger("blackwell")
    await master_handler.sync_extra_valid("task_id", None)

    sync_worker_files_mock.assert_called_once_with("task_id", None, logging.getLogger("blackwell"))
    set_date_end_master_mock.assert_called_once_with(logging.getLogger("blackwell"))
...
def test_set_date_end_master(info_mock):
    ...
    master_handler.set_date_end_master(logging.getLogger("blackwell"))
...
@patch.object(logging.getLogger("blackwell"), "info")
@patch.object(logging.getLogger("blackwell"), "debug")
@patch("blackwell.core.cluster.master.MasterHandler.integrity_sync")
@patch("blackwell.core.cluster.master.MasterHandler.wait_for_file")
@patch("blackwell.core.cluster.master.MasterHandler.send_request", return_value=b"ok")
@patch("blackwell.core.cluster.cluster.decompress_files", return_value=("files_metadata", "/decompressed/files/path"))
async def test_master_handler_integrity_check(decompress_files_mock, send_request_mock, wait_for_file_mock,
    ...
    master_handler.server = ServerMock()
    master_handler.sync_tasks = {"task_id": TaskMock()}
    master_handler.task_loggers["Integrity check"] = logging.getLogger("blackwell")

    with patch("blackwell.core.cluster.cluster.compare_files", return_value=({"extra_valid": compare_result})
               ) as compare_mock:
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.master.MasterHandler.wait_for_file", return_value=Exception())
...
@freeze_time("1970-01-01")
@patch("os.path.join", return_value="/some/path")
@patch('blackwell.core.cluster.master.utils.safe_move')
...
def test_master_handler_process_files_from_worker_ok(gid_mock, uid_mock, basename_mock, safe_move_mock, path_join_mock):
    ...
    path_join_mock.assert_called_once_with(common.BLACKWELL_PATH, "data")
    ...
    with patch("blackwell.core.cluster.cluster.unmerge_info",
        ...
        with patch('os.path.isfile', return_value=True) as isfile_mock:
            with patch('os.stat', return_value=StatMock()) as os_stas_mock:
                ...
                path_join_mock.assert_has_calls([call(common.BLACKWELL_PATH, 'data'),
                                                 call(common.BLACKWELL_PATH, '/file/path'),
                                                 call(common.BLACKWELL_PATH, 'queue', 'cluster', 'blackwell',
                                                      '/os/path/basename')])
...
@patch.object(logging.getLogger("blackwell"), "info")
@patch("blackwell.core.cluster.master.server.AbstractServerHandler.connection_lost")
@patch("blackwell.core.cluster.master.cluster.clean_up") 
def test_master_handler_connection_lost(clean_up_mock, connection_lost_mock, logger_mock, worker_name):
    ...
    master_handler = get_master_handler()
    master_handler.logger = logging.getLogger("blackwell")
...
@patch.object(logging.getLogger("blackwell"), "warning")
@patch('asyncio.get_running_loop', return_value=loop)
@patch("blackwell.core.cluster.master.ProcessPoolExecutor")
def test_master_init(pool_executor_mock, get_running_loop_mock, warning_mock):
    ...
    warning_mock.assert_has_calls([call("In order to take advantage of Blackwell 4.3.0 cluster improvements, the directory "
                                        "'/dev/shm' must be accessible by the 'blackwell' user. Check that this file has "
                                        "permissions to be accessed by all users. Changing the file permissions to 777 "
                                        "will solve this issue."),
                                   call('The Blackwell cluster will be run without the improvements added in Blackwell 4.3.0 '
                                        'and higher versions.')])
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch("blackwell.core.cluster.master.metadata.__version__", "1.0.0")
...
@patch('blackwell.core.cluster.master.perf_counter', return_value=0)
@patch('asyncio.sleep')
async def test_agent_groups_update(sleep_mock, perf_counter_mock):
    ...
    class AsyncBlackwellDBConnectionMock:
        """Auxiliary class."""
    ...
    with patch("blackwell.core.cluster.master.Master.setup_task_logger",
        ...
        with patch('blackwell.core.cluster.master.AsyncBlackwellDBConnection', AsyncBlackwellDBConnectionMock):
            with patch('blackwell.core.cluster.master.c_common.SyncBlackwelldb', SyncBlackwelldbMock):
...
async def test_master_file_status_update_ok(sleep_mock):
    ...
    with patch("blackwell.core.cluster.master.cluster.run_in_pool", side_effect=run_in_pool) as run_in_pool_mock:
        with patch("blackwell.core.cluster.master.Master.setup_task_logger",
                   return_value=logger_mock) as setup_task_logger_mock:
            with pytest.raises(Exception, match='Stop while true'):
...
@freeze_time("2021-11-02")
@patch('asyncio.sleep')
@patch('blackwell.core.cluster.master.cluster.run_in_pool', return_value={})
...
@patch('asyncio.get_running_loop', return_value=loop)
@patch("blackwell.core.agent.Agent.get_agents_overview", return_value={'totalItems': 5})
