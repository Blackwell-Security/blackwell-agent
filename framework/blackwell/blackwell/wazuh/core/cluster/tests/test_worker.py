...
import blackwell.core.exception as exception
...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core.cluster import client, worker, common as cluster_common
        from blackwell.core import common as core_common
...
configuration = {'node_name': 'master', 'nodes': ['master'], 'port': 1111, "name": "blackwell", "node_type": "master"}
...
def get_sync_blackwell_db(worker_handler):
    return cluster_common.SyncBlackwelldb(worker_handler, logging.getLogger("blackwell"), cmd=b"cmd",
...
async def test_rgit_init(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.ReceiveAgentGroupsTask.set_up_coro',
               side_effect=return_coro) as set_up_coro_mock:
        receive_agent_groups_task = worker.ReceiveAgentGroupsTask(blackwell_common=get_worker_handler(event_loop),
                                                                  logger=logging.getLogger("blackwell"), task_id="0101")
        assert isinstance(receive_agent_groups_task.blackwell_common, cluster_common.BlackwellCommon)
...
async def test_rgit_set_up_coro(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.recv_agent_groups_periodic_information',
...
async def test_rgcit_set_up_coro(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.recv_agent_groups_entire_information',
               return_value='') as recv_agent_mock:
        receive_agent_groups_task = worker.ReceiveEntireAgentGroupsTask(blackwell_common=get_worker_handler(event_loop),
                                                                        logger=logging.getLogger("blackwell"),
                                                                        task_id="0101")
        while not receive_agent_groups_task.task.done():
            await asyncio.sleep(0.01)
...
async def test_rgit_done_callback(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.recv_agent_groups_periodic_information',
        ...
        assert receive_agent_groups_task.blackwell_common.sync_agent_groups_free is True
...
async def test_rit_set_up_coro(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.process_files_from_master',
               return_value='') as process_files_mock:
        receive_task = worker.ReceiveIntegrityTask(blackwell_common=get_worker_handler(event_loop), logger=None)
        receive_task.blackwell_common = cluster_common.BlackwellCommon()
...
async def test_rit_done_callback(event_loop):
    ...
    with patch('blackwell.core.cluster.worker.ReceiveIntegrityTask.set_up_coro', side_effect=return_coro):
        receive_task = worker.ReceiveIntegrityTask(blackwell_common=get_worker_handler(event_loop), logger=None)
        receive_task.blackwell_common = cluster_common.BlackwellCommon()
        ...
        assert receive_task.blackwell_common.check_integrity_free is True


# Test SyncBlackwelldb class
@pytest.mark.asyncio
async def test_sync_blackwell_db_init(event_loop):
    """Test the '__init__' method from the SyncBlackwelldb class."""
    sync_blackwell_db = get_sync_blackwell_db(get_worker_handler(event_loop))
...
@patch('blackwell.core.cluster.worker.cluster.run_in_pool', return_value=True)
async def test_sync_blackwell_db_sync_ok(run_in_pool_mock, json_dumps_mock, event_loop):
    ...
    sync_blackwell_db = get_sync_blackwell_db(get_worker_handler(event_loop))
    sync_blackwell_db.data_retriever = callable_mock

    # Test try and if
    with patch.object(logging.getLogger("blackwell"), "debug") as logger_debug_mock:
        with patch("blackwell.core.cluster.worker.WorkerHandler.send_string", return_value=b"OK") as send_string_mock:
            with patch("blackwell.core.cluster.worker.WorkerHandler.send_request") as send_request_mock:
                assert await sync_blackwell_db.sync(start_time=10, chunks=["get_command"]) is True
    ...
    # Test else
    chunks = False
    with patch.object(logging.getLogger("blackwell"), "info") as logger_info_mock:
...
@freeze_time('1970-01-01')
@patch("json.dumps", return_value="")
@patch("blackwell.core.cluster.worker.WorkerHandler.send_string", return_value=b"Error")
async def test_sync_blackwell_db_sync_ko(send_string_mock, json_dumps_mock, event_loop):
    ...
    with pytest.raises(exception.BlackwellClusterError, match=r".* 3016 .*"):
        await sync_blackwell_db.sync(start_time=10, chunks=["get_command"])
...
@patch("blackwell.core.utils.mkdir_with_mode")
@patch("os.path.join", return_value="/some/path")
@patch("blackwell.core.cluster.worker.client.AbstractClient.connection_result")
async def test_worker_handler_connection_result(connection_result_mock, join_mock, mkdir_with_mode_mock, exists_mock,
    ...
    join_mock.assert_called_once_with(core_common.BLACKWELL_PATH, "queue", "cluster", "Testing")
...
@pytest.mark.asyncio
@patch.object(logging.getLogger("blackwell"), "debug")
async def test_worker_handler_process_request_ok(logger_mock, event_loop):
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.logger = logging.getLogger("blackwell")
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.sync_integrity_ok_from_master",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.setup_receive_files_from_master",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.end_receiving_integrity",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.error_receiving_integrity",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.setup_sync_integrity",
    ...
    with patch("blackwell.core.cluster.master.Master.setup_task_logger",
               worker_handler.setup_task_logger('Agent-info sync')) as setup_task_logger_mock:
        with patch("blackwell.core.cluster.worker.c_common.end_sending_agent_information",
    ...
    with patch("blackwell.core.cluster.worker.c_common.error_receiving_agent_information",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.forward_dapi_response",
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.forward_sendsync_response",
    ...
    with patch("blackwell.core.cluster.worker.client.AbstractClient.process_request",
...
@pytest.mark.asyncio
@patch.object(logging.getLogger("blackwell"), "info")
@patch("blackwell.core.cluster.worker.client.AbstractClient.connection_lost")
@patch("blackwell.core.cluster.worker.cluster.clean_up")
...
async def test_worker_handler_process_request_ko(logger_mock, event_loop):
    ...
    class ClientsMock:
        ...
        def send_request(self, command, error_msg):
            raise exception.BlackwellClusterError(1001)
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.server = ManagerMock()
    with pytest.raises(exception.BlackwellClusterError, match=r".* 1001 .*"):
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.BlackwellCommon.setup_receive_file", return_value=b"ok")
...
@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch.object(logging.getLogger("blackwell.Integrity check"), "info")
@patch("blackwell.core.cluster.common.BlackwellCommon.setup_receive_file", return_value="OK")
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.BlackwellCommon.end_receiving_file", return_value=(b"OK", b"OK"))
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.BlackwellCommon.error_receiving_file", return_value=(b"error", b"error"))
...
async def test_worker_compare_agent_groups_checksums(socket_mock, event_loop):
    ...
    sync_object = cluster_common.SyncBlackwelldb(manager=w_handler, logger=logger, cmd=b'syn_g_m_w',
    ...
    with patch('blackwell.core.cluster.worker.c_common.SyncBlackwelldb', return_value=sync_object):
        # Nothing is returned
...
@pytest.mark.asyncio
@patch('blackwell.core.cluster.worker.c_common.Handler.send_request')
async def test_worker_check_agent_groups_checksums(send_request_mock, event_loop):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.compare_agent_groups_checksums', return_value=False):
    ...
    with patch('blackwell.core.cluster.worker.WorkerHandler.compare_agent_groups_checksums', return_value=True):
...
@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch("blackwell.core.cluster.worker.WorkerHandler.recalculate_group_hash", return_value=AsyncMock())
@patch('blackwell.core.cluster.worker.WorkerHandler.check_agent_groups_checksums', return_value='')
@patch('blackwell.core.cluster.common.Handler.send_request', return_value='check')
@patch('blackwell.core.cluster.common.Handler.update_chunks_wdb', return_value={'updated_chunks': 1})
@patch('blackwell.core.cluster.common.Handler.get_chunks_in_task_id', return_value='chunks')
async def test_worker_handler_recv_agent_groups_information(get_chunks_in_task_id_mock, update_chunks_wdb_mock,
    ...
    """Check that the blackwell-db data reception task is created."""
...
@freeze_time('1970-01-01')
@pytest.mark.asyncio
@patch.object(blackwell.core.cluster.worker.json, "dumps", return_value="")
@patch.object(logging.getLogger("blackwell.Integrity check"), "info")
@patch.object(logging.getLogger("blackwell.Integrity check"), "error")
@patch("blackwell.core.cluster.common.SyncFiles.sync", return_value=True)
@patch("blackwell.core.cluster.cluster.get_files_status", return_value={})
@patch("blackwell.core.cluster.worker.client.common.Handler.send_request")
@patch("blackwell.core.cluster.common.SyncTask.request_permission", return_value=True)
async def test_worker_handler_sync_integrity(request_permission_mock,
    ...
    # Test the try
    with (patch('blackwell.core.cluster.worker.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as
          run_in_pool_mock):
        try:
        ...
        run_in_pool_mock.side_effect = exception.BlackwellException(1001)
        try:
            await asyncio.wait_for(worker_handler.sync_integrity(), 0.2)
        except asyncio.exceptions.TimeoutError:
            pass

        error_mock.assert_called_with(f"Error synchronizing integrity: {exception.BlackwellException(1001)}")
        json_dumps_mock.assert_called_with(exception.BlackwellException(1001), cls=cluster_common.BlackwellJSONEncoder)
        ...
        json_dumps_mock.assert_called_with(exception.BlackwellClusterError(code=1000, extra_message=str(Exception())),
...
@patch("blackwell.core.cluster.master.AsyncBlackwellDBConnection")
@patch('blackwell.core.cluster.common.SyncBlackwelldb')
async def test_worker_handler_sync_agent_info(SyncBlackwelldb_mock, AsyncBlackwellDBConnection_mock, sleep_mock, event_loop):
    ...
    SyncBlackwelldb_mock.return_value.request_permission = AsyncMock()
    SyncBlackwelldb_mock.return_value.retrieve_information = AsyncMock()
    SyncBlackwelldb_mock.return_value.sync = AsyncMock()
    ...
    SyncBlackwelldb_mock.assert_called_once_with(manager=w_handler, logger=logger, cmd=b'syn_a_w_m', data_retriever=ANY,
                                             get_data_command='global sync-agent-info-get ',
                                             set_data_command='global sync-agent-info-set')
    SyncBlackwelldb_mock.return_value.request_permission.assert_called_once()
    SyncBlackwelldb_mock.return_value.retrieve_information.assert_called_once()
    SyncBlackwelldb_mock.return_value.sync.assert_called_once_with(start_time=ANY, chunks=ANY)
...
@patch("blackwell.core.cluster.master.AsyncBlackwellDBConnection")
@patch('blackwell.core.cluster.common.SyncBlackwelldb')
async def test_worker_handler_sync_agent_info_ko(SyncBlackwelldb_mock, AsyncBlackwellDBConnection_mock, sleep_mock, event_loop):
    """Check that the agent-info task is properly configured."""
...
@pytest.mark.asyncio
@freeze_time('1970-01-01')
@patch.object(logging.getLogger("blackwell.Integrity sync"), "debug")
@patch("blackwell.core.cluster.common.SyncFiles.sync", return_value=True)
@patch('blackwell.core.cluster.worker.perf_counter', return_value=0)
@patch("blackwell.core.cluster.cluster.merge_info", return_value=("n_files", "merged_file"))
async def test_worker_handler_sync_extra_valid(merge_info_mock, perf_counter_mock, sync_mock, logger_debug_mock,
    ...
    worker_handler = get_worker_handler(event_loop)
    worker_handler.server = ManagerMock()
    with patch.object(logging.getLogger("blackwell.Integrity sync"), "info") as logger_info_mock:
    ...
    with patch("blackwell.core.cluster.worker.WorkerHandler.send_request") as send_request_mock:
        with patch.object(logging.getLogger("blackwell.Integrity sync"), "error") as logger_error_mock:
            merge_info_mock.side_effect = exception.BlackwellException(1001)
            cls = cluster_common.BlackwellJSONEncoder
            await worker_handler.sync_extra_valid(extra_valid)
            ...
            logger_error_mock.assert_called_once_with(
                f"Error synchronizing extra valid files: {exception.BlackwellException(1001)}")
            send_request_mock.assert_called_once_with(command=b'syn_i_w_m_r',
                                                      data=b'None ' + json.dumps(exception.BlackwellException(1001),
                                                                                 cls=cls).encode())
...
@patch("shutil.rmtree")
@patch("json.dumps", return_value="")
@patch("blackwell.core.cluster.cluster.decompress_files")
@patch.object(logging.getLogger("blackwell.Integrity sync"), "info")
@patch.object(logging.getLogger("blackwell.Integrity sync"), "debug")
@patch("blackwell.core.cluster.worker.client.common.Handler.send_request")
@patch("blackwell.core.cluster.worker.WorkerHandler.update_master_files_in_worker")
async def test_worker_handler_process_files_from_master_ok(update_files_mock, send_request_mock, logger_debug_mock,
    ...
    with patch('blackwell.core.cluster.cluster.run_in_pool', side_effect=cluster_run_in_pool_mock) as run_in_pool_mock:
        ...
        # Test first except
        worker_handler.sync_tasks["task_id"] = TaskMock()
        decompress_files_mock.side_effect = exception.BlackwellException(1001)
        event = asyncio.Event()
        await asyncio.gather(worker_handler.process_files_from_master(name="task_id", file_received=event),
                             unlock_event(event))
        ...
        json_dumps_mock.assert_called_once_with(exception.BlackwellException(1001), cls=cluster_common.BlackwellJSONEncoder)
        ...
        logger_debug_mock.assert_not_called()
        logger_info_mock.assert_called_once_with("Starting.")
        json_dumps_mock.assert_called_once_with(exception.BlackwellClusterError(code=1000, extra_message=str(Exception())),
                                                cls=cluster_common.BlackwellJSONEncoder)
        rmtree_mock.assert_not_called()
...
@patch("builtins.open")
@patch("os.path.exists", return_value=False)
@patch("blackwell.core.cluster.worker.safe_move")
@patch("blackwell.core.cluster.worker.utils.mkdir_with_mode")
@patch("os.path.join", return_value="queue/testing/")
@patch("blackwell.core.common.blackwell_uid", return_value="blackwell_uid")
@patch("blackwell.core.common.blackwell_gid", return_value="blackwell_gid")
async def test_worker_handler_update_master_files_in_worker_ok(blackwell_gid_mock, blackwell_uid_mock, path_join_mock,
    ...
    all_mocks = [blackwell_gid_mock, blackwell_uid_mock, path_join_mock, mkdir_with_mode_mock, safe_move_mock, open_mock,
                 path_exists_mock]
    ...
    with patch("blackwell.core.cluster.cluster.unmerge_info", return_value=[("name", "content", "_")]):
        with patch("os.remove") as os_remove_mock:
            ...
            path_join_mock.assert_has_calls([call(core_common.BLACKWELL_PATH, 'filename1'),
                                             call(core_common.BLACKWELL_PATH, 'name'),
                                             call(core_common.BLACKWELL_PATH, 'filename1'),
                                             call('/zip/path', 'filename1'),
                                             call(core_common.BLACKWELL_PATH, 'filename3')])
            blackwell_uid_mock.assert_called_with()
            blackwell_gid_mock.assert_called_with()
    ...
    path_join_mock.assert_has_calls([call(core_common.BLACKWELL_PATH, "filename1"),
                                     call(core_common.BLACKWELL_PATH, "filename2"),
                                     call(core_common.BLACKWELL_PATH, "filename3")])
    blackwell_uid_mock.assert_not_called()
    blackwell_gid_mock.assert_not_called()
    ...
    with patch("os.listdir", return_value="dir_files") as listdir_mock:
        with patch("shutil.rmtree") as rmtree_mock:
            ...
            path_join_mock.assert_has_calls([call(core_common.BLACKWELL_PATH, "filename3"),
                                             call(core_common.BLACKWELL_PATH, "")])
            blackwell_uid_mock.assert_not_called()
            blackwell_gid_mock.assert_not_called()
            mkdir_with_mode_mock.assert_not_called()
            safe_move_mock.assert_not_called()
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.worker.metadata.__version__", "1.0.0")
@patch("blackwell.core.cluster.worker.dapi.APIRequestQueue", return_value="APIRequestQueue object")
async def test_worker_init(api_request_queue, event_loop):
    ...
    assert nested_worker.cluster_name == "blackwell"
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.client.AbstractClientManager.add_tasks", return_value=["task"])
@patch("blackwell.core.cluster.worker.dapi.APIRequestQueue", return_value="APIRequestQueue object")
