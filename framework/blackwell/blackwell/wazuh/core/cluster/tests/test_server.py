...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        from blackwell.core.cluster.server import *
        from blackwell.core.cluster import common as c_common
        from blackwell.core.exception import BlackwellClusterError, BlackwellError, BlackwellResourceNotFound
...
async def test_AbstractServerHandler_init(event_loop):
    ...
    with patch("blackwell.core.cluster.server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("blackwell.core.cluster.server.AbstractServerHandler.hello")
@patch("blackwell.core.cluster.server.AbstractServerHandler.echo_master")
@patch("blackwell.core.cluster.common.Handler.process_request")
async def test_AbstractServerHandler_process_request(mock_process_request, mock_echo_master, mock_hello, event_loop):
    ...
    abstract_server_handler.process_request(command=b"echo-c", data=b"blackwell")
    mock_echo_master.assert_called_once_with(b"blackwell")
...
async def test_AbstractServerHandler_echo_master(event_loop):
    ...
    assert abstract_server_handler.echo_master(data=b"blackwell") == (b"ok-m ", b"blackwell")
    abstract_server_handler.echo_master(data=b"blackwell")
...
def test_AbstractServerHandler_hello(event_loop):
    ...
    with pytest.raises(BlackwellClusterError, match=".* 3029 .*"):
        abstract_server_handler.hello(b"elif_test")

    abstract_server_handler.server.clients["if_test"] = "testing"
    with pytest.raises(BlackwellClusterError, match=f".* 3028 .* if_test"):
        abstract_server_handler.hello(b"if_test")
    assert abstract_server_handler.name == ""


@pytest.mark.asyncio
@patch("blackwell.core.cluster.common.Handler.process_response")
...
@pytest.mark.asyncio
@patch("asyncio.Queue")
@patch("blackwell.core.cluster.server.functools")
...
@patch('blackwell.core.cluster.server.AbstractServer.check_clients_keepalive')
@patch('blackwell.core.cluster.server.AbstractServerHandler')
...
@patch("blackwell.core.cluster.server.uuid4", return_value="abc123")
@patch("asyncio.get_running_loop", new=Mock())
...
@patch("blackwell.core.cluster.server.utils.process_array")
@patch("asyncio.get_running_loop", new=Mock())
def test_AbstractServer_get_connected_nodes(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        abstract_server.get_connected_nodes(search={"value": "blackwell", "negation": True},
                                            sort={"fields": ["nothing"], "order": "desc"}, limit=501, offset=1)
        mock_process_array.assert_called_once_with([basic_dict["info"]], search_text="blackwell", complementary_search=True,
...
def test_AbstractServer_get_connected_nodes_ko(mock_process_array):
    ...
    with patch.object(abstract_server, "to_dict", return_value=basic_dict):
        ...
        with pytest.raises(BlackwellError, match=".* 1724 .* Not a valid select field: no"):
            abstract_server.get_connected_nodes(select={"no": "exists"})

        with pytest.raises(BlackwellError, match=".* 1728 .*"):
            abstract_server.get_connected_nodes(filter_type="None")

        abstract_server.configuration = {"node_name": "master"}
        with pytest.raises(BlackwellResourceNotFound, match=".* 1730 .*"):
...
async def test_AbstractServer_check_clients_keepalive(sleep_mock):
    ...
    with patch.object(logger, "debug") as mock_debug:
        with patch.object(logger, "error") as mock_error:
            with patch("blackwell.core.cluster.server.AbstractServer.setup_task_logger",
                       return_value=logger):
                abstract_server = AbstractServer(performance_test=1, concurrency_test=2, configuration={"test3": 3},
                                                 cluster_items={"test4": 4}, enable_ssl=True, logger=logger)
...
@patch("asyncio.sleep", side_effect=IndexError)
@patch("asyncio.get_running_loop", new=Mock())
@patch('blackwell.core.cluster.server.perf_counter', return_value=0)
