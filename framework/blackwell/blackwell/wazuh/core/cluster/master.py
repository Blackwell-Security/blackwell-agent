...
from blackwell.core import cluster as metadata, common, exception, utils
from blackwell.core.agent import Agent
from blackwell.core.cluster import server, cluster, common as c_common
from blackwell.core.cluster.dapi import dapi
from blackwell.core.cluster.utils import context_tag, log_subprocess_execution
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.integrity_check
    ...
    def done_callback(self, future=None):
        ...
        # if not self.blackwell_common.extra_valid_requested:
        #     self.blackwell_common.sync_integrity_free[0] = True
...
class ReceiveExtraValidTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.sync_extra_valid
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.blackwell_common.extra_valid_requested = False
        self.blackwell_common.sync_integrity_free[0] = True
...
class ReceiveAgentInfoTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.sync_blackwell_db_info
    ...
    def done_callback(self, future=None):
        ...
        """
        super().done_callback(future)
        self.blackwell_common.sync_agent_info_free = True
...
class SendEntireAgentGroupsTask(c_common.SendStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.send_entire_agent_groups_information


class MasterHandler(server.AbstractServerHandler, c_common.BlackwellCommon):
    """
    ...
    async def execute(self, command: bytes, data: bytes, wait_for_complete: bool = False) -> Dict:
        ...
        if command == b'dapi_fwd':
            ...
            if client in self.server.clients:
                ...
                raise exception.BlackwellClusterError(3022, extra_message=client)
        ...
        if command == b'dapi' or command == b'dapi_fwd':
            try:
                ...
                raise exception.BlackwellClusterError(3021)
        # Otherwise, immediately return the result obtained before.
        else:
            status, request_result = result
            if status != b'ok':
                raise exception.BlackwellClusterError(3022, extra_message=request_result.decode())
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            Node name, cluster name, node type and blackwell version all separated by spaces.
        ...
        if self.cluster_name != self.server.configuration['name']:
            raise exception.BlackwellClusterError(3030)
        elif self.version != metadata.__version__:
            raise exception.BlackwellClusterError(3031)

        # Create directory where zips and other files coming from or going to the worker will be managed.
        worker_dir = os.path.join(common.BLACKWELL_PATH, 'queue', 'cluster', self.name)
        ...
        # SyncBlackwelldb instance to send agent-groups data to the worker.
        wdb_conn = AsyncBlackwellDBConnection()
        self.agent_groups = c_common.SyncBlackwelldb(manager=self, logger=self.task_loggers['Agent-groups send'],
    ...
    def get_manager(self) -> server.AbstractServer:
        """Get the Master object that created this MasterHandler. Used in the class BlackwellCommon.
    ...
    def process_dapi_res(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if req_id in self.server.pending_api_requests:
            ...
            raise exception.BlackwellClusterError(3032, extra_message=req_id)
    ...
    async def sync_blackwell_db_info(self, task_id: bytes, info_type: str):
        """Create a process to send to the local blackwell-db the chunks of data received from a worker.
    ...
    async def send_entire_agent_groups_information(self):
        ...
        sync_object = c_common.SyncBlackwelldb(manager=self, logger=logger, cmd=b'syn_g_m_w_c',
                                           data_retriever=AsyncBlackwellDBConnection().run_wdb_command,
    ...
    async def sync_worker_files(self, task_id: str, received_file: asyncio.Event, logger):
        ...
        try:
            ...
            raise exception.BlackwellClusterError(3038, extra_message=str(e))
        finally:
            shutil.rmtree(decompressed_files_path)
                ...
                for file_path, data in files_metadata.items():
                    ...
                    if data['merged']:
                        for unmerged_file_path, file_data, file_time in cluster.unmerge_info(
                            ...
                            try:
                                # Destination path.
                                full_unmerged_name = os.path.join(common.BLACKWELL_PATH, unmerged_file_path)
                                # Path where to create the file before moving it to the destination path.
                                tmp_unmerged_path = os.path.join(common.BLACKWELL_PATH, 'queue', 'cluster', worker_name,
                                ...
                                utils.safe_move(tmp_unmerged_path, full_unmerged_name,
                                                ownership=(common.blackwell_uid(), common.blackwell_gid()),
                    ...
                    # If the file is not 'merged' type, move it directly to the destination path.
                        ...
                        try:
                            ...
                            utils.safe_move(zip_path, full_path, ownership=(common.blackwell_uid(), common.blackwell_gid()),
                                            permissions=cluster_items['files'][item_key]['permissions'])
                        except TimeoutError as e:
                            raise e
                        except Exception as e:
...
class Master(server.AbstractServer):
    ...
    def __init__(self, **kwargs):
        ...
        # Handle exception when the user running Blackwell cannot access /dev/shm
            ...
            self.logger.warning(
                "In order to take advantage of Blackwell 4.3.0 cluster improvements, the directory '/dev/shm' must be "
                "accessible by the 'blackwell' user. Check that this file has permissions to be accessed by all users. "
                "Changing the file permissions to 777 will solve this issue.")
            self.logger.warning(
                "The Blackwell cluster will be run without the improvements added in Blackwell 4.3.0 and higher versions.")
            self.task_pool = None
    ...
    async def agent_groups_update(self):
        ...
        wdb_conn = AsyncBlackwellDBConnection()
        sync_object = c_common.SyncBlackwelldb(manager=self, logger=logger, cmd=b'syn_g_m_w',
