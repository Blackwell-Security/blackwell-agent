...
from blackwell.core import common, exception
from blackwell.core import utils
from blackwell.core.cluster import cluster, utils as cluster_utils
from blackwell.core.wdb import AsyncBlackwellDBConnection, BlackwellDBConnection
...
class SendStringTask:
    ...
    def __init__(self, blackwell_common, logger):
        """Class constructor.

        Parameters
        ----------
        blackwell_common : BlackwellCommon object
            Instance of BlackwellCommon.
        logger : Logger object
            Logger to use during the reception process.
        """
        self.blackwell_common = blackwell_common
...
class ReceiveStringTask:
    ...
    def __init__(self, blackwell_common, logger, task_id, info_type='agent-info'):
        """Class constructor.
    ...
    def done_callback(self, future=None):
        ...
        Remove string and task_id (if exist) from sync_tasks dict. If task was not cancelled, raise stored exception.
        """
        if self.task_id in self.blackwell_common.in_str:
            # pop() is used instead of 'del' so an exception is never raised here
            self.blackwell_common.in_str.pop(self.task_id, None)
        if self.task_id in self.blackwell_common.sync_tasks:
            del self.blackwell_common.sync_tasks[self.task_id]
        if not self.task.cancelled():
            task_exc = self.task.exception()
            if task_exc:
...
class ReceiveFileTask:
    ...
    def __init__(self, blackwell_common, logger, task_id: bytes = b''):
        """Class constructor.
...
class Handler(asyncio.Protocol):
    ...
    def __init__(self, fernet_key: str, cluster_items: Dict, logger: logging.Logger = None, tag: str = "Handler"):
        ...
        self.logger = logging.getLogger('blackwell') if not logger else logger
        # Logging tag.
        self.tag = tag
    ...
    def msg_build(self, command: bytes, counter: int, data: bytes) -> List[bytearray]:
        ...
        if cmd_len > self.cmd_len - len(InBuffer.divide_flag):
            raise exception.BlackwellClusterError(3024, extra_message=command)
        ...
        while parsed:
            if self.in_msg.received == self.in_msg.total:
                ...
                try:
                    ...
                    raise exception.BlackwellClusterError(3025)
    ...
    async def send_request(self, command: bytes, data: bytes) -> Union[exception.BlackwellClusterError, Any]:
        ...
        try:
            ...
            raise exception.BlackwellClusterError(3026)
        except Exception as e:
            raise exception.BlackwellClusterError(3018, extra_message=str(e))
        try:
    ...
    async def get_chunks_in_task_id(self, task_id: bytes, error_command: bytes) -> dict:
        ...
        try:
            ...
            raise exception.BlackwellClusterError(3035, extra_message=f"it should be under task_id {str(e)}, "
                                                                  f"but it's empty.")
        except ValueError as e:
            with contextlib.suppress(Exception):
                await self.send_request(command=error_command,
                                        data=f'error while trying to load JSON: {str(e)}'.encode())
            raise exception.BlackwellClusterError(3036, extra_message=str(e))
    ...
    async def update_chunks_wdb(self, data: dict, info_type: str, logger: logging.Logger, error_command: bytes,
        ...
        ----------
        data : dict
            Dict containing command and list of chunks to be sent to blackwell-db.
        info_type : str
            Information type handled.
        logger : Logger object
            Logger to use.
        ...
        try:
            ...
            raise exception.BlackwellClusterError(3037, extra_message=str(e))
        ...
        for error in result['error_messages']['chunks']:
            ...
            logger.error(f'Blackwell-db response for chunk {error[0] + 1}/{len(data["chunks"])} was not "ok": {error[1]}')

        logger.debug(f'{result["updated_chunks"]}/{len(data["chunks"])} chunks updated in blackwell-db '
    ...
    async def send_file(self, filename: str, task_id: bytes = None) -> int:
        ...
        if not os.path.exists(filename):
            raise exception.BlackwellClusterError(3034, extra_message=filename)

        sent_size = 0
        filename = filename.encode()
        relative_path = filename.replace(common.BLACKWELL_PATH.encode(), b'')

        try:
            # Tell to the destination node where (inside blackwell_path) the file has to be written.
            await self.send_request(command=b'new_file', data=relative_path)
        except exception.BlackwellClusterError as e:
            if e.code != 3020:
                raise e
    ...
    async def forward_dapi_response(self, data: bytes):
        ...
        try:
            ...
            if isinstance(e, exception.BlackwellException):
                exc = json.dumps(e, cls=BlackwellJSONEncoder)
            else:
                exc = json.dumps(exception.BlackwellClusterError(1000, extra_message=str(e)), cls=BlackwellJSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def dispatch(self, command: bytes, counter: int, payload: bytes) -> None:
        ...
        try:
            ...
            command, payload = b'err', json.dumps(e, cls=BlackwellJSONEncoder).encode()
        except Exception as e:
            self.logger.error(f"Unhandled error processing request '{command}': {e}", exc_info=True)
            command, payload = b'err', json.dumps(exception.BlackwellInternalError(1000, extra_message=str(e)),
                                                  cls=BlackwellJSONEncoder).encode()
    ...
    def process_dapi_error(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if dapi_client.decode() in self.server.local_server.clients:
            ...
            except exception.BlackwellClusterError:
                raise exception.BlackwellClusterError(3025)
        else:
            raise exception.BlackwellClusterError(3032, extra_message=dapi_client.decode())
    ...
    def receive_file(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        bytes
            Response message.
        """
        self.in_file[data] = {'fd': open(common.BLACKWELL_PATH + data.decode(), 'wb'), 'checksum': hashlib.sha256()}
    ...
    def cancel_task(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : bytes
            String containing task_id and BlackwellJSONEncoded object with the exception details.
        ...
        error_json = json.loads(error_details, object_hook=as_blackwell_object)
        if task_id != b'None':
    ...
    def process_error_from_peer(self, data: bytes) -> bytes:
        ...
        """
        try:
            exc = json.loads(data.decode(), object_hook=as_blackwell_object)
        except json.JSONDecodeError as e:
            exc = exception.BlackwellClusterError(3000, extra_message=data.decode())
    ...
    async def wait_for_file(self, file, task_id):
        ...
        try:
            ...
            if isinstance(e, asyncio.TimeoutError):
                exc = exception.BlackwellClusterError(3039)
            else:
                exc = exception.BlackwellClusterError(3040, extra_message=str(e))
            # Notify the sending node to stop its task.
            with contextlib.suppress(Exception):
                await self.send_request(command=b"cancel_task",
                                        data=f"{task_id} {json.dumps(exc, cls=BlackwellJSONEncoder)}".encode())
            raise exc
...
class BlackwellCommon:
    """
    Task implementing common methods for both clients and servers that are Blackwell specific.
    """
    ...
    async def recalculate_group_hash(logger) -> None:
        ...
        try:
            ...
            await AsyncBlackwellDBConnection().run_wdb_command(command='global recalculate-agent-group-hashes')
        except (exception.BlackwellInternalError, exception.BlackwellError) as e:
    ...
    def end_receiving_file(self, task_and_file_names: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        if task_id not in self.sync_tasks:
            ...
            if os.path.exists(os.path.join(common.BLACKWELL_PATH, filename)):
                try:
                    os.remove(os.path.join(common.BLACKWELL_PATH, filename))
                except Exception as e:
                    self.get_logger(logger_tag).error(
                        f"Attempt to delete file {os.path.join(common.BLACKWELL_PATH, filename)} failed: {e}")
            raise exception.BlackwellClusterError(3027, extra_message=task_id)

        # Set full path to file for task 'task_id' and notify it is ready to be read, so the lock is released.
        self.sync_tasks[task_id].filename = os.path.join(common.BLACKWELL_PATH, filename)
    ...
    def error_receiving_file(self, task_id_and_error_details: str, logger_tag: str = '') -> Tuple[bytes, bytes]:
        ...
        task_id_and_error_details : str
             BlackwellJSONEncoded object with the exception details.
        logger_tag : str
            Logger task to use. If empty, it will use main class logger.
        ...
        error_details_json = json.loads(error_details, object_hook=as_blackwell_object)
    ...
    async def sync(self, files: Iterable, files_metadata: Dict, metadata_len: int, task_pool=None,
        ...
        try:
            ...
            await self.server.send_request(
                ...
                data=task_id + b' ' + os.path.relpath(compressed_data, common.BLACKWELL_PATH).encode()
            )
        except Exception as e:
            self.logger.error(f"Error sending zip file: {e}")
            if isinstance(e, exception.BlackwellException):
                exc = json.dumps(e, cls=BlackwellJSONEncoder).encode()
            else:
                exc = json.dumps(exception.BlackwellClusterError(1000, extra_message=str(e)), cls=BlackwellJSONEncoder).encode()
            with contextlib.suppress(Exception):
...
class SyncBlackwelldb(SyncTask):
    """
    Define methods to send information to the master/worker node (blackwell-db) through send_string protocol.
    """
    ...
    def __init__(self, manager, logger, data_retriever: Callable, cmd: bytes = b'', get_data_command: str = '',
        ...
        get_data_command : str
            Command to retrieve data from local blackwell-db.
        set_data_command : str
            Command to set data in master/worker's blackwell-db.
        logger : Logger object
            Logger to use during synchronization process.
    ...
    async def retrieve_information(self):
        ...
        try:
            # Retrieve information from local blackwell-db
            ...
            self.logger.error(f"Could not obtain data from blackwell-db: {e}")
            return []
    ...
    async def sync(self, start_time: float, chunks: List):
        ...
        if chunks:
            ...
            if task_id.startswith(b'Error'):
                raise exception.BlackwellClusterError(3016, extra_message=f'String with agents information could '
...
def send_data_to_wdb(data, timeout, info_type='agent-info'):
    """Send chunks of data to Blackwell-db socket.
    ...
    wdb_conn = BlackwellDBConnection()
...
class BlackwellJSONEncoder(json.JSONEncoder):
    """
    Define special JSON encoder for Blackwell.
    """
    ...
    def default(self, obj):
        if callable(obj):
            ...
            if hasattr(obj, '__self__'):
                if isinstance(obj.__self__, Blackwell):
                    attributes['__blackwell__'] = obj.__self__.to_dict()
            attributes['__type__'] = type(obj).__name__
            return result
        elif isinstance(obj, exception.BlackwellException):
            result = {'__blackwell_exception__': {'__class__': obj.__class__.__name__,
                                              '__object__': obj.to_dict()}}
            return result
        elif isinstance(obj, wresults.AbstractBlackwellResult):
            result = {'__blackwell_result__': {'__class__': obj.__class__.__name__,
                                           '__object__': obj.encode_json()}
                      }
            return result
        elif isinstance(obj, (datetime.datetime, datetime.date)):
            return {'__blackwell_datetime__': obj.isoformat()}
...
def as_blackwell_object(dct: Dict):
    try:
        if '__callable__' in dct:
            encoded_callable = dct['__callable__']
            funcname = encoded_callable['__name__']
            if '__blackwell__' in encoded_callable:
                # Encoded Blackwell instance method.
                blackwell = Blackwell()
                return getattr(blackwell, funcname)
            else:
        ...
        elif '__blackwell_exception__' in dct:
            blackwell_exception = dct['__blackwell_exception__']
            return getattr(exception, blackwell_exception['__class__']).from_dict(blackwell_exception['__object__'])
        elif '__blackwell_result__' in dct:
            blackwell_result = dct['__blackwell_result__']
            return getattr(wresults, blackwell_result['__class__']).decode_json(blackwell_result['__object__'])
        elif '__blackwell_datetime__' in dct:
            return datetime.datetime.fromisoformat(dct['__blackwell_datetime__'])
        ...
        raise exception.BlackwellInternalError(1000,
                                           extra_message=f"Blackwell object cannot be decoded from JSON {dct}",
                                           cmd_error=True)
