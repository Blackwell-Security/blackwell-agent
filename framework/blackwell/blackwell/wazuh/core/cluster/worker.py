...
from blackwell.core.cluster import client, cluster, common as c_common
from blackwell.core.cluster.utils import log_subprocess_execution
from blackwell.core.cluster.dapi import dapi
from blackwell.core.utils import safe_move, get_utc_now
...
class ReceiveAgentGroupsTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.recv_agent_groups_periodic_information
    ...
    def done_callback(self, future=None):
        ...
        self.blackwell_common.sync_agent_groups_free = True
...
class ReceiveEntireAgentGroupsTask(c_common.ReceiveStringTask):
    ...
    def set_up_coro(self) -> Callable:
        """Set up the function to be called when the worker sends its Agent groups."""
        return self.blackwell_common.recv_agent_groups_entire_information
...
class ReceiveIntegrityTask(c_common.ReceiveFileTask):
    ...
    def set_up_coro(self) -> Callable:
        ...
        return self.blackwell_common.process_files_from_master
    ...
    def done_callback(self, future=None):
        ...
        future : asyncio.Future object
            Synchronization process result.
        """
        self.blackwell_common.check_integrity_free = True
        super().done_callback(future)


class WorkerHandler(client.AbstractClient, c_common.BlackwellCommon):
    """
    ...
    def connection_result(self, future_result):
        ...
        if self.connected:
            ...
            worker_tmp_files = os.path.join(common.BLACKWELL_PATH, 'queue', 'cluster', self.name)
    ...
    def get_manager(self):
        """Get the Worker object that created this WorkerHandler. Used in the class BlackwellCommon.
    ...
    def error_receiving_integrity(self, data: str) -> Tuple[bytes, bytes]:
        ...
        ----------
        data : str
            Task ID and error formatted as BlackwellJSONEncoder.
        ...
        while True:
            ...
            # If exception is raised during sync process, notify the master so it removes the file if received.
                ...
                if isinstance(e, exception.BlackwellException):
                    exc = json.dumps(e, cls=c_common.BlackwellJSONEncoder)
                else:
                    exc = json.dumps(exception.BlackwellClusterError(1000, extra_message=str(e)),
    ...
    async def sync_agent_info(self):
        ...
        A list of JSON chunks with the information of all local agents is retrieved from local blackwell-db socket
        and sent to the master's blackwell-db.
        """
        logger = self.task_loggers["Agent-info sync"]
        wdb_conn = AsyncBlackwellDBConnection()
        agent_info = c_common.SyncBlackwelldb(manager=self, logger=logger, cmd=b'syn_a_w_m',
                                          data_retriever=wdb_conn.run_wdb_command,
    ...
    async def sync_extra_valid(self, extra_valid: Dict):
        ...
        # If exception is raised during sync process, notify the master, so it removes the file if received.
            ...
            if isinstance(e, exception.BlackwellException):
                ...
                exc = json.dumps(exception.BlackwellClusterError(1000, extra_message=str(e)), cls=c_common.BlackwellJSONEncoder)
            with contextlib.suppress(Exception):
                await self.send_request(command=b'syn_i_w_m_r', data=f"None {exc}".encode())
    ...
    async def process_files_from_master(self, name: str, file_received: asyncio.Event):
        ...
        if isinstance(received_filename, Exception):
            exc_info = json.dumps(exception.BlackwellClusterError(
                1000, extra_message=str(self.sync_tasks[name].filename)), cls=c_common.BlackwellJSONEncoder)
            with contextlib.suppress(Exception):
    ...
    def update_master_files_in_worker(ko_files: Dict, zip_path: str, cluster_items: Dict):
        ...
        def overwrite_or_create_files(filename_: str, data_: Dict):
            ...
            """
            full_filename_path = os.path.join(common.BLACKWELL_PATH, filename)
            ...
            if data_['merged']:  # worker nodes can only receive agent-groups files
                ...
                # and then move each one to the destination directory (<blackwell_path>/filename).
                # The TYPE string used in the 'unmerge_info' function is a placeholder. It corresponds to the
                # directory inside '{blackwell_path}/queue/' path.
                for name, content, _ in cluster.unmerge_info('TYPE', zip_path, filename_):
                    full_unmerged_name = os.path.join(common.BLACKWELL_PATH, name)
                ...
                # Move the file from zipdir (directory containing unzipped files) to <blackwell_path>/filename.
            ...
            # Remove local files marked as extra.
                ...
                for file_to_remove in files:
                    try:
                        ...
                        file_path = os.path.join(common.BLACKWELL_PATH, file_to_remove)
                        try:
                            os.remove(file_path)
                        except OSError as e:
        ...
        for directory in directories_to_check:
            try:
                full_path = os.path.join(common.BLACKWELL_PATH, directory)
