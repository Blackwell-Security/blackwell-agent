...
from blackwell.core import common, exception, utils
...
class AbstractServerHandler(c_common.Handler):
    ...
    def hello(self, data: bytes) -> Tuple[bytes, bytes]:
        ...
        if self.name in self.server.clients:
            self.name = ''
            raise exception.BlackwellClusterError(3028, extra_message=data.decode())
        elif self.name == self.server.configuration['node_name']:
            raise exception.BlackwellClusterError(3029)
        else:
...
class AbstractServer:
    ...
    def get_connected_nodes(self, filter_node: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
        ...
        if select is None:
            ...
            if not set(select).issubset(default_fields):
                raise exception.BlackwellError(1724, extra_message=', '.join(set(select) - default_fields),
                                           extra_remediation=', '.join(default_fields))

        if filter_type != 'all' and filter_type not in {'worker', 'master'}:
            raise exception.BlackwellError(1728)
        ...
        if filter_node is not None:
            ...
            if not filter_node.issubset(set(itertools.chain(self.clients.keys(), [self.configuration['node_name']]))):
                raise exception.BlackwellResourceNotFound(1730)
    ...
    async def start(self):
        ...
        if self.enable_ssl:
            ...
            ssl_context.load_cert_chain(certfile=os.path.join(common.BLACKWELL_PATH, 'etc', 'sslmanager.cert'),
                                        keyfile=os.path.join(common.BLACKWELL_PATH, 'etc', 'sslmanager.key'))
        else:
            ssl_context = None
