...
from blackwell.core.utils import blake2b, get_date_from_timestamp, get_utc_now, mkdir_with_mode, to_relative_path
...
def check_cluster_config(config):
    ...
    if len(config['key']) == 0:
        raise BlackwellError(3004, 'Unspecified key')

    elif not iv.check_name(config['key']) or not iv.check_length(config['key'], 32, eq):
        raise BlackwellError(3004, 'Key must be 32 characters long and only have alphanumeric characters')

    elif config['node_type'] != 'master' and config['node_type'] != 'worker':
        raise BlackwellError(3004, f'Invalid node type {config["node_type"]}. Correct values are master and worker')

    elif not isinstance(config['port'], int):
        raise BlackwellError(3004, "Port has to be an integer.")

    elif not MIN_PORT < config['port'] < MAX_PORT:
        raise BlackwellError(3004, f"Port must be higher than {MIN_PORT} and lower than {MAX_PORT}.")
    ...
    if len(invalid_elements) != 0:
        raise BlackwellError(3004, f"Invalid elements in node fields: {', '.join(invalid_elements)}.")
...
def walk_dir(dirname, recursive, files, excluded_files, excluded_extensions, get_cluster_item_key, previous_status=None,
    ...
    full_dirname = path.join(common.BLACKWELL_PATH, dirname)
    ...
    try:
                ...
                for file_ in files_:
                    ...
                    try:
                        ...
                        if files == ['all'] or file_ in files:
                            relative_file_path = path.join(path.relpath(root_, common.BLACKWELL_PATH), file_)
        ...
        raise BlackwellInternalError(3015, e)
...
def compress_files(name, list_path, cluster_control_json=None, max_zip_size=None):
    ...
    zip_file_path = path.join(common.BLACKWELL_PATH, 'queue', 'cluster', name,
    ...
    with open(zip_file_path, 'ab') as wf:
        for file in list_path:
            ...
            try:
                with open(path.join(common.BLACKWELL_PATH, file), 'rb') as rf:
                    new_file = rf.read()
                    if len(new_file) > max_zip_size:
                        result_logs['warning'][file].append(f'File too large to be synced: '
                                                            f'{path.join(common.BLACKWELL_PATH, file)}')
                        update_cluster_control(file, cluster_control_json)
                        continue
                ...
                raise BlackwellError(3001, str(e))
            except Exception as e:
                result_logs['debug'][file].append("Exception raised: " + str(BlackwellException(3001, str(e))))
...
def clean_up(node_name=""):
    ...
    try:
        rm_path = path.join(common.BLACKWELL_PATH, 'queue', 'cluster', node_name)
...
def merge_info(merge_type, node_name, files=None, file_type=""):
    ...
    ----------
    merge_type : str
        Directory inside {blackwell_path}/PATH where the files to merge can be found.
    node_name : str
    ...
    """
    merge_path = path.join(common.BLACKWELL_PATH, 'queue', merge_type)
    ...
    with open(path.join(common.BLACKWELL_PATH, output_file), 'wb') as o_f:
...
def unmerge_info(merge_type, path_file, filename):
    ...
    ----------
    merge_type : str
        Name of the destination directory inside queue. I.e: {blackwell_path}/PATH/{merge_type}/<unmerge_files>.
    path_file : str
