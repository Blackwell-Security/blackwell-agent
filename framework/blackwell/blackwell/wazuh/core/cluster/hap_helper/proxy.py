...
import httpx
from blackwell.core.cluster.utils import ClusterFilter, context_tag
from blackwell.core.exception import BlackwellHAPHelperError
...
class ProxyAPI:
        ...
        try:
            async with httpx.AsyncClient(verify=self.haproxy_cert, cert=self.client_cert) as client:
                ...
                if response.status_code == 401:
                    raise BlackwellHAPHelperError(3046)
                elif response.status_code == 404:
                    raise BlackwellHAPHelperError(3047)
        except httpx.ConnectError:
            raise BlackwellHAPHelperError(
                3043, extra_message='Check connectivity and the configuration in the `ossec.conf`'
            )
        except httpx.RequestError as req_exc:
            raise BlackwellHAPHelperError(3043, extra_message=str(req_exc))
    ...
    async def _make_hap_request(
        ...
        try:
            ...
            raise BlackwellHAPHelperError(3044, extra_message=str(request_exc))
        ...
        if response.is_success:
            ...
            raise BlackwellHAPHelperError(3045, extra_message=response.json()['message'])
...
class Proxy:
    def __init__(
        self,
        blackwell_backend: str,
        proxy_api: ProxyAPI,
        tag: str,
        blackwell_connection_port: int = 1514,
        resolver: str = None,
    ):
        self.tag = tag
        self.logger = self._get_logger(self.tag)
        self.blackwell_backend = blackwell_backend
        self.blackwell_connection_port = blackwell_connection_port
        self.api = proxy_api
    ...
    def _get_logger(tag: str) -> logging.Logger:
        ...
        """
        logger = logging.getLogger('blackwell').getChild('Proxy')
    ...
    async def initialize(self):
        ...
        try:
            ...
            raise BlackwellHAPHelperError(3048)
    ...
    async def get_current_backend_servers(self) -> dict:
        ...
        dict
            The backend servers.
        """
        api_response = await self.api.get_backend_servers(self.blackwell_backend)
        self.logger.debug2('Obtained proxy servers')
        return {server['name']: server['address'] for server in api_response['data']}

    async def add_blackwell_manager(self, manager_name: str, manager_address: str, resolver: Optional[str] = None) -> dict:
        """Add a new Blackwell manager to the Proxy.

        Parameters
        ----------
        manager_name : str
            Name of the Blackwell manager.
        manager_address : str
            IP or DNS for the Blackwell manager.
        resolver : Optional[str]
        ...
        api_response = await self.api.add_server_to_backend(
            backend=self.blackwell_backend,
            server_name=manager_name,
            server_address=manager_address,
            port=self.blackwell_connection_port,
            resolver=resolver,
        )
        self.logger.debug2(
            f"Added new server '{manager_name}' {manager_address}:{self.blackwell_connection_port} to backend"
            f" '{self.blackwell_backend}'"
        )
        return api_response

    async def remove_blackwell_manager(self, manager_name: str) -> dict:
        """Delete the given Blackwell manager from the Proxy.
        ...
        """
        api_response = await self.api.remove_server_from_backend(backend=self.blackwell_backend, server_name=manager_name)

        self.logger.debug2(f"Removed server {manager_name} from backend '{self.blackwell_backend}'")
        return api_response
    ...
    async def restrain_server_new_connections(self, server_name: str) -> dict:
        ...
        api_response = await self.api.change_backend_server_state(
            backend_name=self.blackwell_backend, server_name=server_name, state=ProxyServerState.DRAIN
        )
        self.logger.debug2(f"Changed Blackwell server '{server_name}' to {ProxyServerState.DRAIN.value.upper()} state")
        return api_response
    ...
    async def allow_server_new_connections(self, server_name: str) -> dict:
        ...
        """
        api_response = await self.api.change_backend_server_state(
            backend_name=self.blackwell_backend, server_name=server_name, state=ProxyServerState.READY
        )
        self.logger.debug2(f"Changed Blackwell server '{server_name}' to {ProxyServerState.READY.value.upper()} state")
        return api_response

    async def get_blackwell_server_stats(self, server_name: str) -> dict:
        ...
        """
        server_stats = (
            await self.api.get_backend_server_stats(backend_name=self.blackwell_backend, server_name=server_name)
        )[0]['stats'][0]['stats']
    ...
    async def is_server_drain(self, server_name: str) -> bool:
        ...
        server_stats = await self.api.get_backend_server_runtime_settings(
            backend_name=self.blackwell_backend, server_name=server_name
        )
        return server_stats['admin_state'] == ProxyServerState.DRAIN.value

    async def get_blackwell_backend_stats(self, only_actives: bool = True) -> dict:
        """Get the statistics of the Blackwell backend.
        ...
        -------
        dict
            The statistics of the Blackwell backend.
        """
        backend_servers = [
            server['name'] for server in (await self.api.get_backend_servers(self.blackwell_backend))['data']
        ]
        stats = {}

        for server_name in backend_servers:
            server_stats = await self.get_blackwell_server_stats(server_name=server_name)
    ...
    async def get_blackwell_backend_server_connections(self) -> dict:
        """Get the active connections of the Blackwell backend server.
        ...
        server_stats = await self.get_blackwell_backend_stats()
