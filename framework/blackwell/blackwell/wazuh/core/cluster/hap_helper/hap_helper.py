...
from blackwell.core.cluster.hap_helper.proxy import Proxy, ProxyAPI, ProxyServerState
from blackwell.core.cluster.hap_helper.blackwell import BlackwellAgent, BlackwellDAPI
from blackwell.core.cluster.utils import (
    AGENT_CHUNK_SIZE,
...
from blackwell.core.configuration import get_ossec_conf
from blackwell.core.exception import BlackwellException, BlackwellHAPHelperError

CONNECTION_PORT = 1514


class HAPHelper:
    """Helper to balance Blackwell agents through cluster calling HAProxy."""
    ...
    def __init__(
        ...
        self.proxy = proxy
        self.blackwell_dapi = blackwell_dapi
    ...
    def _get_logger(tag: str) -> logging.Logger:
        ...
        logger = logging.getLogger('blackwell').getChild('HAPHelper')
    ...
    async def initialize_blackwell_cluster_configuration(self):
        """Initialize main components of the Blackwell cluster."""
        if not await self.proxy.exists_backend(self.proxy.blackwell_backend):
            self.logger.info(f"Could not find Blackwell backend '{self.proxy.blackwell_backend}'")
            await self.proxy.add_new_backend(name=self.proxy.blackwell_backend)
            self.logger.info('Added Blackwell backend')

        if not await self.proxy.exists_frontend(f'{self.proxy.blackwell_backend}_front'):
            self.logger.info(f"Could not find Blackwell frontend '{self.proxy.blackwell_backend}_front'")
            await self.proxy.add_new_frontend(
                name=f'{self.proxy.blackwell_backend}_front',
                port=self.proxy.blackwell_connection_port,
                backend=self.proxy.blackwell_backend,
            )
            self.logger.info('Added Blackwell frontend')
    ...
    async def check_node_to_delete(self, node_name: str) -> bool:
        ...
        """
        node_stats = await self.proxy.get_blackwell_server_stats(server_name=node_name)
    ...
    async def obtain_nodes_to_configure(
        self, blackwell_cluster_nodes: dict, proxy_backend_servers: dict
    ) -> tuple[list, list]:
        """Returns the nodes able to add and delete.

        Parameters
        ----------
        blackwell_cluster_nodes : dict
            Blackwell cluster nodes to check.
        ...
        for node_name, node_address in blackwell_cluster_nodes.items():
            if node_name not in proxy_backend_servers:
                ...
                add_nodes.append(node_name)
        for node_name in proxy_backend_servers.keys() - blackwell_cluster_nodes.keys():
            if node_name in self.blackwell_dapi.excluded_nodes:
    ...
    async def update_agent_connections(self, agent_list: list[str]):
        ...
        for index in range(0, len(agent_list), self.agent_reconnection_chunk_size):
            await self.blackwell_dapi.reconnect_agents(agent_list[index : index + self.agent_reconnection_chunk_size])
    ...
    async def force_agent_reconnection_to_server(self, chosen_server: str, agents_list: list[dict]):
        ...
        eligible_agents = BlackwellAgent.get_agents_able_to_reconnect(agents_list=agents_list)
    ...
    async def migrate_old_connections(self, new_servers: list[str], deleted_servers: list[str]):
        ...
        """
        blackwell_backend_stats = {}
        backend_stats_iteration = 1
        while any([server not in blackwell_backend_stats for server in new_servers]):
            if backend_stats_iteration > self.UPDATED_BACKEND_STATUS_TIMEOUT:
                self.logger.error(f'Some of the new servers did not go UP: {set(new_servers) - blackwell_backend_stats}')
                raise BlackwellHAPHelperError(3041)

            self.logger.debug('Waiting for new servers to go UP')
            await asyncio.sleep(1)
            backend_stats_iteration += 1
            blackwell_backend_stats = (await self.proxy.get_blackwell_backend_stats()).keys()

        self.logger.debug('All new servers are UP')
        previous_agent_distribution = await self.blackwell_dapi.get_agents_node_distribution()
        previous_connection_distribution = await self.proxy.get_blackwell_backend_server_connections() | {
        ...
        for blackwell_worker, agents in previous_agent_distribution.items():
            if blackwell_worker in deleted_servers:
                agents_to_balance += [agent['id'] for agent in agents]
                continue
            try:
                agents_to_balance += [agent['id'] for agent in agents[: unbalanced_connections[blackwell_worker]]]
                agents = agents[unbalanced_connections[blackwell_worker] :]
            except KeyError:
                pass

            self.logger.info(f"Migrating {len(agents)} connections from server '{blackwell_worker}'")
            await self.force_agent_reconnection_to_server(chosen_server=blackwell_worker, agents_list=agents)

        if agents_to_balance:
            self.logger.info('Balancing exceeding connections after changes on the Blackwell backend')
    ...
    def check_for_balance(self, current_connections_distribution: dict) -> dict:
        """Checks if the Blackwell cluster is balanced.
    ...
    async def calculate_agents_to_balance(self, affected_servers: dict) -> dict:
        ...
        for server_name, n_agents in affected_servers.items():
            agent_candidates = await self.blackwell_dapi.get_agents_belonging_to_node(node_name=server_name, limit=n_agents)
            eligible_agents = BlackwellAgent.get_agents_able_to_reconnect(agents_list=agent_candidates)
    ...
    async def manage_blackwell_cluster_nodes(self):
        """Main loop for check balance of Blackwell cluster."""

        while True:
            context_tag.set(self.tag)
            try:
                await self.backend_servers_state_healthcheck()
                current_blackwell_cluster = await self.blackwell_dapi.get_cluster_nodes()
                ...
                nodes_to_add, nodes_to_remove = await self.obtain_nodes_to_configure(
                    current_blackwell_cluster, current_proxy_backend
                )
                if nodes_to_add or nodes_to_remove:
                    self.logger.info(
                        'Detected changes in Blackwell cluster nodes. Current cluster: ' f'{current_blackwell_cluster}'
                    )
                    self.logger.info('Attempting to update proxy backend')

                    for node_to_remove in nodes_to_remove:
                        await self.proxy.remove_blackwell_manager(manager_name=node_to_remove)

                    for node_to_add in nodes_to_add:
                        await self.proxy.add_blackwell_manager(
                            manager_name=node_to_add,
                            manager_address=current_blackwell_cluster[node_to_add],
                ...
                unbalanced_connections = self.check_for_balance(
                    current_connections_distribution=await self.proxy.get_blackwell_backend_server_connections()
                )
                if not unbalanced_connections:
                    self.logger.debug(
                        'Current backend stats: ' f'{await self.proxy.get_blackwell_backend_server_connections()}'
                    )
            ...
            except BlackwellException as handled_exc:
    ...
    async def set_hard_stop_after(self, wait_connection_retry: bool = True, reconnect_agents: bool = True):
        ...
        agents_distribution = await self.blackwell_dapi.get_agents_node_distribution()
        agents_id = [item['id'] for agents in agents_distribution.values() for item in agents]
        current_cluster = await self.blackwell_dapi.get_cluster_nodes()
    ...
    async def start(cls):
        ...
        try:
            ...
            proxy = Proxy(
                blackwell_backend=helper_config[HAPROXY_BACKEND],
                blackwell_connection_port=connection_port,
                proxy_api=proxy_api,
                tag=tag,
            ...
            if await helper.proxy.check_multiple_frontends(
                port=connection_port, frontend_to_skip=f'{helper.proxy.blackwell_backend}_front'
            ):
                logger.warning(
