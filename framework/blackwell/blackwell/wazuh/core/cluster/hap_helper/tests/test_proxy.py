...
import httpx
import pytest
from blackwell.core.cluster.hap_helper.proxy import (
...
class TestProxyAPI:
    ...
    @pytest.fixture
    def get_mock(self):
        with mock.patch('framework.blackwell.core.cluster.hap_helper.proxy.httpx.AsyncClient.get') as get_mock:
            yield get_mock

    @pytest.fixture
    def request_mock(self):
        with mock.patch('framework.blackwell.core.cluster.hap_helper.proxy.httpx.AsyncClient.request') as request_mock:
            yield request_mock
    ...
    async def test_initialize_ko(
        ...
        with pytest.raises(BlackwellHAPHelperError, match=f'.*{expected}.*'):
            await proxy_api.initialize()
...
class TestProxy:
    ...
    def proxy_api_mock(self):
        with mock.patch('framework.blackwell.core.cluster.hap_helper.proxy.ProxyAPI', autospec=True) as proxy_api_mock:
            yield proxy_api_mock

    @pytest.fixture
    def proxy(self, proxy_api_mock: mock.MagicMock):
        proxy = Proxy(blackwell_backend='test', proxy_api=proxy_api_mock, tag='test')
    ...
    async def test_initialize_ko(self, proxy_api_mock: mock.MagicMock, proxy: Proxy, side_effect: Exception):
        ...
        with pytest.raises(BlackwellHAPHelperError, match='.*3048.*'):
            await proxy.initialize()
    ...
    async def test_get_current_backend_servers(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        ...
        proxy_api_mock.get_backend_servers.assert_called_once_with(backend=proxy.blackwell_backend)
        assert ret_val == {server['name']: server['address'] for server in servers}

    async def test_add_blackwell_manager(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        """Check that `add_blackwell_manager` method makes the correct callback."""

        manager_name = 'foo'
        manager_address = '192.168.0.1'
        resolver = 'test-resolver'

        await proxy.add_blackwell_manager(manager_name, manager_address, resolver)

        proxy_api_mock.add_server_to_backend.assert_called_once_with(
            backend=proxy.blackwell_backend,
            server_name=manager_name,
            server_address=manager_address,
            port=proxy.blackwell_connection_port,
            resolver=resolver,
        )

    async def test_remove_blackwell_manager(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        """Check that `remove_blackwell_manager` method makes the correct callback."""

        manager_name = 'foo'

        await proxy.remove_blackwell_manager(manager_name)

        proxy_api_mock.remove_server_from_backend.assert_called_with(
            backend=proxy.blackwell_backend, server_name=manager_name
        )
    ...
    async def test_restrain_server_new_connections(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        ...
        proxy_api_mock.change_backend_server_state.assert_called_once_with(
            backend_name=proxy.blackwell_backend, server_name=server_name, state=ProxyServerState.DRAIN
        )
    ...
    async def test_allow_server_new_connections(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        ...
        proxy_api_mock.change_backend_server_state.assert_called_once_with(
            backend_name=proxy.blackwell_backend, server_name=server_name, state=ProxyServerState.READY
        )

    async def test_get_blackwell_server_stats(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        """Check the correct output of `get_blackwell_server_stats` method."""

        stats = {'foo': 'bar'}
        proxy_api_mock.get_backend_server_stats.return_value = [{'stats': [{'stats': stats}]}]
        server_name = 'foo'
        assert (await proxy.get_blackwell_server_stats(server_name)) == stats
        proxy_api_mock.get_backend_server_stats.assert_called_once_with(
            backend_name=proxy.blackwell_backend, server_name=server_name
        )
    ...
    async def test_get_blackwell_backend_stats(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        """Check the correct output of `get_blackwell_backend_stats` method."""
        ...
        with mock.patch.object(
            proxy, 'get_blackwell_server_stats', return_value={'status': ProxyServerState.UP.value.upper()}
        ) as server_stats_mock:
            stats = await proxy.get_blackwell_backend_stats()
    ...
    async def test_get_blackwell_backend_server_connections(self, proxy_api_mock: mock.MagicMock, proxy: Proxy):
        """Check the correct output of `get_blackwell_backend_server_connections` method."""

        stats = {
            'server1': {'scur': 10},
            'server2': {'scur': 20},
        }
        with mock.patch.object(proxy, 'get_blackwell_backend_stats', return_value=stats) as backend_stats_mock:
            ret_stats = await proxy.get_blackwell_backend_server_connections()
