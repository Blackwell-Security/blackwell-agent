...
import pytest
from blackwell.agent import get_agents, reconnect_agents
from blackwell.cluster import get_nodes_info
from blackwell.core.results import AffectedItemsBlackwellResult

from framework.blackwell.core.cluster.hap_helper.blackwell import BlackwellAgent, BlackwellDAPI


class TestBlackwellAgent:
    ...
    def test_can_reconnect(self, version: str, expected: bool):
        ...
        assert BlackwellAgent.can_reconnect(version) == expected
    ...
    def test_get_agents_able_to_reconnect(self):
        ...
        assert BlackwellAgent.get_agents_able_to_reconnect(agents_list=agents) == [2, 3]


@mock.patch('framework.blackwell.core.cluster.hap_helper.blackwell.DistributedAPI', autospec=True)
class TestBlackwellDAPI:
    blackwell_dapi = BlackwellDAPI(tag='test')

    @pytest.fixture
    def fixture_affected_items_result(self):
        return AffectedItemsBlackwellResult()
    ...
    @mock.patch('framework.blackwell.core.cluster.hap_helper.blackwell.get_system_nodes', return_value={})
    async def test_get_cluster_nodes(
        self,
        get_system_nodes_mock: mock.AsyncMock,
        dapi_mock: mock.MagicMock,
        fixture_affected_items_result: AffectedItemsBlackwellResult,
        nodes_data: list,
        excluded_nodes: list,
    ):
        """Check the correct output of `get_cluster_nodes` function."""

        self.blackwell_dapi.excluded_nodes = excluded_nodes
        ...
        ret_val = await self.blackwell_dapi.get_cluster_nodes()

        dapi_mock.assert_called_once_with(
            f=get_nodes_info,
            f_kwargs=None,
            logger=self.blackwell_dapi.logger,
    ...
    async def test_reconnect_agents(
        ...
        ret_val = await self.blackwell_dapi.reconnect_agents(agent_list=agent_list)
    ...
    async def test_get_agents_node_distribution(
        ...
        ret_val = await self.blackwell_dapi.get_agents_node_distribution()
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': self.blackwell_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.blackwell_dapi.logger,
            request_type='local_master',
        )
    ...
    async def test_get_agents_belonging_to_node(
        self, dapi_mock: mock.MagicMock, fixture_affected_items_result: AffectedItemsBlackwellResult, limit: Optional[int]
    ):
        ...
        ret_val = await self.blackwell_dapi.get_agents_belonging_to_node(node_name=node_name, limit=limit)
        ...
        dapi_mock.assert_called_once_with(
            ...
            f_kwargs={
                ...
                'q': 'id!=000',
                'limit': limit or self.blackwell_dapi.AGENTS_MAX_LIMIT,
            },
            logger=self.blackwell_dapi.logger,
            request_type='local_master',
        )
