...
import pytest
from blackwell.core.cluster.hap_helper.hap_helper import HAPHelper, ProxyServerState, BlackwellHAPHelperError
...
class TestHAPHelper:
    ...
    @pytest.fixture
    def dapi_mock(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.BlackwellDAPI', autospec=True) as dapi_mock:
            yield dapi_mock

    @pytest.fixture
    def proxy_api_mock(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.ProxyAPI', autospec=True) as proxy_api_mock:
            yield proxy_api_mock

    @pytest.fixture
    def proxy_mock(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.Proxy', autospec=True) as proxy_mock:
            yield proxy_mock

    @pytest.fixture
    def helper(self, proxy_mock: mock.MagicMock, dapi_mock: mock.MagicMock):
        helper = HAPHelper(proxy=proxy_mock, blackwell_dapi=dapi_mock, tag='test', **self.CONFIGURATION)
        with mock.patch.object(helper, 'logger'):
            yield helper

    @pytest.fixture
    def sleep_mock(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.asyncio.sleep') as sleep_mock:
            yield sleep_mock

    @pytest.fixture
    def blackwell_agent_mock(self):
        with mock.patch(
            'blackwell.core.cluster.hap_helper.hap_helper.BlackwellAgent.get_agents_able_to_reconnect', autospec=True
        ) as blackwell_agent_mock:
            yield blackwell_agent_mock

    @pytest.fixture
    def read_cluster_config_mock(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.read_cluster_config') as read_cluster_config_mock:
            yield read_cluster_config_mock

    @pytest.fixture
    def get_ossec_conf(self):
        with mock.patch('blackwell.core.cluster.hap_helper.hap_helper.get_ossec_conf') as get_ossec_conf:
    ...
    async def test_initialize_proxy_ko(self, helper: HAPHelper, proxy_mock: mock.MagicMock):
        ...
        proxy_mock.initialize.side_effect = BlackwellHAPHelperError(3046)

        with pytest.raises(BlackwellHAPHelperError):
            await helper.initialize_proxy()
    ...
    async def test_initialize_blackwell_cluster_configuration(
        self, helper: HAPHelper, proxy_mock: mock.MagicMock, exists_backend: bool, exists_frontend: bool
    ):
        """Check that `initialize_blackwell_cluster_configuration` method makes the correct callbacks."""
        backend_name = 'test'
        frontend_name = f'{backend_name}_front'
        port = 1514

        proxy_mock.blackwell_backend = backend_name
        proxy_mock.blackwell_connection_port = port
    ...
    async def test_check_node_to_delete(
        ...
        proxy_mock.get_blackwell_server_stats.return_value = stats
        node_name = 'test_node'

        ret_val = await helper.check_node_to_delete(node_name)

        proxy_mock.get_blackwell_server_stats.assert_called_once_with(server_name=node_name)
    ...
    async def test_obtain_nodes_to_configure_servers(
        ...
        blackwell_cluster_nodes = {WORKER1: '192.168.0.1', WORKER2: '192.168.0.2'}
        ...
        with mock.patch.object(helper, 'check_node_to_delete', return_value=check_node_to_delete) as check_mock:
            add_nodes, remove_nodes = await helper.obtain_nodes_to_configure(blackwell_cluster_nodes, proxy_backend_servers)
    ...
    async def test_force_agent_reconnection_to_server(
        self,
        blackwell_agent_mock: mock.MagicMock,
        helper: HAPHelper,
        ...
        blackwell_agent_mock.return_value = elegible_agents
    ...
    async def test_migrate_old_connections(
        ...
        proxy_mock.get_blackwell_backend_stats.return_value = {WORKER1: {}, WORKER2: {}}
        dapi_mock.get_agents_node_distribution.return_value = {
            WORKER1: AGENTS_TO_FORCE,
            WORKER3: AGENTS_TO_UPDATE,
        }
        proxy_mock.get_blackwell_backend_server_connections.return_value = PREVIOUS_CONNECTION_DIST
    ...
    async def test_migrate_old_connections_ko(
        ...
        proxy_mock.get_blackwell_backend_stats.return_value = {WORKER1: {}}

        with pytest.raises(BlackwellHAPHelperError, match='.*3041.*'):
            await helper.migrate_old_connections(NEW_SERVERS, OLD_SERVERS)
    ...
    async def test_manage_blackwell_cluster_nodes(
        self,
        helper: HAPHelper,
        proxy_mock: mock.MagicMock,
        dapi_mock: mock.MagicMock,
        ...
        """Check that `manage_blackwell_cluster_nodes` method makes the correct callbacks."""
        WORKER1 = 'worker1'
        WORKER2 = 'worker2'
        WORKER3 = 'worker3'
                    ...
                    with mock.patch.object(helper, 'migrate_old_connections'):
                        with mock.patch.object(helper, 'check_for_balance', return_value=unbalanced_connections):
                            with mock.patch.object(helper, 'balance_agents'):
                                try:
                                    await wait_for(helper.manage_blackwell_cluster_nodes(), 0.5)
                                ...
                                if nodes_to_add or nodes_to_remove:
                                    for node_to_remove in nodes_to_remove:
                                        proxy_mock.remove_blackwell_manager.assert_any_call(manager_name=node_to_remove)

                                    for node_to_add in nodes_to_add:
                                        proxy_mock.add_blackwell_manager.assert_any_call(
    ...
    async def test_manage_blackwell_cluster_nodes_ko(
        self,
        helper: HAPHelper,
        sleep_mock: mock.AsyncMock,
    ):
        """Check the correct error handling of `manage_blackwell_cluster_nodes` method."""
        error = BlackwellException(3000, 'Some test exception')
    ...
    async def test_get_connection_retry(self, helper: HAPHelper, proxy_mock: mock.MagicMock):
        ...
        with mock.patch(
            'blackwell.core.cluster.hap_helper.hap_helper.get_cluster_items',
    ...
    async def test_start(
        ...
        HAPROXY_ADDRESS_VALUE = 'blackwell-proxy'
        HAPROXY_PORT_VALUE = 5555
        HAPROXY_PROTOCOL_VALUE = protocol
        HAPROXY_BACKEND_VALUE = 'blackwell_test'
        ...
        proxy = mock.MagicMock(hard_stop_after=hard_stop_after, blackwell_backend=HAPROXY_BACKEND_VALUE)
        ...
        get_ossec_conf.return_value = {'remote': [{'port': BLACKWELL_PORT}]}
        ...
        with mock.patch.object(HAPHelper, 'initialize_proxy', new=mock.AsyncMock()):
            with mock.patch.object(HAPHelper, 'get_connection_retry', return_value=connection_retry):
                with mock.patch.object(HAPHelper, 'initialize_blackwell_cluster_configuration', new=mock.AsyncMock()):
                    with mock.patch.object(HAPHelper, 'set_hard_stop_after', new=mock.AsyncMock()):
                        with mock.patch.object(HAPHelper, 'manage_blackwell_cluster_nodes', new=mock.AsyncMock()):
                            with mock.patch.object(HAPHelper, '_get_logger', return_value=logger_mock):
                                ...
                                proxy_mock.assert_called_once_with(
                                    blackwell_backend=HAPROXY_BACKEND_VALUE,
                                    blackwell_connection_port=BLACKWELL_PORT,
                                    proxy_api=proxy_api,
                                    tag=TAG,
                                ...
                                proxy.check_multiple_frontends.assert_called_once_with(
                                    port=BLACKWELL_PORT, frontend_to_skip=f'{HAPROXY_BACKEND_VALUE}_front'
                                )
                                ...
                                HAPHelper.initialize_blackwell_cluster_configuration.assert_called_once()

                                if hard_stop_after is None:
                                    HAPHelper.set_hard_stop_after.assert_called_once()

                                HAPHelper.manage_blackwell_cluster_nodes.assert_called_once()

    @pytest.mark.parametrize('exception', [KeyError(), KeyboardInterrupt(), BlackwellHAPHelperError(3046)])
