...
from blackwell.core.cluster import client, cluster, server
from blackwell.core.cluster import common as c_common
from blackwell.core.cluster.dapi import dapi
from blackwell.core.cluster.utils import context_tag
...
class LocalServer(server.AbstractServer):
    ...
    async def start(self):
        ...
        socket_path = os.path.join(common.BLACKWELL_PATH, 'queue', 'cluster', 'c-internal.sock')
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise BlackwellClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise BlackwellClusterError(3023)
            asyncio.create_task(self.log_exceptions(
