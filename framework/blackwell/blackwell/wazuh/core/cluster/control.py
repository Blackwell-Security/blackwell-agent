...
from blackwell.core.cluster.common import as_blackwell_object, BlackwellJSONEncoder
from blackwell.core.exception import BlackwellError
from blackwell.core.utils import filter_array_by_query
...
async def get_nodes(lc: local_client.LocalClient, filter_node=None, offset=0, limit=common.DATABASE_LIMIT,
    ...
    result = json.loads(response, object_hook=as_blackwell_object)
...
async def get_node(lc: local_client.LocalClient, filter_node=None, select=None):
    ...
    response = await lc.execute(command=b'get_nodes', data=json.dumps(arguments).encode())
    node_info_array = json.loads(response, object_hook=as_blackwell_object)
...
async def get_agents(lc: local_client.LocalClient, filter_node=None, filter_status=None):
    ...
    response = await lc.execute(command=b'dapi', data=json.dumps(input_json, cls=BlackwellJSONEncoder).encode())
    result = json.loads(response, object_hook=as_blackwell_object)
