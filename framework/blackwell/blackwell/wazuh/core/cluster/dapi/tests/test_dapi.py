...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        from blackwell.core.cluster.dapi.dapi import DistributedAPI, APIRequestQueue, SendSyncRequestQueue
        from blackwell.core.manager import get_manager_status
        from blackwell.core.results import BlackwellResult, AffectedItemsBlackwellResult
        from blackwell import agent, cluster, ciscat, manager, BlackwellError, BlackwellInternalError
        from blackwell.core.exception import BlackwellClusterError
        from api.util import raise_if_exc
        from blackwell.core.cluster import local_client
...
@pytest.mark.parametrize('kwargs', [
    {'f_kwargs': {'select': ['id']}, 'rbac_permissions': {'mode': 'black'}, 'nodes': ['worker1'],
     'basic_services': ('blackwell-modulesd', 'blackwell-db'), 'request_type': 'local_master'},
...
def test_DistributedAPI_debug_log():
    ...
    logger_ = TestingLogger(logger_name="blackwell-api")
    ...
    logger_ = TestingLogger(logger_name="blackwell")
...
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=BlackwellResult({'result': 'local'})))
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.forward_request',
       new=AsyncMock(return_value=BlackwellResult({'result': 'forward'})))
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.execute_remote_request',
       new=AsyncMock(return_value=BlackwellResult({'result': 'remote'})))
...
def test_DistributedAPI_distribute_function(api_request, request_type, node, expected, cluster_enabled, f_kwargs):
    ...
    with patch('blackwell.core.cluster.dapi.dapi.check_cluster_status', return_value=cluster_enabled):
        with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': node}):
...
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
       new=AsyncMock(return_value=BlackwellResult({'result': 'local'})))
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=BlackwellResult({'unknown': ['001', '002']})))
...
def test_DistributedAPI_distribute_function_mock_solver(api_request, request_type, node, expected):
    ...
    expected : str
        Expected result
    """
    with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': node, 'node': 'master'}):
...
def test_DistributedAPI_distribute_function_exception():
    ...
    logger_ = logging.getLogger("blackwell")
    with patch("blackwell.core.cluster.dapi.dapi.get_node_wrapper", side_effect=BlackwellError(4000)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        get_error_result = dapi.get_error_info(Exception("testing"))
        assert 'unknown-node' in get_error_result
        assert get_error_result['unknown-node']['error'] == 'Blackwell Internal Error. See log for more detail'

    with patch("blackwell.core.cluster.dapi.dapi.get_node_wrapper", side_effect=BlackwellError(4001)):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
        with pytest.raises(BlackwellError, match='.* 4001 .*'):
            dapi.get_error_info(Exception("testing"))

    with patch("blackwell.core.cluster.dapi.dapi.get_node_wrapper", return_value=NodeWrapper()):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger_)
...
def test_DistributedAPI_local_request_errors():
    ...
    with patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.execute_local_request',
               new=AsyncMock(side_effect=BlackwellInternalError(1001))):
        dapi_kwargs = {'f': agent.get_agents_summary_status, 'logger': logger}
        ...
        try:
            raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
        except BlackwellInternalError as e:
            assert e.code == 1001
    ...
    with patch('blackwell.cluster.get_nodes_info', side_effect=json.decoder.JSONDecodeError('test', 'test', 1)):
        with patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.check_blackwell_status'):
...
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.check_blackwell_status', side_effect=None)
...
def test_DistributedAPI_local_request(mock_local_request):
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=BlackwellError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=BlackwellInternalError(1001))):
        dapi_kwargs = {'f': manager.status, 'logger': logger}
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1001)
    ...
    exc_code = 3000
    cluster_exc = BlackwellClusterError(exc_code)
    with patch('asyncio.wait_for', new=AsyncMock(side_effect=cluster_exc)):
        with patch.object(TestingLogger, "error") as logger_error_mock:
            # Test BlackwellClusterError caught in execute_local_request and ProblemException raised
            dapi_kwargs = {'f': manager.status, 'logger': testing_logger}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=exc_code)

            # Test BlackwellClusterError is raised when using debug in execute_local_request and distribute_function
            dapi = DistributedAPI(f=manager.status, logger=testing_logger, debug=True)
            try:
                raise_if_exc(loop.run_until_complete(dapi.distribute_function()))
            except BlackwellClusterError as e:
...
@patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'worker'})
@patch('blackwell.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('blackwell.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('blackwell.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
...
@patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'master-node'})
@patch('blackwell.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.get_solver_node', return_value={'worker1': ['001', '002']})
@patch('blackwell.core.cluster.local_client.LocalClient.execute', return_value='invalid_json')
...
@patch('blackwell.core.cluster.local_client.LocalClient.send_file', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('blackwell.core.cluster.local_client.LocalClient.execute', new=AsyncMock(return_value='{"Testing": 1}'))
@patch('blackwell.core.cluster.dapi.dapi.DistributedAPI.get_solver_node',
       new=AsyncMock(return_value=BlackwellResult({'testing': ['001', '002']})))
@patch('blackwell.core.cluster.dapi.dapi.check_cluster_status', return_value=True)
def test_DistributedAPI_tmp_file(mock_cluster_status):
    ...
    with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('blackwell.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsBlackwellResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                           'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            raise_if_exc_routine(dapi_kwargs=dapi_kwargs)

    open('/tmp/dapi_file.txt', 'a').close()
    with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'unk', 'node': 'master'}):
        raise_if_exc_routine(dapi_kwargs=dapi_kwargs)
...
def test_DistributedAPI_tmp_file_cluster_error(mock_cluster_status):
    ...
    with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
        with patch('blackwell.core.cluster.dapi.dapi.get_node_wrapper',
                   return_value=AffectedItemsBlackwellResult(affected_items=[{'type': 'master', 'node': 'unknown'}])):
            with patch('blackwell.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=BlackwellClusterError(3022))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
            ...
            open('/tmp/dapi_file.txt', 'a').close()
            with patch('blackwell.core.cluster.local_client.LocalClient.execute',
                       new=AsyncMock(side_effect=BlackwellClusterError(1000))):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
                               'f_kwargs': {'tmp_file': '/tmp/dapi_file.txt'}}
                raise_if_exc_routine(dapi_kwargs=dapi_kwargs, expected_error=1000)


@patch('blackwell.core.cluster.local_client.LocalClient.execute',
       new=AsyncMock(return_value='{"items": [{"name": "master"}], "totalItems": 1}'))
@patch('blackwell.agent.Agent.get_agents_overview', return_value={'items': [{'id': '001', 'node_name': 'master'},
...
def test_DistributedAPI_get_solver_node(mock_cluster_status, mock_agents_overview):
    ...
    nodes_info_result = AffectedItemsBlackwellResult()
    nodes_info_result.affected_items.append({'name': 'master'})
    common.cluster_nodes.set(['master'])

    with patch('blackwell.core.cluster.dapi.dapi.get_nodes_info', new=AsyncMock(return_value=nodes_info_result)):
        with patch('blackwell.core.cluster.cluster.get_node', return_value={'type': 'master', 'node': 'unknown'}):
            dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
            ...
            expected = AffectedItemsBlackwellResult()
            expected.affected_items = [{'id': '001', 'node_name': 'master'}]
            with patch('blackwell.agent.get_agents_in_group', return_value=expected):
                dapi_kwargs = {'f': manager.status, 'logger': logger, 'request_type': 'distributed_master',
...
@pytest.mark.parametrize('api_request', [
    ...
    blackwell.core.manager.status
])
@patch('blackwell.core.manager.get_manager_status', return_value={process: 'running' for process in get_manager_status()})
def test_DistributedAPI_check_blackwell_status(status_mock, api_request):
    """Test `check_blackwell_status` method from class DistributedAPI."""
    dapi = DistributedAPI(f=api_request, logger=logger)
    data = dapi.check_blackwell_status()
    assert data is None
...
@patch('blackwell.core.cluster.cluster.get_node', return_value={'node': 'random_node'})
def test_DistributedAPI_check_blackwell_status_exception(node_info_mock, status_value):
    """Test exceptions from `check_blackwell_status` method from class DistributedAPI."""
    statuses = {process: status_value for process in sorted(get_manager_status())}
    with patch('blackwell.core.manager.get_manager_status',
               return_value=statuses):
        dapi = DistributedAPI(f=agent.get_agents_summary_status, logger=logger)
        try:
            dapi.check_blackwell_status()
        except BlackwellInternalError as e:
            assert e.code == 1017
            assert statuses
...
@patch("blackwell.core.cluster.common.import_module", return_value="os.path")
...
async def test_APIRequestQueue_run(loop_mock, import_module_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return 'blackwell*request_queue*test ' \
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        logger_mock.assert_called_once_with("Error in DAPI request. The destination node is "
                                            "not connected or does not exist: 'blackwell'.")

        node = NodeMock()
        with patch.object(node, "send_request", side_effect=BlackwellClusterError(3020, extra_message="test")):
            with patch.object(node, "send_string", return_value=b"noerror"):
                with patch("blackwell.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    server.clients = {"blackwell": node}
                    with pytest.raises(Exception):
                        await apirequest.run()

            with patch.object(node, "send_string", Exception("break while true")):
                with patch("blackwell.core.cluster.dapi.dapi.DistributedAPI", return_value=DistributedAPI_mock()):
                    with patch("blackwell.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception()):
                        apirequest.logger = logging.getLogger("apirequest")
                        with pytest.raises(Exception):
                            await apirequest.run()


@patch("blackwell.core.cluster.dapi.dapi.contextlib.suppress", side_effect=Exception())
@patch("asyncio.get_event_loop")
async def test_SendSyncRequestQueue_run(loop_mock, contexlib_mock):
    ...
    class RequestQueueMock:
        async def get(self):
            return "blackwell*request_queue*test {\"daemon_name\": \"test\"}"
    ...
    with patch.object(logger, "error", side_effect=Exception("break while true")) as logger_mock:
        ...
        with patch.object(node, "send_request", Exception("break while true")):
            with patch("blackwell.core.cluster.dapi.dapi.blackwell_sendsync", side_effect=Exception("break while true")):
                server.clients = {"blackwell": node}
                sendsync.logger = logging.getLogger("sendsync")
                with pytest.raises(Exception):
                    await sendsync.run()

            with patch("blackwell.core.cluster.dapi.dapi.blackwell_sendsync", side_effect="noerror"):
                with pytest.raises(Exception):
                    await sendsync.run()
