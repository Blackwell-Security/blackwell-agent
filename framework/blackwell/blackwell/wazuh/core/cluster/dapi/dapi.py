...
import blackwell.core.manager
import blackwell.core.results as wresults
from blackwell import agent
from blackwell.cluster import get_node_wrapper, get_nodes_info
from blackwell.core import common, exception
from blackwell.core.cluster import local_client, common as c_common
from blackwell.core.cluster.cluster import check_cluster_status
from blackwell.core.exception import BlackwellException, BlackwellClusterError, BlackwellError
from blackwell.core.blackwell_socket import blackwell_sendsync
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = blackwell.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        self.node_info = blackwell.core.cluster.cluster.get_node() if node is None else node.get_node()
        ...
        if not basic_services:
            self.basic_services = ('blackwell-modulesd', 'blackwell-analysisd', 'blackwell-execd', 'blackwell-db', 'blackwell-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'blackwell-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.BlackwellException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or BlackwellException
            Dictionary with API response or BlackwellException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_blackwell_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractBlackwellResult, exception.BlackwellException)) \
                else wresults.BlackwellResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.BlackwellInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.BlackwellInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.BlackwellClusterError))
            return e
        except exception.BlackwellError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.BlackwellInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_blackwell_status(self):
        """
        There are some services that are required for blackwell to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * Blackwell must be started before using the API is the services are stopped.

        The basic services blackwell needs to be running are: blackwell-modulesd, blackwell-remoted, blackwell-analysisd, blackwell-execd
        and blackwell-db
        """
        if self.f == blackwell.core.manager.status:
            return

        status = blackwell.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.BlackwellInternalError(1017, extra_message=extra_info)
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    raise exception.BlackwellInternalError(3021)
                except OperationalError as exc:
                    raise exception.BlackwellInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.BlackwellInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.BlackwellInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.BlackwellInternalError(900)
            ...
            # BlackwellClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.BlackwellClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.BlackwellJSONEncoder)
        except (exception.BlackwellError, exception.BlackwellResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.BlackwellJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.BlackwellInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.BlackwellJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.BlackwellException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.BlackwellException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of BlackwellInternalError
        if isinstance(e, exception.BlackwellInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('BLACKWELL_HOME', os.path.relpath(h.baseFilename, start=common.BLACKWELL_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.BLACKWELL_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_blackwell_object)
        os.remove(os.path.join(common.BLACKWELL_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.BlackwellJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_blackwell_object)

    async def forward_request(self) -> [wresults.AbstractBlackwellResult, exception.BlackwellException]:
        ...
        Returns
        -------
        wresults.AbstractBlackwellResult or exception.BlackwellException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractBlackwellResult, exception.BlackwellException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a BlackwellError exception
                if isinstance(result, BlackwellClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = BlackwellError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractBlackwellResult, exception.BlackwellException)) \
                else wresults.BlackwellResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsBlackwellResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractBlackwellResult):
        ...
        # It might be a BlackwellError after reducing
        if isinstance(response, wresults.AffectedItemsBlackwellResult):
...
class BlackwellRequestQueue:
    """Represents a queue of Blackwell requests"""
...
class APIRequestQueue(BlackwellRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('blackwell').getChild('dapi')
        self.logger.addFilter(blackwell.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_blackwell_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.BlackwellJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(BlackwellRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('blackwell').getChild('sendsync')
        self.logger.addFilter(blackwell.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await blackwell_sendsync(**request)
