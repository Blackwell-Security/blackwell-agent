...
from blackwell.core.exception import BlackwellError, BlackwellException, BlackwellInternalError, BlackwellHAPHelperError
from blackwell.core.results import BlackwellResult
from blackwell.core.utils import temporary_cache
from blackwell.core.blackwell_socket import create_blackwell_socket_message
from blackwell.core.wlogging import BlackwellLogger
...
execq_lockfile = os.path.join(common.BLACKWELL_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'blackwell_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise BlackwellError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise BlackwellError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise BlackwellHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise BlackwellError(3006, extra_message=e.message)
    except Exception as e:
        raise BlackwellError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise BlackwellError(3004, extra_message="Cluster port must be an integer.")
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    try:
        ...
        raise BlackwellInternalError(1913, extra_message=str(e))

    processes = ['blackwell-agentlessd', 'blackwell-analysisd', 'blackwell-authd', 'blackwell-csyslogd', 'blackwell-dbd', 'blackwell-monitord',
                 'blackwell-execd', 'blackwell-integratord', 'blackwell-logcollector', 'blackwell-maild', 'blackwell-remoted',
                 'blackwell-reportd', 'blackwell-syscheckd', 'blackwell-clusterd', 'blackwell-modulesd', 'blackwell-db', 'blackwell-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.BLACKWELL_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['blackwell-clusterd'] == 'running' else "no"}
...
def manager_restart() -> BlackwellResult:
    """Restart Blackwell manager.

    Send JSON message with the 'restart-blackwell' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    BlackwellInternalError(1901)
        If the socket path doesn't exist.
    BlackwellInternalError(1902)
    ...
    try:
        ...
        # json msg for restarting Blackwell manager
        msg = json.dumps(create_blackwell_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_BLACKWELL_COMMAND,
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise BlackwellInternalError(1902)
        else:
    ...
    return BlackwellResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.BLACKWELL_PATH, here, 'cluster.json')) as f:
        ...
        raise BlackwellError(3005, str(e))
...
class ClusterLogger(BlackwellLogger):
    """
    Define the logger used by blackwell-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from blackwell-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'blackwell-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `BlackwellResult` instance in case the execution did not fail. Return an exception otherwise.
    """
