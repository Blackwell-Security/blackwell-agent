...
from blackwell.core.common import MAX_SOCKET_BUFFER_SIZE
...
def test_async_init():
    """Verify that AsyncBlackwellDBConnection attributes are correct."""
    async_wdb = AsyncBlackwellDBConnection('test')
...
async def test_async_open_connection(open_unix_connection_mock):
    ...
    async_wdb = AsyncBlackwellDBConnection(loop='test_loop')
...
def test_async_close():
    ...
    async_wdb = AsyncBlackwellDBConnection()
...
async def test_async_send_ko():
    ...
    with pytest.raises(exception.BlackwellInternalError, match=".* 2005 .*"):
    ...
    with pytest.raises(exception.BlackwellInternalError, match=r'\b2010\b'):
        await async_wdb._send('test')

    # Blackwell-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_wdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.BlackwellError, match=r'\b2003\b'):
        await async_wdb._send('test')


@pytest.mark.asyncio
async def test_run_wdb_command():
    """Test `BlackwellDBConnection.run_wdb_command` method."""
    send_result = ('status', '["data"]')
    command = "any wdb command"

    wdb_con = AsyncBlackwellDBConnection()
    with patch('blackwell.core.wdb.AsyncBlackwellDBConnection._send', return_value=send_result) as wdb_send_mock:
...
async def test_run_wdb_command_ko(wdb_response):
    """Test `BlackwellDBConnection.run_wdb_command` method expected exceptions."""
    with patch('blackwell.core.wdb.AsyncBlackwellDBConnection._send', return_value=wdb_response):
        wdb_con = AsyncBlackwellDBConnection()
        with pytest.raises(exception.BlackwellInternalError, match=".* 2007 .*") as expected_exc:
...
def test_failed_connection():
    ...
    with patch('blackwell.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.BlackwellException, match=".* 2005 .*"):
            BlackwellDBConnection()
...
def test_wrong_character_encodings_wdb(send_mock, connect_mock):
    """
    Tests receiving a text with a bad character encoding from blackwell db
    """
...
def test_failed_send_private(send_mock, connect_mock):
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = BlackwellDBConnection()
        with pytest.raises(exception.BlackwellException, match=".* 2003 .*"):
            mywdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        mywdb = BlackwellDBConnection()
        with pytest.raises(exception.BlackwellException, match=".* 2009 .*"):
            mywdb._send('test_msg')
...
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    mywdb = BlackwellDBConnection()
    with pytest.raises(exception.BlackwellException, match=".* 2004 .*"):
...
@patch("socket.socket.connect")
@patch("socket.socket.send")
@patch("blackwell.core.wdb.BlackwellDBConnection._send")
def test_execute(send_mock, socket_send_mock, connect_mock):
    ...
    with patch("blackwell.core.wdb.BlackwellDBConnection._send", new=send_mock):
...
def test_execute_pagination(socket_send_mock, connect_mock):
    ...
    # Test pagination
    with patch("blackwell.core.wdb.BlackwellDBConnection._send",
               side_effect=[[{'total': 5}], exception.BlackwellInternalError(2009), ['ok', '{"total": 5}'],
                            ['ok', '{"total": 5}']]):
        mywdb.execute("agent 000 sql select test from test offset 1 limit 500")

    # Test pagination error
    with patch("blackwell.core.wdb.BlackwellDBConnection._send",
               side_effect=[[{'total': 5}], exception.BlackwellInternalError(2009)]):
        with pytest.raises(exception.BlackwellInternalError, match=".* 2009 .*"):
            mywdb.execute("agent 000 sql select test from test offset 1 limit 1")
...
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    ...
    if not error_type:
        ...
        with patch("blackwell.core.wdb.BlackwellDBConnection._send", return_value=[{'total': 5}]):
            with patch("blackwell.core.wdb.min", side_effect=error_type):
                with pytest.raises(exception.BlackwellException, match=f'.* {expected_exception} .*'):
                    mywdb.execute(error_query, delete=delete, update=update)
...
def test_BlackwellDBConnection_loads(string):
    """Test that the `loads` method from the class `BlackwellDBConnection` cleans empty objects from the result."""
    result = BlackwellDBConnection.loads(string)
    assert len(result) == 1
