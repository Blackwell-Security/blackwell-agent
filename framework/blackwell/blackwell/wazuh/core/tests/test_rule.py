...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core import rule
...
@pytest.mark.parametrize('status, expected_result', [
    ...
    (None, 'all'),
    ('unexistent', BlackwellError(1202))
])
...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', BlackwellError(1201))
])
@patch("blackwell.core.common.BLACKWELL_PATH", new=parent_directory)
@patch("blackwell.core.common.RULES_PATH", new=data_path)
...
@patch("blackwell.core.rule.load_blackwell_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(BlackwellError, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("blackwell.core.rule.load_blackwell_xml", side_effect=OSError(8, 'Error', 'Unknown'))
