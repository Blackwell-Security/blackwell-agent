...
from blackwell.core.exception import BlackwellException, BlackwellError
import pytest
...
@pytest.mark.parametrize('code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type, exc_string', [
        ...
        (1017, {'node_name': 'Node Name', 'not_ready_daemons': 'not ready daemons'}, None, None, None, None, None, 
            'Error 1017 - Some Blackwell daemons are not ready yet in node "Node Name" (not ready daemons)'),        
    ])
def test_blackwell_exception_to_string(code, extra_message, extra_remediation, cmd_error, dapi_errors, title , type, exc_string):
    """Check object constructor """
    exc = BlackwellException(code, extra_message, extra_remediation, cmd_error, dapi_errors, title, type)
    assert str(exc) == exc_string


def test_blackwell_exception__or__():
    """Check that BlackwellException's | operator performs the join of dapi errors properly."""
    excp1 = BlackwellException(1308)
    excp1._dapi_errors = {'test1': 'test error'}
    excp2 = BlackwellException(1308)
    excp2._dapi_errors = {'test2': 'test error'}
    excp3 = excp2 | excp1
    assert excp3._dapi_errors == {'test1': 'test error', 'test2': 'test error'}


def test_blackwell_exception__deepcopy__():
    """Check that BlackwellException's __deepcopy__ magic method works properly."""
    excp1 = BlackwellException(1308)
    excp2 = excp1.__deepcopy__()
    assert excp1 == excp2 and excp1 is not excp2


def test_blackwell_error__or__():
    """Check that BlackwellError's | operator performs the union of id sets properly."""
    error1 = BlackwellError(1309, ids={1, 2, 3})
    error2 = BlackwellError(1309, ids={4, 5, 6})
