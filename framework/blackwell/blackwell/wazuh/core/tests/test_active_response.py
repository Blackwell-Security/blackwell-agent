...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core import active_response
...
@pytest.mark.parametrize('agent_version, builder_type', [
    ('Blackwell v4.2.0', active_response.ARJsonMessage),
    ('Blackwell v4.3.0', active_response.ARJsonMessage),
    ('Blackwell v4.1.0', active_response.ARStrMessage)
])
...
@pytest.mark.parametrize('expected_exception, command, arguments', [
    ...
    (1652, 'random', []),
    (1652, 'invalid_cmd', []),
    (None, 'restart-blackwell0', []),
    (None, '!restart-blackwell0', []),
    (None, 'restart-blackwell0', ["arg1", "arg2"])
])
...
def test_create_message(expected_exception, command, arguments):
    ...
    if expected_exception:
        with pytest.raises(BlackwellError, match=f'.* {expected_exception} .*'):
...
@pytest.mark.parametrize('expected_exception, command, arguments, alert', [
    (1650, None, [], None),
    (None, 'restart-blackwell0', [], None),
    (None, 'restart-blackwell0', [], None),
    (None, 'restart-blackwell0', ["arg1", "arg2"], None),
...
def test_send_ar_message(agent_id, command, arguments, alert):
    ...
    with patch.object(Agent, 'get_basic_information', return_value=mock_agent_info) as mock_get_basic_info, \
        ...
        mock_wq.send_msg_to_agent.assert_called_once_with(msg=mock_message_builder_instance.create_message.return_value,
                                                          agent_id=agent_id,
                                                          msg_type=active_response.BlackwellQueue.AR_TYPE)
