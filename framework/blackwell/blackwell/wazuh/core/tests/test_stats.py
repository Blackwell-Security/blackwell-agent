...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core import common, stats
        from blackwell.core.exception import BlackwellError, BlackwellException, BlackwellInternalError
...
def test_totals_ko_():
    ...
    with patch('blackwell.core.stats.open', side_effect=IOError):
        with pytest.raises(BlackwellException, match=".* 1308 .*"):
            stats.totals_(date(1996, 8, 13))

    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(BlackwellInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))


@patch('blackwell.core.common.STATS_PATH', new=test_data_path)
def test_weekly_():
...
@patch('blackwell.core.common.STATS_PATH', new='')
...
@pytest.mark.parametrize('agent, daemon, expected_value', [
    (0, 'logcollector', os.path.join(common.BLACKWELL_PATH, "queue", "sockets", 'logcollector')),
...
@patch('blackwell.core.blackwell_socket.BlackwellSocketJSON.close')
@patch('blackwell.core.blackwell_socket.BlackwellSocketJSON.send')
@patch('blackwell.core.blackwell_socket.BlackwellSocketJSON.__init__', return_value=None)
def test_get_daemons_stats_socket(mock__init__, mock_send, mock_close, agents_list, expected_socket_response,
    ...
    with patch('blackwell.core.blackwell_socket.BlackwellSocketJSON.receive',
...
def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(BlackwellInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_ko():
    ...
    with patch('blackwell.core.stats.open'):
        with pytest.raises(BlackwellInternalError, match=".* 1104 .*"):
            stats.get_daemons_stats_('filename')
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('blackwell.core.blackwell_socket.BlackwellSocket.__init__', return_value=None) as mock_socket:
        with patch('blackwell.core.blackwell_socket.BlackwellSocket.send', side_effect=None) as mock_send:
            with patch('blackwell.core.blackwell_socket.BlackwellSocket.receive', return_value=response.encode()):
                with patch('blackwell.core.blackwell_socket.BlackwellSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        if agent_id == '000':
            mock_socket.assert_called_once_with(os.path.join(common.BLACKWELL_PATH, "queue", "sockets", "logcollector"))
            mock_send.assert_called_once_with(b'getstate')
        else:
            mock_socket.assert_called_once_with(os.path.join(common.BLACKWELL_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('blackwell.core.blackwell_socket.BlackwellSocket.__init__', return_value=None) as mock_socket:
        with patch('blackwell.core.blackwell_socket.BlackwellSocket.send', side_effect=None) as mock_send:
            with patch('blackwell.core.blackwell_socket.BlackwellSocket.receive', side_effect=responses):
                with patch('blackwell.core.blackwell_socket.BlackwellSocket.close', side_effect=None):
...
def test_get_daemons_stats_from_socket_ko():
    ...
    with pytest.raises(BlackwellError, match=r'\b1307\b'):
        stats.get_daemons_stats_from_socket(None, None)

    with pytest.raises(BlackwellError, match=r'\b1310\b'):
        stats.get_daemons_stats_from_socket('000', 'agent')

    with pytest.raises(BlackwellInternalError, match=r'\b1121\b'):
        stats.get_daemons_stats_from_socket('000', 'logcollector')

    with patch('blackwell.core.blackwell_socket.BlackwellSocket.__init__', return_value=None):
        with patch('blackwell.core.blackwell_socket.BlackwellSocket.close', side_effect=None):
            with patch('blackwell.core.blackwell_socket.BlackwellSocket.send', side_effect=None):
                with patch('blackwell.core.blackwell_socket.BlackwellSocket.receive', side_effect=ValueError):
                    with pytest.raises(BlackwellInternalError, match=r'\b1118\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')

                with patch('blackwell.core.blackwell_socket.BlackwellSocket.receive',
                           return_value=json.dumps({'error': 1}).encode()):
                    with pytest.raises(BlackwellError, match=r'\b1117\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')
