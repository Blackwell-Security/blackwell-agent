...
from blackwell.core.utils import get_date_from_timestamp

with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core import rootcheck
...
@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch("blackwell.core.rootcheck.BlackwellDBQuery.__init__")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_init(mock_info, mock_blackwellDBQuery, mock_backend):
    """Test if BlackwellDBQuery and BlackwellDBBackend are called with expected parameters"""
    test = rootcheck.BlackwellDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='test', count=True, get_data=True, distinct=False, filters=None,
                                           fields={})
    mock_backend.assert_called_with('100')
    mock_blackwellDBQuery.assert_called_with(ANY, offset=1, limit=1, table='pm_event', sort=None, search='test',
...
@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_default_query(mock_info, mock_backend, distinct):
...
@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_parse_filters(mock_info, mock_backend):
...
@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_filter_status(mock_info, mock_backend, status, expected_items):
...
@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_filter_status_ko(mock_info, mock_backend):
    """Test if expected exception is raised when status does not exist"""
    test = rootcheck.BlackwellDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='', count=True, get_data=True, distinct=False, fields={},
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})

    with pytest.raises(BlackwellException, match=".* 1603 .*"):
        test._filter_status({'value': 'test'})


@patch("blackwell.core.rootcheck.BlackwellDBBackend")
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQueryRootcheck_format_data_into_dictionary(mock_info, mock_backend):
    """Test if format_data_into_dictionary() returns expected element"""
    test = rootcheck.BlackwellDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test',
                                           select=['log', 'date_first', 'status', 'date_last', 'cis', 'pci_dss'],
                                           query='', count=True, get_data=True, distinct=False,
                                           fields=rootcheck.BlackwellDBQueryRootcheck.fields,
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})
...
@patch('blackwell.core.wdb.BlackwellDBConnection')
def test_rootcheck_delete_agent(mock_db_conn, agent):
    """Test if proper parameters are being sent to the blackwelldb socket.

    Parameters
    ----------
    agent : str
        Agent whose information is being deleted from the db.
    mock_db_conn : BlackwellDBConnection
        Object used to send the delete message to the blackwelldb socket.
    """
