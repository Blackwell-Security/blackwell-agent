...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core.agent import *
        from blackwell.core.exception import BlackwellException
        from api.util import remove_nones_to_dict
        from blackwell.core.common import reset_context_cache
...
def get_BlackwellDBQuery_params(wdb_class):
    """Get default parameters for the specified BlackwellDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the BlackwellDBQuery class. Example: 'Agents' to get default parameters from `BlackwellDBQueryAgents` class.
    ...
    """
    with patch('blackwell.core.agent.BlackwellDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'BlackwellDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with Blackwell packages availables with their hash
...
@patch("blackwell.core.agent.BlackwellDBBackend")
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of BlackwellDBQueryAgents works properly.
    ...
    if value:
        BlackwellDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(BlackwellException, match=".* 2005 .*"):
            BlackwellDBQueryAgents()


@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of BlackwellDBQueryAgents returns expected query"""
    query_agent = BlackwellDBQueryAgents()
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of BlackwellDBQueryAgents returns expected result
    ...
    """
    query_agent = BlackwellDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of BlackwellDBQueryAgents returns expected query"""
    query_agent = BlackwellDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of BlackwellDBQueryAgents returns expected data"""
    query_agent = BlackwellDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of BlackwellDBQueryAgents returns expected query"""
    query_agent = BlackwellDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of BlackwellDBQueryAgents returns expected query
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of BlackwellDBQueryGroup works properly.
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of BlackwellDBQueryGroup works properly.
        ...
        """
    query_group = BlackwellDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of BlackwellDBQueryGroupByAgents works properly."""
    query_group = BlackwellDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of BlackwellDBQueryGroupByAgents works properly."""
    query_group = BlackwellDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'blackwell-master'},
                         {'count': 1, 'name': 'blackwell-agent1'}]
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if BlackwellDBQueryGroupByAgents works properly."""
    query_group = BlackwellDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of BlackwellDBQueryGroup works properly"""
    query_group = BlackwellDBQueryGroup()
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of BlackwellDBQueryMultigroups works properly."""
    query_multigroups = BlackwellDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of BlackwellDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = BlackwellDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_BlackwellDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of BlackwellDBQueryMultigroups works properly."""
    query_multigroups = BlackwellDBQueryMultigroups(group_id='test')
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_BlackwellDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of BlackwellDBQueryMultigroups works properly."""
    query_multigroups = BlackwellDBQueryMultigroups(group_id='test')
...
@patch('blackwell.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(BlackwellResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(BlackwellError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()


@patch('blackwell.core.agent.BlackwellQueue.send_msg_to_agent')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(BlackwellQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(BlackwellQueue.HC_FORCE_RECONNECT, agent_id)


@patch('blackwell.core.agent.BlackwellQueue')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(BlackwellError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('blackwell.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('blackwell.core.agent.get_manager_status', return_value={'blackwell-authd': 'running'}):
        agent = Agent('000')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(BlackwellError, match='.* 1726 .*'):
        agent = Agent('000')
        agent.remove()


@patch('blackwell.core.agent.BlackwellSocketJSON')
def test_agent_remove_authd(mock_blackwell_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('000')
    agent._remove_authd(purge=True)
    mock_blackwell_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(0).zfill(3), "purge": True}})
    mock_blackwell_socket.return_value.receive.assert_called_once()
    mock_blackwell_socket.return_value.close.assert_called_once()
...
@patch('blackwell.core.agent.Agent._add_authd')
...
@patch('blackwell.core.agent.get_manager_status', return_value={'blackwell-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(BlackwellError, match='.* 1706 .*'):
...
@patch('blackwell.core.agent.BlackwellSocketJSON')
def test_agent_add_authd(mock_blackwell_socket, name, ip, id, key, force):
    ...
    mock_blackwell_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (BlackwellError(9008, cmd_error=True), ".* 1705 .*"),
    (BlackwellError(9007, cmd_error=True), ".* 1706 .*"),
    (BlackwellError(9012, cmd_error=True), ".* 1708 .*"),
    (BlackwellError(9000, cmd_error=True), ".* None")
])
@patch('blackwell.core.agent.BlackwellSocketJSON')
def test_agent_add_authd_ko(mock_blackwell_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(BlackwellError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_blackwell_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(BlackwellError, match=expected_exception):
...
@patch('blackwell.core.agent.rmtree')
@patch('blackwell.core.agent.path.exists', return_value=True)
@patch('blackwell.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('blackwell.core.agent.Agent.get_agent_groups', return_value=['default'])
@patch('blackwell.core.agent.Agent.set_agent_group_relationship')
...
def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('blackwell.core.agent.Agent.get_agent_groups', side_effect=BlackwellError(2003)):
        with pytest.raises(BlackwellInternalError, match='.* 2007 .*'):
...
@patch("blackwell.core.agent.Agent.get_basic_information")
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(BlackwellError, match='.* 1722 .*'):
        Agent.group_exists('default**')


@patch('blackwell.core.agent.BlackwellDBConnection.send', return_value=('ok', '["payload"]'))
@patch('socket.socket.connect')
...
@patch('blackwell.core.agent.BlackwellDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(BlackwellInternalError, match='.* 2005 .*'):
...
@patch('blackwell.core.agent.Agent.set_agent_group_relationship')
@patch('blackwell.core.agent.Agent.group_exists', return_value=True)
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('blackwell.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('blackwell.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(BlackwellResourceNotFound, match='.* 1710 .*'):
            Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('blackwell.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(BlackwellError, match='.* 1734 .*'):
            Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('blackwell.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(BlackwellError, match='.* 1745 .*'):
            Agent.unset_single_group_agent('002', 'default', force=True)


@patch('blackwell.core.blackwell_socket.BlackwellSocket')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_blackwell_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_blackwell_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'Blackwell v4.0.0')
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_blackwell_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(BlackwellError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'Blackwell v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(BlackwellError, match=".* 1307 .*"):
        agent.get_config('com', None, 'Blackwell v4.0.0')
        agent.get_config(None, 'active-response', 'Blackwell v4.0.0')

    # Agent Blackwell version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(BlackwellInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'Blackwell v3.6.0')


@patch('blackwell.core.blackwell_socket.BlackwellSocket')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_blackwell_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_blackwell_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_blackwell_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(BlackwellInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['Blackwell v4.2.0', 'Blackwell v4.0.0', 'Blackwell v4.2.1', 'Blackwell v3.13.2']])
])
@patch('blackwell.core.agent.BlackwellQueue.send_msg_to_agent')
@patch('blackwell.core.agent.BlackwellQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('blackwell.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = BlackwellQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = BlackwellQueue.RESTART_AGENTS_JSON if BlackwellVersion(
                agent_version['version']) >= BlackwellVersion(common.AR_LEGACY_VERSION) else BlackwellQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('blackwell.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('blackwell.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `BlackwellDBConnection.send` method.
    expected_agents : set
    ...
    with patch('blackwell.core.wdb.BlackwellDBConnection.send', side_effect=wdb_response):
...
@patch('blackwell.core.agent.core_upgrade_agents')
