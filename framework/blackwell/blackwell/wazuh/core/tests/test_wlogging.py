...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core import wlogging
...
def test_sizebasedfilerotatinghandler_compute_log_directory(mock_mkdir):
    ...
    with tempfile.TemporaryDirectory() as tmp_dir:
        ...
        with patch("blackwell.core.wlogging.os.path.exists", new=_mock_exists):
...
@patch('blackwell.core.wlogging.SizeBasedFileRotatingHandler')
@patch('blackwell.core.wlogging.TimeBasedFileRotatingHandler')
def test_blackwell_logger_setup_logger(mock_time_handler, mock_size_handler, mock_add_handler, mock_add_level_name,
                                   max_size):
    """Test if method setup_logger of BlackwellLogger setups the logger attribute properly.
    ...
    # To bypass the checking of the existence of a valid Blackwell install
    # Check time handler
    with patch('os.path.join', return_value=tmp_dir.name):
        w_logger = wlogging.BlackwellLogger(foreground_mode=True, log_path=tmp_dir.name,
...
@patch.object(wlogging.BlackwellLogger, 'log_path', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'tag', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'logger', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'foreground_mode', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'debug_level', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'logger_name', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'custom_formatter', create=True, new_callable=PropertyMock)
@patch.object(wlogging.BlackwellLogger, 'max_size', create=True, new_callable=PropertyMock)
@patch('logging.Formatter')
def test_blackwell_logger__init__(mock_lformatter, mock_max_size, mock_formatter, mock_logger_name, mock_debug_level,
                              mock_foreground_mode, mock_logger, mock_tag, mock_log_path):
    """Test if BlackwellLogger __init__ method initialize all attributes properly."""
    # To bypass the checking of the existence of a valid Blackwell install
    with patch('os.path.join'):
        wlogging.BlackwellLogger(foreground_mode=mock_foreground_mode, log_path=mock_log_path, tag=mock_tag,
...
@patch('blackwell.core.wlogging.SizeBasedFileRotatingHandler')
@patch('blackwell.core.wlogging.TimeBasedFileRotatingHandler')
def test_blackwell_logger_getattr(mock_time_handler, mock_size_handler, attribute, expected_exception, expected_value):
    """Test if BlackwellLogger __getattr__ method works properly."""
    tmp_dir = tempfile.TemporaryDirectory()
    # To bypass the checking of the existence of a valid Blackwell install
    with patch('os.path.join'):
        w_logger = wlogging.BlackwellLogger(foreground_mode=True, log_path=tmp_dir.name, tag='%(test)s %(test)s: %(test)s',
