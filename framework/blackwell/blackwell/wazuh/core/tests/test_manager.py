...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core.manager import *
...
@patch('blackwell.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'blackwell-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch("blackwell.core.manager.get_blackwell_active_logging_format", return_value=log_format):
        with pytest.raises(BlackwellInternalError, match=".*1020.*"):
            get_ossec_logs()

        with patch('blackwell.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('blackwell.core.manager.tail', return_value=logs):
        ...
        assert result['blackwell-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('blackwell.core.manager.exists', return_value=True)
@patch('blackwell.core.manager.BlackwellSocket')
def test_validate_ossec_conf(mock_blackwellsocket, mock_exists):
    with patch('socket.socket') as sock:
        # Mock sock response
        json_response = json.dumps({'error': 0, 'message': ""}).encode()
        mock_blackwellsocket.return_value.receive.return_value = json_response
        result = validate_ossec_conf()

        assert result == {'status': 'OK'}
        mock_exists.assert_called_with(os.path.join(common.BLACKWELL_PATH, 'queue', 'sockets', 'com'))
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch("blackwell.core.manager.exists", return_value=False):
            with pytest.raises(BlackwellInternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('blackwell.core.manager.BlackwellSocket.send', side_effect=socket.error):
                with pytest.raises(BlackwellInternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('blackwell.core.manager.BlackwellSocket.receive', side_effect=socket.timeout):
                    with pytest.raises(BlackwellInternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('blackwell.core.manager.BlackwellSocket'):
                    with patch('blackwell.core.manager.parse_execd_output', side_effect=KeyError):
                        with pytest.raises(BlackwellInternalError, match='.* 1904 .*'):
                            validate_ossec_conf()
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(BlackwellException, match=f'.* 1908 .*'):
            parse_execd_output(json_response)


@patch('blackwell.core.manager.configuration.api_conf', new={'experimental_features': True})
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f"v{blackwell.__version__}"
...
async def test_query_update_check_service_request(
    ...
    version = '4.8.0'
    with patch('framework.blackwell.core.manager.blackwell.__version__', version):
        ...
        client_session_get_mock.assert_called_with(
            ...
            headers={
                ...
                BLACKWELL_TAG_KEY: f'v{version}',
                USER_AGENT_KEY: f'Blackwell UpdateCheckService/v{version}'
            },
