...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core.cdb_list import check_path, get_list_from_file, iterate_lists, \
...
RELATIVE_PATH = os.path.join("framework", "blackwell", "core", "tests", "data", "test_cdb_list")
PATH_FILE = os.path.join(RELATIVE_PATH, "test_lists")

CONTENT_FILE = {'test-blackwell-w': 'write',
                'test-blackwell-r': 'read',
                'test-blackwell-a': 'attribute',
                'test-blackwell-x': 'execute',
                'test-blackwell-c': 'command',
                'test-key': 'value:1',
                'test-key:1': 'value',
                'test-key:2': 'value:2',
                'test-key::::::3': 'value3',
...
def test_split_key_value_with_quotes(line, expected_key, expected_value):
    ...
    if expected_key and expected_value:
        ...
        with pytest.raises(BlackwellError) as e:
...
def test_get_list_from_file(raw):
    ...
    """
    full_path = os.path.join(common.BLACKWELL_PATH, PATH_FILE)
    if raw:
...
@pytest.mark.parametrize("error_to_raise, blackwell_error_code", [
...
def test_get_list_from_file_with_errors(error_to_raise, blackwell_error_code):
    """Test `get_list_from_file` core functionality when using invalid files or paths as parameter.

    `get_list_from_file` must raise the proper BlackwellError when facing certain scenarios like a Permission Denied error
    when opening a file.
    ...
    blackwell_error_code : int
        Error code of the `BlackwellError` that must be raised by `get_list_from_file` when the specified `OSError` occurrs.
    """
    with patch("builtins.open", mock_open()) as mock:
        ...
        try:
            ...
            assert e.code == blackwell_error_code
...
def test_validate_cdb_list():
    ...
    with open(os.path.join(common.BLACKWELL_PATH, PATH_FILE)) as f:
        blackwell_cdb_list = f.read()

    try:
        validate_cdb_list(blackwell_cdb_list)
    except BlackwellError:
...
def test_validate_cdb_list_ko():
    ...
    with pytest.raises(BlackwellError, match=r'\b1112\b'):
        validate_cdb_list('')

    # Raise exeption when using invalid CDB list
    with pytest.raises(BlackwellError, match=r'\b1800\b'):
        validate_cdb_list("test:key:testvalue\n")


@patch('blackwell.core.cdb_list.chmod')
...
def test_create_list_file(mock_delete, mock_chmod):
    ...
    with open(os.path.join(common.BLACKWELL_PATH, PATH_FILE)) as f:
        with patch('blackwell.core.cdb_list.common.BLACKWELL_PATH', new='/var/ossec'):
            with patch('builtins.open') as mock_open:
                blackwell_cdb_list = f.read()
                result = create_list_file('/test/path', blackwell_cdb_list, permissions=0o660)
                assert mock_open.return_value.__enter__().write.call_count == len(blackwell_cdb_list.split('\n'))-1
...
def test_create_list_file_ko():
    ...
    with pytest.raises(BlackwellError, match=r'\b1806\b'):
        create_list_file(full_path='/test/path', content=' ')


@patch('blackwell.core.cdb_list.remove')
@patch('blackwell.core.cdb_list.delete_blackwell_file')
def test_delete_list(mock_delete_blackwell_file, mock_remove):
    """Check that delete_list function uses expected params."""
    path = 'etc/list/test_list'
    delete_list(path)
    mock_delete_blackwell_file.assert_called_once_with(os.path.join(common.BLACKWELL_PATH, path))
    mock_remove.assert_called_once_with(os.path.join(common.BLACKWELL_PATH, path + '.cdb'))
