...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core import sca as core_sca
...
@patch('blackwell.core.utils.BlackwellDBBackend.__init__', return_value=None)
@patch('blackwell.core.utils.BlackwellDBQuery.__init__')
def test_BlackwellDBQuerySCA__init__(mock_wdbq, mock_backend, mock_get_basic_info, distinct, agent_id, offset, limit, sort,
                                 search, query, count, get_data, select):
    """Test if method __init__ of BlackwellDBQuerySCA works properly.
    ...
    """
    wdbq_sca = core_sca.BlackwellDBQuerySCA(agent_id=agent_id, offset=offset, limit=limit, sort=sort, search=search,
    ...
    mock_wdbq.assert_called_once_with(ANY, offset=offset, limit=limit, table='sca_policy', sort=sort, search=search,
                                      select=select, fields=core_sca.BlackwellDBQuerySCA.DB_FIELDS,
...
def test_BlackwellDBQuerySCA__format_data_into_dictionary(agent_id, offset, limit, sort, search, select, query, count,
                                                      get_data):
    """Check if BlackwellDBQuerySCA's method _format_data_into_dictionary works properly.
...
@patch('blackwell.core.sca.BlackwellDBQuerySCA.__init__')
def test_BlackwellDBQuerySCACheck__init__(mock_wdbqsca, select, sca_checks_test_list, expected_default_query):
    """Test if method __init__ of BlackwellDBQuerySCACheck works properly.
    ...
    """
    core_sca.BlackwellDBQuerySCACheck(agent_id='000', select=select, sort={'fields': ['title'], 'order': 'asc'},
...
def test_BlackwellDBQuerySCACheck_parse_select_filter(mock_exists, mock_get_basic_info, mock_backend):
    """Test if method _parse_select_filter of BlackwellDBQuerySCACheck works properly."""
    wdbq_sca_check = core_sca.BlackwellDBQuerySCACheck(agent_id='000', sort={'value': 'test'},
...
@patch('blackwell.core.sca.BlackwellDBQuerySCA.__init__')
def test_BlackwellDBQuerySCACheckIDs__init__(mock_wdbqsca, query):
    """Test if method __init__ of BlackwellDBQuerySCACheckIDs works properly.

    Parameters
    ----------
    query : str
        Query used to initialize the BlackwellDBQuerySCACheckIDs object.
    """
    ...
    core_sca.BlackwellDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
                                     search={'value': 'test'},
...
@patch('blackwell.core.utils.BlackwellDBBackend.__init__', return_value=None)
@patch('blackwell.core.agent.Agent.get_basic_information')
def test_BlackwellDBQuerySCACheckIDs_protected_pass_filter(mock_get_basic_info, mock_backend, field, value, expected):
    """Test BlackwellDBQuerySCACheckIDs._pass_filter function."""
    query = core_sca.BlackwellDBQuerySCACheckIDs(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
...
@patch('blackwell.core.sca.BlackwellDBQuerySCA.__init__')
def test_BlackwellDBQuerySCACheckRelational__init__(mock_wdbqsca, select, table, sca_checks_test_list):
    """Test if method __init__ of BlackwellDBQuerySCACheckRelational works properly.
    ...
    select : list or None
        Fields to select.
    table : str
        Table used to initialize the BlackwellDBQuerySCACheckRelational object.
    sca_checks_test_list : list
        List of SCA checks IDs.
    """
    query_sca_check_relational = core_sca.BlackwellDBQuerySCACheckRelational(agent_id='000', table=table,
...
@patch('blackwell.core.sca.BlackwellDBQuerySCA.__enter__')
...
def test_BlackwellDBQueryDistinctSCACheck__init__(mock_exit, mock_wdbqsca, mock_enter, select):
    """Test if method __init__ of BlackwellDBQueryDistinctSCACheck works properly."""
    ...
    core_sca.BlackwellDBQueryDistinctSCACheck(agent_id='000', offset=10, limit=20, filters={'test': 'value'},
    ...
    mock_wdbqsca.assert_has_calls(
        [call(agent_id='000', offset=0, limit=None, sort={'fields': ['title'], 'order': 'asc'},
              ...
              default_query=core_sca.BlackwellDBQueryDistinctSCACheck.INNER_QUERY_PATTERN, search={'value': 'test'}),
