...
from blackwell.core.common import OSSEC_CONF, REMOTED_SOCKET
...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core.exception import BlackwellError, BlackwellInternalError
        from blackwell.core import configuration
...
def mock_blackwell_path():
    with patch('blackwell.core.common.BLACKWELL_PATH', new=os.path.join(parent_directory, tmp_path)):
        yield
...
def test_agentconf2json():
    xml_conf = configuration.load_blackwell_xml(
...
def test_get_ossec_conf():
    with patch('blackwell.core.configuration.load_blackwell_xml', return_value=Exception):
        with pytest.raises(BlackwellError, match=".* 1101 .*"):
...
def test_get_agent_conf():
    with pytest.raises(BlackwellError, match=".* 1710 .*"):
        configuration.get_agent_conf(group_id='noexists')

    with patch('blackwell.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(BlackwellError, match=".* 1006 .*"):
...
def test_get_agent_conf_multigroup():
    ...
    with patch('blackwell.core.common.MULTI_GROUPS_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(BlackwellError, match=".* 1006 .*"):
...
def test_get_file_conf():
    with patch('blackwell.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'noexists')):
        with pytest.raises(BlackwellError, match=".* 1710 .*"):
    ...
    with patch('blackwell.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        ...
        ar_list = ['restart-ossec0 - restart-ossec.sh - 0', 'restart-ossec0 - restart-ossec.cmd - 0',
                   'restart-blackwell0 - restart-ossec.sh - 0', 'restart-blackwell0 - restart-ossec.cmd - 0',
                   'restart-blackwell0 - restart-blackwell - 0', 'restart-blackwell0 - restart-blackwell.exe - 0']
...
def test_parse_internal_options():
    ...
    with patch('blackwell.core.common.INTERNAL_OPTIONS_CONF',
        ...
        with patch('blackwell.core.common.LOCAL_INTERNAL_OPTIONS_CONF',
                   new=os.path.join(parent_directory, tmp_path, 'configuration/local_internal_options.conf')):
            with pytest.raises(BlackwellInternalError, match=".* 1108 .*"):
                configuration.parse_internal_options('ossec', 'python')


def test_get_internal_options_value():
    with patch('blackwell.core.configuration.parse_internal_options', return_value='str'):
        with pytest.raises(BlackwellError, match=".* 1109 .*"):
            configuration.get_internal_options_value('ossec', 'python', 5, 1)

    with patch('blackwell.core.configuration.parse_internal_options', return_value='0'):
        with pytest.raises(BlackwellError, match=".* 1110 .*"):
            configuration.get_internal_options_value('ossec', 'python', 5, 1)

    with patch('blackwell.core.configuration.parse_internal_options', return_value='1'):
        assert configuration.get_internal_options_value('ossec', 'python', 5, 1) == 1


@patch('blackwell.core.configuration.common.blackwell_gid')
@patch('blackwell.core.configuration.common.blackwell_uid')
@patch('builtins.open')
def test_upload_group_configuration(mock_open, mock_blackwell_uid, mock_blackwell_gid):
    with pytest.raises(BlackwellError, match=".* 1710 .*"):
        configuration.upload_group_configuration('noexists', 'noexists')

    with patch('blackwell.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with patch('blackwell.core.configuration.tempfile.mkstemp', return_value=['mock_handle', 'mock_tmp_file']):
            with patch('blackwell.core.configuration.open'):
                with pytest.raises(BlackwellInternalError, match=".* 1743 .*"):
                    configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
            with patch('blackwell.core.configuration.open', return_value=Exception):
                with pytest.raises(BlackwellError, match=".* 1113 .*"):
                    configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
            with patch('builtins.open'):
                with patch('blackwell.core.configuration.subprocess.check_output', return_value=True):
                    with patch('blackwell.core.utils.chown', side_effect=None):
                        with patch('blackwell.core.utils.chmod', side_effect=None):
                            with patch('blackwell.core.configuration.safe_move'):
                                assert isinstance(configuration.upload_group_configuration('default',
                                                                                           "<agent_config>new_config"
                                                                                           "</agent_config>"),
                                                  str)
                            with patch('blackwell.core.configuration.safe_move', side_effect=Exception):
                                with pytest.raises(BlackwellInternalError, match=".* 1016 .*"):
                                    configuration.upload_group_configuration('default',
                                                                             "<agent_config>new_config</agent_config>")
            with patch('blackwell.core.configuration.subprocess.check_output',
                       side_effect=subprocess.CalledProcessError(cmd='ls', returncode=1, output=b'ERROR')):
                with patch('blackwell.core.configuration.re.findall', return_value=None):
                    with pytest.raises(BlackwellError, match=".* 1115 .*"):
                        configuration.upload_group_configuration('default', "<agent_config>new_config</agent_config>")
                with patch('blackwell.core.configuration.re.findall', return_value='1114'):
                    with patch('os.path.exists', return_value=True):
                        with patch('blackwell.core.configuration.remove') as mock_remove:
                            with pytest.raises(BlackwellError, match=".* 1114 .*"):
                                configuration.upload_group_configuration('default',
                                                                         "<agent_config>new_config</agent_config>")
...
@patch('blackwell.core.configuration.common.blackwell_uid')
@patch('builtins.open')
@patch('blackwell.core.configuration.safe_move')
def test_upload_group_file(mock_safe_move, mock_open, mock_blackwell_uid, mock_blackwell_gid):
    with pytest.raises(BlackwellError, match=".* 1710 .*"):
        configuration.upload_group_file('noexists', 'given', 'noexists')

    with patch('blackwell.core.configuration.os_path.exists', return_value=True):
        with pytest.raises(BlackwellError, match=".* 1112 .*"):
    ...
    with patch('blackwell.core.common.SHARED_PATH', new=os.path.join(parent_directory, tmp_path, 'configuration')):
        with pytest.raises(BlackwellError, match=".* 1111 .*"):
...
@pytest.mark.parametrize("agent_id, component, socket, socket_dir, rec_msg", [
    ...
    ('000', 'blackwell-db', 'wdb', 'db', {"error": 0, "data": {"enabled": "yes"}}),
...
@patch('blackwell.core.blackwell_socket.create_blackwell_socket_message')
@patch('os.path.exists')
@patch('blackwell.core.common.BLACKWELL_PATH', new='/var/ossec')
def test_get_active_configuration(mock_exists, mock_create_blackwell_socket_message, agent_id, component, socket,
                                  socket_dir, rec_msg):
    """This test checks the proper working of get_active_configuration function."""
    sockets_json_protocol = {'remote', 'analysis', 'wdb'}
    config = MagicMock()

    socket_class = "BlackwellSocket" if socket not in sockets_json_protocol or agent_id != '000' else "BlackwellSocketJSON"
    with patch(f'blackwell.core.blackwell_socket.{socket_class}.close') as mock_close:
        with patch(f'blackwell.core.blackwell_socket.{socket_class}.send') as mock_send:
            with patch(f'blackwell.core.blackwell_socket.{socket_class}.__init__', return_value=None) as mock__init__:
                with patch(f'blackwell.core.blackwell_socket.{socket_class}.receive',
                           return_value=rec_msg.encode() if socket_class == "BlackwellSocket" else rec_msg) as mock_receive:
                    ...
                    if socket_class == "BlackwellSocket":
                        mock_send.assert_called_with(f"getconfig {config}".encode() if agent_id == '000' else \
                                                         f"{agent_id} {component} getconfig {config}".encode())
                    else:  # socket_class == "BlackwellSocketJSON"
                        mock_create_blackwell_socket_message.assert_called_with(origin={'module': ANY},
                                                                            command="getconfig",
                                                                            parameters={'section': config})
                        mock_send.assert_called_with(mock_create_blackwell_socket_message.return_value)
...
@pytest.mark.parametrize('agent_id, component, config, socket_exist, socket_class, expected_error, expected_id', [
    ...
    ('000', 'test_component', None, ANY, 'BlackwellSocket', BlackwellError, 1307),  # No configuration
    ('000', None, 'test_config', ANY, 'BlackwellSocket', BlackwellError, 1307),  # No component
    ('000', 'test_component', 'test_config', ANY, 'BlackwellSocket', BlackwellError, 1101),  # Component not in components
    ('001', 'syscheck', 'syscheck', ANY, 'BlackwellSocket', BlackwellError, 1116),  # Cannot send request
    ('001', 'syscheck', 'syscheck', ANY, 'BlackwellSocket', BlackwellError, 1117),  # No such file or directory

    # Checks for 000 - Simple messages
    ('000', 'syscheck', 'syscheck', False, 'BlackwellSocket', BlackwellError, 1121),  # Socket does not exist
    ('000', 'syscheck', 'syscheck', True, 'BlackwellSocket', BlackwellInternalError, 1121),  # Error connecting with socket
    ('000', 'syscheck', 'syscheck', True, 'BlackwellSocket', BlackwellInternalError, 1118),  # Data could not be received

    # Checks for 000 - JSON messages
    ('000', 'request', 'global', False, 'BlackwellSocketJSON', BlackwellError, 1121),  # Socket does not exist
    ('000', 'request', 'global', True, 'BlackwellSocketJSON', BlackwellInternalError, 1121),  # Error connecting with socket
    ('000', 'request', 'global', True, 'BlackwellSocketJSON', BlackwellInternalError, 1118),  # Data could not be received

    # Checks for 001
    ('001', 'syscheck', 'syscheck', ANY, 'BlackwellSocket', BlackwellInternalError, 1121),  # Error connecting with socket
    ('001', 'syscheck', 'syscheck', ANY, 'BlackwellSocket', BlackwellInternalError, 1118)  # Data could not be received
...
def test_get_active_configuration_ko(mock_exists, agent_id, component, config, socket_exist, socket_class,
    ...
    with patch(f'blackwell.core.blackwell_socket.{socket_class}.__init__',
               return_value=MagicMock() if expected_id == 1121 and socket_exist else None):
        with patch(f'blackwell.core.blackwell_socket.{socket_class}.send'):
            with patch(f'blackwell.core.blackwell_socket.{socket_class}.receive',
                ...
                with patch(f'blackwell.core.blackwell_socket.{socket_class}.close'):
...
def test_write_ossec_conf():
    content = "New config"
    with patch('blackwell.core.configuration.open', mock_open()) as mocked_file:
...
def test_write_ossec_conf_exceptions():
    with patch('blackwell.core.configuration.open', return_value=Exception):
        with pytest.raises(BlackwellError, match=".* 1126 .*"):
...
@patch('blackwell.core.configuration.get_ossec_conf')
...
def test_update_check_is_enabled_exceptions(error_id, value):
    ...
    with patch('blackwell.core.configuration.get_ossec_conf', side_effect=BlackwellError(error_id), return_value=value):
        if value is not None:
            assert configuration.update_check_is_enabled() == value
        else:
            with pytest.raises(BlackwellError, match=f'.* {error_id} .*'):
