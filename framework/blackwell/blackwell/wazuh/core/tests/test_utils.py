...
from blackwell.core.common import OSSEC_TMP_PATH
...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell import BlackwellException
        from blackwell.core.agent import BlackwellDBQueryAgents
        from blackwell.core import utils, exception
        from blackwell.core.common import BLACKWELL_PATH, AGENT_NAME_LEN_LIMIT
...
@patch('blackwell.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
...
def test_cut_array_ko(limit, offset, expected_exception):
    ...
    """
    with pytest.raises(exception.BlackwellException, match=f'.* {expected_exception} .*'):
...
@patch('blackwell.core.utils.len', return_value=1)
@patch('blackwell.core.utils.cut_array')
@patch('blackwell.core.utils.select_array', return_value=ANY)
@patch('blackwell.core.utils.filter_array_by_query', return_value=ANY)
@patch('blackwell.core.utils.search_array', return_value=ANY)
@patch('blackwell.core.utils.sort_array', return_value=ANY)
...
@patch('blackwell.core.utils.chmod')
...
@patch('blackwell.core.utils.chown')
...
@patch('blackwell.core.utils.common.BLACKWELL_PATH', new='/test/path')
@patch('blackwell.core.utils.path.exists', return_value=True)
@patch('blackwell.core.utils.remove')
def test_delete_blackwell_file(mock_remove, mock_exists):
    """Check delete_file calls functions with expected params"""
    assert utils.delete_blackwell_file('/test/path/etc/file')
    mock_remove.assert_called_once_with('/test/path/etc/file')


@patch('blackwell.core.utils.common.BLACKWELL_PATH', new='/test/path')
def test_delete_blackwell_file_ko():
    """Check delete_file calls functions with expected params"""
    with pytest.raises(utils.BlackwellError, match=r'\b1907\b'):
        utils.delete_blackwell_file('/test/different_path/etc/file')

    with pytest.raises(utils.BlackwellError, match=r'\b1907\b'):
        utils.delete_blackwell_file('/test/path/file/../../home')

    with patch('blackwell.core.utils.path.exists', return_value=False):
        with pytest.raises(utils.BlackwellError, match=r'\b1906\b'):
            utils.delete_blackwell_file('/test/path/etc/file')

    with patch('blackwell.core.utils.path.exists', return_value=True):
        with pytest.raises(utils.BlackwellError, match=r'\b1907\b'):
            utils.delete_blackwell_file('/test/path/etc/file')
...
@patch('blackwell.core.utils.chown')
@patch('blackwell.core.utils.chmod')
@patch('blackwell.core.utils.utime')
...
def test_safe_move_exception(mock_utime, mock_chmod, mock_chown):
    ...
    with TemporaryDirectory() as tmpdirname:
        tmp_file = NamedTemporaryFile(dir=tmpdirname, delete=False)
        target_file = os.path.join(tmpdirname, 'target')
        with patch('blackwell.core.utils.rename', side_effect=OSError(1)):
...
@patch('blackwell.core.utils.chmod')
@patch('blackwell.core.utils.mkdir')
@patch('blackwell.core.utils.curdir', new='var')
def test_mkdir_with_mode(mock_mkdir, mock_chmod, dir_name, path_exists):
    """Test mkdir_with_mode function."""
    with patch('blackwell.core.utils.path.exists', return_value=path_exists):
...
@patch('blackwell.core.utils.mkdir', side_effect=OSError)
def test_mkdir_with_mode_ko(mock_mkdir, dir_name, exists):
    """Test mkdir_with_mode function errors work."""
    with patch('blackwell.core.utils.path.exists', return_value=exists):
        with pytest.raises(OSError):
            utils.mkdir_with_mode(dir_name)


@patch('blackwell.core.utils.open')
@patch('blackwell.core.utils.iter', return_value=['1', '2'])
def test_md5(mock_iter, mock_open):
    """Test md5 function."""
    with patch('blackwell.core.utils.hashlib.md5') as md:
...
def test_blake2b(mock_iter, mock_open):
    """Test md5 function."""
    with patch('blackwell.core.utils.hashlib.blake2b') as blake2b_mock:
...
def test_protected_get_hashing_algorithm_ko():
    ...
    with pytest.raises(exception.BlackwellException, match=".* 1723 .*"):
...
def test_get_hash(mock_open):
    ...
    with patch('blackwell.core.utils.iter', return_value=['1', '2']):
        with patch('blackwell.core.utils.hashlib.new') as md:
            md.return_value.update.side_effect = None
    ...
    with patch('blackwell.core.utils.iter', return_value=[]):
...
@patch('blackwell.core.utils.compile', return_value='Something')
def test_basic_load_blackwell_xml(mock_compile):
    """Test basic load_blackwell_xml functionality."""
    with patch('blackwell.core.utils.open') as f:
        f.return_value.__enter__.return_value = StringIO(test_xml)
        result = utils.load_blackwell_xml('test_file')

        assert isinstance(result, Element)


def test_load_blackwell_xml():
    """Test load_blackwell_xml function."""
    ...
    for rule_file in os.listdir(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_blackwell_xml')):
        path = os.path.join(os.path.join(os.path.dirname(os.path.abspath(__file__)), 'data/test_load_blackwell_xml'),
                            rule_file)
        original = parse(path).getroot()
        result = utils.load_blackwell_xml(path)
...
def test_load_blackwell_xml_ko(expected_exception):
    """Test load_blackwell_xml fails gracefully when reading an invalid utf-8 character sequence"""
    file_path = os.path.join(test_data_path, 'test_load_blackwell_xml_ko/invalid_utf8.xml')
    with pytest.raises(BlackwellException, match=f'.* {expected_exception} .*'):
        utils.load_blackwell_xml(file_path)

@pytest.mark.parametrize('version1, version2', [
    ('Blackwell v3.5.0', 'Blackwell v3.5.2'),
    ('Blackwell v3.6.1', 'Blackwell v3.6.3'),
    ('Blackwell v3.7.2', 'Blackwell v3.8.0'),
    ('Blackwell v3.8.0', 'Blackwell v3.8.1'),
    ('Blackwell v3.9.0', 'Blackwell v3.9.2'),
    ('Blackwell v3.9.10', 'Blackwell v3.9.14'),
    ('Blackwell v3.10.1', 'Blackwell v3.10.10'),
    ('Blackwell v4.10.10', 'Blackwell v4.11.0'),
    ('Blackwell v5.1.15', 'Blackwell v5.2.0'),
    ('v3.6.0', 'v3.6.1'),
    ('v3.9.1', 'v3.9.2'),
    ('v4.0.0', 'v4.0.1'),
    ('3.6.0', '3.6.1'),
...
def test_version_ok(version1, version2):
    """Test BlackwellVersion class."""
    current_version = utils.BlackwellVersion(version1)
    new_version = utils.BlackwellVersion(version2)
...
@pytest.mark.parametrize('version1, version2', [
    ('v3.6.0', 'v.3.6.1'),
    ('Blackwell v4', 'Blackwell v5'),
    ('Blackwell v3.9', 'Blackwell v3.10'),
    ('ABC v3.10.1', 'ABC v3.10.12'),
    ('Blackwellv3.9.0', 'Blackwellv3.9.2'),
    ('3.9', '3.10'),
    ('3.9.0', '3.10'),
    ('3.10', '4.2'),
    ('3', '3.9.1')
...
def test_version_ko(version1, version2):
    """Test BlackwellVersion class."""
    try:
        utils.BlackwellVersion(version1)
        utils.BlackwellVersion(version2)
    except ValueError:
        return


@pytest.mark.parametrize('version1, version2', [
    ('Blackwell v3.10.10', 'Blackwell v3.10.10'),
    ('Blackwell v5.1.15', 'Blackwell v5.1.15'),
    ('v3.6.0', 'v3.6.0'),
    ('v3.9.2', 'v3.9.2')
])
...
def test_BlackwellVersion_to_array():
    """Test BlackwellVersion.to_array function."""
    version = utils.BlackwellVersion('Blackwell v3.10.0-alpha4')

    assert isinstance(version.to_array(), list)


def test_BlackwellVersion__str__():
    """Test BlackwellVersion.__str__ function."""
    version = utils.BlackwellVersion('Blackwell v3.10.0-alpha4')
...
@pytest.mark.parametrize('version1, version2', [
    ('Blackwell v3.5.2', 'Blackwell v4.0.0'),
    ('Blackwell v3.10.0-alpha', 'Blackwell v3.10.0'),
    ('Blackwell v3.10.0-alpha4', 'Blackwell v3.10.0-beta4'),
    ('Blackwell v3.10.0-alpha3', 'Blackwell v3.10.0-alpha4'),
])
def test_BlackwellVersion__ge__(version1, version2):
    """Test BlackwellVersion.__ge__ function."""
    current_version = utils.BlackwellVersion(version1)
    new_version = utils.BlackwellVersion(version2)
...
def test_failed_test_get_timeframe_in_seconds():
    ...
    with pytest.raises(exception.BlackwellException, match=".* 1411 .*"):
...
@patch('blackwell.core.utils.path.exists', return_value=True)
@patch('blackwell.core.utils.glob.glob', return_value=True)
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_BlackwellDBQuery_protected_clean_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                             query_filter, expected_query_filter, expected_wef):
    """Test BlackwellDBQuery._clean_filter function."""
    query = utils.BlackwellDBQuery(offset=0, limit=500, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=1),
...
@patch('socket.socket.connect')
@patch('blackwell.core.utils.common.MAXIMUM_DATABASE_LIMIT', new=10)
def test_BlackwellDBQuery_protected_add_limit_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                   limit, error, expected_exception):
    """Test BlackwellDBQuery._add_limit_to_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=limit, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=1),
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_sort_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests BlackwellDBQuery._sort_query function works"""

    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=1), count=5,
                               get_data=None)
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_add_sort_to_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                                  sort, expected_exception):
    """Test BlackwellDBQuery._add_sort_to_query function."""
    fields = {'1': 'one', '2': 'two', '3': 'three', '4': 'four'}
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=sort,
                               search=None, select=None, filters=None,
                               fields=fields,
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=1),
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_add_search_to_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test BlackwellDBQuery._add_search_to_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('blackwell.core.utils.glob.glob', return_value=True)
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_parse_select_filter(mock_socket_conn, mock_glob, mock_conn_db, mock_exists,
                                                    selector_fields, error, expected_exception):
    """Test BlackwellDBQuery._parse_select_filter function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
                               fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._parse_select_filter')
def test_BlackwellDBQuery_protected_add_select_to_query(mock_parse, mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test BlackwellDBQuery._add_select_to_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent',
                               sort={'order': 'asc'}, search=None, select=None,
                               filters=None, fields={'1': None, '2': None},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.path.exists', return_value=True)
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
def test_BlackwellDBQuery_protected_parse_query_regex(mock_backend_connect, mock_exists, q, expected_query_filters):
    """Test BlackwellDBQuery._parse_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_parse_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_parse_legacy_filters(mock_socket_conn, mock_conn_db, mock_glob,
                                                     mock_exists, filter_):
    """Test BlackwellDBQuery._parse_legacy_filters function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None, filters=filter_,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._parse_legacy_filters')
@patch('blackwell.core.utils.BlackwellDBQuery._parse_query')
def test_BlackwellDBQuery_parse_filters(mock_query, mock_filter, mock_socket_conn, mock_conn_db, mock_glob,
                                    mock_exists, filter, q):
    """Test BlackwellDBQuery._parse_filters function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search={"negation": True, "value": "1"}, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=q,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._filter_status')
@patch('blackwell.core.utils.BlackwellDBQuery._filter_date')
def test_BlackwellDBQuery_protected_process_filter(mock_date, mock_status, mock_socket_conn, mock_conn_db,
                                               mock_glob, mock_exists, field_name, field_filter, q_filter):
    """Tests BlackwellDBQuery._process_filter."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04', 'status': 'active'},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=0), count=5,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._process_filter')
def test_BlackwellDBQuery_protected_add_filters_to_query(mock_process, mock_socket_conn,
                                                     mock_conn_db, mock_glob, mock_exists):
    """Test BlackwellDBQuery._add_filters_to_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=['os.name'],
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query='os.name=ubuntu',
                               backend=utils.BlackwellDBBackend(agent_id=0), distinct=True,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('blackwell.core.utils.path.exists', return_value=True)
def test_BlackwellDBQuery_protected_add_filters_to_query_final_query(mock_conn_db, mock_file_exists,
                                                                 filters, expected_query):
    """Test BlackwellDBQuery._add_filters_to_query final query."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None, search=None, select=None,
                               fields={'id': 'id', 'status': 'status', 'group': 'group'},
                               default_sort_field=None, query='', backend=utils.BlackwellDBBackend(agent_id=0),
                               count=5, get_data=None)
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_get_total_items(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test BlackwellDBQuery._get_total_items function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_get_total_items_mitre(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
                               backend=utils.BlackwellDBBackend(agent_id=0, query_format='mitre'), count=5,
                               get_data=None, date_fields=['date1', 'date2'])
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_substitute_params(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BlackwellDBQuery._get_total_items function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select=None,
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.BlackwellDBQuery._format_data_into_dictionary."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_filter_status(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BlackwellDBQuery._filter_status function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_filter_date(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            date_filter, filter_db_name, time, error):
    """Test utils.BlackwellDBQuery._filter_date function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
    ...
    with patch('blackwell.core.utils.get_timeframe_in_seconds', return_value=time):
        if error:
            with pytest.raises(exception.BlackwellException, match=".* 1412 .*"):
...
@patch('socket.socket.connect')
def test_BlackwellDBQuery_general_run(mock_socket_conn, execute_value, expected_result):
    """Test utils.BlackwellDBQuery.general_run function."""
    with patch('blackwell.core.utils.BlackwellDBBackend.execute', return_value=execute_value):
        query = BlackwellDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('socket.socket.connect')
def test_BlackwellDBQuery_oversized_run(mock_socket_conn, execute_value, rbac_ids, negate,
                                    final_rbac_ids, expected_result):
    """Test utils.BlackwellDBQuery.oversized_run function."""
    with patch('blackwell.core.utils.BlackwellDBBackend.execute', side_effect=[execute_value, final_rbac_ids]):
        query = BlackwellDBQueryAgents(offset=0, limit=None, sort=None, search=None, select={'id'},
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._default_query')
def test_BlackwellDBQuery_reset(mock_query, mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BlackwellDBQuery.reset function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'os.name'},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Test utils.BlackwellDBQuery._default_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                    mock_exists):
    """Test utils.BlackwellDBQuery._default_count_query function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
                               search=None, select={'fields': set(['os.name'])},
                               fields={'os.name': 'ubuntu', 'os.version': '18.04'},
                               default_sort_field=None, query=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQuery_protected_pass_filter(mock_socket_conn, mock_conn_db, mock_glob, mock_exists,
                                            value):
    """Test utils.BlackwellDBQuery._pass_filter function."""
    query = utils.BlackwellDBQuery(offset=0, limit=1, table='agent', sort=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQueryDistinct_protected_default_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                      mock_exists):
    """Test utils.BlackwellDBQueryDistinct._default_query function."""
    query = utils.BlackwellDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQueryDistinct_protected_default_count_query(mock_socket_conn, mock_conn_db, mock_glob,
                                                            mock_exists):
    """Test utils.BlackwellDBQueryDistinct._default_count_query function."""
    query = utils.BlackwellDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._add_filters_to_query')
def test_BlackwellDBQueryDistinct_protected_add_filters_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                             mock_glob, mock_exists):
    """Test utils.BlackwellDBQueryDistinct._add_filters_to_query function."""
    query = utils.BlackwellDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'name'},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True, default_sort_field='`group`',
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._add_select_to_query')
def test_BlackwellDBQueryDistinct_protected_add_select_to_query(mock_add, mock_socket_conn, mock_conn_db,
                                                            mock_glob, mock_exists, select):
    """Test utils.BlackwellDBQueryDistinct._add_select_to_query function."""
    query = utils.BlackwellDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
    ...
    if len(select) > 1:
        with pytest.raises(exception.BlackwellException, match=".* 1410 .*"):
            query._add_select_to_query()
    else:
        query._add_select_to_query()
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQueryDistinct_protected_format_data_into_dictionary(mock_socket_conn, mock_conn_db,
                                                                    mock_glob, mock_exists):
    """Test utils.BlackwellDBQueryDistinct._format_data_into_dictionary function."""
    query = utils.BlackwellDBQueryDistinct(offset=0, limit=1, sort=None, search=None,
                                       query=None, select={'fields': ['name']},
                                       fields={'name': '`group`'}, count=True,
                                       get_data=True,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
def test_BlackwellDBQueryGroupBy__init__(mock_socket_conn, mock_conn_db, mock_glob, mock_exists):
    """Tests utils.BlackwellDBQueryGroupBy.__init__ function works"""
    utils.BlackwellDBQueryGroupBy(filter_fields=None, offset=0, limit=1, table='agent',
                              ...
                              backend=utils.BlackwellDBBackend(agent_id=0), get_data=None,
...
@patch('blackwell.core.utils.BlackwellDBBackend.connect_to_db')
@patch('socket.socket.connect')
@patch('blackwell.core.utils.BlackwellDBQuery._get_total_items')
def test_BlackwellDBQueryGroupBy_protected_get_total_items(mock_total, mock_socket_conn, mock_conn_db,
                                                       mock_glob, mock_exists):
    """Test utils.BlackwellDBQueryGroupBy._get_total_items function."""
    query = utils.BlackwellDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
...
@patch('blackwell.core.utils.BlackwellDBQuery._add_select_to_query')
@patch('blackwell.core.utils.BlackwellDBQuery._parse_select_filter')
def test_BlackwellDBQueryGroupBy_protected_add_select_to_query(mock_parse, mock_add, mock_socket_conn,
                                                           mock_conn_db, mock_glob, mock_exists):
    """Test utils.BlackwellDBQueryGroupBy._add_select_to_query function."""
    query = utils.BlackwellDBQueryGroupBy(filter_fields={'fields': ['name']}, offset=0,
                                      limit=1, table='agent', sort=None, search=None,
                                      select={'name'}, filters=None,
                                      fields={'name': '`group`'}, query=None,
...
def test_filter_array_by_query(q, return_length):
    ...
    if return_length == -1:
        with pytest.raises(exception.BlackwellError, match='.* 1407 .*'):
...
def test_select_array(select, required_fields, expected_result):
    ...
    except utils.BlackwellError as e:
        assert e.code == 1724
...
@patch('blackwell.core.utils.check_blackwell_limits_unchanged')
@patch('blackwell.core.utils.check_remote_commands')
@patch('blackwell.core.utils.check_agents_allow_higher_versions')
@patch('blackwell.core.utils.check_virustotal_integration')
@patch('blackwell.core.utils.check_indexer')
@patch('blackwell.core.manager.common.BLACKWELL_PATH', new=test_files_path)
def test_validate_blackwell_xml(mock_check_indexer, mock_virus_total_integration,
                            mock_agents_versions, mock_remote_commands, mock_unchanged_limits):
    """Test validate_blackwell_xml method works and methods inside are called with expected parameters"""
    ...
    with patch('builtins.open', m):
        utils.validate_blackwell_xml(xml_file)
    ...
    with patch('builtins.open', m):
        utils.validate_blackwell_xml(xml_file, config_file=True)
...
def test_validate_blackwell_xml_ko(effect, expected_exception):
    """Tests validate_blackwell_xml function works when open function raises an exception.
    Parameters
    ----------
    effect : Exception
    ...
    with patch('blackwell.core.utils.load_blackwell_xml', side_effect=effect):
        with pytest.raises(BlackwellException, match=f'.* {expected_exception} .*'):
            utils.validate_blackwell_xml(input_file)


@patch('blackwell.core.utils.full_copy')
...
@patch('blackwell.core.utils.full_copy', side_effect=IOError)
def test_delete_file_with_backup_ko(mock_copyfile):
    """Test delete_file_with_backup function exceptions."""
    with pytest.raises(utils.BlackwellError, match='.* 1019 .*'):
...
def test_to_relative_path():
    ...
    path = 'etc/ossec.conf'
    assert utils.to_relative_path(os.path.join(BLACKWELL_PATH, path)) == path

    assert utils.to_relative_path(path, prefix='etc') == os.path.basename(path)


@patch('blackwell.core.utils.common.RULES_PATH', new=test_files_path)
@patch('blackwell.core.utils.common.USER_RULES_PATH', new=test_files_path)
...
@patch('blackwell.core.utils.common.DECODERS_PATH', new=test_files_path)
@patch('blackwell.core.utils.common.USER_DECODERS_PATH', new=test_files_path)
...
@patch('blackwell.core.utils.common.LISTS_PATH', new=test_files_path)
@patch('blackwell.core.utils.common.USER_LISTS_PATH', new=test_files_path)
...
@patch('blackwell.core.utils.copy2', new=copyfile)
...
def test_check_blackwell_limits_unchanged(new_conf, unchanged_limits_conf, original_conf, limits_conf):
    ...
    with patch('blackwell.core.utils.configuration.api_conf', new=api_conf):
        if limits_conf['eps']['allow'] or unchanged_limits_conf:
            utils.check_blackwell_limits_unchanged(new_conf, original_conf)
        else:
            with pytest.raises(exception.BlackwellError, match=".* 1127 .*"):
                utils.check_blackwell_limits_unchanged(new_conf, original_conf)
...
def test_agents_allow_higher_versions(new_conf, agents_conf):
    ...
    with patch('blackwell.core.utils.configuration.api_conf', new=api_conf):
        if agents_conf['allow_higher_versions']['allow'] or new_conf.find('no') != -1:
            ...
            with pytest.raises(exception.BlackwellError, match=".* 1129 .*"):
                utils.check_agents_allow_higher_versions(new_conf)
...
@patch('blackwell.core.utils.tempfile.mkstemp',
        return_value=('handle', os.path.join(OSSEC_TMP_PATH, 'file.tmp')))
@patch('blackwell.core.utils.chmod')
@patch('blackwell.core.common.blackwell_gid')
@patch('blackwell.core.common.blackwell_uid')
def test_upload_file(mock_uid, mock_gid, 
    ...
    ----------
    mock_uid: Mock
        mock of the blackwell.core.common.blackwell_uid function.
    mock_gid: Mock
        mock of the blackwell.core.common.blackwell_gid function.
    mock_chmod: Mock
        mock of the blackwell.core.utils.chmod function.
    mock_mks
        Mock of the blackwell.core.utils.tempfile.mkstemp function.
    chk_xml: bool
    ...
    with patch('blackwell.core.utils.open', mko):
        with patch('blackwell.core.utils.safe_move') as mock_safe_move:
            result = utils.upload_file(content, file_path=filename,
                                        check_xml_formula_values=chk_xml)
            assert isinstance(result, BlackwellResult)
            handle.write.assert_called_once_with(content)
            tmp_path = os.path.join(OSSEC_TMP_PATH, 'file.tmp')
            file_path = os.path.join(BLACKWELL_PATH, filename)
...
@pytest.mark.parametrize(
    'sm_side_effect, w_side_effect, upload_error', [
        ...
        (None, True, (utils.BlackwellInternalError, 1005)),
        # safe_move raises an Error()
        (Error(), False, (utils.BlackwellInternalError, 1016)),
        # safe_move raises a PermissionError()
        (PermissionError(), False, (utils.BlackwellError, 1006))]
)
@patch('blackwell.core.utils.tempfile.mkstemp',
        return_value=('handle', os.path.join(OSSEC_TMP_PATH, 'file.tmp')))
@patch('blackwell.core.utils.chmod')
...
def test_upload_file_ko(mock_uid, mock_gid, mock_chmod, mock_mks,
    ...
    with patch('blackwell.core.utils.open', mko):
        with patch('blackwell.core.utils.safe_move', side_effect=sm_side_effect) as mock_safe_move:
...
def test_check_virustotal_integration(integrations_conf, new_conf):
    ...
    with patch('blackwell.core.utils.get') as mock_requests_get:
        if not integrations_conf['virustotal']['public_key']['allow']:
            with pytest.raises(exception.BlackwellError, match=".* 1130 .*"):
                mock_response = Mock()
            ...
            with pytest.raises(exception.BlackwellError, match=".* 1131 .*"):
