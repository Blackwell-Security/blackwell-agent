...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.core import decoder
...
@pytest.mark.parametrize("detail, value, details, expected_result", [
    ("parent", "blackwell", {'regex': ['regex00']}, {'parent': 'blackwell', 'regex': ['regex00']}),
...
@pytest.mark.parametrize("filename, relative_dirname, status, permissions, exception", [
    ...
    ('wrong_decoders.xml', 'decoders', "all", 777, BlackwellInternalError(1501)),
    ('non_existing.xml', 'decoders', "disabled", 777, BlackwellError(1502)),
    ('test1_decoders.xml', 'decoders', "all", 000, BlackwellError(1502)),
])
@patch('blackwell.core.common.BLACKWELL_PATH', new=test_data_path)
