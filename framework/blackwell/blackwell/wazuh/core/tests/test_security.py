...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a BlackwellResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('blackwell.core.security.change_keypair', side_effect=None):
        security, BlackwellResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='blackwell')
        with patch("blackwell.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('blackwell.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('blackwell.core.security.TokenManager.add_user_roles_rules')
...
@patch("blackwell.core.security.revoke_tokens")
@patch("blackwell.core.security.check_database_integrity")
@patch("blackwell.core.security.os.remove")
