...
from blackwell.core.common import find_blackwell_path, blackwell_uid, blackwell_gid, context_cached, reset_context_cache, \
    get_context_cache


@pytest.mark.parametrize('fake_path, expected', [
    ('/var/ossec/framework/python/lib/python3.7/site-packages/blackwell-3.10.0-py3.7.egg/blackwell', '/var/ossec'),
    ('/my/custom/path/framework/python/lib/python3.7/site-packages/blackwell-3.10.0-py3.7.egg/blackwell', '/my/custom/path'),
    ('/my/fake/path', '')
])
def test_find_blackwell_path(fake_path, expected):
    with patch('blackwell.core.common.__file__', new=fake_path):
        assert (find_blackwell_path.__wrapped__() == expected)


def test_find_blackwell_path_relative_path():
    with patch('os.path.abspath', return_value='~/framework'):
        assert (find_blackwell_path.__wrapped__() == '~')


def test_blackwell_uid():
    with patch('blackwell.core.common.getpwnam', return_value=getpwnam("root")):
        blackwell_uid()


def test_blackwell_gid():
    with patch('blackwell.core.common.getgrnam', return_value=getgrnam("root")):
        blackwell_gid()
...
@patch('blackwell.core.logtest.create_blackwell_socket_message', side_effect=SystemExit)
...
async def test_origin_module_context_var_api(mock_check_blackwell_status, mock_create_socket_msg):
    ...
    # side_effect used to avoid mocking the rest of functions
    with pytest.raises(SystemExit):
        d = dapi.DistributedAPI(f=logtest.run_logtest, logger=logging.getLogger('blackwell'), is_async=True)
