...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        from blackwell.core.results import BlackwellResult, AffectedItemsBlackwellResult, _goes_before_than, nested_itemgetter, merge
        from blackwell import BlackwellException, BlackwellError

param_name = ['affected_items', 'total_affected_items', 'sort_fields', 'sort_casting', 'sort_ascending',
              'all_msg', 'some_msg', 'none_msg']
BLACKWELL_EXCEPTION_CODE = 1725
FAILED_AGENT_ID = '999'


@pytest.fixture(scope='function')
def get_blackwell_result():
    return BlackwellResult(dct={"data": {"items": [{"item1": "data1"}, {"item2": "OK"}], "message": "Everything ok"}},
                       str_priority=['KO', 'OK'])


@pytest.fixture(scope='function')
def get_blackwell_affected_item():
    def _get_affected(params=None):
        kwargs = {p_name: param for p_name, param in zip(param_name, params)}
        return AffectedItemsBlackwellResult(**kwargs)

    return _get_affected


@pytest.fixture(scope='function')
def get_blackwell_failed_item():
    item = AffectedItemsBlackwellResult()
    item.add_failed_item(id_=FAILED_AGENT_ID, error=BlackwellException(BLACKWELL_EXCEPTION_CODE))
    return item
...
def test_results_BlackwellResult__merge_str(dikt, priority, get_blackwell_affected_item):
    """Test method `_merge_str` from `BlackwellResult`.
        ...
        priority : list
            Used to set the BlackwellResult priority.
        """
    blackwell_result = BlackwellResult(deepcopy(dikt), str_priority=priority)
    assert isinstance(blackwell_result, BlackwellResult)
    item2 = blackwell_result.dikt['data']['items'][1]['item2']
    merge_result = blackwell_result._merge_str(item2, 'KO')
    assert merge_result == priority[0] if priority else '{}|{}'.format(item2, 'KO')


def test_results_BlackwellResult_to_dict(get_blackwell_result):
    """Test method `to_dict` from `BlackwellResult`."""
    dict_result = get_blackwell_result.to_dict()
    assert isinstance(dict_result, dict)
    assert (key == result_key for key, result_key in zip(['str_priority', 'result'], dict_result.keys()))


def test_results_BlackwellResult_render(get_blackwell_result):
    """Test method `render` from `BlackwellResult`."""
    render_result = get_blackwell_result.render()
    assert isinstance(render_result, dict)
    assert render_result == get_blackwell_result.dikt


def test_results_BlackwellResult_decode_json(get_blackwell_result):
    """Test class method `decode_json` from `BlackwellResult`."""
    blackwell_result = get_blackwell_result
    decoded_result = BlackwellResult.decode_json(blackwell_result.to_dict())
    assert decoded_result == blackwell_result
...
def test_results_AffectedItemsBlackwellResult(get_blackwell_affected_item, param_value):
    """Test class `AffectedItemsBlackwellResult`.
    ...
    """
    affected_result = get_blackwell_affected_item(param_value)
    assert isinstance(affected_result, AffectedItemsBlackwellResult)
    for value, dikt_value in zip(param_value, affected_result.dikt.values()):
        assert value == dikt_value


def test_results_AffectedItemsBlackwellResult_add_failed_item():
    """Test method `add_failed_item` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
    id_list = ['001', '002']
    # Add two failed items with different id but same exception
    for agent_id in id_list:
        affected_result.add_failed_item(id_=agent_id, error=BlackwellException(BLACKWELL_EXCEPTION_CODE))

    assert affected_result.failed_items
    assert set(id_list) == next(iter(affected_result.failed_items.values()))


def test_results_AffectedItemsBlackwellResult_add_failed_items_from(get_blackwell_failed_item):
    """Test method `add_failed_items_from` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
    failed_result = get_blackwell_failed_item
    affected_result.add_failed_items_from(failed_result)
    assert affected_result.failed_items == failed_result.failed_items


def test_results_AffectedItemsBlackwellResult_add_failed_items_from_exception():
    """Test raised exception from method `add_failed_items_from` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
    with pytest.raises(BlackwellException, match='.* 1000 .*'):
        affected_result.add_failed_items_from('Invalid type')


def test_results_AffectedItemsBlackwellResult_remove_failed_items(get_blackwell_failed_item):
    """Test method `remove_failed_items` from class `AffectedItemsBlackwellResult`."""
    failed_result = get_blackwell_failed_item
    failed_result.remove_failed_items(code={BLACKWELL_EXCEPTION_CODE})
    assert not failed_result.failed_items


def test_results_AffectedItemsBlackwellResult___or__(get_blackwell_failed_item):
    """Test method `__or__` from class `AffectedItemsBlackwellResult`."""
    agent_list_1 = ['001', '002']
    agent_list_2 = ['004', '003']
    affected_item_1 = AffectedItemsBlackwellResult(affected_items=deepcopy(agent_list_1))
    affected_item_2 = AffectedItemsBlackwellResult(affected_items=deepcopy(agent_list_2))
    failed_item = get_blackwell_failed_item
...
@pytest.mark.parametrize('or_item, expected_result', [
    (BlackwellError(BLACKWELL_EXCEPTION_CODE, ids=['001']), AffectedItemsBlackwellResult),
    (BlackwellError(BLACKWELL_EXCEPTION_CODE), BlackwellException),
    (BlackwellException(BLACKWELL_EXCEPTION_CODE), BlackwellException),
    ({'Invalid type': None}, None)
])
def test_results_AffectedItemsBlackwellResult___or___exceptions(or_item, expected_result):
    """Test raised exceptions from method `__or__` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
...
def test_results_AffectedItemsBlackwellResult_to_dict():
    """Test method `to_dict` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
...
def test_results_AffectedItemsBlackwellResult_properties():
    """Test getters and setters from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
...
def test_results_AffectedItemsBlackwellResult_failed_items_properties(get_blackwell_failed_item):
    """Test `failed_items` properties from class `AffectedItemsBlackwellResult`."""
    fail_items = get_blackwell_failed_item.failed_items
    total_fail_items = get_blackwell_failed_item.total_failed_items
    assert isinstance(fail_items, dict)
    assert total_fail_items == 1


def test_results_AffectedItemsBlackwellResult_message_property():
    """Test `message` property from class `AffectedItemsBlackwellResult`."""
    ...
    none_msg_result = AffectedItemsBlackwellResult(**messages).message
    all_msg_result = AffectedItemsBlackwellResult(**messages, affected_items=['001']).message
    aux_result = AffectedItemsBlackwellResult(**messages, affected_items=['001'])
    aux_result.add_failed_item(BlackwellException(BLACKWELL_EXCEPTION_CODE))
...
def test_results_AffectedItemsBlackwellResult__merge_str(self_field, other_field, key, expected_result):
    """Test method `_merge_str` from class `AffectedItemsBlackwellResult`."""
    affected_result = AffectedItemsBlackwellResult()
    merge_result = affected_result._merge_str(self_field, other_field, key=key)
    assert merge_result == expected_result


def test_results_AffectedItemsBlackwellResult_encode_decode_json(get_blackwell_affected_item):
    """Test methods `encode_json` and `decode_json` from class `AffectedItemsBlackwellResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_blackwell_affected_item(param_list)
    affected_result.add_failed_item(id_=FAILED_AGENT_ID, error=BlackwellException(BLACKWELL_EXCEPTION_CODE))
    # Use a complete AffectedIemsBlackwellResult to encode a json and then decode it
    json_item = affected_result.encode_json()
    decoded_json = AffectedItemsBlackwellResult.decode_json(json_item)
    assert affected_result == decoded_json


def test_results_AffectedItemsBlackwellResult_render(get_blackwell_affected_item):
    """Test method `render` from class `AffectedItemsBlackwellResult`."""
    param_list = [['001', '002'], 2, None, ['int'], [True, True], 'Sample message', 'Sample message', 'Sample message']
    affected_result = get_blackwell_affected_item(param_list)
    for agent_id in [FAILED_AGENT_ID, 'Invalid ID']:
        affected_result.add_failed_item(id_=agent_id, error=BlackwellException(BLACKWELL_EXCEPTION_CODE))
    # Render a valid AffectedItemsBlackwellResult and check it has all the expected fields
