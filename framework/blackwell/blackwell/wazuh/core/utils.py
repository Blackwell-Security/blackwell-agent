...
import blackwell.core.results as results
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.BLACKWELL_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.BLACKWELL_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a Blackwell framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise BlackwellError(1405, extra_message=str(limit))
        elif limit == 0:
            raise BlackwellError(1406)
    ...
    if offset < 0:
        raise BlackwellError(1400)
    elif limit < 1:
        raise BlackwellError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise BlackwellError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'blackwell' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise BlackwellError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise BlackwellError(1724, "{}".format(', '.join(select)))
...
def delete_blackwell_file(full_path: str) -> bool:
    """Delete a Blackwell file.
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.BLACKWELL_PATH) or '..' in full_path:
        raise BlackwellError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise BlackwellError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
...
def check_blackwell_limits_unchanged(new_conf, original_conf):
    """Check if Blackwell limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise BlackwellError(1127, extra_message=f"global > limits > {disabled_limit}")
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise BlackwellError(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise BlackwellError(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise BlackwellError(1130, extra_message='integrations > virustotal')


def load_blackwell_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise BlackwellError(1113, extra_message=str(e))
...
class BlackwellVersion:

    def __init__(self, version):

        pattern = r"(?:Blackwell )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise BlackwellError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise BlackwellError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class BlackwellDBBackend(AbstractDatabaseBackend):
    """
    This class describes a blackwell db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise BlackwellError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return BlackwellDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through BlackwellDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class BlackwellDBQuery(object):
    """This class describes a database query for blackwell."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """Blackwell DB Query constructor.
        ...
        # For example, the following regex -> (name!=blackwell;id>5),group=webserver <- would return 3 different matches:
        #   (name != blackwell ;
        #    id   > 5      ),
        #    group=webserver
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, BlackwellDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise BlackwellError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise BlackwellError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not set_select_fields or not set_select_fields.issubset(set_fields_keys):
                raise BlackwellError(1724, "Allowed select fields: {0}. Fields {1}". \
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise BlackwellError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'BlackwellDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'BlackwellDBQueryGroups':
            resource = 'name'
        else:
            raise BlackwellInternalError(1123)
...
class BlackwellDBQueryDistinct(BlackwellDBQuery):
    ...
    def _add_filters_to_query(self):
        BlackwellDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise BlackwellError(1410)

        BlackwellDBQuery._add_select_to_query(self)
...
def validate_blackwell_xml(content: str, config_file: bool = False):
    """Validate Blackwell XML files (rules, decoders and ossec.conf)
    ...
    try:
        ...
        if config_file:
            ...
            check_blackwell_limits_unchanged(final_xml, current_xml)
        # Check xml format
        load_blackwell_xml(xml_path='', data=final_xml)
    except ExpatError:
        raise BlackwellError(1113)
    except BlackwellError as e:
        raise e
    except Exception as e:
        raise BlackwellError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    Raises
    ------
    BlackwellInternalError(1005)
        Error reading file.
    BlackwellInternalError(1016)
        Error moving file.
    BlackwellError(1006)
    ...
    try:
        ...
        raise BlackwellInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.BLACKWELL_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.blackwell_uid(), common.blackwell_gid()), permissions=0o660)
    except PermissionError as exc:
        raise BlackwellError(1006) from exc
    except Error as exc:
        raise BlackwellInternalError(1016) from exc

    return results.BlackwellResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.BLACKWELL_PATH) -> str:
    """Return a relative path from the Blackwell base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.BLACKWELL_PATH`
