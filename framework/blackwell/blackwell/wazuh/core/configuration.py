...
from blackwell.core import common, blackwell_socket
from blackwell.core.exception import BlackwellError, BlackwellInternalError, BlackwellResourceNotFound
from blackwell.core.utils import cut_array, load_blackwell_xml, safe_move
...
DEFAULT_CTI_URL = 'https://cti.blackwell.com'
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_blackwell_xml' with their real values.
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise BlackwellError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    try:
        # Read XML
        xml_data = load_blackwell_xml(conf_file)

        # Parse XML to JSON
        data = _ossecconf2json(xml_data)
    except Exception as e:
        if not from_import:
            raise BlackwellError(1101, extra_message=str(e))
        else:
            print(f"blackwell-apid: There is an error in the ossec.conf file: {str(e)}")
            sys.exit(0)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise BlackwellError(1102, extra_message=e.args[0])
            else:
                raise BlackwellError(1106, extra_message=e.args[0])
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(os_path.join(common.SHARED_PATH, group_id)):
        raise BlackwellResourceNotFound(1710, group_id)
    ...
    if not os_path.exists(agent_conf):
        raise BlackwellError(1006, agent_conf)
    ...
    try:
        ...
        # Parse XML to JSON
            ...
            # Read XML
            xml_data = load_blackwell_xml(agent_conf)
...
def get_agent_conf_multigroup(multigroup_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if multigroup_id and not os_path.exists(os_path.join(common.MULTI_GROUPS_PATH, multigroup_id)) or not multigroup_id:
        raise BlackwellResourceNotFound(1710, extra_message=multigroup_id if multigroup_id else "No multigroup provided")
    ...
    if not os_path.exists(agent_conf):
        raise BlackwellError(1006, extra_message=os_path.join("BLACKWELL_PATH", "var", "multigroups", agent_conf))
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise BlackwellError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise BlackwellError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    ...
    Raises
    ------
    BlackwellInternalError(1107)
        Internal options file not found.
    BlackwellInternalError(1108)
    ...
    if not os_path.exists(common.INTERNAL_OPTIONS_CONF):
        raise BlackwellInternalError(1107)
    ...
    try:
        ...
        raise BlackwellInternalError(1108, e.args[0])
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    if not option.isdigit():
        raise BlackwellError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise BlackwellError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    BlackwellError(1113)
        XML syntax error.
    BlackwellError(1114)
        Blackwell syntax error.
    BlackwellError(1115)
        Error executing verify-agent-conf.
    BlackwellInternalError(1743)
        Error running Blackwell syntax validator.
    ...
    try:
        # check Blackwell xml format
        try:
            subprocess.check_output([os_path.join(common.BLACKWELL_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            if output_regex:
                raise BlackwellError(1114, ' '.join(output_regex))
            else:
                raise BlackwellError(1115, e.output.decode())
        except Exception as e:
            raise BlackwellInternalError(1743, str(e))

        # move temporary file to group folder
        try:
            new_conf_path = os_path.join(common.SHARED_PATH, group_id, "agent.conf")
            safe_move(tmp_file_path, new_conf_path, ownership=(common.blackwell_uid(), common.blackwell_gid()),
                      permissions=0o660)
        except Exception as e:
            raise BlackwellInternalError(1016, extra_message=str(e))
...
def get_active_configuration(agent_id: str, component: str, configuration: str) -> dict:
    ...
    BlackwellInternalError(1121)
        If the socket cant be created.
    BlackwellInternalError(1118)
    ...
    component_socket_mapping = {'agent': 'analysis', 'agentless': 'agentless', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'blackwell-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'agentless': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'blackwell-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise BlackwellError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise BlackwellError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.BLACKWELL_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise BlackwellError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = blackwell_socket.BlackwellSocket(dest_socket)
            except BlackwellInternalError:
                raise
            except Exception as unhandled_exc:
                raise BlackwellInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise BlackwellInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = blackwell_socket.create_blackwell_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = blackwell_socket.BlackwellSocketJSON(dest_socket)
            except BlackwellInternalError:
                raise
            except Exception as unhandled_exc:
                raise BlackwellInternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.BLACKWELL_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise BlackwellError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
                         extra_message=f'{component}:{configuration}')


def write_ossec_conf(new_conf: str):
    """Replace the current blackwell configuration (ossec.conf) with the provided configuration.
    ...
    try:
        ...
        raise BlackwellError(1126, extra_message=str(e))
