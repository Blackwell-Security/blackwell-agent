...
def create_pid(name: str, pid: int):
    ...
    Raises
    ------
    BlackwellInternalError(3002)
        Error creating pidfile.
    """
    filename = path.join(common.BLACKWELL_PATH, common.OS_PIDFILE_PATH, f'{name}-{pid}.pid')
    ...
    with open(filename, 'a') as fp:
        try:
            ...
            raise BlackwellInternalError(3002, str(e))
...
def delete_child_pids(name: str, ppid: int, logger: logging.Logger):
    ...
    logger : logging.Logger
        Logger object.
    """
    filenames = glob.glob(path.join(common.BLACKWELL_PATH, common.OS_PIDFILE_PATH, f'{name}*.pid'))
