...
from blackwell.core import common, utils
from blackwell.core import blackwell_socket
from blackwell.core.exception import BlackwellError, BlackwellInternalError, BlackwellException
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise BlackwellError(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise BlackwellInternalError(1309)
...
def get_daemons_stats_socket(socket: str, agents_list: Union[list[int], str] = None, last_id: int = None) -> dict:
    """Send message to Blackwell socket to get statistical information.
    ...
    """
    # Create message
    full_message = blackwell_socket.create_blackwell_socket_message(
    ...
    # Connect to socket
    try:
        s = blackwell_socket.BlackwellSocketJSON(socket)
    except Exception:
        raise BlackwellInternalError(1121, extra_message=socket)
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise BlackwellInternalError(1104, extra_message=str(e))
    except IOError:
        raise BlackwellError(1308, extra_message=filename)
...
def get_stats_socket_path(agent_id: Union[str, int], daemon: str) -> str:
    ...
    if is_agent_a_manager(agent_id):
        return os.path.join(common.BLACKWELL_PATH, "queue", "sockets", daemon)
    else:
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    except BlackwellException:
        # Error connecting to socket
        raise BlackwellInternalError(1121)
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise BlackwellError(1117, extra_message=rec_msg)
