...
import blackwell.core.exception as wexception
...
class AbstractBlackwellResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.BlackwellInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractBlackwellResult):
            self.dikt = dct.dikt
        else:
            raise wexception.BlackwellInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractBlackwellResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractBlackwellResult objects. When merged with a BlackwellException, the result is
        always a BlackwellException

        Parameters
        ----------
        other : AbstractBlackwellResult or wexception.BlackwellException

        Raises
        ------
        BlackwellInternalError(1000)
            Blackwell internal error when two object could not be merged.

        Returns
        -------
        AbstractBlackwellResult or wexception.BlackwellException
            Resultant object.
        """
        if isinstance(other, wexception.BlackwellException):
            return other
        elif not isinstance(other, (dict, AbstractBlackwellResult)):
            raise wexception.BlackwellInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractBlackwellResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in BlackwellResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in blackwell.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractBlackwellResult
            Filtered AbstractBlackwellResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in BlackwellResult subclasses.
        ...
        -------
        AbstractBlackwellResult
            Sorted AbstractBlackwellResult
        """
...
class BlackwellResult(AbstractBlackwellResult):
    """
...
class AffectedItemsBlackwellResult(AbstractBlackwellResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.BlackwellException = None):
        ...
        error : wexception.BlackwellException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsBlackwellResult
        ...
        wexception.BlackwellInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsBlackwellResult):
            raise wexception.BlackwellInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsBlackwellResult with self.

        Parameters
        ----------
        other : AffectedItemsBlackwellResult
            AffectedItemsBlackwellResult object to merge.
        ...
        if isinstance(other, wexception.BlackwellError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.BlackwellException):
            return result
        elif not isinstance(other, AffectedItemsBlackwellResult):
            raise wexception.BlackwellInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsBlackwellResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractBlackwellResult
            Instance of AbstractBlackwellResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsBlackwellResult object.

        Returns
        -------
        dict
            Return AffectedItemsBlackwellResult as a dictionary with fields data, message and error.
        """
