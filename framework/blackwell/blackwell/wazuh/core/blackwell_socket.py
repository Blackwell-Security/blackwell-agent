...
from blackwell import common
from blackwell.core.exception import BlackwellException, BlackwellInternalError
...
class BlackwellSocket:
    ...
    def _connect(self):
        try:
            ...
            raise BlackwellInternalError(1013, extra_message=os.path.basename(self.path))
        except ConnectionRefusedError:
            raise BlackwellInternalError(1121, extra_message=f"Socket '{os.path.basename(self.path)}' cannot receive "
                                                         "connections")
        except Exception as e:
            raise BlackwellException(1013, str(e))
    ...
    def send(self, msg_bytes, header_format="<I"):
        if not isinstance(msg_bytes, bytes):
            raise BlackwellException(1105, "Type must be bytes")

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise BlackwellException(1014, "Number of sent bytes is 0")
            return sent
        except Exception as e:
            raise BlackwellException(1014, str(e))
...
class BlackwellSocketJSON(BlackwellSocket):
    MAX_SIZE = 65536

    def __init__(self, path):
        BlackwellSocket.__init__(self, path)

    def send(self, msg, header_format="<I"):
        return BlackwellSocket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format="<I", header_size=4, raw=False):
        response = loads(BlackwellSocket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise BlackwellException(response['error'], response['message'], cmd_error=True)
            return response['data']
        else:
            return response


class BlackwellAsyncProtocol(asyncio.Protocol):
    """Blackwell implementation of asyncio.Protocol class."""
...
class BlackwellAsyncSocket:
    ...
    async def connect(self, path_to_socket: str):
        ...
        try:
            ...
            self.transport, self.protocol = await self.loop.create_connection(
                lambda: BlackwellAsyncProtocol(self.loop), sock=self.s)
    ...
    async def send(self, msg_bytes: bytes, header_format: str = None) -> bytes:
        ...
        Raises
        ------
        BlackwellException(1105)
            If the `msg_bytes` type is not bytes.
        BlackwellException(1014)
        ...
        if self.is_connection_lost():
            await self.close()
            raise BlackwellException(1014, "Socket connection was closed")
...
class BlackwellAsyncSocketJSON(BlackwellAsyncSocket):
    ...
    async def send(self, msg: str, header_format: str = None) -> bytes:
        ...
        bytes
            Bytes sent.
        """
        return await BlackwellAsyncSocket.send(self, dumps(msg).encode(), header_format)
    ...
    async def receive(self, header_size: int = None) -> dict:
        ...
        dict
            Data received.
        """
        response = await BlackwellAsyncSocket.receive(self, header_size)
...
daemons = {
    ...
    "blackwell-db": {"protocol": "TCP", "path": common.WDB_SOCKET, "header_format": "<I", "size": 4},
    "remoted": {"protocol": "TCP", "path": common.REMOTED_SOCKET, "header_format": "<I", "size": 4}
}


async def blackwell_sendasync(daemon_name: str, message: str = None) -> dict:
    ...
    try:
        ...
        raise BlackwellInternalError(1014, extra_message=e)

    return data


async def blackwell_sendsync(daemon_name: str = None, message: str = None) -> dict:
    """Send a message to the specified daemon's socket and wait for its response.
    ...
    """
    try:
        sock = BlackwellSocket(daemons[daemon_name]['path'])
        if isinstance(message, dict):
            message = dumps(message)
...
def create_blackwell_socket_message(origin=None, command=None, parameters=None):
