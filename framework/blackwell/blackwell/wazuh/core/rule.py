...
def load_rules_from_file(rule_filename: str, rule_relative_path: str, rule_status: str) -> list:
    ...
    try:
        rules = list()
        root = load_blackwell_xml(os.path.join(common.BLACKWELL_PATH, rule_relative_path, rule_filename))
        ...
        if e.errno == 2:
            raise BlackwellError(1201)
        elif e.errno == 13:
            raise BlackwellError(1207)
        else:
            raise e
...
def item_format(data, all_items, exclude_filenames):
    for item in glob(all_items):
        ...
        item_dir = os.path.relpath(os.path.dirname(item), start=common.BLACKWELL_PATH)
...
def _create_rule_decoder_dir_dict(ruleset_conf, tag, exclude_filenames, data):
    ...
    for item_dir in items:
        all_rules = f"{common.BLACKWELL_PATH}/{item_dir}/*.xml"
...
def _create_dict(ruleset_conf, tag, exclude_filenames, data):
    ...
    for item in items:
        ...
        item_dir = os.path.relpath(full_dir if full_dir else common.RULES_PATH, start=common.BLACKWELL_PATH)
