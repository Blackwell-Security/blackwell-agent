...
from blackwell.core.utils import find_nth, delete_blackwell_file, to_relative_path
    ...
    for name in dir_content:
        ...
        if (path.isfile(new_absolute_path)) and ('.cdb' not in name) and ('~' not in name) and not pattern.search(name):
            if only_names:
                ...
                items = get_list_from_file(path.join(common.BLACKWELL_PATH, new_relative_path))
...
def split_key_value_with_quotes(line: str, file_path: str = '/CDB_LISTS_PATH') -> tuple:
    ...
    if line.count('"') == 4:
        ...
        if first_quote != 0 or line[second_quote: third_quote + 1] != '":"' or fourth_quote != len(
                line) - 1:
            raise BlackwellError(1800, extra_message={'path': path.join('BLACKWELL_HOME', file_path)})
...
def get_list_from_file(path: str, raw: bool = False) -> Union[dict, str]:
    ...
    try:
        ...
        if e.errno == 2:
            raise BlackwellError(1802)
        elif e.errno == 13:
            raise BlackwellError(1803)
        elif e.errno == 21:
            raise BlackwellError(1804, extra_message="{0} {1}".format(path, "is a directory"))
        else:
            raise e
    except ValueError:
        raise BlackwellError(1800, extra_message={'path': path})
...
def create_list_file(full_path: str, content: str, permissions: int = 0o660) -> str:
    ...
    try:
        ...
        raise BlackwellError(1806)

    return full_path


def delete_list(rel_path: str):
    """Delete a Blackwell CDB list file.
    ...
    """
    delete_blackwell_file(path.join(common.BLACKWELL_PATH, rel_path))

    # Also delete .cdb file (if exists).
    try:
        remove(path.join(common.BLACKWELL_PATH, rel_path + common.COMPILED_LISTS_EXTENSION))
    except (IOError, OSError):
        pass


def get_filenames_paths(filenames_list: list, root_directory: str = common.USER_LISTS_PATH) -> list:
    """Get full paths from filename list. I.e: test_filename -> {blackwell_path}/etc/lists/test_filename
