...
from blackwell.core.utils import load_blackwell_xml, add_dynamic_detail
...
def load_decoders_from_file(decoder_file: str, decoder_path: str, decoder_status: str) -> list:
    ...
    try:
        decoders = list()
        position = 0
        root = load_blackwell_xml(os.path.join(common.BLACKWELL_PATH, decoder_path, decoder_file))
        ...
        raise BlackwellError(1502, extra_message=os.path.join('BLACKWELL_HOME', decoder_path, decoder_file))
    except Exception:
        raise BlackwellInternalError(1501, extra_message=os.path.join('BLACKWELL_HOME', decoder_path, decoder_file))
