...
class ARMessageBuilder:
    ...
    def choose_builder(cls, agent_version: str):
        ...
        raise BlackwellError(1000, "No suitable message builder found for agent version: {}".format(agent_version))
    ...
    def validate_command(self, command: str):
        ...
        """
        if not command:
            raise BlackwellError(1650)

        if not command.startswith('!'):
            commands = get_commands()
            if command not in commands:
                raise BlackwellError(1652)
...
class ARStrMessage(ARMessageBuilder):
    ...
    def can_handle(agent_version: str) -> bool:
        ...
        """
        return BlackwellVersion(agent_version) < BlackwellVersion(common.AR_LEGACY_VERSION)
...
class ARJsonMessage(ARMessageBuilder):
    ...
    def create_message(self, command: str = '', arguments: list = None, alert: dict = None) -> str:
        ...
        msg_queue = json.dumps(
            create_blackwell_socket_message(origin={'name': node_name, 'module': common.origin_module.get()},
                                        command=command,
...
def send_ar_message(agent_id: str = '', wq: BlackwellQueue = None, command: str = '', arguments: list = None,
    ...
    wq.send_msg_to_agent(msg=msg_queue, agent_id=agent_id, msg_type=BlackwellQueue.AR_TYPE)
