...
from blackwell.core.exception import BlackwellError, BlackwellResourceNotFound
from blackwell.core.results import AffectedItemsBlackwellResult, BlackwellResult
from blackwell.core.security import invalid_users_tokens, invalid_roles_tokens, invalid_run_as_tokens, revoke_tokens, \
    load_spec, sanitize_rbac_policy, update_security_conf, REQUIRED_FIELDS, SORT_FIELDS, SORT_FIELDS_GET_USERS
from blackwell.core.utils import process_array
from blackwell.rbac.decorators import expose_resources
from blackwell.rbac.orm import AuthenticationManager, PoliciesManager, RolesManager, RolesPoliciesManager
from blackwell.rbac.orm import SecurityError, MAX_ID_RESERVED
from blackwell.rbac.orm import UserRolesManager, RolesRulesManager, RulesManager

# Minimum eight characters, at least one uppercase letter, one lowercase letter, one number and one special character:
_user_password = re.compile(r'^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{8,}$')


def get_user_me(token: dict) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        User information.
    """
    result = AffectedItemsBlackwellResult(all_msg='Current user information was returned')
    affected_items = list()
...
def get_users(user_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
              ...
              q: str = None, distinct: bool = False) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        User information.
    """
    result = AffectedItemsBlackwellResult(none_msg='No user was returned',
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            affected_items.append(user) if user else result.add_failed_item(id_=user_id, error=BlackwellError(5001))
...
def edit_run_as(user_id: str = None, allow_run_as: bool = False) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Status message.
    """
    result = AffectedItemsBlackwellResult(none_msg=f"The parameter allow_run_as could not be "
    ...
    with AuthenticationManager() as auth:
        ...
        if query is False:
            result.add_failed_item(id_=user_id, error=BlackwellError(5001))
        elif query == SecurityError.INVALID:
            result.add_failed_item(id_=user_id, error=BlackwellError(5010))
        else:
...
def create_user(username: str = None, password: str = None) -> AffectedItemsBlackwellResult:
    """Create a new user.
    ...
    if len(password) > 64 or len(password) < 8:
        raise BlackwellError(5009)
    elif not _user_password.match(password):
        raise BlackwellError(5007)

    result = AffectedItemsBlackwellResult(none_msg='User could not be created',
    ...
    with AuthenticationManager() as auth:
        if auth.add_user(username, password):
            ...
            if operation:
                ...
                result.add_failed_item(id_=username, error=BlackwellError(5000))
        else:
            result.add_failed_item(id_=username, error=BlackwellError(5000))
...
def update_user(user_id: str = None, password: str = None, current_user: str = None) -> AffectedItemsBlackwellResult:
    ...
    if password:
        ...
        if int(user_id[0]) <= MAX_ID_RESERVED and current_user is not None:
            ...
            if current_user_id > MAX_ID_RESERVED:
                raise BlackwellError(5011)

    result = AffectedItemsBlackwellResult(all_msg='User was successfully updated',
                                      none_msg='User could not be updated')
    with AuthenticationManager() as auth:
        query = auth.update_user(int(user_id[0]), password)
        if query is False:
            result.add_failed_item(id_=int(user_id[0]), error=BlackwellError(5001))
        else:
...
def remove_users(user_ids: list) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Status message.
    """
    result = AffectedItemsBlackwellResult(none_msg='No user was deleted',
    ...
    with AuthenticationManager() as auth:
        for user_id in user_ids:
            ...
            if not isinstance(current_user, bool) and user_id == int(current_user['id']) and user_id > MAX_ID_RESERVED:
                result.add_failed_item(id_=user_id, error=BlackwellError(5008))
                continue
            user = auth.get_user_id(user_id)
            ...
            if not query:
                ...
                result.add_failed_item(id_=user_id, error=BlackwellError(5004))
            elif query == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=user_id, error=BlackwellError(4025))
            elif user:
                invalid_users_tokens(users=[user_id])
...
def get_roles(role_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    affected_items = list()
    result = AffectedItemsBlackwellResult(none_msg='No role was returned',
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role != SecurityError.ROLE_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(r_id), error=BlackwellError(4002))
...
def remove_roles(role_ids: list) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(none_msg='No role was deleted',
    ...
    with RolesManager() as rm:
        for r_id in role_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(r_id), error=BlackwellError(4008))
            elif role_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(r_id), error=BlackwellError(4025))
...
def add_role(name: str = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Result of the operation.
    """
    result = AffectedItemsBlackwellResult(none_msg='Role was not created',
                                      all_msg='Role was successfully created')
    with RolesManager() as rm:
        status = rm.add_role(name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=BlackwellError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=BlackwellError(4003))
        else:
...
def update_role(role_id: str = None, name: str = None) -> AffectedItemsBlackwellResult:
    ...
    if name is None:
        raise BlackwellError(4001)
    result = AffectedItemsBlackwellResult(none_msg='Role was not updated',
                                      all_msg='Role was successfully updated')
    with RolesManager() as rm:
        status = rm.update_role(role_id=role_id[0], name=name)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4003))
        elif status == SecurityError.ROLE_NOT_EXIST:
            result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4002))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4008))
        else:
...
def get_policies(policy_ids: list, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    result = AffectedItemsBlackwellResult(none_msg='No policy was returned',
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy != SecurityError.POLICY_NOT_EXIST:
                ...
                result.add_failed_item(id_=int(p_id), error=BlackwellError(4007))
...
def remove_policies(policy_ids: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Result of operation.
    """
    result = AffectedItemsBlackwellResult(none_msg='No policy was deleted',
    ...
    with PoliciesManager() as pm:
        for p_id in policy_ids:
            ...
            if policy_delete == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(p_id), error=BlackwellError(4008))
            elif policy_delete == SecurityError.RELATIONSHIP_ERROR:
                result.add_failed_item(id_=int(p_id), error=BlackwellError(4025))
...
def add_policy(name: str = None, policy: dict = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Result of the operation.
    """
    result = AffectedItemsBlackwellResult(none_msg='Policy was not created',
                                      all_msg='Policy was successfully created')
    sanitize_rbac_policy(policy)
    with PoliciesManager() as pm:
        status = pm.add_policy(name=name, policy=policy)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=name, error=BlackwellError(4009))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=name, error=BlackwellError(4006))
        else:
...
def update_policy(policy_id: str = None, name: str = None, policy: dict = None) -> AffectedItemsBlackwellResult:
    ...
    if name is None and policy is None:
        raise BlackwellError(4001)
    result = AffectedItemsBlackwellResult(none_msg='Policy was not updated',
    ...
    with PoliciesManager() as pm:
        ...
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=BlackwellError(4013))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(policy_id[0]), error=BlackwellError(4006))
        elif status == SecurityError.POLICY_NOT_EXIST:
            result.add_failed_item(id_=int(policy_id[0]), error=BlackwellError(4007))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(policy_id[0]), error=BlackwellError(4008))
        else:
...
def get_rules(rule_ids: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    """
    affected_items = list()
    result = AffectedItemsBlackwellResult(none_msg='No security rule was returned',
    ...
    with RulesManager() as rum:
        for ru_id in rule_ids:
            ...
            if rule != SecurityError.RULE_NOT_EXIST:
                ...
                result.add_failed_item(id_=ru_id, error=BlackwellError(4022))
...
@expose_resources(actions=['security:create'], resources=['*:*:*'])
def add_rule(name: str = None, rule: dict = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Result of the operation.
    """
    result = AffectedItemsBlackwellResult(none_msg='Security rule was not created',
...
def remove_rules(rule_ids: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Result of the operation.
    """
    result = AffectedItemsBlackwellResult(none_msg='No security rule was deleted',
    ...
    with RulesManager() as rum:
        for r_id in rule_ids:
            ...
            if role_delete == SecurityError.ADMIN_RESOURCES:
                ...
                result.add_failed_item(id_=int(r_id), error=BlackwellError(4022))
            elif rule:
...
def update_rule(rule_id: str = None, name: str = None, rule: dict = None) -> AffectedItemsBlackwellResult:
    ...
    if name is None and rule is None:
        raise BlackwellError(4001)
    result = AffectedItemsBlackwellResult(none_msg='Security rule was not updated',
                                      all_msg='Security rule was successfully updated')
    with RulesManager() as rum:
        status = rum.update_rule(rule_id=rule_id[0], name=name, rule=rule)
        if status == SecurityError.ALREADY_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=BlackwellError(4005))
        elif status == SecurityError.INVALID:
            result.add_failed_item(id_=int(rule_id[0]), error=BlackwellError(4003))
        elif status == SecurityError.RULE_NOT_EXIST:
            result.add_failed_item(id_=int(rule_id[0]), error=BlackwellError(4022))
        elif status == SecurityError.ADMIN_RESOURCES:
            result.add_failed_item(id_=int(rule_id[0]), error=BlackwellError(4008))
        else:
...
def get_username(user_id: str) -> AffectedItemsBlackwellResult:
...
def set_user_role(user_id: list, role_ids: list, position: int = None) -> AffectedItemsBlackwellResult:
    ...
    if position is not None and position < 0:
        raise BlackwellError(4018)

    username = get_username(user_id=user_id)
    result = AffectedItemsBlackwellResult(none_msg=f'No link was created to user {username}',
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(role_id), error=BlackwellError(4017))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=BlackwellError(4002))
            elif user_role == SecurityError.USER_NOT_EXIST:
                result.add_failed_item(id_=int(user_id[0]), error=BlackwellError(5001))
                break
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=BlackwellError(4008))
            else:
                success = True
                result.total_affected_items += 1
...
def remove_user_role(user_id: str, role_ids: list) -> AffectedItemsBlackwellResult:
    ...
    Raises
    ------
    BlackwellResourceNotFound(5001)
    ...
    username = get_username(user_id=user_id)
    if username == 'unknown':
        raise BlackwellResourceNotFound(5001)

    result = AffectedItemsBlackwellResult(none_msg=f'No role was unlinked from user {username}',
    ...
    with UserRolesManager() as urm:
        for role_id in role_ids:
            ...
            if user_role == SecurityError.INVALID:
                result.add_failed_item(id_=int(role_id), error=BlackwellError(4016))
            elif user_role == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id), error=BlackwellError(4002))
            elif user_role == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(user_id[0]), error=BlackwellError(4008))
...
def set_role_rule(role_id: str, rule_ids: list, run_as: bool = False) -> AffectedItemsBlackwellResult:
    ...
    result = AffectedItemsBlackwellResult(none_msg=f'No link was created to role {role_id[0]}',
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=int(rule_id), error=BlackwellError(4023))
            elif role_rule == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4002))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=int(rule_id), error=BlackwellError(4022))
...
def remove_role_rule(role_id: str, rule_ids: list) -> AffectedItemsBlackwellResult:
    ...
    Raises
    ------
    BlackwellResourceNotFound(4002)
    ...
    if not role:
        raise BlackwellResourceNotFound(4002)

    result = AffectedItemsBlackwellResult(none_msg=f'No security rule was unlinked from role {role_id[0]}',
    ...
    with RolesRulesManager() as rrm:
        for rule_id in rule_ids:
            ...
            if role_rule == SecurityError.INVALID:
                result.add_failed_item(id_=rule_id, error=BlackwellError(4024))
            elif role_rule == SecurityError.RULE_NOT_EXIST:
                result.add_failed_item(id_=rule_id, error=BlackwellError(4022))
            elif role_rule == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4008))
            else:
                success = True
...
def set_role_policy(role_id: str, policy_ids: list, position: int = None) -> AffectedItemsBlackwellResult:
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.ALREADY_EXIST:
                result.add_failed_item(id_=policy_id, error=BlackwellError(4011))
            elif role_policy == SecurityError.ROLE_NOT_EXIST:
                result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4002))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=BlackwellError(4007))
...
def remove_role_policy(role_id: str, policy_ids: list) -> AffectedItemsBlackwellResult:
    ...
    result = AffectedItemsBlackwellResult(none_msg=f'No policy was unlinked from role {role_id[0]}',
    ...
    with RolesPoliciesManager() as rpm:
        for policy_id in policy_ids:
            ...
            if role_policy == SecurityError.INVALID:
                result.add_failed_item(id_=policy_id, error=BlackwellError(4010))
            elif role_policy == SecurityError.POLICY_NOT_EXIST:
                result.add_failed_item(id_=policy_id, error=BlackwellError(4007))
            elif role_policy == SecurityError.ADMIN_RESOURCES:
                result.add_failed_item(id_=int(role_id[0]), error=BlackwellError(4008))
...
def revoke_current_user_tokens() -> BlackwellResult:
    ...
    return BlackwellResult({'message': f'User {common.current_user.get()} was successfully logged out'})
...
def wrapper_revoke_tokens() -> BlackwellResult:
    """Revoke all tokens.
    ...
    return BlackwellResult({'message': 'Tokens were successfully revoked'})
...
@lru_cache(maxsize=None)
def get_rbac_resources(resource: str = None) -> BlackwellResult:
    ...
    """
    if not resource:
        return BlackwellResult({'data': load_spec()['x-rbac-catalog']['resources']})
    else:
        if resource not in load_spec()['x-rbac-catalog']['resources'].keys():
            raise BlackwellError(4019)
        return BlackwellResult({'data': {resource: load_spec()['x-rbac-catalog']['resources'][resource]}})


@lru_cache(maxsize=None)
def get_rbac_actions(endpoint: str = None) -> BlackwellResult:
    ...
    if endpoint and endpoint not in endpoints_list:
        raise BlackwellError(4020, extra_remediation=endpoints_list)
    info_data = load_spec()
    data = dict()
    for path, path_info in info_data['paths'].items():
    ...
    return BlackwellResult({'data': data})


@expose_resources(actions=['security:read_config'], resources=['*:*:*'])
def get_security_config() -> BlackwellResult:
    ...
    """
    return BlackwellResult({'data': get_security_conf()})
