...
from blackwell.core.cluster.control import get_health, get_nodes, get_node_ruleset_integrity
from blackwell.core.cluster.utils import get_cluster_status, read_cluster_config, read_config
...
from blackwell.rbac.decorators import expose_resources, async_list_handler
...
def read_config_wrapper() -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected information was returned',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_node_wrapper() -> AffectedItemsBlackwellResult:
...
def get_status_json() -> BlackwellResult:
    """Return the cluster status.

    Returns
    -------
    BlackwellResult
        BlackwellResult object with the cluster status.
    """
    return BlackwellResult({'data': get_cluster_status()})
...
async def get_health_nodes(lc: local_client.LocalClient,
                           filter_node: Union[str, list] = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected nodes healthcheck information was returned',
...
async def get_nodes_info(lc: local_client.LocalClient, filter_node: Union[str, list] = None,
                         **kwargs: dict) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected nodes information was returned',
    ...
    for node in non_existent_nodes:
        result.add_failed_item(id_=node, error=BlackwellResourceNotFound(1730))
...
async def get_ruleset_sync_status(master_md5: dict = None):
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg="Nodes ruleset synchronization status was successfully read",
