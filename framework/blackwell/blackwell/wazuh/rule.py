...
import blackwell.core.configuration as configuration
from blackwell.core import common
from blackwell.core.cluster.cluster import get_node
from blackwell.core.cluster.utils import read_cluster_config
from blackwell.core.exception import BlackwellError
from blackwell.core.results import AffectedItemsBlackwellResult
from blackwell.core.rule import check_status, load_rules_from_file, format_rule_decoder_file, REQUIRED_FIELDS, \
    RULE_REQUIREMENTS, SORT_FIELDS, RULE_FIELDS, RULE_FILES_FIELDS, RULE_FILES_REQUIRED_FIELDS
from blackwell.core.utils import process_array, safe_move, \
    validate_blackwell_xml, upload_file, full_copy, to_relative_path
from blackwell.core.logtest import validate_dummy_logtest
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsBlackwellResult(none_msg='No rule was returned',
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=BlackwellError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
                    ...
                    distinct: bool = False) -> AffectedItemsBlackwellResult:
    ...
    ------
    BlackwellError(1200)
        Error reading rules from `BLACKWELL_HOME/etc/ossec.conf`.
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(none_msg='No rule files were returned',
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsBlackwellResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=BlackwellError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.BLACKWELL_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.BLACKWELL_PATH, rules[0]['relative_dirname'], filename))


def get_rule_file(filename: str = None, raw: bool = False,
                  relative_dirname: str = None) -> Union[str, AffectedItemsBlackwellResult]:
    ...
    Returns
    -------
    str or AffectedItemsBlackwellResult
        Content of the file. AffectedItemsBlackwellResult format if `raw=False`.
    """
    result = AffectedItemsBlackwellResult(none_msg='No rule was returned',
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=BlackwellError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=BlackwellError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=BlackwellError(1414, extra_message=f"{filename}"))

    return result


def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, BlackwellError]:
    ...
    Tuple (str, BlackwellError)
        ...
        The second element of the tuple is a BlackwellError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return BlackwellError(1505).
            If relative_dirname is inside the default RULES_PATH return BlackwellError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return BlackwellError(1507).
    ...
    blackwell_error = None
    if not relative_dirname in ruleset_conf['rule_dir']:
        blackwell_error = BlackwellError(1209)
    elif commonpath([join(common.BLACKWELL_PATH, relative_dirname), common.RULES_PATH]) == common.RULES_PATH:
        blackwell_error = BlackwellError(1210)
    elif not exists(join(common.BLACKWELL_PATH, relative_dirname)):
        blackwell_error = BlackwellError(1211)
    return relative_dirname, blackwell_error


@expose_resources(actions=['rules:update'], resources=['*:*:*'])
def upload_rule_file(filename: str, content: str, relative_dirname: str = None, 
                     overwrite: bool = False) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='Rule was successfully uploaded',
                                      none_msg='Could not upload rule')

    backup_file = ''
    try:
        relative_dirname, blackwell_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.BLACKWELL_PATH, relative_dirname, filename)
        if blackwell_error:
            raise blackwell_error
        if len(content) == 0:
            raise BlackwellError(1112)

        validate_blackwell_xml(content)

        # If file already exists and overwrite is False, raise exception
        if not overwrite and exists(full_path):
            raise BlackwellError(1905)
        elif overwrite and exists(full_path):
            backup_file = f'{full_path}.backup'
            try:
                full_copy(full_path, backup_file)
            except IOError as exc:
                raise BlackwellError(1019) from exc
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsBlackwellResult:
    """Delete a rule file.
    ...
    result = AffectedItemsBlackwellResult(all_msg='Rule was successfully deleted',
                                      none_msg='Could not delete rule')

    try:
        relative_dirname, blackwell_error = validate_upload_delete_dir(
            relative_dirname=relative_dirname)
        full_path = join(common.BLACKWELL_PATH, relative_dirname, file)
        if blackwell_error:
            raise blackwell_error
        ...
        if exists(full_path):
            try:
                ...
                raise BlackwellError(1907) from exc
        else:
