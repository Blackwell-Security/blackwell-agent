...
from blackwell.core import common, configuration
from blackwell.core.cluster.cluster import get_node
from blackwell.core.cluster.utils import manager_restart, read_cluster_config
from blackwell.core.configuration import get_ossec_conf, write_ossec_conf
from blackwell.core.exception import BlackwellError, BlackwellInternalError
from blackwell.core.manager import status, get_api_conf, get_update_information_template, get_ossec_logs, \
...
from blackwell.core.utils import process_array, safe_move, validate_blackwell_xml, full_copy
...
def get_status() -> AffectedItemsBlackwellResult:
    """Wrapper for status().
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Processes status was successfully read"
...
def ossec_log(level: str = None, tag: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
              ...
              q: str = '', select: str = None, distinct: bool = False) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Logs were successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def ossec_log_summary() -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Log was successfully summarized"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def get_api_config() -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(**_get_config_default_result_kwargs)
...
def restart() -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def validation() -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(**_validation_default_result_kwargs)
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_config(component: str = None, config: str = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Active configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def read_ossec_conf(section: str = None, field: str = None, raw: bool = False,
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_basic_info() -> AffectedItemsBlackwellResult:
    """Wrapper for Blackwell().to_dict
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Basic information was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read basic information in some nodes',
    ...
    try:
        result.affected_items.append(Blackwell().to_dict())
    except BlackwellError as e:
        result.add_failed_item(id_=node_id, error=e)
    result.total_affected_items = len(result.affected_items)
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:update_config"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def update_ossec_conf(new_conf: str = None) -> AffectedItemsBlackwellResult:
    """Replace blackwell configuration (ossec.conf) with the provided configuration.
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg=f"Configuration was successfully updated"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
    ...
    try:
        ...
        if not new_conf:
            raise BlackwellError(1125)

        # Check if the configuration is valid
        validate_blackwell_xml(new_conf, config_file=True)
...
def get_update_information(installation_uid: str, update_information: dict) -> BlackwellResult:
    """Process update information into a blackwell result.
    ...
    if not update_information:
        ...
        return BlackwellResult({'data': get_update_information_template(uuid=installation_uid, update_check=False)})
    ...
    if status_code != 200:
        ...
        raise BlackwellInternalError(2100, extra_message=extra_message)

    update_information.pop('message', None)

    return BlackwellResult({'data': update_information})
