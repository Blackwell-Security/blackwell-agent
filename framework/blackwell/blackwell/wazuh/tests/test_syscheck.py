...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.syscheck import run, clear, last_scan, files
        from blackwell.syscheck import AffectedItemsBlackwellResult
        from blackwell import BlackwellError, BlackwellInternalError
...
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_data_path, 'client.keys'))
@patch('blackwell.syscheck.BlackwellDBQueryAgents.__exit__')
@patch('blackwell.syscheck.BlackwellDBQueryAgents.__init__', return_value=None)
@patch('blackwell.syscheck.BlackwellQueue._connect')
@patch('blackwell.syscheck.BlackwellQueue.send_msg_to_agent', side_effect=set_callable_list)
@patch('blackwell.syscheck.BlackwellQueue.close')
def test_syscheck_run(close_mock, send_mock, connect_mock, agent_init_mock, agent_exit_mock,
    ...
    """
    with patch('blackwell.syscheck.BlackwellDBQueryAgents.run', return_value=failed_items[0]):
...
@patch('blackwell.core.wdb.BlackwellDBConnection.__init__', return_value=None)
@patch('blackwell.core.wdb.BlackwellDBConnection.execute', return_value=None)
@patch('blackwell.core.wdb.BlackwellDBConnection.close')
def test_syscheck_clear(wdb_close_mock, wdb_execute_mock, wdb_init_mock, agent_list, expected_result, agent_info_list,
    ...
    """
    with patch('blackwell.syscheck.get_agents_info', return_value=set(agent_info_list)), \
            patch('blackwell.syscheck.BlackwellDBQueryAgents') as mock_wdbqa:
...
@patch('blackwell.core.wdb.BlackwellDBConnection.__init__', return_value=None)
@patch('blackwell.core.wdb.BlackwellDBConnection.execute', side_effect=BlackwellError(1000))
@patch('blackwell.core.wdb.BlackwellDBConnection.close')
...
@pytest.mark.parametrize('agent_id, blackwell_version', [
    (['001'], {'version': 'Blackwell v3.6.0'}),
    (['002'], {'version': 'Blackwell v3.8.3'}),
    (['005'], {'version': 'Blackwell v3.5.3'}),
    (['006'], {'version': 'Blackwell v3.9.4'}),
    (['004'], {}),
])
...
@patch("blackwell.syscheck.BlackwellDBConnection.execute", return_value=[{'end': '', 'start': ''}])
@patch('socket.socket.connect')
def test_syscheck_last_scan(socket_mock, wdb_conn_mock, db_mock, exists_mock, agent_id, blackwell_version):
    ...
    agent_id : list
        Agent ID.
    blackwell_version : dict
        Dict with the Blackwell version to be applied.
    """
    with patch('blackwell.syscheck.Agent.get_basic_information', return_value=blackwell_version):
...
@patch('blackwell.core.utils.path.exists', return_value=True)
@patch('socket.socket.connect')
@patch('blackwell.core.common.WDB_PATH', new=test_data_path)
def test_syscheck_files(socket_mock, exists_mock, agent_id, select, filters, distinct, q):
    ...
    with patch('blackwell.core.utils.BlackwellDBConnection') as mock_wdb:
