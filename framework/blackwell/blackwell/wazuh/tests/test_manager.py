...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.manager import *
        from blackwell.core.manager import LoggingFormat
        from blackwell.core.tests.test_manager import get_logs
        from blackwell import BlackwellInternalError
...
def mock_blackwell_path():
    with patch('blackwell.core.common.BLACKWELL_PATH', new=test_data_path):
        yield
...
manager_status = {'blackwell-agentlessd': 'running', 'blackwell-analysisd': 'running', 'blackwell-authd': 'running',
 'blackwell-csyslogd': 'running', 'blackwell-dbd': 'running', 'blackwell-monitord': 'running',
 'blackwell-execd': 'running', 'blackwell-integratord': 'running', 'blackwell-logcollector': 'running',
 'blackwell-maild': 'running', 'blackwell-remoted': 'running', 'blackwell-reportd': 'running',
 'blackwell-syscheckd': 'running', 'blackwell-clusterd': 'running', 'blackwell-modulesd': 'running',
 'blackwell-db': 'running', 'blackwell-apid': 'running'}


@patch('blackwell.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works"""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsBlackwellResult), 'No expected result type'
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('blackwell-modulesd:database', None, 2, None, None),
    ('blackwell-modulesd:syscollector', None, 2, None, None),
    ('blackwell-modulesd:syscollector', None, 2, None, None),
    ('blackwell-modulesd:aws-s3', None, 5, None, None),
    ('blackwell-execd', None, 1, None, None),
    ('blackwell-csyslogd', None, 2, None, None),
...
@patch("blackwell.core.manager.get_blackwell_active_logging_format", return_value=LoggingFormat.plain)
@patch("blackwell.core.manager.exists", return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. blackwell-remoted).
    total_items : int
    ...
    """
    with patch('blackwell.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'blackwell-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'blackwell-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'blackwell-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'blackwell-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'blackwell-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'blackwell-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'blackwell-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0}
    }

    logs = get_logs().splitlines()
    with patch('blackwell.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('blackwell.core.cluster.utils.fcntl')
@patch('blackwell.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with pytest.raises(BlackwellInternalError, match='.* 1901 .*'):
        restart()

    # Socket error
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(BlackwellInternalError, match='.* 1902 .*'):
                restart()

        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                with pytest.raises(BlackwellInternalError, match='.* 1014 .*'):
                    restart()


@pytest.mark.parametrize('error_flag, error_msg', [
    (0, ""),
    (1, "2019/02/27 11:30:07 blackwell-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: "
        "Unspecified key"),
    (1, "2019/02/27 11:30:24 blackwell-authd: ERROR: (1230): Invalid element in the configuration: "
        "'use_source_i'.\n2019/02/27 11:30:24 blackwell-authd: ERROR: (1202): Configuration error at "
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('blackwell.core.manager.BlackwellSocket') as sock:
        # Mock sock response
...
@patch('blackwell.manager.validate_ossec_conf')
def test_validation_ko(mock_validate, exception):
    mock_validate.side_effect = exception

    if isinstance(exception, BlackwellInternalError):
        with pytest.raises(BlackwellInternalError, match='.* 1013 .*'):
            validation()
    else:
        result = validation()
...
@patch('blackwell.core.configuration.get_active_configuration')
...
@patch('blackwell.manager.validate_ossec_conf', return_value={'status': 'OK'})
@patch('blackwell.manager.write_ossec_conf')
@patch('blackwell.manager.validate_blackwell_xml')
@patch('blackwell.manager.full_copy')
@patch('blackwell.manager.exists', return_value=True)
@patch('blackwell.manager.remove')
@patch('blackwell.manager.safe_move')
