...
from blackwell.core.common import USER_RULES_PATH
import pytest
...
def mock_blackwell_paths():
    with patch('blackwell.core.common.RULES_PATH', new=os.path.join(parent_directory, core_data_path)):
        with patch('blackwell.core.common.USER_RULES_PATH', new=os.path.join(parent_directory, tests_data_path)):
            with patch('blackwell.core.common.BLACKWELL_PATH', new=parent_directory):
                with patch('blackwell.rule.to_relative_path', side_effect=lambda x: os.path.relpath(x, parent_directory)):
                    yield
...
@patch("blackwell.rule.configuration.get_ossec_conf", return_value=rule_ossec_conf)
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(BlackwellError, match='.* 1202 .*'):
            func(status=status)
    else:
...
@patch('blackwell.core.configuration.get_ossec_conf', return_value=rule_ossec_conf)
...
@patch('blackwell.core.configuration.get_ossec_conf', return_value=other_rule_ossec_conf)
...
def test_failed_get_rules():
    ...
    with pytest.raises(BlackwellError, match=".* 1203 .*"):
...
def test_get_rule_file_path(filename, relative_dirname, result, mock_blackwell_paths):
    """Test get_rule_file_path function."""
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        res = rule.get_rule_file_path(filename=filename, 
                                            relative_dirname=relative_dirname)
        assert res == os.path.join(blackwell.core.common.BLACKWELL_PATH, result) if result else not res
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsBlackwellResult)
...
@patch('blackwell.rule.delete_rule_file')
@patch('blackwell.rule.full_copy')
@patch('blackwell.rule.upload_file')
@patch('blackwell.rule.remove')
@patch('blackwell.rule.safe_move')
@patch('blackwell.rule.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_xml, mock_full_copy,
    ...
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('blackwell.rule.validate_upload_delete_dir', return_value=ret_validation):
            with patch('blackwell.rule.exists', return_value=overwrite):
                ...
                if overwrite:
                    full_path = os.path.join(blackwell.common.BLACKWELL_PATH, rule_path)
...
@patch('blackwell.rule.delete_rule_file', side_effect=BlackwellError(1019))
@patch('blackwell.rule.upload_file')
@patch('blackwell.rule.safe_move')
...
def test_delete_rule_file(file, relative_dirname):
    ...
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        with patch('blackwell.rule.exists', return_value=True):
            with patch('blackwell.rule.remove'):
...
def test_delete_rule_file_ko():
    ...
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('blackwell.rule.remove', side_effect=IOError()):
