...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.ciscat import get_ciscat_results
...
@patch('blackwell.ciscat.get_agents_info', return_value=['001'])
...
def test_get_ciscat_results_select_ko(agents_info_mock, socket_mock, exists_mock):
    ...
    with patch('blackwell.core.utils.BlackwellDBConnection') as mock_wdb:
        mock_wdb.return_value = InitWDBSocketMock(sql_schema_file=db_file)
        with pytest.raises(BlackwellError, match=r'\b1724\b'):
