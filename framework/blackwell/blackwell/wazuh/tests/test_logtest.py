...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        from blackwell.logtest import run_logtest, end_logtest_session


def send_logtest_msg_mock(**kwargs):
    socket_response = create_blackwell_socket_message(command=kwargs['command'], parameters=kwargs['parameters'])
...
def test_get_logtest_output(logtest_param_values):
    ...
    with patch('blackwell.logtest.send_logtest_msg') as send_mock:
