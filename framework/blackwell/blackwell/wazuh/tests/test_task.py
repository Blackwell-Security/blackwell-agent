...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        from blackwell.tests.util import get_fake_database_data, RBAC_bypasser, InitWDBSocketMock

        blackwell.rbac.decorators.expose_resources = RBAC_bypasser
        from blackwell import task, BlackwellError
        from blackwell.core.task import BlackwellDBQueryTask
...
@patch('blackwell.core.utils.BlackwellDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_tasks_test.sql'))
@patch.object(BlackwellDBQueryTask, '_final_query', fake_final_query)
