...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from blackwell.core.agent import Agent
        ...
        from blackwell.core.tests.test_agent import InitAgent
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'Blackwell v3.9.0', 'count': 1}, {'version': 'Blackwell v3.8.2', 'count': 2},
             {'version': 'Blackwell v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsBlackwellResult), 'The returned object is not an "AffectedItemsBlackwellResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsBlackwellResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, BlackwellResult), 'The returned object is not an "BlackwellResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsBlackwellResult), 'The returned object is not an "BlackwellResult" instance.'
...
@patch('blackwell.core.agent.Agent.reconnect')
@patch('blackwell.agent.get_agents_info', return_value=short_agent_list)
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsBlackwellResult), 'The returned object is not an "AffectedItemsBlackwellResult".'
...
@patch('blackwell.agent.send_restart_command')
@patch('blackwell.agent.get_agents_info', return_value=set(short_agent_list))
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('blackwell.agent.get_agents')
@patch('blackwell.agent.get_groups')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~blackwell,status~active)', 'group=default;((name~blackwell,status~active))'),
    ('default', 'name~blackwell,status~active', 'group=default;(name~blackwell,status~active)')
])
@patch('blackwell.agent.get_agents')
@patch('blackwell.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list[1:], {'status': 'all', 'older_than': '1s', 'version': 'Blackwell v3.6.2'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, BlackwellError(1726), None),
])
@patch('blackwell.agent.Agent.remove')
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, BlackwellException):
...
@patch('blackwell.core.agent.get_manager_status', return_value={'blackwell-authd': 'running'})
...
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('blackwell.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('blackwell.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, BlackwellResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, BlackwellError(1006)),
    (test_shared_path, ['default'], True, BlackwellError(1405), BlackwellError(1405)),
    (test_shared_path, ['default'], True, BlackwellException(1400), BlackwellInternalError(1727))
])
@patch('blackwell.agent.process_array')
@patch('blackwell.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('blackwell.core.common.SHARED_PATH', new=shared_path):
        ...
        except (BlackwellError, BlackwellInternalError) as e:
...
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
@patch('blackwell.core.common.blackwell_gid', return_value=getgrnam('root'))
@patch('blackwell.core.common.blackwell_uid', return_value=getpwnam('root'))
@patch('blackwell.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, BlackwellResult), 'The returned object is not an "BlackwellResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', BlackwellError, 1711),
    ('group-1', BlackwellError, 1711),
    ('invalid!', BlackwellError, 1722),
    ('delete-me', BlackwellInternalError, 1005),
    ('ar.conf', BlackwellError, 1713),
    ('agent-template.conf', BlackwellError, 1713)
])
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the Blackwell Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('blackwell.agent.get_groups')
@patch('blackwell.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [BlackwellResourceNotFound(1710)]),
    (['default'], [BlackwellError(1712)]),
    (['none-1', 'none-2'], [BlackwellResourceNotFound(1710)]),
    (['default', 'none-1'], [BlackwellError(1712), BlackwellResourceNotFound(1710)]),
])
@patch('blackwell.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of BlackwellError
        List of expected BlackwellError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('blackwell.agent.Agent.add_group_to_agent')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], BlackwellResourceNotFound(1710), True),
    (['group-1'], ['100'], BlackwellResourceNotFound(1701), False),
    (['default'], ['000'], BlackwellError(1703), False)
])
@patch('blackwell.agent.Agent.group_exists')
@patch('blackwell.agent.Agent.add_group_to_agent')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    catch_exception : bool
        ...
        `AffectedItemsBlackwellResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except BlackwellException as ex:
...
@patch('blackwell.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('blackwell.core.agent.Agent.unset_single_group_agent')
@patch('blackwell.agent.get_groups')
@patch('blackwell.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', BlackwellResourceNotFound(1701)),
    ('any-group', '000', BlackwellError(1703)),
    ('group-1', '005', BlackwellResourceNotFound(1710)),
])
@patch('blackwell.agent.get_agents_info', return_value=short_agent_list)
@patch('blackwell.agent.get_groups', side_effect={'default'})
def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : BlackwellError
        The BlackwellError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (BlackwellError, BlackwellResourceNotFound) as error:
...
@patch('blackwell.core.agent.Agent.unset_single_group_agent')
@patch('blackwell.agent.get_agents_info', return_value=short_agent_list)
@patch('blackwell.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], BlackwellResourceNotFound(1701), True),
    (['any-group'], ['000'], BlackwellError(1703), True),
    (['any-group'], ['005'], BlackwellResourceNotFound(1710), False),
])
@patch('blackwell.core.agent.Agent.unset_single_group_agent')
@patch('blackwell.agent.get_agents_info', return_value=short_agent_list)
@patch('blackwell.agent.get_groups', return_value={'group-1'})
def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            f'An "BlackwellError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsBlackwellResult), 'The returned object is not an "AffectedItemsBlackwellResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            f'No exception should be raised at this point. An AffectedItemsBlackwellResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], BlackwellResourceNotFound(1710), True),
    (['group-1'], ['000'], BlackwellError(1703), False),
    (['group-1'], ['100'], BlackwellResourceNotFound(1701), False),
])
@patch('blackwell.agent.get_agents_info', return_value=short_agent_list)
@patch('blackwell.agent.get_groups', return_value={'group-1'})
...
def test_agent_get_outdated_agents(socket_mock, send_mock):
    ...
    # Check typing
    assert isinstance(result, AffectedItemsBlackwellResult)
    assert isinstance(result.affected_items, list)
    # Check affected items
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating BlackwellInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('blackwell.agent.get_agents_info', return_value=set(full_agent_list))
@patch('blackwell.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('blackwell.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a Blackwell Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(BlackwellError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(BlackwellInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('blackwell.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a Blackwell Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(BlackwellInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, blackwell_socket_mock, agent_list, component, configuration):
    ...
    """
    blackwell_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    try:
        ...
        assert error == BlackwellError(1740)
...
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
@patch('blackwell.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('blackwell.agent.get_agents_info', return_value=full_agent_list)
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['000'], BlackwellError(1703)),
    (['100'], BlackwellResourceNotFound(1701))
])
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : BlackwellError
        Expected BlackwellError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsBlackwellResult), 'The returned object is not an "AffectedItemsBlackwellResult".'
    # Check failed items
...
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
@patch('blackwell.core.configuration.upload_group_configuration')
...
@patch('blackwell.core.common.SHARED_PATH', new=test_shared_path)
@patch('blackwell.agent.get_distinct_agents')
@patch('blackwell.agent.get_agent_groups')
@patch('blackwell.agent.get_agents_summary_status')
@patch('blackwell.agent.get_agents')
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('blackwell.agent.get_agents_info', return_value=['test', 'test2'])
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('blackwell.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('blackwell.core.agent.Agent.get_agent_groups')
@patch('blackwell.core.agent.Agent.set_agent_group_file')
@patch('blackwell.core.agent.Agent')
...
@patch('blackwell.core.agent.Agent.group_exists', return_value=False)
...
def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected BlackwellException code error.
    """
    with pytest.raises(BlackwellException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = BlackwellResult({'message': 'User has permission to uninstall agents'})
