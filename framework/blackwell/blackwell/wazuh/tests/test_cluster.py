...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        from blackwell import cluster
        from blackwell.core import common
        from blackwell.core.exception import BlackwellError, BlackwellResourceNotFound
        from blackwell.core.cluster.local_client import LocalClient
        from blackwell.core.results import BlackwellResult

default_config = {'disabled': True, 'node_type': 'master', 'name': 'blackwell', 'node_name': 'node01',
                  'key': '', 'port': 1516, 'bind_addr': '0.0.0.0', 'nodes': ['NODE_IP'], 'hidden': 'no'}


@patch('blackwell.cluster.read_config', return_value=default_config)
...
@patch('blackwell.cluster.read_config', side_effect=BlackwellError(1001))
def test_read_config_wrapper_exception(mock_read_config):
    """Verify the exceptions raised in read_config_wrapper."""
    result = cluster.read_config_wrapper()
    assert list(result.failed_items.keys())[0] == BlackwellError(1001)
...
@patch('blackwell.cluster.get_node', side_effect=BlackwellError(1001))
...
def test_get_status_json():
    ...
    expected = BlackwellResult({'data': {"enabled": "no" if default_config['disabled'] else "yes", "running": "no"}})
    assert result == expected


@pytest.mark.asyncio
@patch('blackwell.core.cluster.local_client.LocalClient.start', side_effect=None)
async def test_get_health_nodes(mock_unix_connection):
    ...
    with patch('blackwell.cluster.get_health', side_effect=async_mock):
...
async def test_get_nodes_info():
    ...
    with patch('blackwell.cluster.get_nodes', side_effect=valid_node):
    ...
    assert result.failed_items[BlackwellResourceNotFound(1730)] == {'noexists'}
...
@patch("blackwell.cluster.node_id", new="testing_node")
@pytest.mark.asyncio
async def test_get_ruleset_sync_status(ruleset_integrity):
    ...
    with patch("blackwell.cluster.get_node_ruleset_integrity",
...
async def test_get_ruleset_sync_status_ko():
    ...
    exc = BlackwellError(1000)
    with patch("blackwell.cluster.get_node_ruleset_integrity", side_effect=exc):
