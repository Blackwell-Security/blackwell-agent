...
import pytest
from blackwell.core.common import USER_DECODERS_PATH


with patch('blackwell.core.common.getgrnam'):
    with patch('blackwell.core.common.getpwnam'):
...
def mock_blackwell_paths():
    """Mock blackwell paths."""
    with patch('blackwell.core.common.BLACKWELL_PATH', new=test_data_path):
        with patch('blackwell.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('blackwell.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('blackwell.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('blackwell.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, blackwell.core.common.BLACKWELL_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'blackwell', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'blackwell', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'blackwell', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'blackwell', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, BlackwellInternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('blackwell.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
        ...
        except BlackwellInternalError as exc:
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_blackwell_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename, 
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(blackwell.core.common.BLACKWELL_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_blackwell_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsBlackwellResult
        assert isinstance(result, AffectedItemsBlackwellResult)
...
@patch('blackwell.decoder.delete_decoder_file')
@patch('blackwell.decoder.full_copy')
@patch('blackwell.decoder.validate_blackwell_xml')
@patch('blackwell.decoder.upload_file')
@patch('blackwell.decoder.remove')
@patch('blackwell.decoder.safe_move')
@patch('blackwell.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_blackwell_paths,
    ...
    with patch('blackwell.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('blackwell.decoder.exists', return_value=overwrite):
            with patch('blackwell.decoder.to_relative_path',
                    side_effect=lambda x: os.path.relpath(x, blackwell.core.common.BLACKWELL_PATH)):
            ...
            if overwrite:
                full_path = os.path.join(blackwell.common.BLACKWELL_PATH, decoder_path)
...
@patch('blackwell.decoder.delete_decoder_file', side_effect=BlackwellError(1019))
@patch('blackwell.decoder.upload_file')
@patch('blackwell.decoder.safe_move')
...
def test_upload_file_ko(*_):
    ...
    with patch('blackwell.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('blackwell.decoder.exists'):
    ...
    # clean backup files
    search_pattern = os.path.join(blackwell.core.common.BLACKWELL_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('blackwell.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsBlackwellResult when everything is correct
        with patch('blackwell.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('blackwell.decoder.exists', return_value=True):
        with patch('blackwell.decoder.remove', side_effect=IOError()):
