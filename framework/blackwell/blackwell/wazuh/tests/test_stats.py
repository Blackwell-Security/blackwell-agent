...
with patch('blackwell.core.common.blackwell_uid'):
    with patch('blackwell.core.common.blackwell_gid'):
        ...
        import blackwell.stats as stats
...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'blackwell-remoted',
                               '/var/ossec/queue/sockets/analysis': 'blackwell-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'blackwell-remoted': '/var/ossec/queue/sockets/remote',
                               'blackwell-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('blackwell.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsBlackwellResult), 'The result is not BlackwellResult type'
...
@patch('blackwell.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('blackwell.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('blackwell.core.common.WDB_SOCKET', '/var/ossec/queue/db/wdb')
@patch('blackwell.stats.get_daemons_stats_socket')
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['blackwell-remoted', 'blackwell-analysisd', 'blackwell-db'])
    ...
    assert isinstance(response, AffectedItemsBlackwellResult), 'The result is not AffectedItemsBlackwellResult type'
    assert response.total_affected_items == len(response.affected_items)


@patch('blackwell.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/wrong_socket_name')
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['blackwell-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['blackwell-remoted', 'blackwell-analysisd']),
    (['blackwell-remoted'], ['blackwell-remoted']),
    (['blackwell-remoted', 'blackwell-analysisd'], ['blackwell-remoted', 'blackwell-analysisd'])
])
@patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('blackwell.stats.get_agents_info', return_value={'000', '001', '002', '003', '004', '005'})
@patch('blackwell.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('blackwell.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('blackwell.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, mock_socket_connect,
    ...
    assert isinstance(result, AffectedItemsBlackwellResult), 'The result is not an AffectedItemsBlackwellResult object'
...
@patch('blackwell.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('blackwell.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('blackwell.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('blackwell.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('blackwell.core.agent.Agent.get_stats')
@patch('blackwell.stats.get_agents_info', return_value=['000', '001'])
