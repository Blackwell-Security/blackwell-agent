...
with patch('blackwell.common.blackwell_uid'):
    with patch('blackwell.common.blackwell_gid'):
        ...
        from blackwell import rootcheck
        from blackwell.core.rootcheck import BlackwellDBQueryRootcheck
        from blackwell.core.tests.test_rootcheck import InitRootcheck, send_msg_to_wdb, remove_db, \
...
@patch('blackwell.rootcheck.BlackwellDBQueryAgents.__exit__')
@patch('blackwell.rootcheck.BlackwellDBQueryAgents.__init__', return_value=None)
...
def test_rootcheck_run(close_mock, send_mock, connect_mock, agent_init__mock, agent_exit__mock,
    ...
    agent_list : list
        List of agent IDs.
    failed_items : list
        List with the BlackwellDBQueryAgents response.
    status_list : list
        List of agent statuses.
    expected_result : list
        List of dicts with expected results for every test.
    """
    with patch('blackwell.rootcheck.BlackwellDBQueryAgents.run', return_value=failed_items[0]):
        ...
        assert isinstance(result, rootcheck.AffectedItemsBlackwellResult)
...
@pytest.mark.parametrize('agent_list, expected_affected_items, expected_calls, wdb_side_effect', [
    (['002'], [], [call('agent 002 rootcheck delete')], BlackwellError(2004)),
    (['000', '001', '003'], ['000', '001'], [call('agent 000 rootcheck delete'), call('agent 001 rootcheck delete')],
     [None, None])
])
@patch('blackwell.rootcheck.get_agents_info', return_value={'000', '001', '002'})
...
def test_clear(mock_connect, mock_info, agent_list, expected_affected_items, expected_calls, wdb_side_effect):
    ...
    expected_calls : list
        List of expected calls to the mocked BlackwellDBConnection._send function.
    wdb_side_effect : BlackwellError or list
        Side effect used in the mocked BlackwellDBConnection._send function.
    """
    with patch('blackwell.core.wdb.BlackwellDBConnection._send', side_effect=wdb_side_effect) as mock_wdbconn:
...
def test_get_rootcheck_agent(mock_connect, mock_send, mock_info, mock_exists, limit):
    ...
    for item in result['affected_items']:
        for key in item.keys():
            assert key in BlackwellDBQueryRootcheck.fields
...
def test_get_rootcheck_agent_select(mock_connect, mock_send, mock_info, mock_exists, select):
    ...
    result = rootcheck.get_rootcheck_agent(agent_list=['001'], select=select, filters={'status': 'all'}).render()[
        'data']
    select = select if select else list(BlackwellDBQueryRootcheck.fields.keys())
