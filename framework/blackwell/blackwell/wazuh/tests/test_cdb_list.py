...
from blackwell.core.exception import BlackwellInternalError
...
with patch('blackwell.core.common.getgrnam'):
    with patch('blackwell.core.common.getpwnam'):
        ...
        from blackwell.cdb_list import get_lists, get_path_lists, iterate_lists, get_list_file, upload_list_file,\
            delete_list_file
        from blackwell.core import common
        from blackwell.core.results import AffectedItemsBlackwellResult

RELATIVE_PATH = os.path.join("framework", "blackwell", "tests", "data", "test_cdb_list")
NAME_FILE_1 = "test_lists_1"
NAME_FILE_2 = "test_lists_2"
NAME_FILES = [NAME_FILE_1, NAME_FILE_2]

RESULT_GET_LIST_FILE_1 = [{'items': [{'key': 'test-blackwell-w', 'value': 'write'},
                                     {'key': 'test-blackwell-r', 'value': 'read'},
                                     {'key': 'test-blackwell-a', 'value': 'attribute'},
                                     {'key': 'test-blackwell-x', 'value': 'execute'},
                                     {'key': 'test-blackwell-c', 'value': 'command'}
                                     ],
...
@patch('blackwell.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
@pytest.mark.parametrize("search_text, complementary_search, search_in_fields, paths, expected_result", [
    ...
    ("test-blackwell-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-ossec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-blackwell-w", False, "items", [NAME_FILE_2], []),
    ...
    ("test-blackwell-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-ossec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-blackwell-w", True, "items", [NAME_FILE_2], RESULT_GET_LIST_FILE_2),
])
@patch('blackwell.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsBlackwellResult)
    assert isinstance(result_b, AffectedItemsBlackwellResult)
...
@patch('blackwell.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
@patch('blackwell.cdb_list.iterate_lists', side_effect=lists_path_mock)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    """
    with patch('blackwell.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        result = get_list_file([filename], raw)
        if raw:
            assert result == expected_result
        else:
            isinstance(result, AffectedItemsBlackwellResult)
...
@patch('blackwell.cdb_list.safe_move')
@patch('blackwell.cdb_list.delete_file_with_backup')
@patch('blackwell.cdb_list.upload_file')
@patch('blackwell.cdb_list.delete_list_file')
@patch('blackwell.cdb_list.remove')
@patch('blackwell.cdb_list.exists', return_value=True)
...
@patch('blackwell.cdb_list.common.USER_LISTS_PATH', return_value='/test/path')
@patch('blackwell.cdb_list.remove')
def test_upload_list_file_ko(mock_remove, mock_lists_path):
    ...
    with patch('blackwell.cdb_list.safe_move') as mock_safe_move:
        with patch('blackwell.cdb_list.exists', return_value=True):
            ...
            with patch('blackwell.cdb_list.get_filenames_paths', return_value=['/test']):
        ...
        with patch('builtins.open'):
            with patch('blackwell.cdb_list.exists', return_value=False):
                with patch('blackwell.core.configuration.tempfile.mkstemp', return_value=['mock_handle', 'mock_tmp_file']):
                    with pytest.raises(BlackwellInternalError, match=r'\b1005\b'):
                        upload_list_file(filename='test', content='test:content', overwrite=False)


@patch('blackwell.core.cdb_list.delete_blackwell_file')
def test_delete_list_file(mock_delete_file):
    ...
    try:
        ...
        with patch('blackwell.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH):
            result = delete_list_file(['test_file'])
            assert result.render()['data']['affected_items'][0] ==\
                   'framework/blackwell/tests/data/test_cdb_list/test_file'
    finally:
        try:
            os.remove(test_file)
        except Exception:
