...
from blackwell.core.exception import BlackwellError, BlackwellPermissionError
from blackwell.core.results import BlackwellResult
from blackwell.rbac.auth_context import RBAChecker, get_policies_from_roles
from blackwell.rbac.orm import AuthenticationManager
        ...
        for action in policy['actions']:
            ...
            for resource in policy['resources']:
                if not re.match(resource_regex, resource):
                    raise BlackwellError(4500)
                resource_type = PreProcessor.is_combination(resource)
                if len(resource_type[1]) > 2:
                    raise BlackwellError(4500, extra_remediation="The maximum length for permission combinations is two")
...
def get_permissions(user_id: int = None, auth_context: Union[dict, str] = None) -> BlackwellResult:
