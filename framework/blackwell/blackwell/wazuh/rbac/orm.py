...
from blackwell.core.common import blackwell_uid, blackwell_gid, DEFAULT_RBAC_RESOURCES
from blackwell.core.utils import get_utc_now, safe_move
from blackwell.rbac.utils import clear_cache

logger = logging.getLogger("blackwell-api")

# Max reserved ID value
BLACKWELL_USER_ID = 1
BLACKWELL_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (BLACKWELL_USER_ID, BLACKWELL_WUI_USER_ID):
        ...
        if from_id == BLACKWELL_USER_ID and to_id == BLACKWELL_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set Blackwell ownership and permissions.
        ...
        """
        chown(database, blackwell_uid(), blackwell_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=BLACKWELL_USER_ID,
                                        to_id=BLACKWELL_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(blackwell_uid(), blackwell_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current Blackwell RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
