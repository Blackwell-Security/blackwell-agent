...
from blackwell.rbac.tests.utils import init_db
from blackwell.rbac.orm import BLACKWELL_USER_ID, BLACKWELL_WUI_USER_ID, MAX_ID_RESERVED, User
...
def db_setup():
    with patch('blackwell.core.common.blackwell_uid'), patch('blackwell.core.common.blackwell_gid'):
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import blackwell.rbac.orm as rbac
                    # Clear mappers
...
def fresh_in_memory_db():
    ...
    with patch('blackwell.core.common.blackwell_uid'), patch('blackwell.core.common.blackwell_gid'):
        import blackwell.rbac.orm as orm
        reload(orm)
...
def test_database_init(db_setup):
    ...
    with db_setup.RolesManager() as rm:
        assert rm.get_role('blackwell') != db_setup.SecurityError.ROLE_NOT_EXIST
...
def test_delete_all_expired_rules(db_setup):
    """Check that rules are correctly deleted"""
    with patch('blackwell.rbac.orm.time', return_value=0):
        add_token(db_setup)
    with db_setup.TokenManager() as tm:
...
@patch("blackwell.rbac.orm.create_engine")
@patch("blackwell.rbac.orm.sessionmaker")
...
@patch("blackwell.rbac.orm._Base.metadata.create_all")
...
def test_databasemanager_get_table(fresh_in_memory_db):
    ...
    with patch("blackwell.rbac.orm._new_columns", new={"new_column"}):
...
@patch("blackwell.rbac.orm.safe_move")
@patch("blackwell.rbac.orm.os.remove")
@patch("blackwell.rbac.orm.chown")
@patch("blackwell.rbac.orm.os.chmod")
def test_check_database_integrity(chmod_mock, chown_mock, remove_mock, safe_move_mock, fresh_in_memory_db):
    ...
    db_mock = MagicMock()
    with patch("blackwell.rbac.orm.db_manager", new=db_mock):
        with patch("blackwell.rbac.orm.os.path.exists", return_value=True):
            with patch("blackwell.rbac.orm.CURRENT_ORM_VERSION", new=99999):
                ...
                db_mock.assert_has_calls([
                    ...
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
                                      from_id=fresh_in_memory_db.BLACKWELL_USER_ID,
                                      to_id=fresh_in_memory_db.BLACKWELL_WUI_USER_ID),
                    call.migrate_data(source=fresh_in_memory_db.DB_FILE, target=fresh_in_memory_db.DB_FILE_TMP,
        ...
        safe_move_mock.assert_called_once_with(fresh_in_memory_db.DB_FILE_TMP, fresh_in_memory_db.DB_FILE,
                                               ownership=(fresh_in_memory_db.blackwell_uid(),
                                                          fresh_in_memory_db.blackwell_gid()),
                                               permissions=0o640)
...
@patch("blackwell.rbac.orm.DatabaseManager.close_sessions")
@patch("blackwell.rbac.orm.os.remove")
def test_check_database_integrity_exceptions(remove_mock, close_sessions_mock, exception, fresh_in_memory_db):
    ...
    with patch("blackwell.rbac.orm.os.path.exists", side_effect=mocked_exists) as mock_exists:
        with patch("blackwell.rbac.orm.DatabaseManager.connect", side_effect=exception) as db_manager_mock:
...
@pytest.mark.parametrize('from_id, to_id, users', [
    (BLACKWELL_USER_ID, BLACKWELL_WUI_USER_ID, [
        User('blackwell', 'test', user_id=BLACKWELL_USER_ID),
        User('blackwell-wui', 'test2', user_id=BLACKWELL_WUI_USER_ID)
    ]),
...
def test_migrate_data(db_setup, from_id, to_id, users):
    ...
    if to_id == BLACKWELL_WUI_USER_ID:
        with patch("blackwell.rbac.orm.db_manager.get_data", return_value=users):
            with patch("blackwell.rbac.orm.AuthenticationManager.update_user") as mock_update_user:
        ...
        empty_list = []
        user_exists = False
        with patch("blackwell.rbac.orm.AuthenticationManager.add_user",
                   side_effect=[not user_exists, user_exists, not user_exists]) as mock_add_user:
            with patch("blackwell.rbac.orm.db_manager.get_data",
