        ...
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    from blackwell.rbac.auth_context import RBAChecker
...
def test_auth_roles(db_setup):
    ...
    for index, auth in enumerate(authorization_contexts):
        for role in roles:
            with patch('blackwell.rbac.orm.RolesManager.get_role_id') as _role_rules:
                with patch('blackwell.rbac.orm.RulesManager.get_rule') as _rule:
