        ...
        with patch('sqlalchemy.create_engine', return_value=create_engine("sqlite://")):
            with patch('shutil.chown'), patch('os.chmod'):
                with patch('api.constants.SECURITY_PATH', new=test_data_path):
                    import blackwell.rbac.decorators as decorator
...
def test_expose_resources(db_setup, decorator_params, function_params, rbac, fake_system_resources, allowed_resources,
    ...
    with patch('blackwell.rbac.decorators._expand_resource', side_effect=mock_expand_resource):
        ...
        try:
            ...
            assert output == function_params.get('call_func', True) or isinstance(output, AffectedItemsBlackwellResult)
