...
from blackwell.core.agent import get_agents_info, get_groups, expand_group
from blackwell.core.common import rbac, broadcast, cluster_nodes
from blackwell.core.exception import BlackwellPermissionError
from blackwell.core.results import AffectedItemsBlackwellResult
from blackwell.core.utils import expand_rules, expand_lists, expand_decoders
from blackwell.rbac.orm import RolesManager, PoliciesManager, AuthenticationManager, RulesManager
...
def list_handler(result: AffectedItemsBlackwellResult, original: dict = None, allowed: dict = None, target: dict = None,
                 add_denied: bool = False, **post_proc_kwargs: dict) -> AffectedItemsBlackwellResult:
        ...
        for res_id, target_param in target.items():
            ...
            for denied_item in denied:
                result.add_failed_item(id_=denied_item,
                                       error=BlackwellPermissionError(4000, extra_message=f'Resource type: {res_id}',
                                                                  ids=denied))
    if not add_denied or post_proc_kwargs.get('force'):
        ...
        if 'default_result_kwargs' in post_proc_kwargs and result is None:
            return AffectedItemsBlackwellResult(**post_proc_kwargs['default_result_kwargs'])
...
def expose_resources(actions: list = None, resources: list = None, post_proc_func: callable = list_handler,
    ...
    """Decorator to apply user permissions on a Blackwell framework function based on exposed action:resource pairs.
            ...
            for res_id, target_param in target_params.items():
                try:
                    ...
                    if add_denied:
                        ...
                        if res_id in integer_resources:
                            denied = {int(i) if i.isdigit() else i for i in denied}
                        raise BlackwellPermissionError(4000,
