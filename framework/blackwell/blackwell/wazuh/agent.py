...
from blackwell.core.agent import BlackwellDBQueryAgents, BlackwellDBQueryGroupByAgents, BlackwellDBQueryMultigroups, Agent, \
    BlackwellDBQueryGroup, create_upgrade_tasks, get_agents_info, get_groups, get_rbac_filters, send_restart_command, \
...
from blackwell.core.exception import BlackwellError, BlackwellInternalError, BlackwellException, BlackwellResourceNotFound
from blackwell.core.results import BlackwellResult, AffectedItemsBlackwellResult
from blackwell.core.utils import chmod_r, chown_r, get_hash, mkdir_with_mode, md5, process_array, clear_temporary_caches, \
    full_copy
...
# The rest of codes generated by the socket will be raised as Blackwell internal errors (500)
...
def get_distinct_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, fields: list = None, q: str = None) -> AffectedItemsBlackwellResult:
    ...
    result = AffectedItemsBlackwellResult(all_msg='All selected agents information was returned',
    ...
    if agent_list:
        ...
        with BlackwellDBQueryGroupByAgents(filter_fields=fields, select=fields, offset=offset, limit=limit, sort=sort,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_status(agent_list: list[str] = None) -> BlackwellResult:
    ...
    if agent_list:
        ...
        with BlackwellDBQueryAgents(limit=None, select=['status', 'group_config_status'], query="id!=000",
    ...
    return BlackwellResult({'data': {'connection': connection, 'configuration': sync_configuration}})


@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agents_summary_os(agent_list: list[str] = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(none_msg='Could not get the operative system of the agents',
    ...
    if agent_list:
        ...
        # We don't consider agent 000 in order to get the summary
        with BlackwellDBQueryAgents(select=['os.platform'], default_sort_field='os_platform', min_select_fields=set(),
...
def reconnect_agents(agent_list: Union[list, str] = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='Force reconnect command was sent to all agents',
...
@expose_resources(actions=["agent:restart"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707]})
def restart_agents(agent_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='Restart command was sent to all agents',
    ...
    try:
        ...
        result.add_failed_item('000', BlackwellError(1703))
    except KeyError:
        pass
    ...
    if agent_list:
        ...
        with BlackwellDBQueryAgents(limit=None, select=["id", "status", "version"], **rbac_filters) as query_data:
        ...
        [result.add_failed_item(id_=agent['id'], error=BlackwellError(1707))
...
def restart_agents_by_node(agent_list: list = None) -> AffectedItemsBlackwellResult:
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701, 1703, 1707], 'force': True})
def restart_agents_by_group(agent_list: list = None) -> AffectedItemsBlackwellResult:
...
def get_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
    ...
    if agent_list:
        ...
        for agent_id in agent_list:
            if agent_id not in system_agents:
                result.add_failed_item(id_=agent_id, error=BlackwellResourceNotFound(1701))

        rbac_filters = get_rbac_filters(system_resources=system_agents, permitted_resources=agent_list, filters=filters)

        with BlackwellDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"],
                  post_proc_kwargs={'exclude_codes': [1701]})
def get_agents_keys(agent_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='Obtained keys for all selected agents',
...
def delete_agents(agent_list: list = None, purge: bool = False, filters: dict = None,
                  q: str = None) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected agents were deleted',
    ...
    if agent_list:
        ...
        with BlackwellDBQueryAgents(limit=None, select=["id"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        list(map(lambda ag: result.add_failed_item(id_=ag, error=BlackwellError(
            1731,
...
def add_agent(name: str = None, agent_id: str = None, key: str = None, ip: str = 'any',
              force: dict = None) -> BlackwellResult:
    ...
    if len(name) > common.AGENT_NAME_LEN_LIMIT:
        raise BlackwellError(1738)

    new_agent = Agent(name=name, ip=ip, id=agent_id, key=key, force=force)

    return BlackwellResult({'data': {'id': new_agent.id, 'key': new_agent.key}})
...
def get_agent_groups(group_list: list = None, offset: int = 0, limit: int = None, sort_by: list = None,
    ...
    result = AffectedItemsBlackwellResult(all_msg='All selected groups information was returned',
    ...
    if group_list:
        ...
        for invalid_group in set(group_list) - system_groups:
            result.add_failed_item(id_=invalid_group, error=BlackwellResourceNotFound(1710))

        rbac_filters = get_rbac_filters(system_resources=system_groups, permitted_resources=group_list)

        with BlackwellDBQueryGroup(**rbac_filters, limit=None) as group_query:
...
def get_group_files(group_list: list = None, offset: int = 0, limit: int = None, search_text: str = None,
                    ...
                    select: str = None, distinct: bool = False) -> BlackwellResult:
    ...
    -------
    BlackwellResult
        BlackwellResult object with the groups files.
    """
    ...
    result = AffectedItemsBlackwellResult(all_msg='All selected groups files were returned',
                                      some_msg='Some groups files were not returned',
                                      none_msg='No groups files were returned'
                                      )
    if group_id:
        if not Agent.group_exists(group_id):
            result.add_failed_item(id_=group_id, error=BlackwellResourceNotFound(1710))
            return result
        group_path = path.join(common.SHARED_PATH, group_id)

    if not path.exists(group_path):
        result.add_failed_item(id_=group_path, error=BlackwellError(1006))
        return result
    ...
    try:
        ...
        raise BlackwellInternalError(1727, extra_message=str(e))

    return result


@expose_resources(actions=["group:create"], resources=["*:*:*"], post_proc_func=None)
def create_group(group_id: str) -> BlackwellResult:
    """Creates a group.
    ...
    -------
    BlackwellResult
        BlackwellResult object with a operation message.
    """
    ...
    if group_id.lower() == "default" or path.exists(group_path):
        if not path.isfile(group_path):
            raise BlackwellError(1711, extra_message=group_id)
        else:
            raise BlackwellError(1713, extra_message=group_id)
    ...
    try:
        ...
        chown_r(group_path, common.blackwell_uid(), common.blackwell_gid())
        chmod_r(group_path, 0o660)
        chmod(group_path, 0o700)
        msg = f"Group '{group_id}' created."
    except Exception as e:
        raise BlackwellInternalError(1005, extra_message=str(e))

    return BlackwellResult({'message': msg})


@expose_resources(actions=["group:delete"], resources=["group:id:{group_list}"],
                  post_proc_kwargs={'exclude_codes': [1710, 1712]})
def delete_groups(group_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Affected items.
    """
    result = AffectedItemsBlackwellResult(all_msg='All selected groups were deleted',
...
def assign_agents_to_group(group_list: list = None, agent_list: list = None, replace: bool = False,
                           replace_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsBlackwellResult(all_msg=f'All selected agents were assigned to {group_id}'
    ...
    if '000' in agent_list:
        agent_list.remove('000')
        result.add_failed_item(id_='000', error=BlackwellError(1703))
...
def remove_agent_from_group(group_list: list = None, agent_list: list = None) -> BlackwellResult:
    ...
    return BlackwellResult({'message': Agent.unset_single_group_agent(agent_id=agent_id, group_id=group_id, force=True)})
...
def remove_agent_from_groups(agent_list: list = None, group_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    """
    agent_id = agent_list[0]
    result = AffectedItemsBlackwellResult(all_msg='Specified agent was removed from returned groups',
...
def remove_agents_from_group(agent_list: list = None, group_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    """
    group_id = group_list[0]
    result = AffectedItemsBlackwellResult(all_msg=f'All selected agents were removed from group {group_id}',
...
def get_outdated_agents(agent_list: list = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None,
                        search: dict = None, select: str = None, q: str = None) -> AffectedItemsBlackwellResult:
...
def upgrade_agents(agent_list: list = None, wpk_repo: str = None, version: str = None, force: bool = False,
                   ...
                   filters: dict = None, q: str = None) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='All upgrade tasks were created',
    ...
    if agent_list:
        ...
        with BlackwellDBQueryAgents(limit=None, select=["id", "status"], query=q, **rbac_filters) as db_query:
            data = db_query.run()
        ...
        [result.add_failed_item(id_=agent, error=BlackwellError(1707)) for agent in non_active_agents]
        non_active_agents = set(non_active_agents)

        # Add non eligible agents to failed_items
        non_eligible_agents = agent_list - not_found_agents - non_active_agents - filtered_agents
        [result.add_failed_item(id_=ag, error=BlackwellError(
            1731,
            extra_message="some of the requirements are not met -> {}".format(
                ', '.join(f"{key}: {value}" for key, value in filters.items() if key != 'rbac_ids') +
                (f', q: {q}' if q else '')
        ...
        for agent_result_chunk in tasks_results:
            for agent_result in agent_result_chunk['data']:
                ...
                # Upgrade error for specific agents
                    ...
                    error = BlackwellError(error_code, cmd_error=True, extra_message=agent_result['message'])
                ...
                # Upgrade error for all agents, bad request
                    ...
                    raise BlackwellError(error_code, cmd_error=True, extra_message=agent_result['message'])

                # Upgrade error for all agents, internal server error
                else:
                    raise BlackwellInternalError(error_code, cmd_error=True, extra_message=agent_result['message'])
...
def get_upgrade_result(agent_list: list = None, filters: dict = None, q: str = None) -> AffectedItemsBlackwellResult:
    ...
    AffectedItemsBlackwellResult
        Upgrade results.
    """
    result = AffectedItemsBlackwellResult(all_msg='All upgrade tasks were returned',
        ...
        for task_result_chunk in task_results:
            for task_result in task_result_chunk['data']:
                ...
                # Upgrade error for specific agents (no task in DB)
                    ...
                    error = BlackwellError(error_code, cmd_error=True, extra_message=task_result['message'])
                    result.add_failed_item(id_=str(task_result['agent']).zfill(3), error=error)

                # Upgrade error for all agents, internal server error
                else:
                    raise BlackwellInternalError(error_code, cmd_error=True, extra_message=task_result['message'])
...
@expose_resources(actions=["agent:read"], resources=["agent:id:{agent_list}"], post_proc_func=None)
def get_agent_config(agent_list: list = None, component: str = None, config: str = None) -> BlackwellResult:
...
def get_agents_sync_group(agent_list: list = None) -> AffectedItemsBlackwellResult:
    ...
    """
    result = AffectedItemsBlackwellResult(all_msg='Sync info was returned for all selected agents',
...
def get_file_conf(group_list: list = None, type_conf: str = None, raw: bool = False,
                  filename: str = None) -> BlackwellResult:
    ...
    -------
    BlackwellResult
        BlackwellResult object with the configuration.
    """
    ...
    return BlackwellResult({'data': configuration.get_file_conf(filename, group_id=group_id, type_conf=type_conf,
                                                            raw=raw)})


@expose_resources(actions=["group:read"], resources=["group:id:{group_list}"], post_proc_func=None)
def get_agent_conf(group_list: list = None, filename: str = 'agent.conf', offset: int = 0,
                   limit: int = common.DATABASE_LIMIT) -> BlackwellResult:
...
def upload_group_file(group_list: list = None, file_data: str = None, file_name: str = 'agent.conf') -> BlackwellResult:
    """Update a group file.
    ...
    -------
    BlackwellResult
        BlackwellResult object with the confirmation message.
    """
    # We access unique group_id from list, this may change if and when we decide to add option to update files for
    # a list of groups
    group_id = group_list[0]

    return BlackwellResult({'message': configuration.upload_group_file(group_id, file_data, file_name=file_name)})


def get_full_overview() -> BlackwellResult:
    """Get information about agents.

    Returns
    -------
    BlackwellResult
        BlackwellResult object with information about agents.
    """
    # We don't consider agent 000 in order to get the summary
    q = "id!=000"
    ...
    return BlackwellResult({'data': result})


@expose_resources(actions=["agent:uninstall"], resources=["*:*:*"], post_proc_func=None)
def check_uninstall_permission() -> BlackwellResult:
    ...
    -------
    BlackwellResult
        BlackwellResult object containing a message indicating if the
        user has the permission to uninstall agents.
    """
    return BlackwellResult({'message': 'User has permission to uninstall agents'})
