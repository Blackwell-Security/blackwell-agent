...
# Modified by Santiago Bassett (http://www.blackwell.com) - Feb 2016
...
UPGRADE_FILE_FLAG="${DIR}/BLACKWELL_PKG_UPGRADE"
...
if [ -z "${upgrade}" ]; then
    echo "Generating Blackwell configuration for a fresh installation."
    ...
    if [ -f "${INSTALLATION_SCRIPTS_DIR}/gen_ossec.sh" ]; then
        ...
        chown root:blackwell ${DIR}/etc/ossec.conf
...
if [[ $(dscl . -read /Groups/ossec) ]]; then
    echo "Changing group from Ossec to Blackwell"
    find ${DIR}/ -group ossec -user root -exec chown root:blackwell {} \ > /dev/null 2>&1 || true
    if [[ $(dscl . -read /Users/ossec) ]]; then
        echo "Changing user from Ossec to Blackwell"
        find ${DIR}/ -group ossec -user ossec -exec chown blackwell:blackwell {} \ > /dev/null 2>&1 || true
...
if [ -n "${upgrade}" ] && [ -n "${restart}" ]; then
    echo "Restarting Blackwell..."
    ${DIR}/bin/blackwell-control restart
fi
