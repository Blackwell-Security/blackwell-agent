#!/bin/bash
set -e
# Program to build and package OSX blackwell-agent
...
SERVICE_PATH="/Library/LaunchDaemons/com.blackwell.agent.plist"
STARTUP_PATH="/Library/StartupItems/BLACKWELL/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/BLACKWELL/Blackwell-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/BLACKWELL/BLACKWELL"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $BLACKWELL_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in Blackwell's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${BLACKWELL_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${BLACKWELL_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${BLACKWELL_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/blackwell-agent/payload

    if [ -d "$CURRENT_PATH/blackwell-agent" ]; then

        echo "\nThe blackwell agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/blackwell-agent
    fi

    munkipkg --create --json $CURRENT_PATH/blackwell-agent

    cp -f $build_info_file $CURRENT_PATH/blackwell-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/blackwell-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/blackwell-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/blackwell-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/blackwell-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/blackwell-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        BLACKWELL_PATH="${SOURCES_DIRECTORY}/blackwell"
        git clone --depth=1 -b ${BRANCH_TAG} ${BLACKWELL_SOURCE_REPOSITORY} "${BLACKWELL_PATH}"
    else
        BLACKWELL_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${BLACKWELL_PATH}" && git rev-parse --short HEAD)"

    export CONFIG="${BLACKWELL_PATH}/etc/preloaded-vars.conf"
    BLACKWELL_PACKAGES_PATH="${BLACKWELL_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${BLACKWELL_PACKAGES_PATH}/entitlements.plist"

    VERSION=$(cat ${BLACKWELL_PATH}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="blackwell-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="blackwell-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe blackwell agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${BLACKWELL_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${BLACKWELL_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/blackwell-agent ; then
        echo "The blackwell agent package for macOS has been successfully built."
        mv $CURRENT_PATH/blackwell-agent/build/* $DESTINATION/
        symbols_pkg_name="${pkg_name}_debug_symbols"
        cp -R "${BLACKWELL_PATH}/src/symbols"  "${DESTINATION}"
