...
if [ -d ${install_path}/logs/ossec ]; then
  if [ -z "$(ls -A ${install_path}/logs/ossec)" ]; then
    ...
    rm -rf ${install_path}/logs/blackwell
    mv ${install_path}/logs/ossec ${install_path}/logs/blackwell
  fi
fi
...
if grep "^ossec:" /etc/group > /dev/null 2>&1; then
  find ${install_path}/ -group ossec -user root -exec chown root:blackwell {} \; > /dev/null 2>&1 || true
  if grep "^ossec" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossec -exec chown blackwell:blackwell {} \; > /dev/null 2>&1 || true
    userdel ossec
  fi
  if grep "^ossecm" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossecm -exec chown blackwell:blackwell {} \; > /dev/null 2>&1 || true
    userdel ossecm
  fi
  if grep "^ossecr" /etc/passwd > /dev/null 2>&1; then
    find ${install_path}/ -group ossec -user ossecr -exec chown blackwell:blackwell {} \; > /dev/null 2>&1 || true
    userdel ossecr
  fi
  groupdel ossec
fi
