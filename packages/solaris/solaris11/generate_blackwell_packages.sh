...
# Blackwell Solaris 11 Package builder.

REPOSITORY="https://github.com/wazuh/wazuh"
blackwell_branch="master"
...
set_control_binary() {
    if [ -e ${SOURCE}/src/VERSION ]; then
        blackwell_version=`cat ${SOURCE}/src/VERSION`
        number_version=`echo "${blackwell_version}" | cut -d v -f 2`
...
build_environment() {
    ...
    curl -LO http://packages-dev.blackwell.com/deps/solaris/precompiled-solaris-gcc-5.5.0.tar.gz
    ...
    curl -LO http://packages-dev.blackwell.com/deps/solaris/precompiled-solaris-cmake-3.18.3.tar.gz
...
compute_version_revision() {
    blackwell_version=$(cat ${SOURCE}/src/VERSION | cut -d "-" -f1 | cut -c 2-)
    revision="$(cat ${SOURCE}/src/REVISION)"

    echo $blackwell_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${blackwell_branch}" != "trunk" ]] || [[ "${blackwell_branch}" != "master" ]]; then
        cd $SOURCE
        git checkout $blackwell_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
#Compile and install blackwell-agent
compile() {
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/blackwell-agent.mog-template > ${current_path}/blackwell-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/blackwell-agent.mog-template > ${current_path}/blackwell-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/blackwell-agent.mog-aux > ${current_path}/blackwell-agent.mog

    echo "Building the package blackwell-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > blackwell-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p blackwell-agent.p5m.1 # Fix p5m.1 file
    mv blackwell-agent.p5m.1.aux.fixed blackwell-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  blackwell-agent.p5m.1 > blackwell-agent.p5m.1.aux_sed
        mv blackwell-agent.p5m.1.aux_sed blackwell-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> blackwell-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> blackwell-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> blackwell-agent.p5m.1
    echo "file blackwell-agent path=etc/init.d/blackwell-agent owner=root group=sys mode=0744" >> blackwell-agent.p5m.1
    echo "file S97blackwell-agent path=etc/rc2.d/S97blackwell-agent owner=root group=sys mode=0744" >> blackwell-agent.p5m.1
    echo "file S97blackwell-agent path=etc/rc3.d/S97blackwell-agent owner=root group=sys mode=0744" >> blackwell-agent.p5m.1

    # Add user and group blackwell
    echo "group groupname=blackwell" >> blackwell-agent.p5m.1
    echo "user username=blackwell group=blackwell" >> blackwell-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> blackwell-agent.p5m.1
    echo "user username=ossec group=ossec" >> blackwell-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` blackwell-agent.p5m.1 blackwell-agent.mog | pkgfmt > blackwell-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} blackwell-agent.p5m.2 > pack
    package=`cat pack | grep blackwell | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="blackwell-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/blackwell
    zfs set mountpoint=/blackwell rpool/blackwell
    pkgrepo create /blackwell
    ls /blackwell
    pkgrepo -s /blackwell set publisher/prefix=blackwell
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/blackwell
...
uninstall() {
    ...
    rm -f `find /etc | grep blackwell`
    rm -f /etc/rc3.d/S97blackwell-agent
    rm -f /etc/rc2.d/S97blackwell-agent
}
...
clean() {
    ...
    umount /blackwell
    zfs destroy rpool/blackwell
    rm -rf /blackwell
    rm -rf $SOURCE/blackwell
    rm -f blackwell-agent.p5m*
    rm -f blackwell-agent.mog
    rm -f blackwell-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${blackwell_branch}."
    echo -e ""
    echo -e "        -c, --checksum"
