...
# Blackwell Solaris 10 Package builder.


# CONFIGURATION VARIABLES
blackwell_branch="$(echo "$2" | cut -d "/" -f2)"
...
clone(){
    cd ${CURRENT_PATH}
    git clone --depth=1 $REPOSITORY -b $blackwell_branch ${SOURCE} || return 1
    cd $SOURCE
    compute_version_revision
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "blackwell-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "blackwell-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "blackwell-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "blackwell-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "blackwell-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "blackwell-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "blackwell-agent_$VERSION.proto"
    echo "f none /etc/init.d/blackwell-agent  0755 root root" >> "blackwell-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97blackwell-agent=/etc/init.d/blackwell-agent" >> "blackwell-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97blackwell-agent=/etc/init.d/blackwell-agent" >> "blackwell-agent_$VERSION.proto"
    cat "blackwell-agent_$VERSION.list" | pkgproto >> "blackwell-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "blackwell-agent_$VERSION.proto"
    pkg_name="blackwell-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" blackwell-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf blackwell-agent blackwell *.list *proto
    rm -f *.new
