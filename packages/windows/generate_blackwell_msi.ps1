...
param (
    [string]$MSI_NAME = "blackwell-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows Blackwell agent msi package.

    PARAMETERS TO BUILD BLACKWELL-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * BLACKWELL:
          $ ./generate_blackwell_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_blackwell_msi.ps1 -MSI_NAME blackwell-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_blackwell_msi.ps1 -MSI_NAME blackwell-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildBlackwellMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        $filesToSign = @(
            ...
            "..\blackwell_modules\syscollector\build\bin\syscollector.dll",
    ...
    & $CANDLE_EXE -nologo .\blackwell-installer.wxs -out "blackwell-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\blackwell-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
	...
	$exeFiles +=  Get-ChildItem -Filter "blackwell_modules\syscollector\build\bin\syscollector.dll"
