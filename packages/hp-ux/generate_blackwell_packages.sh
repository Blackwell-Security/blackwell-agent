...
# Copyright (C) 2018 Blackwell Inc.
# This program is a free software; you can redistribute
# it and/or modify it under the terms of GPLv2
# Blackwell HP-UX Package builder.
...
source_directory=/blackwell-sources
configuration_file="${source_directory}/etc/preloaded-vars.conf"
target_dir="${current_path}/output/"
blackwell_version=""
blackwell_revision="1"
depot_path=""
control_binary=""
compute_checksums="no"

# Needed variables to build Blackwell with custom GCC and cmake
...
build_environment() {
    ...
    curl -k -SO http://packages.blackwell.com/utils/gcc/gcc_9.4_HPUX_build.tar.gz
    ...
    cd ${build_tools_path}
    curl -k -SO http://packages.blackwell.com/utils/cmake/cmake_3.22.2_HPUX_build.tar.gz
...
compute_version_revision() {
    blackwell_version=$(cat ${source_directory}/src/VERSION | cut -d "-" -f1 | cut -c 2-)

    echo ${blackwell_version} > /tmp/VERSION
    echo ${blackwell_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/unzip /blackwell.zip
    mv blackwell-* ${source_directory}
    compute_version_revision
}

check_version() {
    blackwell_version=`cat ${source_directory}/src/VERSION`
...
compile() {
    echo "Compiling code"
    # Compile and install blackwell
    cd ${source_directory}/src
    config
    check_version
    gmake deps RESOURCES_URL=http://packages.blackwell.com/deps/${deps_version} TARGET=agent
...
create_package() {
    ...
    blackwell_version=`echo "${blackwell_version}" | cut -d v -f 2`
    pkg_tar_file="blackwell-agent-${blackwell_version}-${blackwell_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_tar_file} ${install_path} /sbin/init.d/blackwell-agent /sbin/rc2.d/S97blackwell-agent /sbin/rc3.d/S97blackwell-agent
...
clean() {
    ...
    find /sbin -name "*blackwell-agent*" -exec rm {} \;
    userdel blackwell
    groupdel blackwell
...
show_help() {
    ...
    echo "    -r, --revision Revision to naming Blackwell agent package."
