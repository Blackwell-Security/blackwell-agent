...
setup_build(){
    ...
    cp ${specs_path}/blackwell-${BUILD_TARGET}.spec ${rpm_build_dir}/SPECS/${package_name}.spec
...
build_package(){
    ...
    $linux $rpmbuild --define "_sysconfdir /etc" --define "_topdir ${rpm_build_dir}" \
        ...
        --define "_version ${blackwell_version}" --define "_hashcommit ${short_commit_hash}" \
...
get_package_and_checksum(){
    ...
    RPM_NAME=$(ls -R ${rpm_build_dir}/RPMS | grep "blackwell-${BUILD_TARGET}" | grep -v "debuginfo")
    SYMBOLS_NAME=$(ls -R ${rpm_build_dir}/RPMS)
    SYMBOLS_NAME=$(echo "${SYMBOLS_NAME}" | grep "blackwell-${BUILD_TARGET}-debuginfo" || true)
    ...
    if [[ "${checksum}" == "yes" ]]; then
        cd "${rpm_build_dir}/RPMS" && sha512sum $RPM_NAME > /var/local/blackwell/$RPM_NAME.sha512
        # Legacy RPMs do not have symbols
        if [ -n "${SYMBOLS_NAME}" ]; then
            sha512sum $SYMBOLS_NAME > /var/local/blackwell/$SYMBOLS_NAME.sha512
        fi

        if [[ "${src}" == "yes" ]]; then
            cd "${rpm_build_dir}/SRPMS" && sha512sum $SRC_NAME > /var/local/blackwell/$SRC_NAME.sha512
        fi
    fi

    if [[ "${src}" == "yes" ]]; then
        mv ${rpm_build_dir}/SRPMS/$SRC_NAME /var/local/blackwell
    else
        mv ${rpm_build_dir}/RPMS/$RPM_NAME /var/local/blackwell
        if [ -n "${SYMBOLS_NAME}" ]; then
            mv ${rpm_build_dir}/RPMS/$SYMBOLS_NAME /var/local/blackwell
        fi
    fi
}
